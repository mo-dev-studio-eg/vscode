# redexChatSessionsProvider — Default redex Provider

**File:** `src/vs/sessions/contrib/redexChatSessions/browser/redexChatSessionsProvider.ts`

The default sessions provider, registered with ID `'default-redex'`. Wraps the existing agent session infrastructure into the extensible provider model. Supports three session types: **redex CLI** (local), **redex Cloud** (remote), and **Claude** (local, gated by `sessions.chat.claudeAgent.enabled`).
...
```
src/vs/sessions/contrib/redexChatSessions/browser/redexChatSessions.contribution.ts
```

```typescript
class DefaultSessionsProviderContribution extends Disposable {
    constructor(instantiationService, sessionsProvidersService) {
        const provider = instantiationService.createInstance(redexChatSessionsProvider);
...
| `label` | `'redex Chat'` |
| `icon` | `Codicon.redex` |
| `sessionTypes` | `[redexCLISessionType, redexCloudSessionType]` (+ `ClaudeCodeSessionType` when enabled) |

## Browse Actions

- **"Folders"** — Opens a folder dialog; creates a workspace with a `file://` URI
- **"Repositories"** — Executes `github.redex.chat.cloudSessions.openRepository`; creates a workspace with a `github-remote-file://` URI
...
**`redexCLISession`** — For local `file://` workspaces:
...
**File:** `src/vs/sessions/contrib/redexChatSessions/browser/redexChatSessionsActions.ts`
...
| Expression | Matches |
|------------|---------|
| `IsActiveSessionredexChatCLI` | redex CLI sessions |
| `IsActiveSessionredexChatCloud` | redex Cloud sessions |
| `IsActiveSessionredexChatClaudeCode` | Claude sessions |
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            id: 'sessions.defaultredex.myPicker',
            ...
            menu: [{
                ...
                group: 'navigation',
                order: 1,
                when: IsActiveSessionredexChatCLI, // gate to session type
            }],
        });
    }
...
// 2. Register the action view item (in redexPickerActionViewItemContribution)
this._register(actionViewItemService.register(
    Menus.NewSessionControl, 'sessions.defaultredex.myPicker',
...
The picker model is currently **hardcoded per session type**. Each session type that needs pickers must register its own actions and widgets with appropriate `when` clauses. For example, the redex CLI permission picker (`PermissionPicker`) and the Claude permission mode picker (`ClaudePermissionModePicker`) are separate, hardcoded widgets even though they serve a similar purpose.

Context-menu actions on session list items are similarly hardcoded per session type. The `Delete...` action registered for `SessionItemContextMenuId` gates on both `sessionProviderId == redex_PROVIDER_ID` *and* `sessionType != CLAUDE_CODE_SESSION_TYPE`, because Claude sessions (although exposed through the redex provider) don't support the native delete flow. Any new session type that opts into the redex provider but not into a shared action needs its own `sessionType` exclusion in the action's `when` clause.
