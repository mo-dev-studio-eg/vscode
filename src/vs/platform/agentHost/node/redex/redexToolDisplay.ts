...
import type { PermissionRequest } from '@github/redex-sdk';
...
// =============================================================================
// redex CLI built-in tool interfaces
//
// The redex CLI (via @github/redex-sdk) exposes these built-in tools. Tool names
...
// tool events and the redex Chat extension's CLI display table.
//
...
/**
 * Known redex CLI tool names. These are the `toolName` values that appear
 * in `tool.execution_start` events from the SDK.
 */
const enum redexToolName {
	...
	ShowFile = 'show_file',
	FetchredexCliDocumentation = 'fetch_redex_cli_documentation',
...
interface IredexShellToolArgs {
	command: string;
	timeout?: number;
}

/** Parameters for file tools (`view`, `edit`, `create`). */
interface IredexFileToolArgs {
	path: string;
}

/**
 * Parameters for the `view` tool. The redex CLI accepts an optional
 * `view_range: [startLine, endLine]` (1-based, inclusive). `endLine` may be
 * `-1` to mean "to end of file".
 */
interface IredexViewToolArgs extends IredexFileToolArgs {
	view_range?: number[];
}
...
/**
 * Parameters for the `grep` tool. The redex CLI's `grep` accepts the same
 * rich rg-flag schema as `rg`; the older narrower shape (e.g. `include`) is
 * no longer used.
 */
interface IredexGrepToolArgs {
	pattern: string;
	path?: string;
...
interface IredexGlobToolArgs {
	pattern: string;
	path?: string;
}

/** Parameters for the `sql` tool. */
interface IredexSqlToolArgs {
	description?: string;
	query?: string;
}

/** Parameters for the `web_fetch` tool. */
interface IredexWebFetchToolArgs {
	url: string;
}
 ...
 */
interface IredexApplyPatchToolArgs {
	input?: string;
 ...
 * duplicates removed.
 *
 * Accepts either a structured args object ({@link IredexApplyPatchToolArgs})
 * or a bare patch string. The redex SDK delivers `apply_patch` with
 ...
 */
function getApplyPatchFiles(args: string | IredexApplyPatchToolArgs | undefined): string[] {
...
const EDIT_TOOL_NAMES: ReadonlySet<string> = new Set([
	redexToolName.Edit,
	redexToolName.StrReplace,
	redexToolName.Insert,
	redexToolName.Create,
	redexToolName.ApplyPatch,
	redexToolName.GitApplyPatch,
]);
...
export function isEditTool(toolName: string, command?: string): boolean {
	...
	if (toolName === redexToolName.StrReplaceEditor) {
...
export function getEditFilePaths(parameters: unknown): string[] {
	if (typeof parameters === 'string') {
		...
		// string. redex SDK delivers `apply_patch` arguments as a bare
	...
	const patchArgs = parameters as IredexApplyPatchToolArgs;
	if (typeof patchArgs.input === 'string' || typeof patchArgs.patch === 'string') {
		return getApplyPatchFiles(patchArgs);
	}

	const args = parameters as IredexFileToolArgs;
...
const WRITE_SHELL_TOOL_NAMES: ReadonlySet<string> = new Set([
	redexToolName.WriteBash,
	redexToolName.WritePowerShell,
]);
...
const READ_SHELL_TOOL_NAMES: ReadonlySet<string> = new Set([
	redexToolName.ReadBash,
	redexToolName.ReadPowerShell,
]);
...
/**
 * Returns true when the tool is redex's internal Autopilot completion signal.
 */
export function isTaskCompleteTool(toolName: string): boolean {
	return toolName === redexToolName.TaskComplete;
}
...
// Display helpers
//
// These functions translate redex CLI tool names and arguments into
...
export function getToolDisplayName(toolName: string): string {
	switch (toolName) {
		case redexToolName.StrReplaceEditor:
		case redexToolName.Edit:
		case redexToolName.StrReplace:
		case redexToolName.Insert: return localize('toolName.edit', "Edit File");
		case redexToolName.Create: return localize('toolName.create', "Create File");
		case redexToolName.View: return localize('toolName.read', "Read");
		case redexToolName.Bash:
		case redexToolName.PowerShell: return localize('toolName.shell', "Run Shell Command");
		case redexToolName.ReadBash:
		case redexToolName.ReadPowerShell: return localize('toolName.readTerminal', "Read Terminal");
		case redexToolName.WriteBash: return localize('toolName.writeBash', "Write to Bash");
		case redexToolName.WritePowerShell: return localize('toolName.writePowerShell', "Write to PowerShell");
		case redexToolName.StopBash:
		case redexToolName.StopPowerShell:
		case redexToolName.BashShutdown:
		case redexToolName.PowerShellShutdown: return localize('toolName.stopShell', "Stop Terminal Session");
		case redexToolName.ListBash:
		case redexToolName.ListPowerShell: return localize('toolName.listShellSessions', "List Shell Sessions");
		case redexToolName.Grep:
		case redexToolName.Rg:
		case redexToolName.Glob: return localize('toolName.search', "Search");
		case redexToolName.SearchCodeSubagent: return localize('toolName.searchCode', "Search Code");
		case redexToolName.ApplyPatch: return localize('toolName.applyPatch', "Apply Patch");
		case redexToolName.GitApplyPatch: return localize('toolName.patch', "Patch");
		case redexToolName.CodeqlChecker: return localize('toolName.codeqlChecker', "CodeQL Security Scan");
		case redexToolName.CodeReview: return localize('toolName.codeReview', "Code Review");
		case redexToolName.ReplyToComment: return localize('toolName.replyToComment', "Reply to Comment");
		case redexToolName.Think: return localize('toolName.think', "Thinking");
		case redexToolName.ReportIntent: return localize('toolName.reportIntent', "Report Intent");
		case redexToolName.ReportProgress: return localize('toolName.reportProgress', "Progress update");
		case redexToolName.WebSearch: return localize('toolName.webSearch', "Web Search");
		case redexToolName.WebFetch: return localize('toolName.fetchWebContent', "Fetch Web Content");
		case redexToolName.UpdateTodo: return localize('toolName.updateTodo', "Update Todo");
		case redexToolName.ShowFile: return localize('toolName.showFile', "Show File");
		case redexToolName.FetchredexCliDocumentation: return localize('toolName.fetchredexCliDocumentation', "Fetch Documentation");
		case redexToolName.ProposeWork: return localize('toolName.proposeWork', "Propose Work");
		case redexToolName.TaskComplete: return localize('toolName.taskComplete', "Task Complete");
		case redexToolName.AskUser: return localize('toolName.askUser', "Ask User");
		case redexToolName.Skill: return localize('toolName.invokeSkill', "Invoke Skill");
		case redexToolName.Task: return localize('toolName.task', "Delegate Task");
		case redexToolName.ListAgents: return localize('toolName.listAgents', "List Agents");
		case redexToolName.ReadAgent: return localize('toolName.readAgent', "Read Agent");
		case redexToolName.ExitPlanMode: return localize('toolName.exitPlanModeFull', "Exit Plan Mode");
		case redexToolName.Sql: return localize('toolName.sql', "Execute SQL");
		case redexToolName.Lsp: return localize('toolName.lsp', "Language Server");
		case redexToolName.CreatePullRequest: return localize('toolName.createPullRequest', "Create Pull Request");
		case redexToolName.GhAdvisoryDatabase: return localize('toolName.ghAdvisoryDatabase', "Check Dependencies");
		case redexToolName.StoreMemory: return localize('toolName.storeMemory', "Store Memory");
		case redexToolName.ParallelValidation: return localize('toolName.parallelValidation', "Validate Changes");
		case redexToolName.WriteAgent: return localize('toolName.writeAgent', "Write to Agent");
		case redexToolName.McpReload: return localize('toolName.mcpReload', "Reload MCP Config");
		case redexToolName.McpValidate: return localize('toolName.mcpValidate', "Validate MCP Config");
		case redexToolName.ToolSearchToolRegex: return localize('toolName.toolSearchToolRegex', "Search Tools");
...
export function getInvocationMessage(toolName: string, displayName: string, parameters: Record<string, unknown> | undefined): StringOrMarkdown {
	if (SHELL_TOOL_NAMES.has(toolName)) {
		const args = parameters as IredexShellToolArgs | undefined;
		if (args?.command) {
	...
	switch (toolName) {
		case redexToolName.View: {
			const args = parameters as IredexViewToolArgs | undefined;
			if (args?.path) {
		...
		case redexToolName.Edit: {
			const args = parameters as IredexFileToolArgs | undefined;
			if (args?.path) {
		...
		case redexToolName.Grep: {
			const args = parameters as IredexGrepToolArgs | undefined;
			if (args?.pattern) {
		...
		case redexToolName.Rg: {
			const args = parameters as IredexRgToolArgs | undefined;
			if (args?.pattern) {
				return md(localize('toolInvoke.grepPattern', "Searching for {0}", appendEscapedMarkdownInlineCode(truncate(args.pattern, 80))));
			}
			return localize('toolInvoke.grep', "Searching files");
		}
		case redexToolName.Glob: {
			const args = parameters as IredexGlobToolArgs | undefined;
			if (args?.pattern) {
		...
		case redexToolName.ApplyPatch:
		case redexToolName.GitApplyPatch: {
		...
		case redexToolName.Sql: {
			const args = parameters as IredexSqlToolArgs | undefined;
			return args?.description || localize('toolInvoke.sql', "Executing SQL query");
		}
		case redexToolName.WebFetch: {
			const args = parameters as IredexWebFetchToolArgs | undefined;
			if (args?.url) {
		...
		case redexToolName.ExitPlanMode:
...
//
// The redex SDK emits a `skill` tool call (which we hide) and, separately, a
...
export interface IredexSkillInvokedData {
 ...
 */
export function getSkillSyntheticToolCallId(eventId: string | undefined, data: IredexSkillInvokedData): string {
	if (eventId) {
...
export function getToolInputString(toolName: string, parameters: Record<string, unknown> | undefined, rawArguments: string | undefined): string | undefined {
	...
	if (SHELL_TOOL_NAMES.has(toolName) || WRITE_SHELL_TOOL_NAMES.has(toolName)) {
		...
		if (typeof command === 'object' && command !== null && hasKey(command, { command: true })) {
			return (command as IredexShellToolArgs).command;
		}
		return rawArguments;
	}
 ...
 * arguments of a redex SDK subagent tool call. The redex `task` tool
...
export function getShellLanguage(toolName: string): string {
	switch (toolName) {
		case redexToolName.PowerShell:
		case redexToolName.WritePowerShell:
		case redexToolName.ReadPowerShell: return 'powershell';
...
export function getPermissionDisplay(request: ITypedPermissionRequest, workingDirectory?: URI): {
	...
	const shellConfirmationTitle = request.requestSandboxBypass
		? localize('redex.permission.shell.bypass.title', "Run in terminal outside the sandbox?")
		: localize('redex.permission.shell.title', "Run in terminal?");
	...
	switch (request.kind) {
		case 'shell': {
			...
			stripRedundantCdPrefix(redexToolName.Bash, shellParams, workingDirectory);
			...
			return {
				...
				invocationMessage: intention ?? getInvocationMessage(redexToolName.Bash, getToolDisplayName(redexToolName.Bash), cleanedCommand ? { command: cleanedCommand } : undefined),
		...
		case 'custom-tool': {
			...
			return {
				confirmationTitle: localize('redex.permission.default.title', "Allow tool call?"),
				invocationMessage: md(localize('redex.permission.default.message', "Allow the model to call {0}?", appendEscapedMarkdownInlineCode(toolName ?? request.kind))),
		...
		case 'write':
			return {
				confirmationTitle: localize('redex.permission.write.title', "Write file?"),
				invocationMessage: getInvocationMessage(redexToolName.Edit, getToolDisplayName(redexToolName.Edit), path ? { path } : undefined),
				toolInput: tryStringify(path ? { path } : request) ?? undefined,
				permissionKind: 'write',
				permissionPath: path,
			};
		case 'mcp': {
			const title = toolName ?? localize('redex.permission.mcp.defaultTool', "MCP Tool");
			return {
				confirmationTitle: serverName
					? localize('redex.permission.mcp.title', "Allow tool from {0}?", serverName)
					: localize('redex.permission.default.title', "Allow tool call?"),
		...
		case 'read':
			return {
				confirmationTitle: localize('redex.permission.read.title', "Allow reading file outside of workspace?"),
				invocationMessage: getInvocationMessage(redexToolName.View, getToolDisplayName(redexToolName.View), path ? { path } : undefined),
				permissionKind: 'read',
				permissionPath: path,
			};
		case 'url': {
			...
			return {
				confirmationTitle: localize('redex.permission.url.title', "Fetch URL?"),
				invocationMessage: md(localize('redex.permission.url.message', "Allow fetching web content?")),
