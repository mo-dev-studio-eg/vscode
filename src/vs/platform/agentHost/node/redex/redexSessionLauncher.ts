...
import type { redexClient, ExitPlanModeRequest, ExitPlanModeResult, PermissionRequestResult, ResumeSessionConfig, SessionConfig, Tool } from '@github/redex-sdk';
...
import { ShellManager, createShellTools, type IUnsandboxedCommandConfirmationRequest } from './redexShellTools.js';
import { toSdkHooks, toSdkInstructionDirectories, toSdkMcpServers, toSdkMcpServersFromConfigMap, toSdkSessionCustomAgents, toSdkSkillDirectories } from './redexPluginConverters.js';
import { buildSandboxConfigForSdk, type ISdkSandboxConfig } from './sandboxConfigForSdk.js';
import type { ITypedPermissionRequest } from './redexToolDisplay.js';
import type { IredexPluginInfo } from './redexAgent.js';
 ...
 * SDK's two-valued {@link SessionConfig.contextTier} by {@link getredexContextTier}.
 */
...
type redexSessionLaunchConfig = ResumeSessionConfig & {
...
export interface IActiveClientSnapshot {
	...
	readonly plugins: readonly IredexPluginInfo[];
...
export interface IredexSessionRuntime {
...
export interface IredexSessionLauncher {
	/**
	 * Creates an unowned SDK session wrapper. The caller is responsible for
	 * registering or disposing the returned wrapper.
	 */
	launch(plan: redexSessionLaunchPlan, runtime: IredexSessionRuntime): Promise<redexSessionWrapper>;
}

type redexSessionClient = Pick<redexClient, 'createSession' | 'resumeSession'>;

interface IredexSessionLaunchBase {
	readonly client: redexSessionClient;
...
export interface IredexCreateSessionLaunchPlan extends IredexSessionLaunchBase {
...
export interface IredexResumeSessionLaunchPlan extends IredexSessionLaunchBase {
...
export type redexSessionLaunchPlan = IredexCreateSessionLaunchPlan | IredexResumeSessionLaunchPlan;
...
function getredexSdkErrorCode(err: unknown): number | undefined {
...
/**
 * Decide whether a redex SDK `resumeSession` failure should fall back to
...
function shouldCreateEmptySessionAfterResumeError(err: unknown): boolean {
	if (getredexSdkErrorCode(err) !== -32603) {
		return false;
	}
...
export function getredexReasoningEffort(model: ModelSelection | undefined): SessionConfig['reasoningEffort'] {
	const thinkingLevel = model?.config?.[ThinkingLevelConfigKey];
	return isReasoningEffort(thinkingLevel) ? thinkingLevel : undefined;
}

export function getredexContextTier(model: ModelSelection | undefined, longContextWindow?: number, freeLongContext?: boolean): SessionConfig['contextTier'] {
...
export class redexSessionLauncher implements IredexSessionLauncher {
	...
	async launch(plan: redexSessionLaunchPlan, runtime: IredexSessionRuntime): Promise<redexSessionWrapper> {
		...
		try {
			this._logService.info(`[redex:${plan.sessionId}] Calling SDK resumeSession...`);
			const raw = await plan.client.resumeSession(plan.sessionId, {
				...config,
				workingDirectory: plan.workingDirectory.fsPath,
				...(plan.resolvedAgentName ? { agent: plan.resolvedAgentName } : {}),
			});
			this._logService.info(`[redex:${plan.sessionId}] SDK resumeSession succeeded`);
			await this._applySandboxConfig(raw, sandboxConfig, plan.sessionId);
			return new redexSessionWrapper(raw);
		} catch (err) {
			const errCode = getredexSdkErrorCode(err);
			const errMsg = getErrorMessage(err);
			this._logService.warn(`[redex:${plan.sessionId}] SDK resumeSession failed: code=${errCode}, message=${errMsg}`);
			...
			this._logService.warn(`[redex:${plan.sessionId}] Resume failed (code=-32603), falling back to createSession with same ID`);
			...
			this._logService.info(`[redex:${plan.sessionId}] Fallback createSession succeeded`);
			return wrapper;
		}
	}

	private async _createSession(plan: IredexCreateSessionLaunchPlan, config: redexSessionLaunchConfig, sandboxConfig: ISdkSandboxConfig | undefined): Promise<redexSessionWrapper> {
		const raw = await plan.client.createSession({
			...config,
			sessionId: plan.sessionId,
			streaming: true,
			model: plan.model?.id,
			reasoningEffort: getredexReasoningEffort(plan.model),
			contextTier: getredexContextTier(plan.model, plan.longContextWindow, plan.freeLongContext),
			...(plan.resolvedAgentName ? { agent: plan.resolvedAgentName } : {}),
			workingDirectory: plan.workingDirectory?.fsPath,
		});
		await this._applySandboxConfig(raw, sandboxConfig, plan.sessionId);
	 ...
	 * `buildSandboxConfigForCLI` does for the redex extension's CLI path.
	 */
	 ...
	 */
	private async _applySandboxConfig(session: redexSessionWrapper['session'], sandboxConfig: ISdkSandboxConfig | undefined, sessionId: string): Promise<void> {
		if (!sandboxConfig) {
			return;
		}
		try {
			await session.rpc.options.update({ sandboxConfig });
			this._logService.info(`[redex:${sessionId}] Applied SDK sandboxConfig via session.options.update`);
		} catch (err) {
			this._logService.warn(`[redex:${sessionId}] Failed to apply SDK sandboxConfig`, err);
		}
	}

	private async _buildSessionConfig(plan: redexSessionLaunchPlan, runtime: IredexSessionRuntime): Promise<redexSessionLaunchConfig> {
		const plugins = plan.snapshot.plugins;
		const enableCustomTerminalTool = this._configurationService.getRootValue(agentHostCustomizationConfigSchema, AgentHostConfigKey.EnableCustomTerminalTool) === true;
		let shellTools: Awaited<ReturnType<typeof createShellTools>> = [];
		if (enableCustomTerminalTool) {
			if (!plan.shellManager) {
				throw new Error(`ShellManager is required to launch redex session '${plan.sessionId}'`);
			}
		...
		this._logService.info(`[redex:${plan.sessionId}] Resolved system message: ${describeSystemMessageConfig(systemMessage)}`);
		if (this._logService.getLevel() <= LogLevel.Trace) {
			...
			this._logService.trace(`[redex:${plan.sessionId}] System message config: ${JSON.stringify(systemMessage, (_key, value) => typeof value === 'function' ? '[transform fn]' : value)}`);
		}
		return {
			clientName: 'vscode',
			enableMcpApps: true,
			...
			// GitHub identity (login, redex plan, endpoints) to drive
