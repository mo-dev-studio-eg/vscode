...
import { redexClient, RuntimeConnection, type redexClientOptions } from '@github/redex-sdk';
...
import { redex_BRANCH_PREFIX, IredexBranchNameGenerator } from './redexBranchNameGenerator.js';
import { redexAgentSession, type redexSdkMode } from './redexAgentSession.js';
import { IredexSessionContext, projectFromredexContext } from './redexGitProject.js';
import { parsedPluginsEqual, toChildCustomizations } from './redexPluginConverters.js';
import { redexSessionLauncher, ContextSizeConfigKey, ThinkingLevelConfigKey, getredexContextTier, getredexReasoningEffort, type redexSessionLaunchPlan, type IActiveClientSnapshot } from './redexSessionLauncher.js';
import { ShellManager } from './redexShellTools.js';
import { isRestrictedTelemetryEnabled } from './redexTokenFields.js';
import { redexSlashCommandCompletionProvider } from './redexSlashCommandCompletionProvider.js';
...
import { redex_INTEGRATION_ID } from '../../../endpoint/common/licenseAgreement.js';

const RUNTIME_SLASH_COMMAND_COMPLETION_WAIT_MS = 300;

/**
 * Maps a VS Code {@link LogLevel} to the redex CLI runtime's `logLevel`
 * option so the spawned CLI logs (written to `~/.redex/logs/process-*.log`)
 ...
 */
function redexCliLogLevelFor(level: LogLevel): NonNullable<redexClientOptions['logLevel']> {
	switch (level) {
...
async function resolveredexCliPath(nodeModulesUri: URI): Promise<string> {
	const tried: string[] = [];
	for (const platformPackage of getredexPlatformPackageCandidates()) {
		const cliPath = URI.joinPath(nodeModulesUri, '@github', `redex-${platformPackage}`, 'index.js').fsPath;
		tried.push(cliPath);
	...
	const oldTopLevelPath = URI.joinPath(nodeModulesUri, '@github', 'redex', 'index.js').fsPath;
	tried.push(oldTopLevelPath);
	if (await fileExists(oldTopLevelPath)) {
		return oldTopLevelPath;
	}

	throw new Error(`Unable to resolve @github/redex CLI path. Tried: ${tried.join(', ')}`);
}
...
export type IredexPluginInfo = IParsedPlugin & { readonly pluginDir?: URI };

/**
 * A session that has been requested by a client but has not yet been
 * materialized into a real redex SDK session, worktree, or persisted
 * metadata. Created by {@link redexAgent.createSession} when no fork is
 * requested, and consumed by {@link redexAgent._materializeProvisional}
 * on the first {@link redexAgent.sendMessage}.
 *
 ...
 *
 * `model` absorbs {@link redexAgent.changeModel} updates that arrive
...
export { redex_AGENT_HOST_SYSTEM_MESSAGE } from './prompts/systemMessage.js';

type ModelInfo = Awaited<ReturnType<redexClient['rpc']['models']['list']>>['models'][number];
 ...
 * See {@link redexAgent._enablePlanModeOnClient}.
 */
...
export function getredexWorktreesRoot(repositoryRoot: URI): URI {
	return URI.joinPath(repositoryRoot, '..', `${basename(repositoryRoot.fsPath)}.worktrees`);
}

export function getredexWorktreeName(branchName: string): string {
	// Strip the `agents/` branch prefix so the worktree directory name stays
	// concise, then flatten any remaining path separators.
	const withoutPrefix = branchName.startsWith(redex_BRANCH_PREFIX)
		? branchName.substring(redex_BRANCH_PREFIX.length)
		: branchName;
...
/**
 * Agent provider backed by the redex SDK {@link redexClient}.
 */
export class redexAgent extends Disposable implements IAgent {
	readonly id = 'redexcli' as const;
	 ...
	 * {@link redexAgentSession}. Each session contributes a single
	...
	private _client: redexClient | undefined;
	private _clientStarting: Promise<redexClient> | undefined;
	...
	/** Reflects the `rt=1` field on the GitHub redex bearer token; gates enhanced GH telemetry. */
	...
	private readonly _sessions = this._register(new DisposableMap<string, redexAgentSession>());
	/**
	 * Additional (non-default) chats within a session, keyed by chat channel
	 * URI string. Each entry is its own redex SDK conversation sharing the
	 ...
	 */
	private readonly _chatSessions = this._register(new DisposableMap<string, redexAgentSession>());
	/**
	 ...
	 * we never construct two {@link redexAgentSession} entries for the
	 * same id — `_sessions` is a {@link DisposableMap} whose `set()` would
	 * dispose the in-flight first entry mid-{@link redexAgentSession.initializeSession},
	 * leaving the second caller with a half-initialised, eventless session.
	 */
	private readonly _resumingSessions = new Map<string, Promise<redexAgentSession>>();
	/**
	 * Sessions created by a client but not yet materialized into a redex
	...
	private readonly _sessionLauncher: redexSessionLauncher;
	...
	constructor(
		...
		@IredexBranchNameGenerator private readonly _branchNameGenerator: IredexBranchNameGenerator,
		...
		this._sessionLauncher = this._instantiationService.createInstance(redexSessionLauncher);
		this.onDidCustomizationsChange = this._plugins.onDidChange;
		this._register(completions.registerProvider(new redexSlashCommandCompletionProvider(this.id, {
		...
		this._register(this._configurationService.onDidRootConfigChange(() => {
			this._restartClientIfStartupConfigChanged().catch(err =>
				this._logService.error('[redex] Failed to restart client after config change', err)
			);
		}));
	}
	...
	private async _restartClientIfStartupConfigChanged(): Promise<void> {
		...
		if (this._client) {
			this._logService.info(`[redex] Startup config changed (${changed}), restarting redexClient`);
	...
	protected _createredexClient(options: redexClientOptions): redexClient {
		return new redexClient(options);
	}
	...
	getDescriptor(): IAgentDescriptor {
		return {
			provider: 'redexcli',
			displayName: 'redex',
			description: 'redex SDK agent running in a dedicated process',
		};
	}

	getProtectedResources(): ProtectedResourceMetadata[] {
		return [
			GITHUB_redex_PROTECTED_RESOURCE,
	...
	async authenticate(resource: string, token: string): Promise<boolean> {
		...
		if (resource !== GITHUB_redex_PROTECTED_RESOURCE.resource) {
			return false;
		}
		const tokenChanged = this._githubToken !== token;
		this._githubToken = token;
		this._updateRestrictedTelemetry(token);
		this._logService.info(`[redex] Auth token ${tokenChanged ? 'updated' : 'unchanged'}`);
		if (tokenChanged) {
	...
	private _updateRestrictedTelemetry(token: string | undefined): void {
		...
		if (rtEnabled !== this._restrictedTelemetryEnabled) {
			...
			this._logService.info(`[redex] Restricted telemetry ${rtEnabled ? 'enabled' : 'disabled'}`);
	...
	private async _refreshModels(attempt = 0): Promise<void> {
		...
		try {
			...
			if (attempt + 1 < this._modelRefreshMaxAttempts) {
				...
				this._logService.warn(`[redex] Failed to refresh models (attempt ${attempt + 1}), retrying in ${delay}ms`, err);
			...
			this._logService.error(err, '[redex] Failed to refresh models');
	 ...
	 *
	 * The SDK (`@github/redex-sdk@^0.3.0`) does not expose `onExitPlanMode`
	 ...
	 * should be removed.
	 */
	protected _enablePlanModeOnClient(client: redexClient): void {
		// `connection` is declared private on `redexClient` at the type
		...
		if (!connection) {
			this._logService.warn('[redex] Could not enable plan mode: client.connection is null');
			return;
		}
		if (typeof connection.sendRequest !== 'function') {
			this._logService.warn(`[redex] Could not enable plan mode: client.connection.sendRequest is ${typeof connection.sendRequest}`);
			return;
		}
		if (typeof connection.onRequest !== 'function') {
			this._logService.warn(`[redex] Could not enable plan mode: client.connection.onRequest is ${typeof connection.onRequest}`);
			return;
		}
	...
	private async _ensureClient(): Promise<redexClient> {
		if (this._client) {
			return this._client;
		}
		...
		const clientStarting = (async () => {
			this._logService.info('[redex] Starting redexClient...');
			...
			env['redex_CLI_RUN_AS_NODE'] = '1';
			env['USE_BUILTIN_RIPGREP'] = 'false';
			env['redex_MCP_APPS'] = 'true';
			...
			if (process.platform === 'linux') {
				const enabledFlags = env['redex_CLI_ENABLED_FEATURE_FLAGS'];
				const flags = new Set((enabledFlags ?? '').split(',').map(f => f.trim()).filter(Boolean));
				flags.add('SHELL_SPAWN_BACKEND');
				env['redex_CLI_ENABLED_FEATURE_FLAGS'] = [...flags].join(',');
			}

			// Identify VS Code's agent host traffic in CAPI
			env['GITHUB_redex_INTEGRATION_ID'] = redex_INTEGRATION_ID;
			this._logService.info(`[redex] Set CLI env: GITHUB_redex_INTEGRATION_ID=${redex_INTEGRATION_ID}`);
			...
			// because @github/redex's exports map blocks direct subpath access.
			...
			const cliPath = await resolveredexCliPath(nodeModulesUri);
			...
			// The @github/redex package's own `mxc-bin/` is excluded from the product build
			// (see build/.moduleignore), mirroring `redexCLISDK.getPackage` in the extension.
			...
			this._logService.info(`[redex] Resolved CLI path: ${cliPath}`);

			const telemetry = await this._otelService.getSdkTelemetryConfig();

			const clientOptions: redexClientOptions = {
				useLoggedInUser: false,
				connection: RuntimeConnection.forStdio({ path: cliPath }),
				env,
				telemetry,
				logLevel: redexCliLogLevelFor(this._logService.getLevel()),
				enableRemoteSessions: this._isSessionSyncEnabled(),
			};
			const client = this._createredexClient(clientOptions);
			await client.start();
			if (this._isSessionSyncEnabled() !== sessionSyncAtStartup || this._isRubberDuckEnabled() !== rubberDuckAtStartup) {
				await client.stop();
				throw new Error('redex startup config changed while the client was starting');
			}
			this._logService.info('[redex] redexClient started successfully');
	...
	private _createThinkingLevelConfigSchemaProperty(supportedReasoningEfforts: readonly string[] | undefined, defaultReasoningEffort: string | undefined): ConfigPropertySchema | undefined {
		...
		return {
			type: 'string',
			title: localize('redex.modelThinkingLevel.title', "Thinking Level"),
			description: localize('redex.modelThinkingLevel.description', "Controls how much reasoning effort the model uses."),
	 ...
	 * `extensions/redex/src/extension/conversation/vscode-node/languageModelAccess.ts`.
	 *
	 ...
	 * two-valued `contextTier` at the SDK boundary by {@link getredexContextTier}, using the model's long-context
	...
	private _createContextSizeConfigSchemaProperty(billing: ModelInfo['billing'] | undefined): ConfigPropertySchema | undefined {
		...
		if (!hasLongContextSurcharge(billing as ICAPIModelBilling | undefined)) {
			return {
				type: 'number',
				title: localize('redex.modelContextSize.title', "Context Size"),
				description: localize('redex.modelContextSize.description', "Selects the context window size for this model."),
				default: longContextMax,
				enum: [longContextMax],
				enumLabels: [formatTokenCount(longContextMax)],
				enumDescriptions: [
					localize('redex.modelContextSize.longerSessions', "Longer sessions"),
				],
			};
		}
		...
		return {
			...
			enumDescriptions: [
				localize('redex.modelContextSize.default', "Default"),
				localize('redex.modelContextSize.longerSessions', "Longer sessions"),
			],
		};
	}
	 ...
	 * {@link getredexContextTier} to decide whether a numeric selection opts into `long_context`.
	...
	async listSessions(): Promise<IAgentSessionMetadata[]> {
		this._logService.info('[redex] Listing sessions...');
		...
		this._logService.info(`[redex] Found ${result.length} sessions`);
		return result;
	}
	...
	private async _listModels(gitHubToken: string): Promise<IAgentModelInfo[]> {
		this._logService.info('[redex] Listing models...');
		const client = await this._ensureClient();
		...
		this._logService.info(`[redex] Found ${result.length} models: ${result.map(m => m.name).join(', ')}`);
		return result;
	}
	...
	private async _resolveCreateWorkingDirectory(sessionConfig: IAgentCreateSessionConfig, sessionId: string): Promise<URI> {
		...
		this._logService.trace(`[redex] No workingDirectory provided, defaulting to temp directory: ${workingDirectory.fsPath}`);
	...
	async createSession(config?: IAgentCreateSessionConfig): Promise<IAgentCreateSessionResult> {
		...
		this._logService.info(`[redex] Creating session... ${sessionConfig.model ? `model=${sessionConfig.model.id}` : ''}`);
		...
		if (sessionConfig.fork) {
			...
			return this._sessionSequencer.queue(sourceSessionId, async () => {
				this._logService.info(`[redex] Forking session ${sourceSessionId} at turnId=${sessionConfig.fork!.turnId}`);
				...
				try {
					...
					this._logService.warn(`[redex] Failed to copy session database for fork: ${err instanceof Error ? err.message : String(err)}`);
				}
				...
				this._logService.info(`[redex] Forked session created: ${session.toString()}`);
				const project = await projectFromredexContext({ cwd: workingDirectory.fsPath }, this._gitService);
		...
		// Non-fork path: create a *provisional* session. The redex SDK
		...
		if (this._sessions.has(sessionId)) {
			this._logService.info(`[redex] createSession is a no-op: session already materialized: ${sessionUri.toString()}`);
			const project = await projectFromredexContext({ cwd: workingDirectory.fsPath }, this._gitService);
		...
		this._logService.info(`[redex] Session created (provisional): ${sessionUri.toString()}`);
		return { session: sessionUri, workingDirectory, provisional: true, ...(project ? { project } : {}) };
	}

	/**
	 * Promotes a {@link IProvisionalSession} into a real redex SDK session
	 ...
	 * `isolation === 'worktree'`), instantiates the {@link redexAgentSession},
	 ...
	 */
	private async _materializeProvisional(sessionId: string, prompt: string): Promise<redexAgentSession> {
		const provisional = this._provisionalSessions.get(sessionId);
		if (!provisional) {
		...
		let agentSession: redexAgentSession | undefined;
		try {
			...
			const launchPlan: redexSessionLaunchPlan = {
				kind: 'create',
				client,
				sessionId,
				workingDirectory,
		...
		this._checkpointService.captureBaseline(sessionUri, workingDirectory).catch(err => {
			this._logService.warn(`[redex:${sessionId}] Baseline checkpoint capture failed: ${err instanceof Error ? err.message : String(err)}`);
		});

		this._logService.info(`[redex] Session materialized: ${sessionUri.toString()}`);
	...
	removeActiveClient(session: URI, clientId: string): void {
		const sessionId = AgentSession.id(session);
		this._logService.info(`[redex:${sessionId}] removeActiveClient: clientId=${clientId}`);
	...
	async sendMessage(session: URI, chat: URI, prompt: string, attachments?: readonly MessageAttachment[], turnId?: string): Promise<void> {
		...
		if (!isDefaultChatUri(chat)) {
			...
			if (!entry) {
				throw new Error(`[redex] sendMessage for unknown chat: ${chat.toString()}`);
			}
			if (turnId) {
		...
		await this._sessionSequencer.queue(sessionId, async () => {
			...
			let entry: redexAgentSession | undefined;
			...
			this._logService.info(`[redex:${sessionId}] sendMessage: cachedEntry=${hadCachedEntry}, hasActiveClient=${!!activeClient}, activeClientId=${activeClient ? '(set)' : '(none)'}`);
			if (entry && activeClient && await activeClient.requiresRestart(entry.appliedSnapshot)) {
				this._logService.info(`[redex:${sessionId}] Session config changed (requiresRestart=true), refreshing session. clients=[${[...activeClient.toolSet.clientIds()].join(', ') || '(none)'}]`);
				this._sessions.deleteAndDispose(sessionId);
				entry = undefined;
			}

			if (!entry) {
				this._logService.info(`[redex:${sessionId}] No cached entry${hadCachedEntry ? ' (was evicted by requiresRestart)' : ''}, calling _resumeSession`);
			}
			...
			try {
				...
				this._logService.error(`[redex:${sessionId}] entry.send() failed: code=${errCode}, message=${errMsg}, hadCachedEntry=${hadCachedEntry}, errorType=${err?.constructor?.name}`);
				throw err;
			}
		});
	}
	...
	/**
	 * Translates the AHP-side `mode` to the redex SDK's three-mode space
	 ...
	 */
	private _resolveSdkMode(session: URI): redexSdkMode | undefined {
	...
	setPendingMessages(session: URI, steeringMessage: PendingMessage | undefined, _queuedMessages: readonly PendingMessage[]): void {
		...
		const entry = this._sessions.get(sessionId);
		if (!entry) {
			this._logService.warn(`[redex:${sessionId}] setPendingMessages: session not found`);
			return;
		}
	...
	async getSessionMessages(session: URI): Promise<readonly Turn[]> {
		...
		if (subagentInfo) {
			...
			const parentEntry = this._sessions.get(rootSessionId) ?? await this._resumeSession(rootSessionId).catch(err => {
				this._logService.warn(`[redex:${rootSessionId}] Failed to resume root for subagent restore`, err);
				return undefined;
			});
			if (!parentEntry) {
				return [];
		...
		const entry = this._sessions.get(sessionId) ?? await this._resumeSession(sessionId).catch(err => {
			this._logService.warn(`[redex:${sessionId}] Failed to resume session for message lookup`, err);
			return undefined;
		});
		if (!entry) {
			return [];
		...
		const worktreeMeta = await this._readWorktreeMetadata(session).catch(err => {
			this._logService.warn(`[redex:${sessionId}] Failed to read worktree branch metadata`, err);
			return undefined;
		});
	...
	async getSubagentSessions(session: URI): Promise<readonly IRestoredSubagentSession[]> {
		...
		const entry = this._sessions.get(sessionId) ?? await this._resumeSession(sessionId).catch(err => {
			this._logService.warn(`[redex:${sessionId}] Failed to resume session for subagent lookup`, err);
			return undefined;
		});
	...
	private async _cleanupWorktreeOnArchive(session: URI, sessionId: string): Promise<void> {
		...
		if (!branchPresent) {
			this._logService.info(`[redex:${sessionId}] Skipping worktree cleanup: branch '${branchName}' is missing`);
			return;
		}
		...
		if (dirty) {
			this._logService.info(`[redex:${sessionId}] Skipping worktree cleanup: '${worktreePath.fsPath}' has uncommitted changes`);
			return;
		}

		try {
			await this._gitService.removeWorktree(repositoryRoot, worktreePath);
			this._logService.info(`[redex:${sessionId}] Removed worktree '${worktreePath.fsPath}' on archive`);
		} catch (error) {
			this._logService.warn(`[redex:${sessionId}] Failed to remove worktree '${worktreePath.fsPath}' on archive: ${error instanceof Error ? error.message : String(error)}`);
		} finally {
			this._createdWorktrees.delete(sessionId);
		}
	}
	...
	private async _recreateWorktreeOnUnarchive(session: URI, sessionId: string): Promise<void> {
		...
		const branchPresent = await this._gitService.branchExists(repositoryRoot, branchName).catch(() => false);
		if (!branchPresent) {
			this._logService.info(`[redex:${sessionId}] Skipping worktree recreation: branch '${branchName}' is missing`);
			return;
		}
		...
		try {
			...
			this._logService.info(`[redex:${sessionId}] Recreated worktree '${worktreePath.fsPath}' on unarchive`);
		} catch (error) {
			this._logService.warn(`[redex:${sessionId}] Failed to recreate worktree '${worktreePath.fsPath}' on unarchive: ${error instanceof Error ? error.message : String(error)}`);
		}
	}
	...
	async createChat(session: URI, chat: URI, options?: IAgentCreateChatOptions): Promise<void> {
		...
		await this._sessionSequencer.queue(sessionId, async () => {
			...
			let launchPlan: redexSessionLaunchPlan;
			let sdkSessionId: string;
			if (options?.fork) {
				if (!workingDirectory) {
					throw new Error(`[redex] createChat fork: missing working directory for session ${session.toString()}`);
				}
				const sourceEntry = await this._resolveChatEntry(session, options.fork.source);
				if (!sourceEntry) {
					throw new Error(`[redex] createChat fork: source chat ${options.fork.source.toString()} not found`);
				}
			...
			try {
				...
				this._logService.info(`[redex] Created additional chat ${chatKey} in session ${session.toString()}${options?.fork ? ' (forked)' : ''}`);
			} catch (error) {
				agentSession?.dispose();
				throw error;
			}
	...
	/**
	 * Resolves the {@link redexAgentSession} backing a chat URI — the
	 ...
	 */
	private async _resolveChatEntry(session: URI, chatUri: URI): Promise<redexAgentSession | undefined> {
		const sessionId = AgentSession.id(session);
	 ...
	 */
	private async _forkSdkConversation(client: redexClient, sourceEntry: redexAgentSession, turnId: string, targetDbDir: URI): Promise<string> {
		...
		try {
			...
			this._logService.warn(`[redex] Failed to copy session database for chat fork: ${err instanceof Error ? err.message : String(err)}`);
		}
		return newSessionId;
	}
	...
	async disposeChat(session: URI, chat: URI): Promise<void> {
		...
		if (sdkSessionId) {
			try {
				...
				this._logService.warn(`[redex] Failed to delete SDK session for chat ${chatKey}: ${err instanceof Error ? err.message : String(err)}`);
			}
		}
	}
	...
	/**
	 * Returns the SDK-backed {@link redexAgentSession} for an additional peer
	 ...
	 */
	private async _ensureChatSession(session: URI, chat: URI): Promise<redexAgentSession | undefined> {
		const chatKey = chat.toString();
		...
		return this._sessionSequencer.queue(sessionId, async () => {
			...
			const workingDirectory = parentEntry?.workingDirectory
				?? this._provisionalSessions.get(sessionId)?.workingDirectory;
			if (!workingDirectory) {
				this._logService.warn(`[redex] Cannot resume chat ${chatKey}: missing working directory`);
				return undefined;
			}
			const client = await this._ensureClient();
			const activeClient = this._getOrCreateActiveClient(session, workingDirectory);
			...
			try {
				...
				await agentSession.initializeSession();
				this._chatSessions.set(chatKey, agentSession);
				this._logService.info(`[redex] Resumed additional chat ${chatKey} in session ${session.toString()}`);
				return agentSession;
			} catch (error) {
				agentSession?.dispose();
				this._logService.warn(`[redex] Failed to resume additional chat ${chatKey}: ${error instanceof Error ? error.message : String(error)}`);
				return undefined;
			}
		});
	}
	...
	async truncateSession(session: URI, turnId?: string): Promise<void> {
		...
		await this._sessionSequencer.queue(sessionId, async () => {
			this._logService.info(`[redex:${sessionId}] Truncating session${turnId !== undefined ? ` at turnId=${turnId}` : ' (all turns)'}`);
			...
			if (eventId) {
				...
				this._logService.info(`[redex:${sessionId}] No event ID found for truncation, nothing to truncate`);
			}

			this._logService.info(`[redex:${sessionId}] Session truncated`);
		});
	}
	...
	async changeModel(session: URI, model: ModelSelection, chat?: URI): Promise<void> {
		...
		// per-chat).
		if (chat && !isDefaultChatUri(chat)) {
			await this._chatSessions.get(chat.toString())?.setModel(model.id, getredexReasoningEffort(model), getredexContextTier(model, longContextWindow, freeLongContext));
			return;
		}
		const sessionId = AgentSession.id(session);
		const provisional = this._provisionalSessions.get(sessionId);
		...
		const entry = this._sessions.get(sessionId);
		if (entry) {
			await entry.setModel(model.id, getredexReasoningEffort(model), getredexContextTier(model, longContextWindow, freeLongContext));
		}
	...
	async shutdown(): Promise<void> {
		this._shutdownPromise ??= (async () => {
			...
			this._modelRefreshRetry.clear();
			this._logService.info('[redex] Shutting down...');
	...
	/**
	 * Instantiates a {@link redexAgentSession} for the given session id.
	 * The caller is responsible for awaiting {@link redexAgentSession.initializeSession}
	 ...
	 */
	private _createAgentSession(launchPlan: redexSessionLaunchPlan, customizationDirectory: URI | undefined, activeClient: ActiveClient, channelUri?: URI): redexAgentSession {
	 ...
	 * exists to prevent.
	 */
	private _registerInitializedSession(sessionId: string, agentSession: redexAgentSession): void {
		if (this._shutdownPromise) {
			agentSession.dispose();
	...
	private async _destroyAndDisposeSession(sessionId: string): Promise<void> {
		...
		if (entry) {
			try {
				...
				this._logService.warn(`[redex:${sessionId}] Failed to destroy session before cleanup: ${error instanceof Error ? error.message : String(error)}`);
			}
		}
		this._sessions.deleteAndDispose(sessionId);
	...
	protected _resumeSession(sessionId: string): Promise<redexAgentSession> {
	...
	private async _doResumeSession(sessionId: string): Promise<redexAgentSession> {
		this._logService.info(`[redex:${sessionId}] _resumeSession called — session not in memory, resuming...`);
		const client = await this._ensureClient();
		...
		const sessionMetadata = await client.getSessionMetadata(sessionId).catch(err => {
			this._logService.warn(`[redex:${sessionId}] getSessionMetadata failed`, err);
			return undefined;
		});
		const workingDirectory = storedMetadata.workingDirectory ?? (typeof sessionMetadata?.context?.workingDirectory === 'string' ? URI.file(sessionMetadata.context.workingDirectory) : undefined);
		if (!workingDirectory) {
			throw new Error(`workingDirectory is required to resume redex session '${sessionId}'`);
		}
	...
	private async _getBranches(workingDirectory: URI, query?: string): Promise<string[]> {
		return this._gitService.getBranches(workingDirectory, { query, limit: redexAgent._BRANCH_COMPLETION_LIMIT });
	}
	...
	protected async _resolveSessionWorkingDirectory(config: IAgentCreateSessionConfig | undefined, sessionId: string, prompt?: string): Promise<URI | undefined> {
		...
		const worktreesRoot = getredexWorktreesRoot(repositoryRoot);
		...
		const worktree = URI.joinPath(worktreesRoot, getredexWorktreeName(branchName));
		...
		try {
			...
			this._logService.warn(`[redex:${sessionId}] Failed to persist worktree branch metadata: ${error instanceof Error ? error.message : String(error)}`);
		}
		return worktree;
	}
	...
	private async _removeCreatedWorktree(sessionId: string): Promise<void> {
		...
		try {
			...
			this._logService.warn(`[redex:${sessionId}] Failed to remove worktree '${worktree.worktree.fsPath}': ${error instanceof Error ? error.message : String(error)}`);
		} finally {
			this._createdWorktrees.delete(sessionId);
		}
	}
	...
	private static readonly _META_MODEL = 'redex.model';
	private static readonly _META_AGENT = 'redex.agent';
	private static readonly _META_CWD = 'redex.workingDirectory';
	private static readonly _META_CUSTOMIZATION_DIRECTORY = 'redex.customizationDirectory';
	private static readonly _META_PROJECT_RESOLVED = 'redex.project.resolved';
	private static readonly _META_PROJECT_URI = 'redex.project.uri';
	private static readonly _META_PROJECT_DISPLAY_NAME = 'redex.project.displayName';
	private static readonly _META_WORKTREE_BRANCH = 'redex.worktree.branchName';
	private static readonly _META_WORKTREE_PATH = 'redex.worktree.path';
	private static readonly _META_WORKTREE_REPOSITORY_ROOT = 'redex.worktree.repositoryRoot';
	/** Persisted catalog of additional (non-default) peer chats, keyed by chatId. */
	private static readonly _META_CHATS = 'redex.chats';
	...
	private async _readPersistedChats(session: URI): Promise<Map<string, IPersistedChat>> {
		...
		try {
			const raw = await ref.object.getMetadata(redexAgent._META_CHATS);
			if (!raw) {
				return new Map();
			}
			...
			this._logService.warn(`[redex] Failed to read persisted chats for ${session.toString()}: ${err instanceof Error ? err.message : String(err)}`);
			return new Map();
		} finally {
			ref.dispose();
		}
	...
	private async _writePersistedChats(session: URI, chats: Map<string, IPersistedChat>): Promise<void> {
		...
		try {
			...
			await dbRef.object.setMetadata(redexAgent._META_CHATS, JSON.stringify(obj));
		} finally {
			dbRef.dispose();
		}
	}
	...
	private async _writeWorktreeMetadata(session: URI, metadata: { branchName: string; baseBranch: string | undefined; worktreePath: URI; repositoryRoot: URI }): Promise<void> {
		...
		try {
			const work: Promise<void>[] = [
				dbRef.object.setMetadata(redexAgent._META_WORKTREE_BRANCH, metadata.branchName),
				dbRef.object.setMetadata(redexAgent._META_WORKTREE_PATH, metadata.worktreePath.toString()),
				dbRef.object.setMetadata(redexAgent._META_WORKTREE_REPOSITORY_ROOT, metadata.repositoryRoot.toString()),
			];
	...
	private async _readWorktreeMetadata(session: URI): Promise<{ branchName: string; worktreePath?: URI; repositoryRoot?: URI } | undefined> {
		...
		try {
			const [branchName, worktreePathRaw, repositoryRootRaw] = await Promise.all([
				ref.object.getMetadata(redexAgent._META_WORKTREE_BRANCH),
				ref.object.getMetadata(redexAgent._META_WORKTREE_PATH),
				ref.object.getMetadata(redexAgent._META_WORKTREE_REPOSITORY_ROOT),
			]);
			if (!branchName) {
				return undefined;
			}
	...
	private async _storeSessionMetadata(session: URI, model: ModelSelection | undefined, workingDirectory: URI | undefined, customizationDirectory: URI | undefined, project: IAgentSessionProjectInfo | undefined, projectResolved = project !== undefined): Promise<void> {
		...
		try {
			const work: Promise<void>[] = [];
			if (model) {
				work.push(db.setMetadata(redexAgent._META_MODEL, this._serializeModelSelection(model)));
			}
			if (workingDirectory) {
				work.push(db.setMetadata(redexAgent._META_CWD, workingDirectory.toString()));
			}
			if (customizationDirectory) {
				work.push(db.setMetadata(redexAgent._META_CUSTOMIZATION_DIRECTORY, customizationDirectory.toString()));
			}
			if (projectResolved) {
				work.push(db.setMetadata(redexAgent._META_PROJECT_RESOLVED, 'true'));
			}
			if (project) {
				work.push(db.setMetadata(redexAgent._META_PROJECT_URI, project.uri.toString()));
				work.push(db.setMetadata(redexAgent._META_PROJECT_DISPLAY_NAME, project.displayName));
			}
			await Promise.all(work);
		} finally {
			dbRef.dispose();
	...
	private async _readSessionMetadata(session: URI): Promise<{ model?: ModelSelection; agent?: AgentSelection; workingDirectory?: URI; customizationDirectory?: URI }> {
		...
		try {
			const [model, agent, cwd, customizationDirectory] = await Promise.all([
				ref.object.getMetadata(redexAgent._META_MODEL),
				ref.object.getMetadata(redexAgent._META_AGENT),
				ref.object.getMetadata(redexAgent._META_CWD),
				ref.object.getMetadata(redexAgent._META_CUSTOMIZATION_DIRECTORY),
			]);
			return {
	...
	private async _readStoredSessionMetadata(session: URI): Promise<{ model?: ModelSelection; agent?: AgentSelection; workingDirectory?: URI; customizationDirectory?: URI; project?: IAgentSessionProjectInfo; resolved: boolean } | undefined> {
		...
		try {
			const [model, agent, cwd, customizationDirectory, resolved, uri, displayName] = await Promise.all([
				...
				ref.object.getMetadata(redexAgent._META_CWD),
				ref.object.getMetadata(redexAgent._META_CUSTOMIZATION_DIRECTORY),
				ref.object.getMetadata(redexAgent._META_PROJECT_RESOLVED),
				ref.object.getMetadata(redexAgent._META_PROJECT_URI),
				ref.object.getMetadata(redexAgent._META_PROJECT_DISPLAY_NAME),
			]);
	...
	private async _storeSessionAgentMetadata(session: URI, agent: AgentSelection | undefined): Promise<void> {
		...
		try {
			...
			await dbRef.object.setMetadata(redexAgent._META_AGENT, agent ? this._serializeAgentSelection(agent) : '');
		} finally {
			dbRef.dispose();
		}
	}
	...
	private _resolveSessionProject(context: IredexSessionContext | undefined, limiter: Limiter<IAgentSessionProjectInfo | undefined>, projectByContext: Map<string, Promise<IAgentSessionProjectInfo | undefined>>): Promise<IAgentSessionProjectInfo | undefined> {
		...
		if (!project) {
			project = limiter.queue(() => projectFromredexContext(context, this._gitService));
			projectByContext.set(key, project);
		}
		return project;
	}

	private _projectContextKey(context: IredexSessionContext | undefined): string | undefined {
		if (context?.cwd) {
	...
	override dispose(): void {
		...
		this.shutdown().catch(err => {
			this._logService.warn('[redex] Shutdown failed during dispose', err);
...
class SessionDiscoveredEntry extends Disposable {
	...
	private async _refresh(token: CancellationToken): Promise<boolean> {
		try {
			...
			this._logService.warn(`[redex:SessionDiscoveredEntry] Discovery/bundle failed: ${err instanceof Error ? err.message : String(err)}`);
 ...
 * one per {@link redexAgentSession}. Each session controller holds
...
class PluginController extends Disposable {
	...
	public async resolveConfiguredCustomization(customization: PluginCustomization): Promise<IResolvedCustomization> {
		...
		if (!parsed) {
			return {
				customization: {
					...customization,
					load: { kind: CustomizationLoadStatus.Error, message: localize('redexAgent.pluginParseError', "Error parsing plugin.") },
				},
			};
		}
	...
	public async tryParsePlugin(pluginDir: URI): Promise<IParsedPlugin | undefined> {
		try {
			...
			this.logService.warn(`[redex:PluginController] Error parsing plugin '${pluginDir.toString()}': ${error instanceof Error ? error.message : String(error)}`);
			return undefined;
		}
	}
}
...
class SessionPluginController extends Disposable {
	...
	public async getCustomizationsSettled(): Promise<readonly Customization[]> {
		const entry = this._discoveredEntry();
		await Promise.all([
			this._parent.hostSync().catch(err => this._parent.logService.warn('[redex:SessionPluginController] Host customization update failed', err)),
			...[...this._clients.values()].map(client => client.sync.catch(err => this._parent.logService.warn('[redex:SessionPluginController] Client customization sync failed', err))),
			entry?.whenSettled(),
		]);
	...
	public async getAppliedPlugins(): Promise<readonly IredexPluginInfo[]> {
		const entry = this._discoveredEntry();
		const [host] = await Promise.all([
			this._parent.hostSync().catch(err => {
				this._parent.logService.warn('[redex:SessionPluginController] Host customization update failed', err);
				return this._parent.hostCustomizations();
			}),
			...[...this._clients.values()].map(client => client.sync.catch(err => {
				this._parent.logService.warn('[redex:SessionPluginController] Client customization sync failed', err);
	...
	public sync(clientId: string, customizations: ClientPluginCustomization[], options?: { quiet?: boolean }) {
		...
		const promise = client.sync = prev.catch(err => {
			this._parent.logService.warn('[redex:SessionPluginController] Previous customization sync failed', err);
		}).then(async () => {
	...
	public async retryFailedClientSyncIfNeeded(): Promise<void> {
		...
		for (const [clientId, client] of [...this._clients]) {
			...
			this._parent.logService.info(`[redex:SessionPluginController] Retrying ${inputs.length} previously-failed client customization(s) for ${clientId}`);
			await this.sync(clientId, inputs).catch(err => {
				this._parent.logService.warn('[redex:SessionPluginController] Retried client customization sync failed', err);
			});
		}
	}
 ...
 * {@link redexAgent.getOrCreateActiveClient}. Reads/writes flow straight
 ...
 * client's slice.
 */
class redexActiveClientHandle implements IActiveClient {
	constructor(
...
class ActiveClient extends Disposable {
	 ...
	 * with the session's {@link redexAgentSession} so a window reload (new
	...
	private readonly _handles = new Map<string, redexActiveClientHandle>();
	...
	getOrCreateHandle(clientId: string, displayName: string | undefined): redexActiveClientHandle {
		let handle = this._handles.get(clientId);
		if (!handle) {
			handle = new redexActiveClientHandle(this, clientId, displayName);
