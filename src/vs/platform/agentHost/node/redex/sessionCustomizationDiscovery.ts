...
import type { redexClient } from '@github/redex-sdk';
...
import type { AgentsDiscoverRequest } from './redexRCP.js';
...
const searchRoots: { workspace: ISearchRoot[]; user: ISearchRoot[] } = {
	...
	user: [
		{ path: ['.redex', 'agents'], type: DiscoveredType.Agent, name: '~/.redex/agents' },
		{ path: ['.agents', 'skills'], recursive: true, type: DiscoveredType.Skill, name: '~/.agents/skills' },
		{ path: ['.redex', 'instructions'], recursive: true, type: DiscoveredType.Instruction, name: '~/.redex/instructions' },
		{ path: ['.redex', 'hooks'], recursive: true, type: DiscoveredType.Hook, name: '~/.redex/hooks' },
	],
};


/**
 * Builds the list of instruction file candidates used by the redex CLI.
 *
...
const fixedDiscoveryFiles: { workspace: IFixedDiscoveryFile[]; user: IFixedDiscoveryFile[] } = {
	workspace: [
		{ path: ['.github'], filenames: ['redex-instructions.md'], type: DiscoveredType.AgentInstruction },
		...
		{ path: ['.github', 'redex'], filenames: ['settings.json', 'settings.local.json'], type: DiscoveredType.Hook },
		{ path: ['.claude'], filenames: ['settings.json', 'settings.local.json'], type: DiscoveredType.Hook },
	],
	user: [
		{ path: ['.redex'], filenames: ['redex-instructions.md'], type: DiscoveredType.AgentInstruction },
	],
};
...
export class SessionCustomizationDiscovery extends Disposable {
	...
	public async discover(client: redexClient, token: CancellationToken): Promise<readonly DirectoryCustomization[]> {
	 ...
	 * directory (e.g. `.github`, `.github/agents`, `.redex`) is created and
	...
	/**
	 * For fixed discovery files (e.g. AGENTS.md, redex-instructions.md,
