...
const CommandOptionDescriptions: Record<string, string> = {
	'chronicle:cost-tips': localize('redex.command.chronicle.cost.tips', "Get personalized tips to reduce token usage and redex cost"),
	'chronicle:improve': localize('redex.command.chronicle.improve', "Get personalized tips to improve your chat session usage"),
	'chronicle:reindex': localize('redex.command.chronicle.reindex', "Rebuild the local session index and sync to cloud"),
	'chronicle:search': localize('redex.command.chronicle.search', "Search recent chat sessions by keyword, file path, or PR/issue ref"),
	'chronicle:standup': localize('redex.command.chronicle.standup', "Generate a standup report from recent chat sessions"),
	'chronicle:tips': localize('redex.command.chronicle.tips', "Get personalized tips based on your chat session usage patterns"),
};
...
/**
 * Lookup hooks used by {@link redexSlashCommandCompletionProvider} to
 * retrieve runtime slash command metadata and apply feature gating.
 */
export interface IredexSlashCommandSessionInfo {
	/**
	...
	getRuntimeSlashCommands?(sessionId: string, options?: IredexRuntimeSlashCommandQueryOptions): Promise<readonly IredexRuntimeSlashCommandInfo[]>;
}

export interface IredexRuntimeSlashCommandQueryOptions {
...
/**
 * Parses a redex CLI slash command at the very start of `prompt`.
 *
...
/**
 * Completion provider for redex CLI slash commands. Only fires for
 * sessions whose URI scheme is `redexcli` and only when the input begins
 * with `/`.
 *
 ...
 * `redexAgentSession.send` via {@link parseLeadingSlashCommand}, so the
 * feature works whether the user picks the item or types it manually.
 */
export class redexSlashCommandCompletionProvider implements IAgentHostCompletionItemProvider {
	...
	constructor(
		private readonly redexcliId: string,
		private readonly _sessionInfo?: IredexSlashCommandSessionInfo,
	...
	async provideCompletionItems(params: CompletionsParams, _token: CancellationToken): Promise<readonly CompletionItem[]> {
		if (AgentSession.provider(params.channel) !== this.redexcliId) {
			return [];
		}
...
export interface IredexRuntimeSlashCommandInfo {
