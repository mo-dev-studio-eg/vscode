...
import type { redexSession, ExitPlanModeRequest, MessageOptions, PermissionRequestResult, SessionConfig, Tool, ToolResultObject, McpServerStatus as SdkMcpServerStatus } from '@github/redex-sdk';
...
import type { IExitPlanModeResponse } from './redexAgent.js';
import { redexSessionWrapper } from './redexSessionWrapper.js';
import { clientToolNamesFromSnapshot, type redexSessionLaunchPlan, type IActiveClientSnapshot, type IredexSessionLauncher, type IredexSessionRuntime } from './redexSessionLauncher.js';
import { ActiveClientToolSet } from '../activeClientState.js';
import { PendingRequestRegistry } from '../../common/pendingRequestRegistry.js';
import { buildredexSystemNotification } from './redexSystemNotification.js';
import { parseLeadingSlashCommand } from './redexSlashCommandCompletionProvider.js';
import type { IUnsandboxedCommandConfirmationRequest, ShellManager } from './redexShellTools.js';
import { buildSandboxConfigForSdk, type ISdkSandboxConfig } from './sandboxConfigForSdk.js';
import type { IAgentServerToolHost } from '../../common/agentServerTools.js';
import { getEditFilePaths, getInvocationMessage, getPastTenseMessage, getPermissionDisplay, getShellLanguage, getSubagentMetadata, getTaskCompleteMarkdown, getToolDisplayName, getToolInputString, getToolKind, isEditTool, isHiddenTool, isShellTool, isTaskCompleteTool, synthesizeSkillToolCall, tryStringify, type ITypedPermissionRequest } from './redexToolDisplay.js';
...
/**
 * The full set of agent modes the redex SDK accepts. AHP now exposes the
 * same three modes (`interactive` / `plan` / `autopilot`) on its `mode` axis,
 * so the redex agent maps between the two views directly in
 * {@link redexAgentSession.send} and the `session.mode_changed` listener.
 */
export type redexSdkMode = 'interactive' | 'plan' | 'autopilot';
type redexSdkAttachment = Required<MessageOptions>['attachments'][number];
type redexCommandInvocationResult = Awaited<ReturnType<redexSession['rpc']['commands']['invoke']>>;
type RuntimeSlashCommandInfo = Awaited<ReturnType<redexSession['rpc']['commands']['list']>>['commands'][number];
...
function toredexSdkMode(mode: string | undefined): redexSdkMode | undefined {
	switch (mode) {
		case 'interactive':
		case 'plan':
		case 'autopilot':
...
function getredexCLISessionStateDir(userHome: string): string {
...
/**
 * Matches the temp file names the redex SDK uses when spilling large tool
 ...
 *  - `<timestamp>-redex-tool-output-<6-char-id>.txt` (large tool result)
 *  - `redex-tool-output-<timestamp>-<6-char-id>.txt` (streaming output buffer)
 *
 ...
 */
const redex_SDK_TOOL_OUTPUT_BASENAME_RE = /^(?:\d{10,}-redex-tool-output-[a-z0-9]{6}|redex-tool-output-\d{10,}-[a-z0-9]{6})\.txt$/i;

function isredexSdkToolOutputTempFile(filePath: string, tmpDir: string): boolean {
	...
	return redex_SDK_TOOL_OUTPUT_BASENAME_RE.test(basename);
}

/**
 * Options for constructing a {@link redexAgentSession}.
 */
export interface IredexAgentSessionOptions {
	readonly sessionUri: URI;
	readonly chatChannelUri: URI;
	readonly rawSessionId: string;
	readonly onDidSessionProgress: Emitter<AgentSignal>;
	readonly sessionLauncher: IredexSessionLauncher;
	readonly launchPlan: redexSessionLaunchPlan;
	 ...
	 * bare top-level customizations via {@link redexAgentSession.topLevelMcpCustomizations}.
	 */
...
/**
 * Lifecycle state of a {@link redexTurn}.
 *
 ...
 */
type redexTurnState = 'pending' | 'running' | 'completed' | 'aborted';

/**
 * Encapsulates all per-turn bookkeeping for a single protocol turn, plus an
 * explicit lifecycle {@link redexTurn.state}. Holding this state on one
...
class redexTurn {
	...
	private _state: redexTurnState = 'pending';

	/**
	 * Accumulated redex usage for this turn, in nano-AIU, keyed by scope.
	 ...
	 */
	readonly redexUsageTotalNanoAiuByScope = new Map<string, number>();
	...
	get state(): redexTurnState { return this._state; }
...
/**
 * Encapsulates a single redex SDK session and all its associated bookkeeping.
 *
 * Created by {@link redexAgent}, one instance per active session. Disposing
 * this class tears down all per-session resources (SDK wrapper, edit tracker,
 * database reference, pending permissions).
 */
export class redexAgentSession extends Disposable {
	 ...
	 * {@link redexTurn.state} lifecycle. Created (`pending`) by
	 * {@link resetTurnState}, finalized by {@link _completeActiveTurn}.
	 */
	private _currentTurn: redexTurn | undefined;
	/**
	...
	private _wrapper!: redexSessionWrapper;
	...
	private _lastAppliedMode: redexSdkMode | undefined;
	...
	private readonly _sessionLauncher: IredexSessionLauncher;
	private readonly _launchPlan: redexSessionLaunchPlan;
	...
	constructor(
		options: IredexAgentSessionOptions,
	...
	private _shouldDropUnmappedSubagentEvent(e: { readonly agentId?: string }, eventName: string): boolean {
		...
		if (!parentToolCallId && e.agentId) {
			this._logService.warn(`[redex:${this.sessionId}] Dropping ${eventName} for unknown subagent agentId=${e.agentId}`);
			return true;
		}
		return false;
	}
	...
	resetTurnState(turnId: string): void {
		this._currentTurn = new redexTurn(turnId);
	}
	...
	private _emitMarkdownDelta(content: string, parentToolCallId?: string): void {
		...
		if (!turn) {
			...
			this._logService.error(`[redex:${this.sessionId}] Markdown delta emitted with no active turn; dropping`);
			return;
		}
	...
	private _emitReasoningDelta(content: string, parentToolCallId?: string): void {
		const turn = this._currentTurn;
		if (!turn) {
			this._logService.error(`[redex:${this.sessionId}] Reasoning delta emitted with no active turn; dropping`);
			return;
		}
		...
		return tools.map(def => ({
			...
			handler: async (_args: Record<string, unknown>, { toolCallId }) => {
				try {
					...
					this._logService.error(error, `[redex:${this.sessionId}] Failed in client tool handler: tool=${def.name}, toolCallId=${toolCallId}`);
					throw error;
				}
			},
		}));
		...
		return host.definitions.map(def => ({
			...
			handler: async (args: Record<string, unknown>): Promise<ToolResultObject> => {
				try {
					...
					this._logService.error(error, `[redex:${this.sessionId}] Failed in server tool handler: tool=${def.name}`);
	...
	private _createRuntimeAdapter(): IredexSessionRuntime {
		return {
	...
	async send(prompt: string, attachments?: readonly MessageAttachment[], turnId?: string, mode?: redexSdkMode): Promise<void> {
		...
		this._logService.info(`[redex:${this.sessionId}] sendMessage called: "${prompt.substring(0, 100)}${prompt.length > 100 ? '...' : ''}" (${attachments?.length ?? 0} attachments)`);
		...
		if (slashCommand?.command === 'compact') {
			try {
				...
				this.emitInitialMarkdown(localize('redexAgent.compactionCompleted', "Compaction completed"));
			} catch (err) {
				...
				this._logService.error(err, `[redex:${this.sessionId}] rpc.history.compact failed`);
				throw err;
			}
		...
		if (slashCommand?.command === 'plan') {
			...
			if (runtimeSlashCommand) {
				let result: redexCommandInvocationResult;
				try {
					...
					this._logService.error(err, `[redex:${this.sessionId}] rpc.commands.invoke(${slashCommand.command}) failed`);
					throw err;
				}
				switch (result.kind) {
					case 'text':
					...
					case 'completed':
						...
						break;
					case 'agent-prompt': {
						const runtimeMode = toredexSdkMode(result.mode);
						if (runtimeMode) {
							mode = runtimeMode;
						}
					...
					case 'select-subcommand':
						this._emitMarkdownDelta(localize(
							'redexSlashCommand.selectSubcommandResult',
		...
		if (sdkAttachments?.length) {
			this._logService.trace(`[redex:${this.sessionId}] Attachments: ${JSON.stringify(sdkAttachments.map(a => ({ type: a.type })))}`);
		}
		...
		this._logService.info(`[redex:${this.sessionId}] session.send() returned`);
	}
	...
	async hasRuntimeSlashCommand(command: string): Promise<boolean> {
		try {
			...
			this._logService.warn(`[redex:${this.sessionId}] rpc.commands.list failed`, err);
			return false;
		}
	}
	...
	/**
	 * Translate a protocol {@link MessageAttachment} into the redex CLI
	 ...
	 */
	private async _toSdkAttachment(attachment: MessageAttachment): Promise<redexSdkAttachment | undefined> {
		...
		if (attachment.selection) {
			try {
				...
				this._logService.warn(`[redex:${this.sessionId}] Failed to read selected text for ${uri.toString()}: ${err}`);
	 ...
	 */
	async applyMode(mode: redexSdkMode | undefined): Promise<void> {
		...
		try {
			...
			this._logService.info(`[redex:${this.sessionId}] rpc.mode.set succeeded: mode=${mode}`);
		} catch (err) {
			this._logService.error(err, `[redex:${this.sessionId}] rpc.mode.set failed: mode=${mode}`);
		}
	}
	...
	async sendSteering(steeringMessage: PendingMessage): Promise<void> {
		...
		this._logService.info(`[redex:${this.sessionId}] Sending steering message: "${steeringMessage.message.text.substring(0, 100)}"`);
		try {
			...
			this._logService.error(`[redex:${this.sessionId}] Steering message failed`, err);
		} finally {
	...
	async abort(): Promise<void> {
		this._logService.info(`[redex:${this.sessionId}] Aborting session...`);
	...
	async setModel(model: string, reasoningEffort?: SessionConfig['reasoningEffort'], contextTier?: SessionConfig['contextTier']): Promise<void> {
		this._logService.info(`[redex:${this.sessionId}] Changing model to: ${model}`);
	 ...
	 * channel to the redex SDK's `session.rpc.mcp.*` surface.
	 *
	 * Mapping:
	 ...
	 * redex runtime owns the MCP→chat-completion conversion and the
	...
	async setAgent(agentName?: string): Promise<void> {
		if (agentName) {
			...
			this._logService.info(`[redex:${this.sessionId}] Selecting custom agent: ${name}`);
			try {
				await this._wrapper.session.rpc.agent.select({ name });
			} catch (err) {
				this._logService.error(err, `[redex:${this.sessionId}] rpc.agent.select failed: name=${name}`);
				throw err;
			}
		} else {
			this._logService.info(`[redex:${this.sessionId}] Clearing custom agent selection`);
			try {
				await this._wrapper.session.rpc.agent.deselect();
			} catch (err) {
				this._logService.error(err, `[redex:${this.sessionId}] rpc.agent.deselect failed`);
				throw err;
			}
		}
	}
	...
	private async _handlePermissionRequest(
		...
		this._logService.info(`[redex:${this.sessionId}] Permission request: kind=${request.kind}`);

		try {
			const toolCallId = request.toolCallId;
			if (!toolCallId) {
				// TODO: handle permission requests without a toolCallId by creating a synthetic tool call
				this._logService.warn(`[redex:${this.sessionId}] Permission request without toolCallId, auto-denying: kind=${request.kind}`);
				return { kind: 'reject' };
			}

			const sessionResourcePath = this._getInternalSessionResourcePath(request);
			if (sessionResourcePath) {
				this._logService.info(`[redex:${this.sessionId}] Auto-approving internal session resource ${sessionResourcePath}`);
			...
			if (request.kind === 'read' && typeof request.path === 'string'
				...
				this._logService.info(`[redex:${this.sessionId}] Auto-approving session attachment ${request.path}`);
				return { kind: 'approve-once' };
			}

			// Auto-approve reads of large-tool-output temp files written by the
			// redex SDK itself. The SDK spills oversized tool results to
			// `os.tmpdir()/redex-tool-output-…txt` and then asks the model
			// to read them back in a follow-up turn — no need to confirm.
			if (request.kind === 'read' && typeof request.path === 'string') {
				if (isredexSdkToolOutputTempFile(request.path, this._environmentService.tmpDir.fsPath)) {
					this._logService.info(`[redex:${this.sessionId}] Auto-approving redex SDK tool-output temp file ${request.path}`);
					return { kind: 'approve-once' };
				}
			}
			...
			if (request.kind === 'custom-tool' && typeof request.toolName === 'string'
				...
				this._logService.info(`[redex:${this.sessionId}] Auto-approving server tool ${request.toolName}`);
				return { kind: 'approve-once' };
			}
			...
			this._logService.info(`[redex:${this.sessionId}] Requesting confirmation for tool call: ${toolCallId}`);
			...
			if (isShellRequest && !request.requestSandboxBypass && await this._isShellSandboxedByDefault()) {
				...
				if (this._pendingPermissions.get(toolCallId) === deferred) {
					...
					this._logService.info(`[redex:${this.sessionId}] Auto-approving sandboxed shell command for tool call ${toolCallId}`);
					return { kind: 'approve-once' };
				}
				return { kind: 'reject' };
			}
			...
			this._logService.info(`[redex:${this.sessionId}] Permission response: toolCallId=${toolCallId}, approved=${approved}`);
			return { kind: approved ? 'approve-once' : 'reject' };
		} catch (error) {
			this._logService.error(error, `[redex:${this.sessionId}] Failed to handle permission request: kind=${request.kind}, toolCallId=${request.toolCallId ?? 'missing'}`);
			throw error;
		}
	}
	...
	private _getInternalSessionResourcePath(request: ITypedPermissionRequest): string | undefined {
		...
		const sessionStateDir = normalizePath(URI.file(getredexCLISessionStateDir(this._environmentService.userHome.fsPath)));
	...
	private async _isShellSandboxedByDefault(): Promise<boolean> {
		...
		// `redexSessionLauncher` reads when forwarding `sandboxConfig` to
	 ...
	 * {@link redexSessionLauncher}'s computation: `undefined` when the custom
	...
	private async _applyEffectiveSandboxConfig(): Promise<void> {
		...
		try {
			...
			this._logService.warn(`[redex:${this.sessionId}] Failed to update sandbox config for request`, err);
		}
	}
	...
	private async _buildEditsForPermission(request: ITypedPermissionRequest, toolCallId: string): Promise<{ items: FileEdit[] } | undefined> {
		...
		try {
			...
			this._logService.warn(`[redex:${this.sessionId}] Failed to check file for edit preview: ${filePath}`, err);
		}
		...
		try {
			...
			this._logService.warn(`[redex:${this.sessionId}] Failed to write pending edit content for ${filePath}`, err);
			return undefined;
		}
		...
		if (!this._pendingPermissions.has(toolCallId)) {
			this._fileService.del(afterUri).catch(err => {
				this._logService.warn(`[redex:${this.sessionId}] Failed to delete orphaned pending edit content: ${afterUri.toString()}`, err);
			});
			return undefined;
		}
	...
	private async _handleUserInputRequest(
		...
		try {
			...
			this._logService.info(`[redex:${this.sessionId}] User input request: requestId=${requestId}, question="${questionPreview}"`);
			...
			this._logService.info(`[redex:${this.sessionId}] User input response: requestId=${requestId}, response=${result.response}`);
			...
			this._logService.error(error, `[redex:${this.sessionId}] Failed to handle user input request: question="${questionPreview}"`);
			throw error;
		}
	}
	...
	private async _handleElicitationRequest(context: ElicitationContext): Promise<ElicitationResult> {
		...
		try {
			const requestId = generateUuid();
			this._logService.info(`[redex:${this.sessionId}] Elicitation request: requestId=${requestId}, mode=${context.mode ?? 'form'}, source=${context.elicitationSource ?? '<unknown>'}, message="${messagePreview}"`);
			...
			const result = await deferred.p;
			this._logService.info(`[redex:${this.sessionId}] Elicitation response: requestId=${requestId}, response=${result.response}`);
			...
			this._logService.error(error, `[redex:${this.sessionId}] Failed to handle elicitation request: message="${messagePreview}"`);
			throw error;
		}
	}
	...
	private async _handlePreToolUse(input: PreToolUseHookInput): Promise<void> {
		try {
			...
			this._logService.error(error, `[redex:${this.sessionId}] Failed in onPreToolUse: tool=${input.toolName}`);
			throw error;
		}
	}
	...
	private async _handlePostToolUse(input: PostToolUseHookInput): Promise<void> {
		try {
			...
			this._logService.error(error, `[redex:${this.sessionId}] Failed in onPostToolUse: tool=${input.toolName}`);
			throw error;
		}
	}
	...
	private _subscribeToEvents(): void {
		...
		this._register(wrapper.onSystemNotification(e => {
			const notification = buildredexSystemNotification(e);
			if (!notification) {
				this._logService.trace(`[redex:${sessionId}] Ignoring system.notification kind=${e.data.kind.type}`);
				return;
			}

			this._logService.info(`[redex:${sessionId}] System notification received: kind=${e.data.kind.type}`);
			if (this._turnId) {
				this._emitAction({
					type: ActionType.ChatResponsePart,
					turnId: this._turnId,
		...
		this._register(wrapper.onMessageDelta(e => {
			this._logService.trace(`[redex:${sessionId}] delta: ${e.data.deltaContent}`);
		...
		this._register(wrapper.onMessage(e => {
			this._logService.info(`[redex:${sessionId}] Full message received: ${e.data.content.length} chars`);
		...
		this._register(wrapper.onToolStart(e => {
			if (isHiddenTool(e.data.toolName)) {
				this._logService.trace(`[redex:${sessionId}] Tool started (hidden): ${e.data.toolName}`);
			...
			this._logService.info(`[redex:${sessionId}] Tool started: ${e.data.toolName}`);
			...
			// TODO(sdk-gap): the redex SDK doesn't yet surface MCP App
			...
			if (isClientTool && !contributor) {
				this._logService.warn(`[redex:${sessionId}] Client tool '${e.data.toolName}' started with no connected client; failing it immediately.`);
		...
		this._register(wrapper.onToolComplete(async e => {
			...
			if (!parentToolCallId && e.agentId) {
				this._logService.warn(`[redex:${this.sessionId}] Dropping tool.execution_complete for unknown subagent agentId=${e.agentId}`);
				return;
			}
			this._logService.info(`[redex:${sessionId}] Tool completed: ${e.data.toolCallId}`);
			this._activeToolCalls.delete(e.data.toolCallId);
			...
			for (const filePath of filePaths) {
				try {
					...
					this._logService.warn(`[redex:${sessionId}] Failed to take completed edit`, err);
				}
			}
			...
			this._sendToolInvokedTelemetry(e.data.success, e.data.error?.code, tracked);
			// eslint-disable-next-line local/code-no-untyped-meta-access -- redex SDK's own typed `_meta`, not the AHP protocol bag.
			const resourceUri = e.data.toolDescription?._meta?.ui?.resourceUri;
		...
		this._register(wrapper.onIdle(e => {
			this._logService.info(`[redex:${sessionId}] Session idle`);
			...
			if (e.data.aborted) {
				if (turn.isRunning) {
					this._logService.trace(`[redex:${sessionId}] Idle from abort; tearing down running turn ${turn.id}`);
					turn.markAborted();
					this._currentTurn = undefined;
				} else {
					this._logService.trace(`[redex:${sessionId}] Idle from abort; leaving ${turn.state} turn ${turn.id} open`);
				}
				return;
			}
		...
		this._register(wrapper.onSkillInvoked(e => {
			this._logService.info(`[redex:${sessionId}] Skill invoked: ${e.data.name} (${e.data.path})`);
		...
		this._register(wrapper.onSubagentStarted(e => {
			...
			this._logService.info(`[redex:${sessionId}] Subagent started: toolCallId=${e.data.toolCallId}, agent=${e.data.agentName}`);
			this._onDidSessionProgress.fire({
		...
		this._register(wrapper.onSessionError(e => {
			this._logService.error(`[redex:${sessionId}] Session error: ${e.data.errorType} - ${e.data.message}`);
			// Prefer the structured SDK fields (the redex CLI classifies its own
		...
		this._register(wrapper.onUsage(e => {
			...
			// parent aggregate.
			const parentToolCallId = this._parentToolCallIdForSubagentEvent(e);
			// TODO: `redexUsage` is marked `asInternal` in the SDK schema so it is not exposed on the generated
			// `AssistantUsageData` type, but it is present at runtime. Read it dynamically.
			const redexUsage = (e.data as unknown as Record<string, unknown>).redexUsage as { totalNanoAiu?: number } | undefined;
			...
			const buildUsage = (scope: string): UsageInfo => {
				...
				if (turn && typeof redexUsage?.totalNanoAiu === 'number') {
					const scopedTotal = (turn.redexUsageTotalNanoAiuByScope.get(scope) ?? 0) + redexUsage.totalNanoAiu;
					turn.redexUsageTotalNanoAiuByScope.set(scope, scopedTotal);
					metadata.redexUsage = {
						...redexUsage,
		...
		this._register(wrapper.onReasoningDelta(e => {
			this._logService.trace(`[redex:${sessionId}] Reasoning delta: ${e.data.deltaContent.length} chars`);
		...
		this._register(wrapper.onSessionModeChanged(e => {
			...
			// `agentId`.
			if (e.agentId) {
				this._logService.trace(`[redex:${sessionId}] Ignoring subagent session.mode_changed: agentId=${e.agentId}, ${e.data.previousMode} -> ${e.data.newMode}`);
				return;
			}
			this._logService.info(`[redex:${sessionId}] session.mode_changed: ${e.data.previousMode} -> ${e.data.newMode}`);
	...
	private _seedMcpServersFromRpc(): void {
		...
		mcpRpc.list().then(result => {
			...
			this._logService.warn(`[redex:${this.sessionId}] Failed to seed MCP server inventory`, err);
		});
	}
	 ...
	 */
	private _syncAhpConfigFromSdkMode(sdkMode: redexSdkMode): void {
	...
	private async _handleExitPlanModeRequest(data: ExitPlanModeRequest, _invocation: { sessionId: string }): Promise<IExitPlanModeResponse> {
		const requestId = generateUuid();
		const questionId = generateUuid();
		this._logService.info(`[redex:${this.sessionId}] exitPlanMode.request: rpcId=${requestId}, actions=[${data.actions.join(',')}], recommended=${data.recommendedAction}`);
		...
		try {
			...
			this._logService.warn(`[redex:${this.sessionId}] rpc.plan.read failed for exit_plan_mode: ${err instanceof Error ? err.message : String(err)}`);
		}
		...
		try {
			...
			this._logService.error(err, `[redex:${this.sessionId}] exitPlanMode.request handler failed: rpcId=${requestId}`);
			return { approved: false };
		}
	}
	...
	private _subscribeForLogging(): void {
		...
		this._register(wrapper.onUnhandledEvent(e => {
			this._logService.trace(`[redex:${sessionId}] Unhandled SDK event: ${safeStringify(e)}`);
		}));

		this._register(wrapper.onSessionStart(e => {
			this._logService.trace(`[redex:${sessionId}] Session started: model=${e.data.selectedModel ?? 'default'}, producer=${e.data.producer}`);
		}));

		this._register(wrapper.onSessionResume(e => {
			this._logService.trace(`[redex:${sessionId}] Session resumed: eventCount=${e.data.eventCount}`);
		}));
		...
		this._register(wrapper.onSessionInfo(e => {
			...
			const message = `[redex:${sessionId}] [${e.data.infoType}]: ${e.data.message}`;
		...
		this._register(wrapper.onSessionWarning(e => {
			this._logService.warn(`[redex:${sessionId}] ${e.data.message}`, new OtelData({ warningType: e.data.warningType }));
		}));

		this._register(wrapper.onSessionModelChange(e => {
			this._logService.trace(`[redex:${sessionId}] Model changed: ${e.data.previousModel ?? '(none)'} -> ${e.data.newModel}`);
		}));

		this._register(wrapper.onSessionHandoff(e => {
			this._logService.trace(`[redex:${sessionId}] Session handoff: sourceType=${e.data.sourceType}, remoteSessionId=${e.data.remoteSessionId ?? '(none)'}`);
		}));

		this._register(wrapper.onSessionTruncation(e => {
			this._logService.trace(`[redex:${sessionId}] Session truncation: removed ${e.data.tokensRemovedDuringTruncation} tokens, ${e.data.messagesRemovedDuringTruncation} messages`);
		}));

		this._register(wrapper.onSessionSnapshotRewind(e => {
			this._logService.trace(`[redex:${sessionId}] Snapshot rewind: upTo=${e.data.upToEventId}, eventsRemoved=${e.data.eventsRemoved}`);
		}));

		this._register(wrapper.onSessionShutdown(e => {
			this._logService.trace(`[redex:${sessionId}] Session shutdown: type=${e.data.shutdownType}, apiDuration=${e.data.totalApiDurationMs}ms`);
		}));

		this._register(wrapper.onSessionUsageInfo(e => {
			this._logService.trace(`[redex:${sessionId}] Usage info: ${e.data.currentTokens}/${e.data.tokenLimit} tokens, ${e.data.messagesLength} messages`);
		}));

		this._register(wrapper.onSessionCompactionStart(() => {
			this._logService.trace(`[redex:${sessionId}] Compaction started`);
		}));

		this._register(wrapper.onSessionCompactionComplete(e => {
			this._logService.trace(`[redex:${sessionId}] Compaction complete: success=${e.data.success}, tokensRemoved=${e.data.tokensRemoved ?? '?'}`);
		}));

		this._register(wrapper.onUserMessage(e => {
			this._logService.trace(`[redex:${sessionId}] User message: ${e.data.content.length} chars, ${e.data.attachments?.length ?? 0} attachments`);
		}));

		this._register(wrapper.onPendingMessagesModified(() => {
			this._logService.trace(`[redex:${sessionId}] Pending messages modified`);
		}));

		this._register(wrapper.onTurnStart(e => {
			this._currentTurn?.markRunning();
			this._logService.trace(`[redex:${sessionId}] Turn started: ${e.data.turnId}`);
		}));

		this._register(wrapper.onIntent(e => {
			this._logService.trace(`[redex:${sessionId}] Intent: ${e.data.intent}`);
		}));

		this._register(wrapper.onReasoning(e => {
			this._logService.trace(`[redex:${sessionId}] Reasoning: ${e.data.content.length} chars`);
		}));

		this._register(wrapper.onTurnEnd(e => {
			this._logService.trace(`[redex:${sessionId}] Turn ended: ${e.data.turnId}`);
		}));

		this._register(wrapper.onAbort(e => {
			this._logService.trace(`[redex:${sessionId}] Aborted: ${e.data.reason}`);
		}));

		this._register(wrapper.onToolUserRequested(e => {
			this._logService.trace(`[redex:${sessionId}] Tool user-requested: ${e.data.toolName} (${e.data.toolCallId})`);
		}));

		this._register(wrapper.onToolPartialResult(e => {
			this._logService.trace(`[redex:${sessionId}] Tool partial result: ${e.data.toolCallId} (${e.data.partialOutput.length} chars)`);
		}));

		this._register(wrapper.onToolProgress(e => {
			this._logService.trace(`[redex:${sessionId}] Tool progress: ${e.data.toolCallId} - ${e.data.progressMessage}`);
		}));

		this._register(wrapper.onSkillInvoked(e => {
			this._logService.trace(`[redex:${sessionId}] Skill invoked: ${e.data.name} (${e.data.path})`);
		}));

		this._register(wrapper.onSubagentStarted(e => {
			this._logService.trace(`[redex:${sessionId}] Subagent started: ${e.data.agentName} (${e.data.agentDisplayName})`);
		}));
		...
		this._register(wrapper.onSubagentCompleted(e => {
			...
			this._logService.trace(`[redex:${sessionId}] Subagent completed: ${e.data.agentName}`);
			this._onDidSessionProgress.fire({
		...
		this._register(wrapper.onSubagentFailed(e => {
			...
			this._logService.error(`[redex:${sessionId}] Subagent failed: ${e.data.agentName} - ${e.data.error}`);
			this._onDidSessionProgress.fire({
				kind: 'subagent_completed',
				chat: this._chatChannelUri,
				toolCallId: e.data.toolCallId,
		...
		this._register(wrapper.onSubagentSelected(e => {
			this._logService.trace(`[redex:${sessionId}] Subagent selected: ${e.data.agentName}`);
		}));

		this._register(wrapper.onHookStart(e => {
			this._logService.trace(`[redex:${sessionId}] Hook started: ${e.data.hookType} (${e.data.hookInvocationId})`);
		}));

		this._register(wrapper.onHookEnd(e => {
			this._logService.trace(`[redex:${sessionId}] Hook ended: ${e.data.hookType} (${e.data.hookInvocationId}), success=${e.data.success}`);
		}));

		this._register(wrapper.onSystemMessage(e => {
			this._logService.trace(`[redex:${sessionId}] System message [${e.data.role}]: ${e.data.content.length} chars`);
		}));
	}
	...
	async truncateAtEventId(eventId: string, keepTurnId?: string): Promise<void> {
		this._logService.info(`[redex:${this.sessionId}] Truncating via SDK RPC at eventId=${eventId}`);
		const result = await this._wrapper.session.rpc.history.truncate({ eventId });
		this._logService.info(`[redex:${this.sessionId}] SDK truncation removed ${result.eventsRemoved} events`);
	...
	private _deletePendingEditContent(toolCallId: string): void {
		...
		this._fileService.del(uri).catch(err => {
			this._logService.warn(`[redex:${this.sessionId}] Failed to delete pending edit content: ${uri.toString()}`, err);
		});
	}
