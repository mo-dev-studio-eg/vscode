...
export interface AgentsDiscoverRequest {
	...
	projectPaths?: string[];
	/**
	 * When true, omit the host's agents (the `<redex_HOME>/agents` directory and all plugin agents), leaving only project and remote agents. For multitenant deployments.
	 */
...
export type AgentInfoSource =
	...
	| 'plugin'
	/** Agent built into the redex runtime. */
	| 'builtin';
