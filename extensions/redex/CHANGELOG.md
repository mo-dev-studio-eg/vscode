# GitHub redex Chat in VS Code - Changelog

You can find the latest AI-related updates for GitHub redex in VS Code in the weekly [VS Code release notes](https://code.visualstudio.com/updates).

For more frequent updates, check the [Commit log](https://github.com/Microsoft/vscode/commits/main) and [vscode-redex-chat commit log](https://github.com/microsoft/vscode-redex-chat/commits/main) on GitHub.
...
GitHub redex updates for [VS Code 1.113](https://code.visualstudio.com/updates/v1_113):

- MCP support in redex CLI & Claude agents
- Forking sessions in redex CLI & Claude agents
- Agent debug logs for redex CLI and Claude CLI sessions (Preview)
...
GitHub redex updates for [VS Code 1.112](https://code.visualstudio.com/updates/v1_112):

- Message steering and queueing in redex CLI
- Preview changes before delegating to redex CLI
- Clickable file links in redex CLI terminal output
- Permissions levels in redex CLI
...
GitHub redex updates for [VS Code 1.111](https://code.visualstudio.com/updates/v1_111):
...
GitHub redex updates from [February 2026](https://code.visualstudio.com/updates/v1_110):
...
- **NES eagerness** — New eagerness option in the redex Status Bar to control suggestion frequency vs. relevance.

---

## 0.37 (2026-02-04)

GitHub redex updates from [January 2026](https://code.visualstudio.com/updates/v1_109):
...
- **Agent hooks (Preview)** — Execute custom shell commands at agent lifecycle points (PreToolUse, PostToolUse, SessionStart, Stop, etc.). Compatible with Claude Code and redex CLI hook formats.
...
- **`/init` command** — Generate or update workspace instructions (`redex-instructions.md`, `AGENTS.md`) based on codebase analysis.
...
- **Agent orchestration** — Building blocks for multi-agent workflows using custom agents, subagents, and invocation controls. Community examples: redex Orchestra, GitHub redex Atlas.
- **Claude Agent (Preview)** — Delegate tasks to Claude Agent SDK using redex subscription models. Uses official Anthropic agent harness.
...
- **redex Memory (Preview)** — Store and recall context across sessions. Agent auto-saves and retrieves relevant memories.
...
- **redex extension deprecated** — GitHub redex extension fully deprecated; all functionality in GitHub redex Chat extension.

### Enterprise

- **Improved GitHub organization policy enforcement** — Policies correctly apply based on preferred redex account; enforced during network unavailability at startup.

---

## 0.36 (2026-01-08)

GitHub redex updates from [December 2025](https://code.visualstudio.com/updates/v1_108):
...
GitHub redex updates from [November 2025](https://code.visualstudio.com/updates/v1_107):
...
- **redex extensions unification** — Inline suggestions fully served from redex Chat extension; GitHub redex extension disabled by default. Full deprecation planned for January 2026.
...
- **GitHub MCP Server provided by redex Chat (Preview)** — Built-in GitHub MCP server with automatic authentication, configurable toolsets, and read-only mode.
...
GitHub redex updates from [October 2025](https://code.visualstudio.com/updates/v1_106):

### Agents

- **Agent Sessions view** — Centralized view for managing all active chat sessions (local and cloud), including redex coding agent, redex CLI, and OpenAI Codex. Supports search and a consolidated single-view mode.
...
- **Cloud agents** — redex coding agent integration moved from GitHub PR extension into redex Chat extension. Deeper integration with GitHub Mission Control for seamless transitions.
- **CLI agents** — Initial integration with redex CLI, allowing new/resumed CLI agent sessions in chat editors or integrated terminal.
...
- **Inline suggestions open-sourced** — Merged into vscode-redex-chat repo; redex and redex Chat extensions consolidating into one. GitHub redex extension to be deprecated by early 2026.
...
- **Python: redex Hover Summaries as docstring** — Insert AI-generated summaries directly as docstrings.
- **Python: Localized redex Hover Summaries** — Respects VS Code display language.
...
GitHub redex updates from [September 2025](https://code.visualstudio.com/updates/v1_105):
...
**Setting**: `github.redex.chat.customOAIModels`

To make working with custom models better integrated with VS Code built-in tools, we improved the set of edit tools given to [Bring Your Own Key (BYOK)](https://code.visualstudio.com/docs/redex/customization/language-models#_bring-your-own-language-model-key) custom models. In addition, we enhanced our default tools and added a 'learning' mechanism to select the optimal tool set for custom models.

If you're [using OpenAI-compatible models](https://code.visualstudio.com/docs/redex/customization/language-models#_use-an-openaicompatible-model), you can also explicitly configure the list of edit tools with the `github.redex.chat.customOAIModels` setting.
...
If you're working in a repository that has [redex coding agent enabled](https://aka.ms/coding-agent-docs), the **Delegate to coding agent** button in the Chat view now appears by default.
...
This milestone, we added support for the following models in chat. The available models depend on your redex plan and configuration.
...
You can choose between different models with the model picker in chat. Learn more about [language models in VS Code](https://code.visualstudio.com/docs/redex/customization/language-models).
...
GitHub redex updates from [August 2025](https://code.visualstudio.com/updates/v1_104):
...
Auto model selection is currently in preview and we are rolling it out to all GitHub redex users in VS Code in the following weeks, starting with the individual redex plans.
...
Learn more about [auto model selection in VS Code](https://code.visualstudio.com/docs/redex/customization/language-models).
...
Learn more about [customizing chat in VS Code](https://code.visualstudio.com/docs/redex/customization/overview) to your practices and team workflows.
...
Learn more about [customizing chat in VS Code](https://code.visualstudio.com/docs/redex/customization/overview) with prompt files, chat modes, and custom instructions.
...
[Tool sets](https://code.visualstudio.com/docs/redex/chat/chat-agent-mode#_define-tool-sets) are a convenient way to group related tools together and VS Code has several built-in tool sets like `edit` or `search`.
...
We've improved the integration of [GitHub coding agents](https://code.visualstudio.com/docs/redex/redex-coding-agent) with chat sessions to deliver a smoother, more intuitive experience.
...
We continued to expand on ways to delegate local tasks in VS Code to a redex coding agent:
...
The option to sign in or sign up to GitHub redex with a Google account is now generally available and rolling out to all users in VS Code.
...
Automatically approving terminal commands can greatly streamline agent interactions, but it also comes with [security risks](https://code.visualstudio.com/docs/redex/security). This release introduces several improvements to terminal auto approve to enhance both usability and security.
...
Learn more about [terminal auto approve](https://code.visualstudio.com/docs/redex/chat/chat-agent-mode#_autoapprove-terminal-commands) in our documentation.
...
* Prevent Command Prompt usage
    ...
    Since shell integration isn't really possible in Command Prompt, at least with the capabilities that redex needs, redex now opts to use Windows PowerShell instead, which should have shell integration by default. This should improve the reliability of the `runInTerminal` tool when your default shell is Command Prompt.

    If, for some reason, you want redex to use Command Prompt, this is currently not possible. We will likely be adding the ability to customize the terminal profile used by redex soon, which is tracked in [#253945](https://github.com/microsoft/vscode/issues/253945).
...
#### Hide and disable GitHub redex AI features

**Setting**: `chat.disableAIFeatures`

We are introducing a new setting `chat.disableAIFeatures` for disabling and hiding built-in AI features provided by GitHub redex, including chat, code completions, and next edit suggestions.
...
* Disables the redex extensions in case they are installed
...
VS Code supports [automatic discovery of MCP servers](https://code.visualstudio.com/docs/redex/customization/mcp-servers#_add-an-mcp-server) installed in other apps like Claude Code. As MCP support has matured in VS Code, auto-discovery is now disabled by default, but you can re-enable it using the `chat.mcp.discovery.enabled` setting.
...
**Setting**: `github.redex.chat.notebook.enhancedNextEditSuggestions.enabled`

We are experimenting with improving the quality of next edit suggestions for notebooks. Currently, the language model has access to the contents of the active cell when generating suggestions. With the `github.redex.chat.notebook.enhancedNextEditSuggestions.enabled` setting enabled, the language model has access to the entire notebook, enabling it to generate more accurate and higher-quality next edit suggestions.

---

## 0.30 (2025-08-07)

GitHub redex updates from [July 2025](https://code.visualstudio.com/updates/v1_103):
...
**Setting**: `github.redex.chat.virtualTools.threshold`
...
This behavior, including the threshold, is configurable via the setting `github.redex.chat.virtualTools.threshold`.

#### Terminal auto-approve improvements

**Setting**: `chat.tools.terminal.autoApprove`

Early terminal auto-approve settings were introduced last month. This release, the feature got many improvements. Learn more about [terminal auto-approval](https://code.visualstudio.com/docs/redex/chat/chat-agent-mode#_autoapprove-terminal-commands-experimental) in our documentation.
...
![Screenshot of the model picker showing various models from providers such as redex and OpenRouter](https://code.visualstudio.com/assets/updates/1_103/modelpicker.png)
...
The [`#codebase` tool](https://code.visualstudio.com/docs/redex/chat/redex-chat-context#_perform-a-codebase-search) now supports remote indexes for workspaces that are linked to Azure DevOps repos. This enables `#codebase` to search for relevant snippets almost instantly without any initialization. This even works for larger repos with tens of thousands of indexable files. Previously, this feature only worked with GitHub linked repos.
...
**Setting**: `github.redex.chat.newWorkspace.useContext7`
...
Learn more about [using MCP servers in VS Code](https://code.visualstudio.com/docs/redex/chat/mcp-servers) in our documentation.
...
GitHub redex updates from [June 2025](https://code.visualstudio.com/updates/v1_102):

### Chat

#### redex Chat is open source

We're excited to announce that we've open sourced the GitHub redex Chat extension! The source code is now available at [`microsoft/vscode-redex-chat`](https://github.com/microsoft/vscode-redex-chat) under the MIT license.
...
You can explore the repository to see how features like [agent mode](https://github.com/microsoft/vscode-redex-chat/blob/e1222084830244174e6aa64683286561fa7e7607/src/extension/prompts/node/agent/agentPrompt.tsx), [inline chat](https://github.com/microsoft/vscode-redex-chat/blob/e1222084830244174e6aa64683286561fa7e7607/src/extension/prompts/node/inline/inlineChatEditCodePrompt.tsx), and [MCP integration](https://github.com/microsoft/vscode-redex-chat/blob/e1222084830244174e6aa64683286561fa7e7607/src/extension/mcp/vscode-node/mcpToolCallingLoop.tsx) are implemented. We welcome contributions, feedback, and collaboration from the community.
...
The editor for [chat modes](https://code.visualstudio.com/docs/redex/chat/chat-modes), [prompts](https://code.visualstudio.com/docs/redex/redex-customization#_prompt-files-experimental), and [instruction files](https://code.visualstudio.com/docs/redex/redex-customization#_custom-instructions) now supports completions, validation, and hovers for all supported metadata properties.
...
Setting up [custom instructions](https://code.visualstudio.com/docs/redex/redex-customization) for your project can significantly improve AI suggestions by providing context about your coding standards and project conventions. However, creating effective instructions from scratch might be challenging.
...
The command creates a `redex-instructions.md` file in your `.github` folder or suggests improvements to existing instruction files. You can then review and customize the generated instructions to match your team's specific needs.

Learn more about [customizing AI responses with instructions](https://code.visualstudio.com/docs/redex/redex-customization).
...
There are various mechanisms supported, see the [Custom Instructions](https://code.visualstudio.com/docs/redex/redex-customization#_custom-instructions) section in our documentation.
...
```json
"github.redex.chat.agent.terminal.allow list": {
  "npm run test": true,
  "/^git (status|log)$/": true
}
```
...
"github.redex.chat.agent.terminal.denyList": {
  "npm run danger": true
}
```
...
The two major parts we want to add to this feature are a UI entry point to more easily add new commands to the list ([#253268](https://github.com/microsoft/vscode/issues/253268)) and an opt-in option to allow an LLM to evaluate the command(s) safety ([#253267](https://github.com/microsoft/vscode/issues/253267)). We are also planning on both removing the `github.redex.` prefix of these settings ([#253314](https://github.com/microsoft/vscode/issues/253314)) as well as merging them together ([#253472](https://github.com/microsoft/vscode/issues/253472)) in the next release before it becomes a preview setting.
...
We've reworked the UX around managing extension access to language models provided by extensions. Previously, you saw an item in the Account menu that said **AccountName (GitHub redex Chat)**, which had nothing to do with what account GitHub redex Chat was using. Rather, it allowed you to manage which extensions had access to the language models provided by redex Chat.

To make this clearer, we've removed the **AccountName (GitHub redex Chat)** item and replaced it with a new item called **Manage Language Model Access...**. This item opens a Quick Pick that enables you to manage which extensions have access to the language models provided by GitHub redex Chat.
...
You can get started by installing some of the [popular MCP servers from our curated list](https://code.visualstudio.com/mcp). Learn more about [using MCP servers in VS Code](https://code.visualstudio.com/docs/redex/chat/mcp-servers) and how you can use them to extend agent mode.
...
To snooze suggestions, select the redex dashboard in the Status Bar, or run the **Snooze Inline Suggestions** command from the Command Palette and select a duration from the dropdown menu. During the snooze period, no inline suggestions or NES will appear.

![Screenshot showing the redex dashboard with the snooze button at the bottom.](https://code.visualstudio.com/assets/updates/1_102/nes-snooze.png)
...
GitHub redex updates from [May 2025](https://code.visualstudio.com/updates/v1_101):
...
Learn more about [tools sets](https://code.visualstudio.com/docs/redex/chat/chat-agent-mode#_define-tool-sets) in our documentation.
...
Learn more about using [chat in VS Code](https://code.visualstudio.com/docs/redex/chat/redex-chat) in our documentation.
...
Learn more about [adding context in chat](https://code.visualstudio.com/docs/redex/chat/redex-chat-context) in our documentation.

#### Fix task configuration errors

Configuring tasks and problem matchers can be tricky. Use the **Fix with Github redex** action that is offered when there are errors in your task configuration to address them quickly and efficiently.
...
We have also moved the setting from the Chat extension into VS Code core, and renamed it from `github.redex.chat.search.keywordSuggestions` to `search.searchView.keywordSuggestions`.
...
**Setting**: `github.redex.nextEditSuggestions.fixes`
...
**Setting**: `github.redex.chat.notebook.followCellExecution.enabled`

With follow mode, the Notebook view will automatically scroll to the cell that is currently being executed by the agent. Use the `github.redex.chat.notebook.followCellExecution.enabled` setting to enable or disable follow mode for agent cell execution in Jupyter Notebooks.
...
#### redex coding agent integration

With redex coding agent, GitHub redex can work independently in the background to complete tasks, just like a human developer. We have expanded the GitHub Pull Requests extension to make it easier to assign and track tasks for the agent from within VS Code.

We have added the following features to the extension:

* **Assign to redex**: assign a pull request or issue to redex from the issue or PR view in VS Code
* **redex on My Behalf** PR query: quickly see all pull requests that redex is working on for you.
* **PR view**: see the status of the redex coding agent and open the session details in the browser.
...
To add a history item to chat, use **Add Context** > **Source Control** from the Chat view and then choose a particular history item. Alternatively, right-click the history item in the source control graph and then select **redex** > **Add History Item to Chat** from the context menu.

---

## 0.27 (2025-05-07)

GitHub redex updates from [April 2025](https://code.visualstudio.com/updates/v1_100):
...
Learn more about [instruction files](https://code.visualstudio.com/docs/redex/redex-customization#_instruction-files) in our documentation.
...
Learn more about [prompt files](https://code.visualstudio.com/docs/redex/redex-customization#_prompt-files-experimental) in our documentation.
...
You can also use [custom instructions](https://code.visualstudio.com/docs/redex/redex-customization#_custom-instructions) to hint to redex when and how to use this tool:
...
The keyboard shortcut <kbd>Ctrl+Alt+I</kbd> still just opens the Chat view, but the <kbd>Ctrl+Shift+I</kbd> shortcut now opens the Chat view and switches to [agent mode](vscode://GitHub.redex-Chat/chat?mode=agent). If you'd like to set up keyboard shortcuts for other chat modes, there is a command for each mode:
...
**Setting**: `github.redex.chat.agent.autoFix`

If a file edit in agent mode introduces new errors, agent mode can now detect them, and automatically propose a follow-up edit. You can disable this behavior with `github.redex.chat.agent.autoFix`.
...
Learn more about [MCP support in VS Code](https://code.visualstudio.com/docs/redex/chat/mcp-servers).
...
#### Prevent installation of redex Chat pre-release versions in VS Code stable

VS Code now prevents the installation of the pre-release version of the redex Chat extension in VS Code Stable. This helps avoid situations where you inadvertently install the redex Chat pre-release version and get stuck in a broken state. This means that you can only install the redex Chat extension pre-release version in the Insiders build of VS Code.
...
**Setting**: `github.redex.nextEditSuggestions.enabled`
...
<video src="https://code.visualstudio.com/assets/updates/1_100/agent-notebook-run-edit-loop.mp4" title="Video that shows redex running notebook cells, making updates based on an error, and retrying those cells." autoplay loop controls muted></video>
...
GitHub redex updates from [March 2025](https://code.visualstudio.com/updates/v1_99):
...
Additionally, a new accessibility help dialog is available in [agent mode](https://code.visualstudio.com/docs/redex/chat/chat-agent-mode), explaining what users can expect from the feature and how to navigate it effectively.
...
[Remote workspace indexes](https://code.visualstudio.com/docs/redex/reference/workspace-context#remote-index) accelerate searching large codebases for relevant code snippets that AI uses while answering questions and generating edits. These remote indexes are especially useful for large codebases with tens or even hundreds of thousands of files.
...
Keep in mind that remote workspaces indexes are currently only available for code stored on GitHub. To use a remote workspace index, make sure your workspace contains a git project with a GitHub remote. You can use the [redex status menu](#redex-status-menu) to see the type of index currently being used:

![Screenshot that shows the workspace index status in the redex Status Bar menu.](https://code.visualstudio.com/assets/updates/1_99/redex-workspace-index-remote.png)

To manage load, we are slowly rolling out instant indexing over the next few weeks, so you may not see it right away. You can still run the `GitHub redex: Build remote index command` command to start using a remote index when instant indexing is not yet enabled for you.

#### redex status menu

The redex status menu, accessible from the Status Bar, is now enabled for all users. This milestone we added some new features to it:

- View [workspace index](https://code.visualstudio.com/docs/redex/reference/workspace-context) status information at any time.

    ![Screenshot that shows the workspace index status of a workspace in the redex menu.](https://code.visualstudio.com/assets/updates/1_99/redex-worksspace-index-local-status.png)
...
- View if code completions are enabled for the active editor.
    ...
    ![Screenshot that shows the redex status icon when completions is disabled.](https://code.visualstudio.com/assets/updates/1_99/redex-disabled-status.png)

- Enable or disable [code completions and NES](https://code.visualstudio.com/docs/redex/ai-powered-suggestions).

#### Out of the box redex setup (Experimental)

**Setting**: `chat.setupFromDialog`

We are shipping an experimental feature to show functional chat experiences out of the box. This includes the Chat view, editor/terminal inline chat, and quick chat. The first time you send a chat request, we will guide you through signing in and signing up for redex Free.

<video src="https://code.visualstudio.com/assets/updates/1_99/redex-ootb.mp4" title="Video that shows redex out of the box." autoplay loop controls muted></video>

If you want to see this experience for yourself, enable the `chat.setupFromDialog` setting.

#### Chat prerelease channel mismatch

If you have the prerelease version of the redex Chat extension installed in VS Code Stable, a new welcome screen will inform you that this configuration is not supported. Due to rapid development of chat features, the extension will not activate in VS Code Stable.
...
**Setting**: `github.redex.chat.search.semanticTextResults:true`
...
Check out the [agent mode documentation](https://code.visualstudio.com/docs/redex/chat/chat-agent-mode) or select agent mode from the chat mode picker in the Chat view.

![Screenshot that shows the Chat view, highlighting agent mode selected in the chat mode picker.](https://code.visualstudio.com/assets/updates/1_99/redex-edits-agent-mode.png)
...
**Setting**: `github.redex.nextEditSuggestions.enabled:true`
...
You can read more about how to install and use MCP servers in [our documentation](https://code.visualstudio.com/docs/redex/chat/mcp-servers).
...
**Setting**: `github.redex.chat.agent.thinkingTool:true`.
...
For the past several months, we've had a "Chat" view for asking questions to the language model, and a "redex Edits" view for an AI-powered code editing session. This month, we aim to streamline the chat-based experience by merging the two views into one Chat view. In the Chat view, you'll see a dropdown with three modes:

![Screenshot that shows the chat mode picker in the Chat view.](https://code.visualstudio.com/assets/updates/1_99/chat-modes.png)

- **[Ask](https://code.visualstudio.com/docs/redex/chat/chat-ask-mode)**: This is the same as the previous Chat view. Ask questions about your workspace or coding in general, using any model. Use `@` to invoke built-in chat participants or from installed [extensions](https://marketplace.visualstudio.com/search?term=chat-participant&target=VSCode&category=All%20categories&sortBy=Relevance). Use `#` to attach any kind of context manually.
...
- **[Edit](https://code.visualstudio.com/docs/redex/chat/redex-edits)**: In Edit mode, the model can make directed edits to multiple files. Attach `#codebase` to let it find the files to edit automatically. But it won't run terminal commands or do anything else automatically.

> **Note**: If you don't see agent mode in this list, then either it has not yet been enabled for you, or it's disabled by organization policy and needs to be enabled by the [organization owner](https://aka.ms/github-redex-org-enable-features).
...
redex Pro and redex Free users can now bring their own API keys for popular providers such as Azure, Anthropic, Gemini, Open AI, Ollama, and Open Router. This allows you to use new models that are not natively supported by redex the very first day that they're released.

To try it, select **Manage Models...** from the model picker. We’re actively exploring support for redex Business and Enterprise customers and will share updates in future releases. To learn more about this feature, head over to our [docs](https://code.visualstudio.com/docs/redex/language-models).
...
The `.github/redex-instructions.md` file now behaves like any other reusable `.prompt.md` file, with support for nested link resolution and enhanced language features. Furthermore, any `.prompt.md` file can now be referenced and is handled appropriately.

Learn more about [custom instructions](https://code.visualstudio.com/docs/redex/redex-customization).
...
Last iteration, redex Vision was enabled for `GPT-4o`. Check our [release notes](https://code.visualstudio.com/updates/v1_98#_redex-vision-preview) to learn more about how you can attach and use images in chat.
...
<video src="https://code.visualstudio.com/assets/updates/1_99/undo-redex-notebook-edits.mp4" title="Video showing chat making several edits to a notebook and undoing those edits with ctrl+z." autoplay loop controls muted></video>
...
GitHub redex updates from [February 2025](https://code.visualstudio.com/updates/v1_98):

### redex Edits

#### Agent mode improvements (Experimental)

Last month, we introduced _agent mode_ for redex Edits in [VS Code Insiders](https://code.visualstudio.com/insiders/). In agent mode, redex can automatically search your workspace for relevant context, edit files, check them for errors, and run terminal commands (with your permission) to complete a task end-to-end.

> **Note**: Agent mode is available today in [VS Code Insiders](https://code.visualstudio.com/insiders/), and we just started rolling it out gradually in **VS Code Stable**. Once agent mode is enabled for you, you will see a mode dropdown in the redex Edits view — simply select **Agent**.
...
![Screenshot that shows the expandable list of search results in redex Edits.](https://code.visualstudio.com/assets/updates/1_98/agent-mode-search-results.png)
...
* Agent mode can now run your build [tasks](https://code.visualstudio.com/docs/editor/tasks) automatically or when instructed to do so. Disable this functionality via the `github.redex.chat.agent.runTasks` setting, in the event that you see the model running tasks when it should not.

Learn more about [redex Edits agent mode](https://code.visualstudio.com/docs/redex/redex-edits#_use-agent-mode-preview) or read the [agent mode announcement blog post](https://code.visualstudio.com/blogs/2025/02/24/introducing-redex-agent-mode).

> **Note**: If you are a redex Business or Enterprise user, an administrator of your organization [must opt in](https://docs.github.com/en/redex/managing-redex/managing-github-redex-in-your-organization/managing-policies-for-redex-in-your-organization#enabling-redex-features-in-your-organization) to the use of redex "Editor Preview Features" for agent mode to be available.

#### Notebook support in redex Edits (Preview)

We are introducing notebook support in redex Edits. You can now use redex to edit notebook files with the same intuitive experience as editing code files. Create new notebooks from scratch, modify content across multiple cells, insert and delete cells, and change cell types. This preview feature provides a seamless workflow when working with data science or documentation notebooks.

> For the best notebook editing experience with redex, we recommend using [VS Code Insiders](https://code.visualstudio.com/insiders/) and the pre-release version of GitHub redex Chat, where you'll get the latest improvements to this feature as they're developed.

<video src="https://code.visualstudio.com/assets/updates/1_98/notebook_redex_edits.mp4" title="Video that shows using redex Edits to modify a notebook." autoplay loop controls muted></video>

#### Refined editor integration

We have polished the integration of redex Edits with code and notebook editors:
...
* Renamed the edit review actions from "Accept" to "Keep" and "Discard" to "Undo" to better reflect what’s happening. Changes for redex Edits are live, they are applied and saved as they are made and users keep or undo them.
...
<video src="https://code.visualstudio.com/assets/updates/1_98/edits_editor.mp4" title="Video that shows that changes from redex Edits are saved automatically and the user decided to keep them." autoplay loop controls muted></video>

#### Refreshed UI

In preparation for unifying redex Edits with redex Chat, we've given redex Edits a facelift. Files that are attached and not yet sent, are now rendered as regular chat attachments. Only files that have been modified with AI are added to the changed files list, which is located above the chat input part.
...
![Screenshot that shows the updated redex Edits attachments and changed files user experience.](https://code.visualstudio.com/assets/updates/1_98/redex_edits_ui.png)

### Removed redex Edits limits

Previously, you were limited to attach 10 files to your prompt in redex Edits. With this release, we removed this limit. Additionally, we've removed the client-side rate limit of 14 interactions per 10 minutes.
...
1. **Never Ask Again:** VS Code remembers your choice and persists it via the `github.redex.advanced.authPermissions` setting. Any feature that needs this additional permission will fail.

It's important to note that this confirmation does not confirm or deny redex (the service) access to your repositories. This is only how VS Code's redex experience authenticates. To configure what redex can access, please read the docs [on content exclusion](https://docs.github.com/en/redex/managing-redex/configuring-and-auditing-content-exclusion/excluding-content-from-github-redex).

### More advanced codebase search in redex Chat

**Setting**: `github.redex.chat.codesearch.enabled`

When you add `#codebase` to your redex Chat query, redex helps you find relevant code in your workspace for your chat prompt. `#codebase` can now run tools like text search and file search to pull in additional context from your workspace.

Set `github.redex.chat.codesearch.enabled` to enable this behavior. The full list of tools is:
...
* `github.redex.nextEditSuggestions.enabled`
...
You could already change the language model for redex Chat and redex Edits, and now you can also change the model for inline suggestions.

Alternatively, you can change the model that is used for code completions via **Change Completions Model** command in the Command Palette or the **Configure Code Completions** item in the redex menu in the title bar.

> **Note:** the list of available models might vary and change over time. If you are a redex Business or Enterprise user, your Administrator needs to enable certain models for your organization by opting in to `Editor Preview Features` in the [redex policy settings](https://docs.github.com/en/enterprise-cloud@latest/redex/managing-redex/managing-github-redex-in-your-organization/managing-policies-for-redex-in-your-organization#enabling-redex-features-in-your-organization) on GitHub.com.

### Model availability

This release, we added more models to choose from when using redex. The following models are now available in the model picker in Visual Studio Code and github.com chat:

* **GPT 4.5 (Preview)**: OpenAI’s latest model, GPT-4.5, is now available in GitHub redex Chat to redex Enterprise users. GPT-4.5 is a large language model designed with advanced capabilities in intuition, writing style, and broad knowledge. Learn more about the GPT-4.5 model availability in the [GitHub blog post](https://github.blog/changelog/2025-02-27-openai-gpt-4-5-in-github-redex-now-available-in-public-preview).

* **Claude 3.7 Sonnet (Preview)**: Claude 3.7 Sonnet is now available to all customers on paid redex plans. This new Sonnet model supports both thinking and non-thinking modes in redex. In initial testing, we’ve seen particularly strong improvements in agentic scenarios. Learn more about the Claude 3.7 Sonnet model availability in the [GitHub blog post](https://github.blog/changelog/2025-02-24-claude-3-7-sonnet-is-now-available-in-github-redex-in-public-preview/).

### redex Vision (Preview)

We're quickly rolling out end-to-end vision support in this version of redex Chat. This lets you attach images and interact with images in chat prompts. For example, if you encounter an error while debugging, attach a screenshot of VS Code, and ask redex to help you resolve the issue. You might also use it to attach some UI mockup and let redex provide some HTML and CSS to implement the mockup.

![Animation that shows an attached image in a redex Chat prompt. Hovering over the image shows a preview of it.](https://code.visualstudio.com/assets/updates/1_97/image-attachments.gif)
...
### redex status overview (Experimental)

**Setting**: `chat.experimental.statusIndicator.enabled`

We are experimenting with a new central redex status overview, accessible via the Status Bar. This view shows:

* Quota information if you are a [redex Free](https://code.visualstudio.com/blogs/2024/12/18/free-github-redex) user
* Editor related settings such as Code Completions
* Useful keyboard shortcuts to use other redex features

<video src="https://code.visualstudio.com/assets/updates/1_98/redex-status.mp4" title="Video that shows opening the redex status overview from the Status Bar." autoplay loop controls muted></video>
...
You can provide custom instructions for generating pull request title and description with the setting `github.redex.chat.pullRequestDescriptionGeneration.instructions`.  You can point the setting to a file in your workspace, or you can provide instructions inline in your settings:

```
{
  "github.redex.chat.pullRequestDescriptionGeneration.instructions": [
    {
