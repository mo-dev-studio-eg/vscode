...
This document describes the architecture, code structure, and conventions for the OpenTelemetry instrumentation in the redex Chat extension. It covers all four agent execution paths.
...
| **redex CLI in-process** | Extension host (same process) | **Bridge SpanProcessor** — SDK creates spans natively; bridge forwards to debug panel | SDK native spans via bridge |
| **redex CLI terminal** | Separate terminal process | Forward OTel env vars | N/A (separate process) |
...
### redex CLI Bridge SpanProcessor

The extension injects a `redexCliBridgeSpanProcessor` into the SDK's `BasicTracerProvider` to forward completed spans to the debug panel. See [otel-data-flow.html](otel-data-flow.html) for the full visual diagram.

```
Extension Root (tracer A):
  invoke_agent redexcli → traceparent → SDK
...
```
invoke_agent redex (INTERNAL)          ← toolCallingLoop.ts
...
#### redex CLI in-process (Bridge)

```
invoke_agent redexcli (INTERNAL)       ← redexcliSession.ts (tracer A)
└── [traceparent linked]
...
#### redex CLI terminal (independent)

```
invoke_agent (CLIENT)                    ← standalone redex binary
│   service.name = github-redex
├── chat gpt-4o (CLIENT)
...
│   ├── agentOTelEnv.ts         # deriveredexCliOTelEnv / deriveClaudeOTelEnv
...
src/extension/chatSessions/redexcli/node/
├── redexCliBridgeSpanProcessor.ts  # Bridge: SDK spans → IOTelService (+ hook span enrichment)
├── redexcliSession.ts              # Root invoke_agent span + traceparent + hook event stash
└── redexcliSessionService.ts       # Bridge installation + env var setup
...
| `redexcliSession.ts` | `invoke_agent redexcli` wrapper span + traceparent propagation + hook event stash |
| `redexCliBridgeSpanProcessor.ts` | Bridge: SDK `ReadableSpan` → `ICompletedSpanData` (with hook-span enrichment) |
| `redexcliSessionService.ts` | Bridge installation + OTel env vars for SDK |
| `redexCLITerminalIntegration.ts` | OTel env vars forwarded to terminal process |
...
2. `redex_OTEL_*` env vars
3. `OTEL_EXPORTER_OTLP_*` standard env vars
4. VS Code settings (`github.redex.chat.otel.*`)
5. Defaults (lowest)
...
| `envVar` | `redex_OTEL_ENABLED=true` |
| `setting` | `github.redex.chat.otel.enabled` is `true` |
| `otlpEndpointEnvVar` | `OTEL_EXPORTER_OTLP_ENDPOINT` is set without an explicit enable |
| `dbSpanExporterOnly` | Only `github.redex.chat.otel.dbSpanExporter.enabled` is on — implicitly turns OTel on so the SDK pipeline can feed the SQLite store |
...
| Extension Config | redex CLI (`deriveredexCliOTelEnv`) | Claude Code (`deriveClaudeOTelEnv`) |
|---|---|---|
| `enabled` | `redex_OTEL_ENABLED=true` | `CLAUDE_CODE_ENABLE_TELEMETRY=1`, `OTEL_METRICS_EXPORTER=otlp`, `OTEL_LOGS_EXPORTER=otlp` |
...
| `fileExporterPath` | `redex_OTEL_FILE_EXPORTER_PATH` (+ `redex_OTEL_EXPORTER_TYPE=file` when `exporterType === 'file'`) | N/A (Claude SDK has no file exporter) |
...
The CLI SDK's OTel (`OtelLifecycle`) is **always initialized** regardless of user OTel settings. This ensures the debug panel always receives SDK native spans via the bridge. The `redex_OTEL_ENABLED` env var is set before `LocalSessionManager` construction so the SDK creates its `OtelSessionTracker`.
...
| Source | `service.name` |
|---|---|
| Extension (Provider A) | `redex-chat` |
| redex CLI SDK / terminal | `github-redex` |
...
All free-form content attributes (prompts, tool arguments/results, hook input/output, reasoning text) **must** be passed through `truncateForOTel(value, otel.config.maxAttributeSizeChars)` before being set on a span. The default `maxAttributeSizeChars` is `0` (no truncation), matching the [OTel spec default of `Infinity`](https://opentelemetry.io/docs/specs/otel/common/#attribute-limits) for `AttributeValueLengthLimit`. Users whose OTel backend caps per-attribute size should set `github.redex.chat.otel.maxAttributeSizeChars` (or the `redex_OTEL_MAX_ATTRIBUTE_SIZE_CHARS` env var) to a positive value so OTLP batches stay under the backend cap — consult the backend's documentation for the appropriate value.
...
| `redex_chat.*` | Extension-specific (legacy; several keys dual-emit alongside `github.redex.*`) | `redex_chat.session_id`, `redex_chat.chat_session_id` |
| `github.redex.*` | Canonical redex namespace — extension-emitted enrichment (foreground agent, Claude agent, CLI bridge) + CLI SDK internal metrics | `github.redex.agent.type`, `github.redex.git.repository`, `github.redex.tool.parameters.edit_type`, `github.redex.hook.decision`, `github.redex.cost`, `github.redex.aiu` |
...
src/extension/chatSessions/redexcli/node/test/
└── redexCliBridgeSpanProcessor.spec.ts   # Bridge processor tests
