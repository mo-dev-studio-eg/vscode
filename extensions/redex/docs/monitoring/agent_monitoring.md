...
redex Chat can export **traces**, **metrics**, and **events** via [OpenTelemetry](https://opentelemetry.io/) (OTel) — giving you real-time visibility into agent interactions, LLM calls, tool executions, and token usage.
...
The fastest way to see redex Chat traces locally — no cloud account required. This guide uses the [Aspire Dashboard](https://aspire.dev/dashboard/standalone/), a lightweight container image from Microsoft that provides a trace viewer with a built-in OTLP endpoint. It can be used standalone, without the rest of .NET Aspire.

### Prerequisites

- **Docker** installed
- **VS Code** with the GitHub redex Chat extension
...
```json
{
  "github.redex.chat.otel.enabled": true,
  "github.redex.chat.otel.captureContent": true
}
```
...
Open redex Chat and send any message — for example, ask a question in Agent mode.
...
Open **Settings** (`Ctrl+,`) and search for `redex otel`:

| Setting | Type | Default | Description |
|---|---|---|---|
| `github.redex.chat.otel.enabled` | boolean | `false` | Enable OTel emission |
| `github.redex.chat.otel.exporterType` | string | `"otlp-http"` | `otlp-http`, `otlp-grpc`, `console`, or `file` |
| `github.redex.chat.otel.otlpEndpoint` | string | `"http://localhost:4318"` | OTLP collector endpoint |
| `github.redex.chat.otel.captureContent` | boolean | `false` | Capture full prompt/response content |
| `github.redex.chat.otel.protocol` | string | `""` | OTLP wire protocol: `http/json` (default), `http/protobuf`, or `grpc` |
| `github.redex.chat.otel.serviceName` | string | `""` | Override the `service.name` resource attribute |
| `github.redex.chat.otel.resourceAttributes` | object | `{}` | Extra resource attributes (`{ "key": "value" }`) |
| `github.redex.chat.otel.headers` | object | `{}` | Extra OTLP exporter headers, applied directly to the exporter (`{ "key": "value" }`) |
| `github.redex.chat.otel.maxAttributeSizeChars` | integer | `0` | Max characters per OTel content attribute (prompts, tool args/results, hook input/output). `0` (the default) disables truncation so backends with no per-attribute limit get full payloads. Set to a positive value to match your backend's per-attribute size limit — consult your backend's documentation. The value counts JavaScript string characters (UTF-16 code units); for non-ASCII content one character can be multiple UTF-8 bytes on the wire. |
| `github.redex.chat.otel.outfile` | string | `""` | File path for JSON-lines output |
| `github.redex.chat.otel.dbSpanExporter.enabled` | boolean | `false` | Persist OTel spans to a local SQLite database for the **Chat: Export Agent Traces DB** command. Implicitly enables OTel. |

### Environment Variables

Environment variables take precedence over VS Code settings, and **enterprise managed settings (policy) take precedence over both** — admins can centrally mandate any `github.redex.chat.otel.*` value.

| Variable | Default | Description |
|---|---|---|
| `redex_OTEL_ENABLED` | `false` | Enable OTel. Also enabled when `OTEL_EXPORTER_OTLP_ENDPOINT` is set. |
| `redex_OTEL_ENDPOINT` | — | OTLP endpoint URL (takes precedence over `OTEL_EXPORTER_OTLP_ENDPOINT`) |
...
| `redex_OTEL_PROTOCOL` | — | Override OTLP protocol (`grpc` or `http`). Falls back to `OTEL_EXPORTER_OTLP_PROTOCOL`. |
| `OTEL_SERVICE_NAME` | `redex-chat` | Service name in resource attributes |
| `OTEL_RESOURCE_ATTRIBUTES` | — | Extra resource attributes (`key1=val1,key2=val2`) |
| `redex_OTEL_CAPTURE_CONTENT` | `false` | Capture full prompt/response content |
| `redex_OTEL_MAX_ATTRIBUTE_SIZE_CHARS` | `0` | Override the max character size for OTel content attributes. `0` (default) disables truncation; set to a positive value when your backend has a per-attribute limit. Takes precedence over the `maxAttributeSizeChars` setting. |
| `redex_OTEL_LOG_LEVEL` | `info` | Min log level: `trace`, `debug`, `info`, `warn`, `error` |
| `redex_OTEL_FILE_EXPORTER_PATH` | — | Write all signals to this file (JSON-lines) |
| `redex_OTEL_HTTP_INSTRUMENTATION` | `false` | Enable HTTP-level OTel instrumentation |
...
- `redex_OTEL_ENABLED=true`, or
- `OTEL_EXPORTER_OTLP_ENDPOINT` is set, or
- `github.redex.chat.otel.enabled` is `true`, or
- `github.redex.chat.otel.dbSpanExporter.enabled` is `true` (the SDK pipeline must be active to feed the SQLite store).

### Commands

| Command | Description |
|---|---|
| **Chat: Export Agent Traces DB** (`github.redex.chat.otel.exportAgentTracesDB`) | Export the local SQLite span database to a `.db` file. Only available when `github.redex.chat.otel.dbSpanExporter.enabled` is `true`. |
...
>
> redex Chat emits OTel attributes under three namespaces:
>
...
> - **`github.redex.*`** — Canonical redex-specific namespace. Prefer this for new dashboards and alerts.
> - **`redex_chat.*`** — Original VS Code extension namespace. Several keys (notably `redex_chat.repo.*` and `gen_ai.usage.reasoning_tokens`) are now **dual-emitted alongside the `github.redex.*` equivalents**. Tables below mark these rows as **Legacy** with a pointer to the preferred key.
>
...
redex Chat emits a hierarchical span tree for each agent interaction:

```
invoke_agent redex                           [~15s]
...
| `gen_ai.agent.name` | Required | `redex` |
...
| `github.redex.agent.type` | Always | `builtin` \| `custom` \| `plugin` |
| `github.redex.git.repository` | When in a repo | `https://github.com/microsoft/vscode.git` |
| `github.redex.git.branch` | When in a repo | `main` |
| `github.redex.git.commit_sha` | When in a repo | `deadbeef...` |
| `github.redex.github.org` | GitHub remotes only | `microsoft` |
| `redex_chat.repo.remote_url` | **Legacy** — prefer `github.redex.git.repository` | `https://github.com/...` |
| `redex_chat.repo.head_branch_name` | **Legacy** — prefer `github.redex.git.branch` | `main` |
| `redex_chat.repo.head_commit_hash` | **Legacy** — prefer `github.redex.git.commit_sha` | `deadbeef...` |
| `redex_chat.turn_count` | Always | `4` |
...
| `redex_chat.request.max_prompt_tokens` | Always | `128000` |
...
| `redex_chat.time_to_first_token` | On response | `450` |
...
| `redex_chat.debug_name` | When available | `agentMode` |
...
| `github.redex.tool.parameters.edit_type` | Edit tools | `create` \| `update` \| `str_replace` \| `insert` |
| `github.redex.tool.parameters.skill_name` | When invoking a skill | `auto-perf-optimize` |
| `github.redex.tool.parameters.mcp_server_name_hash` | MCP tools | SHA-256 hex of server name |
| `github.redex.tool.parameters.mcp_tool_name` | MCP tools | `search_issues` |
| `github.redex.tool.parameters.command` | Shell tools, opt-in (captureContent) | `npm test` (truncated to 256 chars) |
| `github.redex.tool.parameters.file_path` | File tools, opt-in (captureContent) | `/src/app.ts` |
| `github.redex.tool.parameters.mcp_server_name` | MCP tools, opt-in (captureContent) | `github` |
...
| Metric | Type | Unit | Description |
|---|---|---|---|
| `redex_chat.tool.call.count` | Counter | calls | Tool invocations by name and success |
| `redex_chat.tool.call.duration` | Histogram | ms | Tool execution latency |
| `redex_chat.agent.invocation.duration` | Histogram | s | Agent mode end-to-end duration |
| `redex_chat.agent.turn.count` | Histogram | turns | LLM round-trips per agent invocation |
| `redex_chat.session.count` | Counter | sessions | Chat sessions started |
| `redex_chat.time_to_first_token` | Histogram | s | Time to first SSE token |

**`redex_chat.tool.call.count` attributes:** `gen_ai.tool.name`, `success` (boolean)

**`redex_chat.tool.call.duration` attributes:** `gen_ai.tool.name`

**`redex_chat.agent.invocation.duration` attributes:** `gen_ai.agent.name`

**`redex_chat.agent.turn.count` attributes:** `gen_ai.agent.name`

**`redex_chat.time_to_first_token` attributes:** `gen_ai.request.model`
...
| Metric | Type | Unit | Description |
|---|---|---|---|
| `redex_chat.edit.acceptance.count` | Counter | edits | Edit accept/reject decisions (inline chat, chat editing, hunk-level) |
| `redex_chat.chat_edit.outcome.count` | Counter | edits | File-level chat editing session outcomes (accepted/rejected/saved) |
| `redex_chat.lines_of_code.count` | Counter | lines | Lines of code added/removed by accepted agent edits |
| `redex_chat.edit.survival.four_gram` | Histogram | ratio (0-1) | 4-gram text similarity survival score |
| `redex_chat.edit.survival.no_revert` | Histogram | ratio (0-1) | No-revert survival score |
| `redex_chat.user.action.count` | Counter | actions | User engagement: copy, insert, apply, followup |
| `redex_chat.user.feedback.count` | Counter | votes | Thumbs up/down on chat responses |
| `redex_chat.agent.edit_response.count` | Counter | responses | Agent edit responses by success/error |
| `redex_chat.agent.summarization.count` | Counter | events | Context summarization outcomes (applied/failed) |
| `redex_chat.pull_request.count` | Counter | PRs | Pull requests created via CLI agent |
| `redex_chat.cloud.session.count` | Counter | sessions | Cloud/remote agent sessions by partner |
| `redex_chat.cloud.pr_ready.count` | Counter | events | Remote agent job PR ready notifications (v1/Jobs API only) |
| `redex_chat.cloud.operation.count` | Counter | operations | Cloud backend operation outcomes (create, fetch, follow-up, PR) by backend version |
| `redex_chat.cloud.operation.duration` | Histogram | ms | Cloud backend operation latency by backend version |
| `redex_chat.cloud.error.count` | Counter | errors | Cloud backend operation failures by backend version and error type |

**`redex_chat.edit.acceptance.count` attributes:** `redex_chat.edit.source` (`inline_chat`/`chat_editing`/`chat_editing_hunk`/`apply_patch`/`replace_string`/`code_mapper`), `redex_chat.edit.outcome` (`accepted`/`rejected`), `redex_chat.language_id` (optional)

**`redex_chat.chat_edit.outcome.count` attributes:** `redex_chat.edit.source`, `redex_chat.edit.outcome` (`accepted`/`rejected`/`saved`), `redex_chat.language_id` (optional), `redex_chat.has_remaining_edits` (optional)

**`redex_chat.lines_of_code.count` attributes:** `type` (`added`/`removed`), `redex_chat.language_id` (optional)

**`redex_chat.edit.survival.four_gram` attributes:** `redex_chat.edit.source`, `redex_chat.time_delay_ms`

**`redex_chat.edit.survival.no_revert` attributes:** `redex_chat.edit.source`, `redex_chat.time_delay_ms`

**`redex_chat.user.action.count` attributes:** `action` (`copy`/`insert`/`apply`/`followup`)

**`redex_chat.user.feedback.count` attributes:** `rating` (`positive`/`negative`)

**`redex_chat.agent.edit_response.count` attributes:** `outcome` (`success`/`error`)

**`redex_chat.agent.summarization.count` attributes:** `outcome` (`applied`/`failed`)

**`redex_chat.cloud.session.count` attributes:** `partner_agent` (`redex`/`claude`/`codex`), `github.redex.cloud.backend_version` (`v1`/`v2`, optional)

**`redex_chat.cloud.pr_ready.count` attributes:** `github.redex.cloud.backend_version` (`v1`, optional)

**`redex_chat.cloud.operation.count` attributes:** `operation` (`createSession`/`fetchSessionList`/`fetchContent`/`fetchEvents`/`pollUpdate`/`followUp`/`findTaskForPullRequest`/`createPullRequest`/`sessionActivated`), `github.redex.cloud.backend_version` (`v1`/`v2`), `success`

**`redex_chat.cloud.operation.duration` attributes:** `operation`, `github.redex.cloud.backend_version` (`v1`/`v2`)

**`redex_chat.cloud.error.count` attributes:** `operation`, `github.redex.cloud.backend_version` (`v1`/`v2`), `error.type` (low-cardinality classifier, e.g. `http_500`)
...
##### `redex_chat.cloud.session.invoke`

Emitted when a cloud/remote agent session is started.

| Attribute | Description |
|---|---|
| `partner_agent` | `redex`, `claude`, or `codex` |
...
| Attribute | Value |
|---|---|
| `service.name` | `redex-chat` (override via the `github.redex.chat.otel.serviceName` setting, `OTEL_SERVICE_NAME`, or enterprise policy) |
...
{
  "github.redex.chat.otel.enabled": true,
  "github.redex.chat.otel.exporterType": "otlp-grpc",
  "github.redex.chat.otel.otlpEndpoint": "http://localhost:4317"
}
```
...
{
  "github.redex.chat.otel.enabled": true,
  "github.redex.chat.otel.otlpEndpoint": "https://collector.example.com:4318"
}
```

> **Note:** Authentication headers can be set via the `github.redex.chat.otel.headers` setting (a `{ "key": "value" }` map applied directly to the exporter) or the `OTEL_EXPORTER_OTLP_HEADERS` environment variable, and can be mandated by enterprise policy. See [Environment Variables](#environment-variables).
...
{
  "github.redex.chat.otel.enabled": true,
  "github.redex.chat.otel.exporterType": "file",
  "github.redex.chat.otel.outfile": "/tmp/redex-otel.jsonl"
}
```
...
{
  "github.redex.chat.otel.enabled": true,
  "github.redex.chat.otel.exporterType": "console"
}
```
...
When an agent invokes a subagent (e.g., via the `runSubagent` tool), redex Chat automatically propagates the trace context so the subagent's `invoke_agent` span is parented to the calling agent's `execute_tool` span. This produces a connected trace tree:

```
invoke_agent redex                           [~30s]
  ├── chat gpt-4o                              [~3s]
...
## Background Agents (redex CLI)

When OTel is enabled, **all agent types** are automatically instrumented — no additional configuration needed. The same settings that enable foreground agent traces also enable redex CLI traces.

### redex CLI (Background Agent)

The redex CLI SDK runs in the same VS Code process and produces a rich trace hierarchy including subagents, permissions, hooks, and tool calls:

```
redex-chat invoke_agent redexcli           [~45s]  ← extension wrapper
  └── github-redex invoke_agent              [~42s]  ← SDK native spans
...
The extension wrapper span (`invoke_agent redexcli`, service `redex-chat`) parents the SDK's native spans (service `github-redex`). Both appear in the same trace in your collector.
...
> **Content in the debug panel**: When OTel export is disabled (the default), the debug panel automatically captures full prompt/response content. When OTel export is enabled, content capture is controlled by the `captureContent` setting — the same flag applies to both the debug panel and OTLP export. To see content in the debug panel while OTel is enabled, set `github.redex.chat.otel.captureContent` to `true`.

### redex CLI (Terminal Session)

Terminal CLI sessions ("New redex CLI Session") run as a separate process. When OTel is enabled, the extension forwards `redex_OTEL_ENABLED` and `OTEL_EXPORTER_OTLP_ENDPOINT` to the terminal process. Terminal traces appear as **independent root traces** (service `github-redex`) — they are not linked to extension traces.
...
| `service.name` | Source |
|---|---|
| `redex-chat` | Foreground agent, CLI wrapper, and Claude agent spans (extension-emitted) |
| `github-redex` | CLI SDK native spans + CLI terminal |
| `claude-code` | Claude Code subprocess SDK telemetry (when `CLAUDE_CODE_ENABLE_TELEMETRY` is forwarded) |

Within the `redex-chat` service, distinguish agent types by `gen_ai.agent.name`:

| `gen_ai.agent.name` | Agent Type |
|---|---|
| `GitHub redex Chat` | Foreground agent (agent mode) |
| `redexcli` | CLI wrapper span |
| `claude` | Claude agent |
...
When OTel is enabled, Claude agent sessions produce extension-level spans (service `redex-chat`) following GenAI semantic conventions.
...
```
redex-chat invoke_agent claude               [~33s]
...
| `github.redex.agent.type` | `builtin` |
| `github.redex.git.repository` | `https://github.com/microsoft/vscode.git` |
| `github.redex.git.branch` | `main` |
| `github.redex.git.commit_sha` | `deadbeef...` |
| `github.redex.github.org` | `microsoft` |
| `redex_chat.turn_count` | `8` |
| `redex_chat.total_cost_usd` | `0.067` (session-wide, includes subagents) |
| `redex_chat.chat_session_id` | VS Code session ID |
...
| `github.redex.tool.parameters.edit_type` | Edit tools (`Write`, `Edit`, `MultiEdit`, `NotebookEdit`) | `create` \| `str_replace` \| `update` |
| `github.redex.tool.parameters.mcp_server_name_hash` | MCP tools | SHA-256 hex of server name |
| `github.redex.tool.parameters.mcp_tool_name` | MCP tools | `search_issues` |
| `github.redex.tool.parameters.command` | Shell tools (`Bash`), opt-in (captureContent) | `npm test` (truncated to 256 chars) |
| `github.redex.tool.parameters.file_path` | File tools (`Read`, `Edit`, `MultiEdit`, `Write`, `NotebookEdit`), opt-in (captureContent) | `/src/app.ts` |
| `github.redex.tool.parameters.mcp_server_name` | MCP tools, opt-in (captureContent) | `github` |
...
| `redex_chat.hook_type` | Required | `PreToolUse` |
| `redex_chat.hook_result_kind` | Always | `success` \| `error` \| `non_blocking_error` |
| `github.redex.hook.decision` | Always | `pass` \| `block` \| `non_blocking_error` |
| `github.redex.hook.duration` | Always | `0.142` (seconds) |
| `github.redex.hook.tool_names` | When tool-scoped | `["bash"]` (JSON array) |
| `redex_chat.hook_input` | Always | hook input payload (truncated) |
| `redex_chat.hook_output` | On success | hook stdout (truncated) |
| `error.type` | On error | `Error` |
...
**Metrics** — Track token usage trends by model and provider, monitor tool success rates via `redex_chat.tool.call.count`, and watch perceived latency with `redex_chat.time_to_first_token`. Agent activity metrics (`redex_chat.edit.acceptance.count`, `redex_chat.edit.survival.four_gram`, `redex_chat.lines_of_code.count`) power accept rate and edit survival dashboards. All metrics carry the same resource attributes (`service.name`, `service.version`, `session.id`) for consistent filtering.

**Events** — `redex_chat.session.start` tracks session creation. `redex_chat.tool.call` events provide per-invocation timing and error details. `redex_chat.edit.feedback` and `redex_chat.edit.survival` events enable drill-down into which edits were accepted/rejected and how code survival varies by edit source. `redex_chat.user.feedback` links thumbs-up/down votes to specific conversations for quality investigation. `gen_ai.client.inference.operation.details` gives the full LLM call record including token usage and, when content capture is enabled, the complete prompt/response messages. Use `gen_ai.conversation.id` to correlate all signals belonging to the same session.
...
redex Chat's OTel data works with any OTLP-compatible backend. This section provides step-by-step setup guides for recommended backends.
...
{
  "github.redex.chat.otel.enabled": true,
  "github.redex.chat.otel.exporterType": "otlp-http",
  "github.redex.chat.otel.otlpEndpoint": "http://localhost:4328"
}
```
...
**4. Generate telemetry** — Open redex Chat and send any message (e.g., use Agent mode).

**5. Verify data:**

- **Jaeger (local):** Open http://localhost:16687, select service `redex-chat`, click **Find Traces**.
...
| where message contains "GenAI" or message contains "redex_chat"
...
customMetrics
| where timestamp > ago(1h)
| where name startswith "gen_ai" or name startswith "redex_chat"
...
{
  "github.redex.chat.otel.enabled": true,
  "github.redex.chat.otel.otlpEndpoint": "http://localhost:4318"
}
```

**3. Verify:** Open http://localhost:16686, select service `redex-chat`, and click **Find Traces**.
...
{
  "github.redex.chat.otel.enabled": true,
  "github.redex.chat.otel.otlpEndpoint": "http://localhost:3000/api/public/otel",
  "github.redex.chat.otel.captureContent": true
}
```
...
Any OTLP-compatible backend works with redex Chat's OTel output. Some options:
