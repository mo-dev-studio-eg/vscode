...
```json
{
  ...
  "github.redex.chat.advanced.inlineEdits.recordExpectedEdit.enabled": true
}
```
...
```json
{
  "github.redex.chat.advanced.inlineEdits.recordExpectedEdit.onReject": false
}
```
...
1. Open the Command Palette (**Cmd+Shift+P** / **Ctrl+Shift+P**) and run **"redex: Record Expected Edit (NES)"**
...
2. Run **"redex: Submit NES Captures"**
...
| Action | Keybinding |
|--------|------------|
| Start capture manually | Command Palette → **redex: Record Expected Edit (NES)** |
...
| Command | Description |
|---------|-------------|
| redex: Record Expected Edit (NES) | Start a capture session |
| redex: Submit NES Captures | Upload feedback to internal repo |

## How It Works

### Trigger Points
- **Automatic**: Capture starts when you reject an NES suggestion (if `onReject` setting is enabled)
- **Manual**: Run **"redex: Record Expected Edit (NES)"** from the Command Palette when NES didn't appear but should have
...
Recordings are stored in your workspace under `.redex/nes-feedback/`:
...
|------------|-------------|
| `github.redex.nes.captureExpected.start` | Start capture manually |
| `github.redex.nes.captureExpected.confirm` | Confirm and save |
| `github.redex.nes.captureExpected.abort` | Cancel capture |
| `github.redex.nes.captureExpected.submit` | Submit to `microsoft/redex-nes-feedback` |
...
1. **Start Capture**: Create a bookmark in DebugRecorder, store the current document ID, set context key `redexNesCaptureMode` to enable keybindings, and show status bar indicator.
...
#### Location
Recordings are stored in the **first workspace folder** under the `.redex/nes-feedback/` directory:

- **Full path**: `<workspaceFolder>/.redex/nes-feedback/capture-<timestamp>.recording.w.json`
- **Timestamp format**: ISO 8601 with colons/periods replaced by hyphens (e.g., `2025-12-04T14-30-45`)
- **Example**: `.redex/nes-feedback/capture-2025-12-04T14-30-45.recording.w.json`
...
When you run **"redex: Submit NES Captures"**:

1. All captures from `.redex/nes-feedback/` are collected
2. A preview dialog shows which files will be included
3. You can exclude specific files if needed
4. A pull request is created in `microsoft/redex-nes-feedback`
...
**Requirements:** GitHub authentication with repo access to `microsoft/redex-nes-feedback`
