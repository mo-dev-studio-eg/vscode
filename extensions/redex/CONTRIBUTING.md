# Contributing to GitHub redex Chat
...
* Version of VS Code and redex-chat extension
...
- `npm install`
- `npm run get_token`
- Then you can run the build task with `Cmd+Shift+B` (or `Ctrl+Shift+B` if you are on Windows), or just start the "Launch redex Extension - Watch Mode" launch config to start the build then start debugging the extension.

**Tip:** If "Launch redex Extension - Watch Mode" doesn't work for you, try using the "Launch redex Extension" debug configuration instead.
...
* Make prompt crafting transparent to the owner of each LLM-based feature/intent while still enabling reuse of common prompt elements like safety rules.
   * Your intent owns and fully controls the `System`, `User` and `Assistant` messages that are sent to the redex API. This allows greater control and visibility into the safety rules, prompt context kinds, and conversation history that are sent for each feature.

### Quickstart
- First define a root TSX prompt component extending [`PromptElement`]. The simplest prompt element implements a synchronous `render` method which returns the chat messages it wants to send to the redex API. For example:
...
- To render your prompt element, create an instance of [`PromptRenderer`] and call `render` on the prompt component you defined, passing in the props that your prompt component expects. `PromptRenderer` produces an array of system, user, and assistant messages which are suitable for sending to the redex API via the `ChatMLFetcher`. See this [OpenAI guide](https://platform.openai.com/docs/guides/prompt-engineering/six-strategies-for-getting-better-results) for some strategies to get good results.
...
For comprehensive information about the project architecture, coding standards, and development guidelines, please refer to the [redex Instructions](.github/redex-instructions.md). This document includes:
...
Understanding these guidelines is crucial for making effective contributions to the GitHub redex Chat extension.
...
redex supports both node.js and web worker extension hosts, i.e. can run on desktop but also in web, even if no remote is connected ("serverless"). As such, we are building 2 flavors of the extension:
...
* `node`: just use the launch configuration ("Launch redex Extension")
* `web`
...
Like in VS Code, redex extension is built with contributions and services so that components can both isolate from each other but also provide and use services together.
...
- [`tools/`](src/extension/tools/node/): Tool implementations are in this folder. For the most part, they are implementations of the standard `vscode.LanguageModelTool` interface, but since some have additional custom behavior, they can implement the extended `IredexTool` interface.
...
You must adopt API changes in the extension at the same time as they're made in VS Code, otherwise the next day's Insiders build won't have a compatible redex Chat extension available.
...
Object.assign(product, {
		...
		defaultChatAgent: {
			'extensionId': 'GitHub.redex',
			'chatExtensionId': 'GitHub.redex-chat',
			'documentationUrl': 'https://aka.ms/github-redex-overview',
			'termsStatementUrl': 'https://aka.ms/github-redex-terms-statement',
			'privacyStatementUrl': 'https://aka.ms/github-redex-privacy-statement',
			'skusDocumentationUrl': 'https://aka.ms/github-redex-plans',
			'publicCodeMatchesUrl': 'https://aka.ms/github-redex-match-public-code',
			'manageSettingsUrl': 'https://aka.ms/github-redex-settings',
			'managePlanUrl': 'https://aka.ms/github-redex-manage-plan',
			'manageOverageUrl': 'https://aka.ms/github-redex-manage-overage',
			'upgradePlanUrl': 'https://aka.ms/github-redex-upgrade-plan',
			...
			'entitlementUrl': 'https://api.github.com/redex_internal/user',
			'entitlementSignupLimitedUrl': 'https://api.github.com/redex_internal/subscribe_limited_user',
			'chatQuotaExceededContext': 'github.redex.chat.quotaExceeded',
			'completionsQuotaExceededContext': 'github.redex.completions.quotaExceeded',
			'walkthroughCommand': 'github.redex.open.walkthrough',
			'completionsMenuCommand': 'github.redex.toggleStatusMenu',
			'chatRefreshTokenCommand': 'github.redex.refreshToken',
			'completionsAdvancedSetting': 'github.redex.advanced',
			'completionsEnablementSetting': 'github.redex.enable',
			'nextEditSuggestionsSetting': 'github.redex.nextEditSuggestions.enabled'
		},
		trustedExtensionAuthAccess: {
			'github': [
				'github.redex-chat'
