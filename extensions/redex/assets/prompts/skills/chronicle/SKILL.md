---
name: chronicle
description: Analyze redex session history for standup reports, usage tips, session search, and session reindexing. Use when the user asks for a standup, daily summary, usage tips, workflow recommendations, wants to search or find past sessions by keyword/file/PR, wants to reindex their session store, or asks about deleting session data.
---

# Chronicle

Analyze the user's redex session history using the `redex_sessionStoreSql` tool. This skill handles standup reports, usage analysis, session search, and session store maintenance.
...
**Prerequisite:** Chronicle requires the `github.redex.chat.localIndex.enabled` setting to be `true`. If the `redex_sessionStoreSql` tool is not available, tell the user to enable this setting in VS Code Settings.

## Available Tool Actions

The `redex_sessionStoreSql` tool supports two actions:
...
Use `redex_sessionStoreSql` with `action: "query"` and follow the SQL dialect shown in the tool description (SQLite locally, DuckDB on cloud — see the **Database Schema** and **Query Guidelines** sections below).
...
Use `redex_sessionStoreSql` with `action: "query"` to explore their recent sessions. The goal is to understand their patterns — how they prompt, what tools they use, and where they spend time.
...
If the current workspace has a `.github/` folder, check for `.github/redex-instructions.md`, `.github/skills/`, and `.github/agents/` to see what custom configuration exists. Do NOT look outside the workspace. Look for gaps between what's available and what the user actually uses.
...
When the user asks for cost tips, ways to reduce token usage, or how to lower redex spend (e.g. `/chronicle cost-tips`):
...
- **sessions.agent_name** / **agent_description** (both backends): the interactive VS Code chat surface is stored as `'VS Code Chat'` on cloud and `'GitHub redex Chat'` on local. Other values include `redex CLI` / `redexcli`, `redex Coding Agent`, subagents (`Explore`, `summarizeConversationHistory`, `panel/editAgent`, …), and custom agents.
...
Use `redex_sessionStoreSql` with `action: "query"`. Every query in this step must filter `sessions.agent_name` to the interactive VS Code chat value for the active backend — `'VS Code Chat'` on cloud, `'GitHub redex Chat'` on local. What to investigate depends on the active backend.
...
*Local (SQLite) — no token data; use proxies* (filter `sessions.agent_name = 'GitHub redex Chat'` on every query):
...
- **Repeated work** — the same file/topic showing up in many sessions, or the same agent stumbling block recurring (suggesting a custom skill, agent, or `redex-instructions.md` entry would let the model do the work in one shot).
...
If the current workspace has a `.github/` folder, check `.github/redex-instructions.md`, `.github/skills/`, and `.github/agents/` to see what custom configuration already exists. Do NOT look outside the workspace. Cost-relevant capabilities to keep in mind:
...
- `.github/redex-instructions.md` to encode project conventions the model otherwise has to be told every session.
- For cloud-enabled users, the redex usage view to inspect current premium-request spend.
...
- **Stay within VS Code chat scope** — tips should target interactive VS Code chat usage (compaction, model picker, fresh chats, `.github/redex-instructions.md`, custom skills/agents, subagent delegation). Do not propose CLI- or Coding-Agent–specific changes unless the user has explicitly broadened scope.
...
Read whichever instructions file the project uses (`.github/redex-instructions.md` or `AGENTS.md`) to understand what already exists.
...
Use `redex_sessionStoreSql` to explore. Scope all queries to sessions from the current repository or working directory.
...
Call `redex_sessionStoreSql` with `action: "query"` and `description: "Search sessions for <query>"`. Follow the SQL dialect shown in the tool description (SQLite vs DuckDB).
...
1. Call `redex_sessionStoreSql` with `action: "reindex"` and `description: "Reindex sessions"`.
...
- Guide them to run the **Delete Session Sync Data** command from the Command Palette (`github.redex.sessionSync.deleteSessions`).
...
- **Cloud enabled**: Queries go to the cloud DuckDB backend which contains ALL sessions across devices and agents (VS Code, CLI, redex Coding Agent, PR reviews). The tool description will show DuckDB SQL syntax — follow it.
