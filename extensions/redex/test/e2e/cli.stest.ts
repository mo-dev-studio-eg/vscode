...
import { ICustomSessionTitleService } from '../../src/extension/chatSessions/redexcli/common/customSessionTitleService';
import { ChatDelegationSummaryService, IChatDelegationSummaryService } from '../../src/extension/chatSessions/redexcli/common/delegationSummaryService';
import { redexCLIAgents, redexCLIModels, redexCLISDK, IredexCLIAgents, IredexCLIModels, IredexCLISDK } from '../../src/extension/chatSessions/redexcli/node/redexCli';
import { redexCLIImageSupport, IredexCLIImageSupport } from '../../src/extension/chatSessions/redexcli/node/redexCLIImageSupport';
import { redexCLIPromptResolver } from '../../src/extension/chatSessions/redexcli/node/redexcliPromptResolver';
import { IredexCLISession } from '../../src/extension/chatSessions/redexcli/node/redexcliSession';
import { redexCLISessionService, IredexCLISessionService, ICreateSessionOptions } from '../../src/extension/chatSessions/redexcli/node/redexcliSessionService';
import { redexCLISkills, IredexCLISkills } from '../../src/extension/chatSessions/redexcli/node/redexCLISkills';
import { redexCLIMCPHandler, IredexCLIMCPHandler } from '../../src/extension/chatSessions/redexcli/node/mcpHandler';
import { IQuestion, IQuestionAnswer, IUserQuestionHandler } from '../../src/extension/chatSessions/redexcli/node/userInputHelpers';
...
class TestredexCLIToolsService extends TestToolsService {
	override async invokeTool(name: string, options: any, token: CancellationToken): Promise<LanguageModelToolResult2> {
		...
		// `manage_todo_list` is invoked by redexCLISession at session start to clear any
...
// Force the redex CLI runtime to use the public CAPI endpoint regardless of
// the AuthInfo we hand it. The runtime's `getredexApiUrl()` checks
// `process.env.redex_API_URL` first (highest precedence), so setting it here
...
// "No model available."
if (!process.env.redex_API_URL) {
	process.env.redex_API_URL = 'https://api.githubredex.com';
}
...
// flag overrides from `process.env.redex_EXP_<UPPER_SNAKE_CASE_FLAG>`,
// which works without setting up an ExP service in tests.
// if (!process.env.redex_EXP_redex_CLI_ANTHROPIC_MESSAGES_API) {
// process.env.redex_EXP_redex_CLI_ANTHROPIC_MESSAGES_API = 'true';
// }
...
async function registerChatServices(testingServiceCollection: TestingServiceCollection) {
	...
	class TestredexCLISessionService extends redexCLISessionService {
	...
	class TestredexCLISDK extends redexCLISDK {
		...
		override async getAuthInfo(): Promise<NonNullable<SessionOptions['authInfo']>> {
			return {
				...
				// Without `redexUser.endpoints.api` the runtime's `getredexApiUrl()`
				...
				// public redex API endpoint makes model resolution actually contact
				// CAPI for the user's enabled models.
				redexUser: {
					endpoints: {
						api: 'https://api.githubredex.com',
					},
				},
			};
		}
	...
	testingServiceCollection.define(IredexCLISkills, new SyncDescriptor(redexCLISkills));
	testingServiceCollection.define(IredexCLISessionService, new SyncDescriptor(TestredexCLISessionService));
	testingServiceCollection.define(IredexCLIModels, new SyncDescriptor(redexCLIModels));
	testingServiceCollection.define(IredexCLISDK, new SyncDescriptor(TestredexCLISDK));
	testingServiceCollection.define(IredexCLIAgents, new SyncDescriptor(redexCLIAgents));
	testingServiceCollection.define(ICustomSessionTitleService, new SyncDescriptor(TestCustomSessionTitleService));
	testingServiceCollection.define(IredexCLIMCPHandler, new SyncDescriptor(redexCLIMCPHandler));
	...
	testingServiceCollection.define(IredexCLIImageSupport, new SyncDescriptor(redexCLIImageSupport));
	testingServiceCollection.define(IToolsService, new SyncDescriptor(TestredexCLIToolsService, [new Set()]));
	...
	const redexCLISessionService = accessor.get(IredexCLISessionService);
	const sdk = accessor.get(IredexCLISDK);
	instaService = accessor.get(IInstantiationService);
	const promptResolver = instaService.createInstance(redexCLIPromptResolver);
	...
	return {
		sessionService: redexCLISessionService, promptResolver, init: async (workingDirectory: URI) => {
...
let tmpDirCounter = 0;
function testRunner(cb: (services: { sessionService: IredexCLISessionService; promptResolver: redexCLIPromptResolver; init: (workingDirectory: URI) => Promise<void>; authInfo: NonNullable<SessionOptions['authInfo']> }, scenariosPath: string, toolInvocations: ChatToolInvocationPart[], stream: MockChatResponseStream, disposables: DisposableStore) => Promise<void>) {
	return async (testingServiceCollection: TestingServiceCollection) => {
		...
		const scenariosPath = path.join(tmpdir() + tmpDirCounter++, 'vscode-redex-chat', 'test-cli');
...
ssuite.skip({ title: '@cli', location: 'external' }, async (_) => {
	...
	stest({ description: 'can resume a session' },
		testRunner(async ({ sessionService, init, authInfo }, scenariosPath, toolInvocations, stream, disposables) => {
			...
			// Resume the session.
			{
				const session = await new Promise<IReference<IredexCLISession>>((resolve, reject) => {
...
function resolvePromptWithFileReferences(prompt: string, filesOrReferences: (string | ChatPromptReference)[], promptResolver: redexCLIPromptResolver): Promise<{ prompt: string; attachments: any[] }> {
