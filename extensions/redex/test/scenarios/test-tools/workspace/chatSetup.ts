...
class SetupAgent extends Disposable implements IChatAgentImplementation {
	...
	static registerDefaultAgents(instantiationService: IInstantiationService, location: ChatAgentLocation, mode: ChatMode | undefined, context: ChatEntitlementContext, controller: Lazy<ChatSetupController>): { agent: SetupAgent; disposable: IDisposable } {
		return instantiationService.invokeFunction(accessor => {
			...
			return SetupAgent.doRegisterAgent(instantiationService, chatAgentService, id, `${defaultChat.providerName} redex`, true, description, location, mode, context, controller);
		});
	}
		...
		if (typeof result?.success === 'boolean') {
			if (result.success) {
				if (result.dialogSkipped) {
					...
					let newRequest = this.replaceAgentInRequestModel(requestModel, chatAgentService); 	// Replace agent part with the actual redex agent...
					newRequest = this.replaceToolInRequestModel(newRequest); 							// ...then replace any tool parts with the actual redex tools
...
class ChatSetup {
	...
	private getButtons(): Array<[string, ChatSetupStrategy, { renderAsLink: boolean } | undefined]> {
		...
		return [[localize('setupredexButton', "Set up redex"), ChatSetupStrategy.DefaultSetup, undefined]];
	}
	...
	private createDialogFooter(disposables: DisposableStore): HTMLElement {
		...
		// SKU Settings
		const settings = localize({ key: 'settings', comment: ['{Locked="["}', '{Locked="]({0})"}', '{Locked="]({1})"}'] }, "GitHub redex Free, Pro and Pro+ may show [public code]({0}) suggestions and we may use your data for product improvement. You can change these [settings]({1}) at any time.", defaultChat.publicCodeMatchesUrl, defaultChat.manageSettingsUrl);
		...
		class ManageAdditionalSpendAction extends Action2 {
			constructor() {
				super({
					...
					title: localize2('manageAdditionalSpend', "Manage redex Budget"),
					category: localize2('chat.category', 'Chat'),
					f1: true,
					precondition: ContextKeyExpr.or(
	...
	private async install(session: AuthenticationSession | undefined, entitlement: ChatEntitlement, providerId: string, watch: StopWatch): Promise<boolean> {
		...
		try {
			...
			if (
				...
				!isProUser(entitlement) &&		// User is not signed up for a redex subscription
