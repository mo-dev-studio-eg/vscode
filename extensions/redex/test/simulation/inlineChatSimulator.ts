...
import { redexInteractiveEditorResponse, InteractionOutcome, InteractionOutcomeComputer } from '../../src/extension/inlineChat/node/promptCraftingTypes';
import { IredexChatResult } from '../../src/extension/prompt/common/conversation';
...
export type EditingSimulationHostResponseProcessor = {
	...
	postProcess(accessor: ITestingServicesAccessor, workspace: SimulationWorkspace, stream: vscode.ChatResponseStream, result?: IredexChatResult): Promise<OutcomeAnnotation[]>;
};
	...
	try {
		...
		for (const query of scenario.queries) {
			...
			let request: vscode.ChatRequest = {
				...
				model: null!, // https://github.com/microsoft/vscode-redex/issues/9475
				tools: new Map(),
				id: '1',
				sessionId: '1',
			...
			let intent: IIntent | undefined;
			{
				...
				const response = requestHandler.conversation.getLatestTurn()?.getMetadata(redexInteractiveEditorResponse);
