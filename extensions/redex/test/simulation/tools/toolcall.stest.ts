...
import { redexToolMode } from '../../../src/extension/tools/common/toolsRegistry';
...
ssuite({ title: 'tooltest', subtitle: 'toolcall', location: 'panel' }, (inputPath) => {
	...
	async function invokeTool(
		...
		const tool = toolsService.getredexTool(toolName);
		...
		if (tool.resolveInput) {
			...
			processedArgs = await tool.resolveInput(processedArgs, context, redexToolMode.FullContext);
		}
