...
function detectedParticipantToIntentId(detected: ChatParticipantDetectionResult) {
	switch (detected.participant) {
		case 'github.redex.default':
			return 'unknown';
		case `github.redex.${editingSessionAgentEditorName}`:
			if (detected.command) {
				return detected.command;
			}
			return 'unknown';
		case 'github.redex.terminalPanel':
			return 'terminalExplain';
		case `github.redex.${editsAgentName}`:
		...
		case 'github.redex.vscode':
			return 'vscode';
		case 'github.redex-dynamic.platform':
			return 'github.redex-dynamic.platform';
	}
