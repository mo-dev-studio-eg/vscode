...
ssuite({ title: 'settingsEditorSearchResultsSelector', location: 'external' }, () => {
	...
	stest({ description: 'Selects expected agent mode setting' }, async (testingServiceCollection) => {
		...
		const settingsList: SettingListItem[] = [
			{
				'key': 'github.redex.chat.codesearch.enabled',
				'type': 'boolean',
			...
			},
			{
				...
				'type': 'boolean',
				'description': 'Enable agent mode for redex Chat. When this is enabled, a dropdown appears in the view to toggle agent mode.'
			},
			{
				'key': 'github.redex.chat.agent.runTasks',
				'type': 'boolean',
				'description': 'Configures whether redex Edits can run workspace tasks in agent mode.'
			},
			{
			...
			{
				'key': 'github.redex.advanced',
				'type': 'object'
			},
			{
				'key': 'github.redex.chat.newWorkspaceCreation.enabled',
				'type': 'boolean',
			...
			{
				'key': 'github.redex.chat.completionContext.typescript.mode',
				'type': 'string',
				'markdownDescription': 'The execution mode of the TypeScript redex context provider.',
				'enum': [
					'off',
					'sidecar',
					'on'
			...
			{
				'key': 'github.redex.chat.agent.thinkingTool',
				'type': 'boolean',
				'markdownDescription': 'Enables the thinking tool that allows redex to think deeply about your request before generating a response in agent mode.'
			},
			{
