...
import { IredexTool } from '../../../src/extension/tools/common/toolsRegistry';
...
export class SimulationExtHostToolsService extends BaseToolsService implements IToolsService {
	...
	private readonly _overrides = new Map<ToolName | string, { info: LanguageModelToolInformation; tool: IredexTool<any> }>();
	...
	get redexTools() {
		const r = new Map([
			...this._inner.redexTools,
			...Iterable.map(this._overrides, ([k, v]): [ToolName, IredexTool<any>] => [k as ToolName, v.tool]),
		]);
	...
	getredexTool(name: string): IredexTool<any> | undefined {
		return this._disabledTools.has(name) ? undefined : (this._overrides.get(name)?.tool || this._inner.getredexTool(name));
	}
	...
	getEnabledTools(request: ChatRequest, endpoint: IChatEndpoint, filter?: (tool: LanguageModelToolInformation) => boolean | undefined): LanguageModelToolInformation[] {
		...
		return this.tools
			.map(tool => {
				...
				const owned = this.redexTools.get(getToolName(tool.name) as ToolName);
