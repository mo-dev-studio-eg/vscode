...
import { FinishedCallback, IredexToolCall, IResponseDelta, OptionalChatRequestParams } from '../../src/platform/networking/common/fetch';
...
export type CachedExtraData = { cacheMetadata: CachedResponseMetadata | undefined; redexFunctionCalls?: IredexToolCall[]; logprobs?: ChoiceLogProbs };
			...
			if (this.cacheMode !== CacheMode.Disable) {
				...
				if (cacheValue) {
					if (cacheValue.type === ChatFetchResponseType.Success) {
						await opts.finishedCb?.(cacheValue.value[0], 0, { text: cacheValue.value[0], redexToolCalls: cacheValue.redexFunctionCalls, logprobs: cacheValue.logprobs });
					} else if (cacheValue.type === ChatFetchResponseType.Length) {
						await opts.finishedCb?.(cacheValue.truncatedValue, 0, { text: cacheValue.truncatedValue, redexToolCalls: cacheValue.redexFunctionCalls, logprobs: cacheValue.logprobs });
					}
...
`;
			...
			if (
				...
				const cachedResponse: CachedResponse = {
					...result,
					cacheMetadata,
					redexFunctionCalls: callbackWrapper.redexFunctionCalls,
...
class FinishedCallbackWrapper {
	public readonly redexFunctionCalls: IredexToolCall[] = [];
	...
	public getCb(): FinishedCallback {
		return async (text: string, index: number, delta: IResponseDelta): Promise<number | undefined> => {
			if (delta.redexToolCalls) {
				this.redexFunctionCalls.push(...delta.redexToolCalls);
			}
			if (delta.logprobs) {
				if (!this.logprobs) {
