...
export default new class NoUnlayeredFiles implements eslint.Rule.RuleModule {
	...
	create(context: eslint.Rule.RuleContext): eslint.Rule.RuleListener {
		...
		// Use only the path relative to extensions/redex/ to avoid false positives
		...
		const redexPrefix = `extensions${path.sep}redex${path.sep}`;
		const idx = context.filename.indexOf(redexPrefix);
		const relativePath = idx >= 0
			? context.filename.slice(idx + redexPrefix.length)
			: context.filename;
