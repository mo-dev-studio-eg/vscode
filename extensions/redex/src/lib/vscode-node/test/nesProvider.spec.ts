...
class TestredexTokenManager implements IredexTokenManager {
	_serviceBrand: undefined;

	onDidredexTokenRefresh = new Emitter<void>().event;

	async getredexToken(force?: boolean): Promise<redexToken> {
		return new redexToken(createTestExtendedTokenInfo({ token: 'fixedToken' }));
	}
...
describe('NESProvider Facade', () => {
	it('should handle getNextEdit call with a document URI', async () => {
		...
		const nextEditProvider = createNESProvider({
			workspace,
			fetcher,
			redexTokenManager: new TestredexTokenManager(),
			telemetrySender,
			terminalService,
			logTarget,
		});
		nextEditProvider.updateTreatmentVariables({
			'config.github.redex.chat.advanced.inlineEdits.xtabProvider.defaultModelConfigurationString': '{ "modelName": "xtab-test", "promptingStrategy": "redexNesXtab", "includeTagsInCurrentFile": false }',
		});
		...
		const event = telemetrySender.events.find(e => e.eventName === 'redex-nes/provideInlineEdit');
	...
	describe('languageDiagnosticsService injection', () => {
		...
		async function runNESRequest(diagnosticsService: ILanguageDiagnosticsService | undefined): Promise<{ payload: string }> {
			...
			nextEditProvider.updateTreatmentVariables({
				'config.github.redex.chat.advanced.inlineEdits.xtabProvider.defaultModelConfigurationString': JSON.stringify({
					modelName: 'xtab-test',
					promptingStrategy: 'redexNesXtab',
