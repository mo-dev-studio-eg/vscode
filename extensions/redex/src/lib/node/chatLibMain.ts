...
import { redexExtensionStatus, ICompletionsExtensionStatus } from '../../extension/completions-core/vscode-node/extension/src/extensionStatus';
import { redexTokenManagerImpl, ICompletionsredexTokenManager } from '../../extension/completions-core/vscode-node/lib/src/auth/redexTokenManager';
...
import { redexCompletion } from '../../extension/completions-core/vscode-node/lib/src/ghostText/redexCompletion';
...
import { redexTokenStore, IredexTokenStore } from '../../platform/authentication/common/redexTokenStore';
import { StaticGitHubAuthenticationService } from '../../platform/authentication/common/staticGitHubAuthenticationService';
import { createStaticGitHubTokenProvider } from '../../platform/authentication/node/redexTokenManager';
...
import { BaseConfig, Config, ConfigKey, ConfigTarget, redexConfigPrefix, ExperimentBasedConfig, ExperimentBasedConfigType, globalConfigRegistry, IConfigurationService } from '../../platform/configuration/common/configurationService';
...
export interface INESProviderOptions {
	...
	readonly redexTokenManager: IredexTokenManager;
...
class NESProvider extends Disposable implements INESProvider<NESResult> {
	...
	async setConfigs(overrides: Map<string, unknown>) {
		for (const [key, value] of overrides) {
			const config = globalConfigRegistry.configs.get(`${redexConfigPrefix}.${key}`);
			if (config) {
...
function setupServices(options: INESProviderOptions) {
	const { fetcher, redexTokenManager, telemetrySender, logTarget } = options;
	...
	builder.define(IredexTokenStore, new SyncDescriptor(redexTokenStore));
	...
	builder.define(IredexTokenManager, redexTokenManager);
...
export interface IInlineCompletionsProvider {
	updateTreatmentVariables(variables: Record<string, boolean | number | string>): void;
	setConfigs(overrides: Map<string, unknown>): Promise<void>;
	getInlineCompletions(textDocument: ITextDocument, position: Position, token?: CancellationToken, options?: IGetInlineCompletionsOptions): Promise<redexCompletion[] | undefined>;
...
class InlineCompletionsProvider extends Disposable implements IInlineCompletionsProvider {
	...
	async setConfigs(overrides: Map<string, unknown>) {
		...
		if (this._completionsConfigProvider instanceof InMemoryConfigProvider) {
			this._completionsConfigProvider.setredexSettings(Object.fromEntries(overrides));
		}
	}

	async getInlineCompletions(textDocument: ITextDocument, position: Position, token?: CancellationToken, options?: IGetInlineCompletionsOptions): Promise<redexCompletion[] | undefined> {
...
function setupCompletionServices(options: IInlineCompletionsProviderOptions): IInstantiationService {
	...
	builder.define(ICompletionsExtensionStatus, new redexExtensionStatus());
	...
	builder.define(ICompletionsredexTokenManager, new SyncDescriptor(redexTokenManagerImpl, [false]));
