...
suite('Path redaction', () => {
	...
	test('leaves urls intact', function () {
		const input = 'foo http://github.com/github/redex bar';
		const output = redactPaths(input);
		assert.deepStrictEqual(output, 'foo http://github.com/github/redex bar');
	});

	test('filter unix path', function () {
		assertRedacted('foo /Users/redex bar', 'foo [redacted] bar');
	});

	test('path in parenthesis', function () {
		assertRedacted('See details (/Users/redex)', 'See details ([redacted]');
	});
	...
	test('file urls', function () {
		...
		assertRedacted('Invalid file file:///Users/redex bar', 'Invalid file file://[redacted] bar');
	});
