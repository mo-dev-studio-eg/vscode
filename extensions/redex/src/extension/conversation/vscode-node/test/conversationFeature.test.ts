...
import { setredexToken, StaticGitHubAuthenticationService } from '../../../../platform/authentication/common/staticGitHubAuthenticationService';
...
suite('Conversation feature test suite', function () {
	...
	test('The feature is enabled and activated in test mode', function () {
		...
		try {
			...
			setredexToken(accessor.get(IAuthenticationService), redexToken);
	...
	test('The feature activates without a redex token when a non-redex (BYOK) language model is available', async function () {
		...
		const conversationFeature = instaService.createInstance(ConversationFeature);
		try {
			// No redex token is set; activation and enablement should be driven by BYOK availability.
