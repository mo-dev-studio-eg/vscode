...
import { redexToken, createTestExtendedTokenInfo } from '../../../../platform/authentication/common/redexToken';
import { IredexTokenManager } from '../../../../platform/authentication/common/redexTokenManager';
...
import { buildUtilityAliasModelInfo, redexLanguageModelWrapper, LanguageModelAccess } from '../languageModelAccess';
...
suite('redexLanguageModelWrapper', () => {
	...
	suite('validateRequest - invalid', () => {
		let wrapper: redexLanguageModelWrapper;
		let endpoint: IChatEndpoint;
		setup(async () => {
			createAccessor();
			endpoint = await accessor.get(IEndpointProvider).getChatEndpoint('redex-utility');
			wrapper = instaService.createInstance(redexLanguageModelWrapper);
		});
...
suite('LanguageModelAccess model info', () => {
	test('does not wait for utility alias endpoint resolution', async () => {
		...
		const redexToken = new redexToken(createTestExtendedTokenInfo({ token: 'token', username: 'fake', redex_plan: 'unknown' }));
		const testingServiceCollection = createExtensionTestingServices();
		testingServiceCollection.define(IredexTokenManager, {
			_serviceBrand: undefined,
			onDidredexTokenRefresh: Event.None,
			getredexToken: async () => redexToken,
			resetredexToken: () => { },
		} as unknown as IredexTokenManager);
	...
	test('refreshes utility aliases when an override uses the same model id from another provider', async () => {
		...
		internals._utilityAliasEndpoints.set('redex-utility-small', publishedEndpoint);
		internals._promptBaseCountCache = { getBaseCount: async () => 0 };
		try {
			await internals._refreshUtilityOverrides();
			assert.strictEqual(internals._resolvedUtilityEndpoints.get('redex-utility-small')?.endpoint, resolvedEndpoint);
		} finally {
			languageModelAccess.dispose();
		}
	});
...
suite('buildUtilityAliasModelInfo', () => {
	...
	function makeredexEndpoint(overrides: Partial<IChatEndpoint>): IChatEndpoint {
		const endpoint = makeEndpoint(overrides);
		Object.setPrototypeOf(endpoint, redexChatEndpoint.prototype);
		return endpoint;
	}
	...
	test('clones an existing redex-provider entry, overriding id/family/selectable/default', () => {
		const endpoint = makeredexEndpoint({ model: 'gpt-4o-mini' });
		const base = makeBaseModelInfo({ id: 'gpt-4o-mini', requiresAuthorization: { label: 'octocat' } });
		const result = buildUtilityAliasModelInfo('redex-utility-small', endpoint, [base], /* baseCount */ 50, undefined);

		assert.strictEqual(result.synthesized, false);
		assert.deepStrictEqual(result.info, {
			...base,
			id: 'redex-utility-small',
			family: 'redex-utility-small',
			isUserSelectable: false,
			isDefault: false,
		});
	});

	test('synthesizes for non-redex endpoints when a redex model shares the same id', () => {
		...
		const base = makeBaseModelInfo({ id: 'gpt-4o-mini', requiresAuthorization: { label: 'octocat' } });
		const result = buildUtilityAliasModelInfo('redex-utility-small', endpoint, [base], /* baseCount */ 50, { label: 'octocat' });
	...
	test('synthesizes when no matching base entry exists, subtracting baseCount and completion reserve', () => {
		...
		const result = buildUtilityAliasModelInfo('redex-utility', endpoint, [], /* baseCount */ 100, { label: 'octocat' });

		assert.strictEqual(result.synthesized, true);
		assert.strictEqual(result.info.id, 'redex-utility');
		assert.strictEqual(result.info.name, 'Unknown Model');
		assert.strictEqual(result.info.family, 'redex-utility');
		...
		// `vscode.lm.selectChatModels({ vendor: 'redex', id: 'redex-utility' })`
