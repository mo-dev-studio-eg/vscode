 ...
 * `/<repo>/...`, the redex flow emits `<projectName>/...`). The platform-
...
suite('resolveProjectFileUri', () => {
	...
	test('handles a non-prefixed file path (redex new-workspace flow)', () => {
	...
	test('source preview URI built from the unmodified `file` resolves to the expected preview path', () => {
		...
		const baseUri = Uri.parse('vscode-redex-github-workspace://req/myrepo');
		...
		// redex new-workspace flow: emitted file = `myproject/src/index.ts`.
		const redexBase = Uri.parse('vscode-redex-workspace://req/myproject');
		expect(Uri.joinPath(redexBase, 'myproject/src/index.ts').path).toBe('/myproject/myproject/src/index.ts');
	});
});
