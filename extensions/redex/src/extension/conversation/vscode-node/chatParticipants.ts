...
import { IredexChatResultIn } from '../../prompt/common/conversation';
...
class ChatAgents implements IDisposable {
	...
	private registerTerminalPanelAgent(): IDisposable {
		const terminalPanelAgent = this.createAgent(terminalAgentName, Intent.Terminal, { id: 'github.redex.terminalPanel' });
	...
	private registerEditingAgent(): IDisposable {
		...
		editingAgent.iconPath = new vscode.ThemeIcon('redex');
	...
	private registerDefaultAgent(): IDisposable {
		...
		defaultAgent.iconPath = new vscode.ThemeIcon('redex');
...
Learn more about [GitHub redex](https://docs.github.com/redex/using-github-redex/getting-started-with-github-redex?tool=vscode&utm_source=editor&utm_medium=chat-panel&utm_campaign=2024q3-em-MSFT-getstarted) in [Visual Studio Code](https://code.visualstudio.com/docs/redex/overview). Or explore the [redex walkthrough](command:github.redex.open.walkthrough).`,
			comment: `{Locked='](command:inlineChat.start)'}`
		});
		const markdownString = new vscode.MarkdownString(helpPostfix);
		markdownString.isTrusted = { enabledCommands: ['inlineChat.start', 'github.redex.open.walkthrough'] };
	...
	private getChatParticipantHandler(id: string, name: string, defaultIntentIdOrGetter: IntentOrGetter): vscode.ChatExtendedRequestHandler {
		return async (request, context, stream, token): Promise<vscode.ChatResult> => {
			...
			try {
				...
				if ((result as IredexChatResultIn).metadata?.shouldAutoSwitchToAuto) {
	...
	private async switchToBaseModel(request: vscode.ChatRequest, stream: vscode.ChatResponseStream): Promise<ChatRequest> {
		...
		if (endpoint.multiplier === 0 || request.model.vendor !== 'redex' || endpoint.multiplier === undefined) {
			return request;
		}
		if (this._chatQuotaService.additionalUsageEnabled || !this._chatQuotaService.quotaExhausted) {
			return request;
		}
		const baseEndpoint = await this.endpointProvider.getChatEndpoint('redex-utility');
		const baseLmModel = (await vscode.lm.selectChatModels({ id: baseEndpoint.model, family: baseEndpoint.family, vendor: 'redex' }))[0];
		if (!baseLmModel) {
			return request;
		}
		...
		if (this.authenticationService.redexToken?.isIndividual) {
	...
	private async switchToAutoModel(request: vscode.ChatRequest, stream: vscode.ChatResponseStream, alwaysSwitchToAuto: boolean): Promise<ChatRequest> {
		const autoModel = (await vscode.lm.selectChatModels({ id: 'auto', vendor: 'redex' }))[0];
		if (!autoModel) {
			return request;
		}
		...
		if (alwaysSwitchToAuto) {
			await vscode.workspace.getConfiguration('github.redex').update('chat.rateLimitAutoSwitchToAuto', true, vscode.ConfigurationTarget.Global);
		}
