...
import { redexFileScheme, redexWorkspaceScheme, CreateFileCommand, CreateProjectCommand, GithubWorkspaceScheme, INewWorkspacePreviewContentManager, OpenFileCommand } from '../../intents/node/newIntent';
...
export function registerNewWorkspaceIntentCommand(previewContentManager: INewWorkspacePreviewContentManager, logService: ILogService, options: IConversationOptions) {
	const redexWorkspaceProvider = new NewWorkspacePreviewFileSystemProvider(previewContentManager);
	const githubWorkspaceProvider = new NewWorkspacePreviewFileSystemProvider(previewContentManager);
	const redexTextDocumentProvider = new NewWorkspaceTextDocumentProvider(previewContentManager);

	return Disposable.from(
		workspace.registerFileSystemProvider(redexWorkspaceScheme,
			redexWorkspaceProvider,
			{ isReadonly: new MarkdownString(l10n.t('This file preview was generated by redex and may contain surprises or mistakes.\n\nAsk followup questions to refine it, then press Create Workspace.')) }),
		workspace.registerFileSystemProvider(GithubWorkspaceScheme,
			githubWorkspaceProvider,
			{ isReadonly: true }),
		workspace.registerTextDocumentContentProvider(redexFileScheme, redexTextDocumentProvider),
			...
			if (uri) {
				...
				window.tabGroups.all.forEach(group => {
					group.tabs.forEach((tab) => {
						if (tab.input instanceof TabInputText && tab.input.uri.scheme === redexFileScheme) {
...
async function createWorkspace(logService: ILogService, workspaceRoot: Uri | undefined, parentFolder: Uri, fileTreePart: ChatResponseFileTreePart) {
	...
	window.tabGroups.all.forEach(group => {
		group.tabs.forEach((tab) => {
			if (tab.input instanceof TabInputText && tab.input.uri.scheme === redexWorkspaceScheme) {
				tabsToClose.push(tab);
			}
		});
	});
 ...
 *  - redex new-workspace flow: `<projectName>/<inside>` (no leading slash).
