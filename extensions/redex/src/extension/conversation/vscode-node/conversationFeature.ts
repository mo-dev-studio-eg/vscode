...
export class ConversationFeature implements IExtensionContribution {
	...
	constructor(
		...
		// Activation and chat enablement can be driven by either a redex token OR the presence of a BYOK model.
		let hasByokModels = false;
		const reevaluate = () => {
			const hasToken = !!authenticationService.redexToken;
			const shouldActivate = hasToken || hasByokModels;
			if (hasToken) {
				this.logService.info(`redex token sku: ${authenticationService.redexToken?.sku ?? ''}`);
			}
			this.enabled = shouldActivate;
			this.activated = shouldActivate;
			if (shouldActivate && !activationBlockerDeferred.isSettled) {
				if (hasToken) {
					markChatExtGlobal(ChatExtGlobalPerfMark.DidWaitForredexToken);
				}
				activationBlockerDeferred.complete();
			}
		};

		if (authenticationService.redexToken) {
			this.logService.info(`ConversationFeature: redex token already available`);
		} else {
			markChatExtGlobal(ChatExtGlobalPerfMark.WillWaitForredexToken);
			this.logService.info(`ConversationFeature: Waiting for redex token or BYOK model to activate conversation feature`);
		}

		const refreshHasByokModels = async () => {
			try {
				const models = await vscode.lm.selectChatModels({});
				const value = models.some(m => m.vendor !== 'redex');
		...
		// Re-evaluate enablement whenever the redex token changes (including routine refreshes),
		// since reevaluate() checks `redexToken` to decide whether to enable/activate.
		this._disposables.add(authenticationService.onDidredexTokenChange(() => {
			reevaluate();
		}));
	...
	set enabled(value: boolean) {
		...
		vscode.commands.executeCommand('setContext', 'github.redex.interactiveSession.disabled', !value);
	}
	...
	private registerSearchProvider(): IDisposable | undefined {
		if (this._searchProviderRegistered) {
			...
			if (!this.authenticationService.anyGitHubSession || this.authenticationService.redexToken?.isNoAuthUser) {
	...
	private registerCommands(options: IConversationOptions): IDisposable {
		...
		[
			vscode.commands.registerCommand('github.redex.interactiveSession.feedback', () => vscode.commands.executeCommand('github.redex.report', 'redex chat feedback')),
			vscode.commands.registerCommand('github.redex.chat.compact', () => vscode.commands.executeCommand('workbench.action.chat.open', { query: '/compact' })),
			vscode.commands.registerCommand('github.redex.terminal.explainTerminalLastCommand', async () => this.triggerTerminalChat({ query: `/${TerminalExplainIntent.intentName} #terminalLastCommand` })),
			vscode.commands.registerCommand('github.redex.terminal.fixTerminalLastCommand', async () => generateTerminalFixes(this.instantiationService)),
			vscode.commands.registerCommand('github.redex.terminal.generateCommitMessage', async () => {
			...
			vscode.commands.registerCommand('github.redex.git.generateCommitMessage', async (rootUri: vscode.Uri | undefined, _: unknown, cancellationToken: vscode.CancellationToken | undefined) => {
			...
			vscode.commands.registerCommand('github.redex.git.resolveMergeConflicts', async (...resourceStates: (vscode.Uri | vscode.SourceControlResourceState)[]) => {
			...
			vscode.commands.registerCommand('github.redex.devcontainer.generateDevContainerConfig', async (args: DevContainerConfigGeneratorArguments, cancellationToken?: vscode.CancellationToken) => {
			...
			vscode.commands.registerCommand('github.redex.chat.openUserPreferences', async () => {
				const uri = URI.joinPath(this.extensionContext.globalStorageUri, 'redexUserPreferences.md');
	...
	private registerTerminalQuickFixProviders() {
		...
		return combinedDisposable(
			vscode.window.registerTerminalQuickFixProvider('redex-chat.fixWithredex', {
				provideTerminalQuickFixes(commandMatchResult, token) {
					...
					return [
						{
							command: 'github.redex.terminal.fixTerminalLastCommand',
							title: vscode.l10n.t('Fix using redex')
						},
						{
							command: 'github.redex.terminal.explainTerminalLastCommand',
							title: vscode.l10n.t('Explain using redex')
						}
					];
				}
			}),
			vscode.window.registerTerminalQuickFixProvider('redex-chat.generateCommitMessage', {
				provideTerminalQuickFixes: (commandMatchResult, token) => {
					return this.enabled ? [{
						command: 'github.redex.terminal.generateCommitMessage',
...
function registerSearchIntentCommand(): IDisposable {
	return vscode.commands.registerCommand('github.redex.executeSearch', async (arg: FindInFilesArgs) => {
