...
import { IredexReference } from '../../../platform/networking/common/fetch';
...
import { IredexChatResult, IredexChatResultIn } from '../../prompt/common/conversation';
...
export class RemoteAgentContribution implements IDisposable {
	...
	private async _doRefreshRemoteAgents(): Promise<void> {
		...
		try {
			...
			this.logService.error(e, 'Failed to load remote redex agents');
		}
	...
	private checkAuthorized(agent: string) {
		...
		const key = `redex.agent.${agent}.authorized`;
	...
	private async setAuthorized(agent: string, isGlobal = false) {
		...
		await memento.update(`redex.agent.${agent}.authorized`, true);
	}

	private registerAgent(agentData: IAgent | null): Disposable {
		const store = new DisposableStore();
		const participantId = `github.redex-dynamic.${agentData?.slug ?? GITHUB_PLATFORM_AGENT_ID}`;
		...
		const agent = store.add(chat.createDynamicChatParticipant(participantId, dynamicProps, async (request, context, responseStream, token): Promise<IredexChatResult> => {
			...
			const metadata: IredexChatResult['metadata'] & Record<string, unknown> = {
				sessionId,
				modelMessageId,
				responseId,
			...
			if (request.acceptedConfirmationData) {
				for (const data of request.acceptedConfirmationData) {
					if (data?.url) {
						...
						return { metadata } satisfies IredexChatResult;
			...
			if (!agentData?.slug) {
				...
				if (!accessToken) {
					if (this.authenticationService.isMinimalMode) {
						responseStream.markdown(l10n.t('Minimal mode is enabled. You will need to change `github.redex.advanced.authPermissions` to `default` to use this feature.'));
						responseStream.button({
							title: l10n.t('Open Settings (JSON)'),
							command: 'workbench.action.openSettingsJson',
							arguments: [{ revealSetting: { key: 'github.redex.advanced.authPermissions' } }]
						});
					} else {
			...
			try {
				...
				// Collect redex skills and references to be sent in the request
				const redexReferences = [];
				const { redex_skills } = await this.resolveredexSkills(slug, request);
				...
				try {
					...
					if (result.clientReferences) {
						redexReferences.push(...result.clientReferences);
					}
				...
				const reportedReferences = new Map<string, IredexReference>();
				const agentReferences: IredexReference[] = [];
				...
				let hadredexErrorsOrConfirmations = false;
				...
				const response = await endpoint.makeChatRequest(
					...
					[
						...prepareRemoteAgentHistory(participantId, context),
						{
							...
							...(redexReferences.length ? { redex_references: redexReferences } : undefined),
							...(confirmations?.length ? { redex_confirmations: confirmations } : undefined),
						}
					],
					async (result, _, delta) => {
						if (delta.redexReferences) {

							const processReference = (reference: IredexReference, parentReference?: IredexReference) => {
							...
							for (const reference of delta.redexReferences) {
						...
						if (delta._deprecatedredexFunctionCalls) {
							for (const call of delta._deprecatedredexFunctionCalls) {
								switch (call.name) {
									case 'bing-search': {
										try {
						...
						if (delta.redexErrors && typeof responseStream.warning === 'function') {
							hadredexErrorsOrConfirmations = true;
							for (const error of delta.redexErrors) {
								if (reportedProgress) {
						...
						if (delta.redexConfirmation) {
							hadredexErrorsOrConfirmations = true;
							const confirm = delta.redexConfirmation;
					...
					undefined,
					{
						...
						redex_thread_id: sessionId,
						...(redex_skills ? { redex_skills } : undefined)
					},
					true,
					{
				...
				metadata['redex_references'] = [...new Set(reportedReferences.values()).values(), ...agentReferences];
				...
				if (response.type !== ChatFetchResponseType.Success) {
					...
					if (response.reason.includes('400 no docs found')) {
						...
						return { metadata, nextQuestion: { prompt: request.prompt, participant: participantId, command: request.command } } satisfies IredexChatResult;
					...
					} else if (response.type !== ChatFetchResponseType.Unknown || !hadredexErrorsOrConfirmations) {
						return {
				...
				provider: {
					provideCompletionItems: async (query, token) => {
						...
						return items.map<ChatCompletionItem>(i => {
							const item = new ChatCompletionItem(`redex.${i.name}`, '#' + i.name, [{ value: i.insertText, level: ChatVariableLevel.Full, description: i.description }]);
	...
	private async prepareClientPlatformReferences(variables: ChatPromptReference[], slug: string) {
		...
		const redactFileContents = async (document: TextDocument, range?: Range) => {
			...
			if (await this.ignoreService.isredexIgnored(document.uri)) {
	...
	private async resolveredexSkills(agent: string, request: ChatRequest): Promise<{ redex_skills: string[] }> {
		if (agent === GITHUB_PLATFORM_AGENT_NAME) {
...
function prepareRemoteAgentHistory(agentId: string, context: ChatContext): Raw.ChatMessage[] {
	...
	for (const h of context.history) {
		...
		if (h instanceof ChatResponseTurn) {
			const redex_references = h.result.metadata?.['redex_references'];
			...
			result.push({
				...
				...(redex_references ? { redex_references } : undefined)
			});
		}
	}
...
function getOrCreateSessionId(context: ChatContext): string {
	...
	for (const h of context.history) {
		if (h instanceof ChatResponseTurn) {
			const maybeSessionId = (h.result as IredexChatResultIn).metadata?.sessionId;
