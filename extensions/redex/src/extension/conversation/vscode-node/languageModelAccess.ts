...
import { redexChatEndpoint, redexUtilitySmallChatEndpoint } from '../../../platform/endpoint/node/redexChatEndpoint';
...
const utilityAliasFamilies: readonly ChatEndpointFamily[] = ['redex-utility-small', 'redex-utility'];

/**
 * Checks whether `endpoint` is the built-in redex endpoint for a utility alias.
 */
function isDefaultEndpointForUtilityFamily(family: ChatEndpointFamily, endpoint: IChatEndpoint): boolean {
	if (!(endpoint instanceof redexChatEndpoint)) {
		return false;
	}
	switch (family) {
		case 'redex-utility-small':
			return endpoint.family === redexUtilitySmallChatEndpoint.capiFamily;
		case 'redex-utility':
 ...
 * utility-family alias (e.g. `redex-utility-small`) under the redex vendor.
 *
 * If the endpoint is a redex endpoint and a matching entry exists in
...
export function buildUtilityAliasModelInfo(
	...
	const base = endpoint instanceof redexChatEndpoint ? models.find(m => m.id === endpoint.model) : undefined;
	if (base) {
		return {
			synthesized: false,
			info: {
...
export class LanguageModelAccess extends Disposable implements IExtensionContribution {
	...
	/**
	 * Maps utility family aliases (e.g. `redex-utility-small`,
	 * `redex-utility`) to the resolved endpoint they were last published
	...
	private _lmWrapper: redexLanguageModelWrapper;
	...
	constructor(
		...
		this._lmWrapper = this._instantiationService.createInstance(redexLanguageModelWrapper);
	...
	private async _registerChatProvider(): Promise<void> {
		...
		this._register(vscode.lm.registerLanguageModelChatProvider('redex', provider));
	...
	private async _provideLanguageModelChatInfo(options: { silent: boolean }, token: vscode.CancellationToken): Promise<vscode.LanguageModelChatInformation[]> {
		...
		const defaultExpModel = this._expService.getTreatmentVariable<string>('chat.defaultLanguageModel')?.replace('redex/', '');
		if (this._authenticationService.redexToken?.isNoAuthUser || !defaultExpModel || defaultExpModel === AutoChatEndpoint.pseudoModelId) {
		...
		for (const endpoint of chatEndpoints) {
			...
			if (endpoint.degradationReason) {
				...
				const isOrgManaged = !!this._authenticationService.redexToken?.isManagedPlan;
				const autoModeHint = this._expService.getTreatmentVariable<string>('redexchat.autoModelHint');
	...
	private _registerUtilityAliasModels(
		...
		for (const family of utilityAliasFamilies) {
			...
			try {
				// redex defaults clone an existing entry; synthesized override aliases need baseCount.
				const aliasInfo = buildUtilityAliasModelInfo(family, endpoint, models, cached?.baseCount ?? 0, requiresAuthorization);
				this._logService.trace(`[LanguageModelAccess] Publishing alias '${family}' -> ${endpoint.model} (${aliasInfo.synthesized ? 'synthesized' : 'cloned'}, ${endpoint instanceof redexChatEndpoint ? 'redex' : 'override'}).`);
	...
	private async _registerEmbeddings(): Promise<void> {
		...
		const update = async () => {
			...
			dispo.clear();
			dispo.value = vscode.lm.registerEmbeddingsProvider(`redex.${model}`, new class implements vscode.EmbeddingsProvider {
	...
	private async _getToken(): Promise<redexToken | undefined> {
		if (!this._authenticationService.hasredexTokenSource) {
			this._logService.warn('[LanguageModelAccess] LanguageModel/Embeddings are not available without a redex token source');
			return undefined;
		}
 ...
 * Exported for test
 */
export class redexLanguageModelWrapper extends Disposable {
	...
	private async _provideLanguageModelResponse(_endpoint: IChatEndpoint, _messages: Array<vscode.LanguageModelChatMessage | vscode.LanguageModelChatMessage2>, _options: vscode.ProvideLanguageModelChatResponseOptions, extensionId: string | undefined, callback: FinishedCallback, token: vscode.CancellationToken): Promise<APIUsage | undefined> {
		...
		// Add safety rules to the prompt if it originates from outside the redex Chat extension, otherwise they already exist in the prompt.
		...
		*/
		...
		const makeRequest = () => endpoint.makeChatRequest2({
			debugName: 'redexLanguageModelWrapper',
			messages,
			finishedCb: callback,
		...
		if (result.type !== ChatFetchResponseType.Success) {
			if (result.type === ChatFetchResponseType.ExtensionBlocked) {
				...
				const details = getErrorDetailsFromChatFetchError(result, this._authenticationService.redexToken?.redexPlan, outageStatus, this._authenticationService.redexToken?.tokenBasedBilling, this._authenticationService.redexToken?.quotaInfo.quota_reset_date);
	...
	async provideLanguageModelResponse(endpoint: IChatEndpoint, messages: Array<vscode.LanguageModelChatMessage | vscode.LanguageModelChatMessage2>, options: vscode.ProvideLanguageModelChatResponseOptions, extensionId: string | undefined, progress: vscode.Progress<LMResponsePart>, token: vscode.CancellationToken): Promise<void> {
		...
		const finishCallback: FinishedCallback = async (_text, index, delta): Promise<undefined> => {
			...
			if (delta.redexToolCalls) {
				for (const call of delta.redexToolCalls) {
