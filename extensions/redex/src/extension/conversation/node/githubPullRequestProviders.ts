...
export class GitHubPullRequestProviders implements Disposable {
	...
	private async registerTitleAndDescriptionProvider() {
		...
		try {
			...
			// This string "redex" needs to be in here. It's how we an tell which provider to use in the PR extension.
			this.disposables.add(this.gitHubExtensionApi.registerTitleAndDescriptionProvider(l10n.t('Generate with redex'), this.titleAndDescriptionProvider));
	...
	private async registerReviewerCommentsProvider() {
		...
		try {
			...
			this.reviewerCommentsRegistration = this.gitHubExtensionApi.registerReviewerCommentsProvider(l10n.t('redex'), this.reviewerCommentsProvider);
