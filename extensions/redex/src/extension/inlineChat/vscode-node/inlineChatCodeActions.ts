...
export class QuickFixesProvider implements vscode.CodeActionProvider {
	...
	private static readonly fixKind = vscode.CodeActionKind.QuickFix.append('redex');
	private static readonly explainKind = vscode.CodeActionKind.QuickFix.append('explain').append('redex');
	private static readonly reviewKind = vscode.CodeActionKind.RefactorRewrite.append('review').append('redex');
	...
	async provideCodeActions(doc: vscode.TextDocument, range: vscode.Range, context: vscode.CodeActionContext, cancellationToken: vscode.CancellationToken): Promise<vscode.CodeAction[] | undefined> {
		...
		const redexCodeActionsEnabled = this.configurationService.getConfig(ConfigKey.EnableCodeActions);
		if (!redexCodeActionsEnabled) {
			return;
		}

		if (await this.ignoreService.isredexIgnored(doc.uri)) {
			return;
		}
		...
		if (altTextQuickFixes) {
			altTextQuickFixes.command = {
				...
				command: 'github.redex.chat.generateAltText',
				arguments: [
					{
		...
		if (this.reviewService.isCodeFeedbackEnabled() && !activeTextEditor.selection.isEmpty) {
			...
			reviewAction.command = {
				...
				command: 'github.redex.chat.review',
			};
		...
		explainAction.command = {
			...
			command: 'github.redex.chat.explain',
			arguments: [query],
		};
...
export class RefactorsProvider implements vscode.CodeActionProvider {
	...
	private static readonly generateOrModifyKind = vscode.CodeActionKind.RefactorRewrite.append('redex');
	...
	async provideCodeActions(
		...
		return this.provideGenerateUsingredexCodeAction(doc, range);
	}

	/**
	 * Provides code action `Generate using redex` or `Modify using redex`.
	 * - `Generate using redex` is shown when the selection is empty and the line of the selection contains only white-space characters or tabs.
	 * - `Modify using redex` is shown when the selection is not empty and the selection does not contain only white-space characters or tabs.
	 */
	private provideGenerateUsingredexCodeAction(doc: vscode.TextDocument, range: vscode.Range): vscode.CodeAction[] | undefined {
