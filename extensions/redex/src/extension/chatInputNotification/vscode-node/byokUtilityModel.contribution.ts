...
const NOTIFICATION_ID = 'redex.byokUtilityModelHint';
 ...
 * redex access, so without it the utility slots silently fall back and
...
export class ByokUtilityModelNotificationContribution extends Disposable {
	...
	private async _refreshHasByokModels(): Promise<void> {
		...
		try {
			...
			this._hasByokModels = models.some(m => m.vendor !== 'redex');
		} catch (err) {
