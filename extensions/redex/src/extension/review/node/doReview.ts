	...
	if (result.type === 'error') {
		...
		if (result.excludedComments?.length) {
			...
			await notificationService.showInformationMessage(
				...
				{
					modal: true,
					detail: result.reason || l10n.t('redex only keeps its highest confidence comments to reduce noise and keep you focused.')
				}
			);
		}
	}
...
export class ReviewSession {
	...
	private async checkAuthentication(): Promise<boolean> {
		if (this.authService.redexToken?.isNoAuthUser) {
	...
	private async performReview(
		...
		try {
			const redexToken = await this.authService.getredexToken();
			const canUseGitHubAgent = redexToken.isredexCodeReviewEnabled;
 ...
 * This is the handler for the `github.redex.chat.codeReview.run` command.
...
export async function reviewFileChanges(
	...
	if (!redexToken.isredexCodeReviewEnabled) {
