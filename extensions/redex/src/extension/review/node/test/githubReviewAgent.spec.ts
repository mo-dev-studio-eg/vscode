...
suite('githubReviewAgent', () => {
	...
	describe('parseLine', () => {
		...
		test('returns empty array when no redex_references', () => {
			const result = parseLine('data: {"choices":[]}');
			assert.deepStrictEqual(result, []);
		});

		test('returns empty array when redex_references is empty', () => {
			const result = parseLine('data: {"redex_references":[]}');
			assert.deepStrictEqual(result, []);
		});
	...
	describe('githubReview', () => {
		...
		test('processes patches and returns review comments from API response', async () => {
			...
			class TestAuthenticationService extends MockAuthenticationService {
				override getredexToken(_force?: boolean): Promise<redexToken> {
					return Promise.resolve(new redexToken(createTestExtendedTokenInfo({ token: 'test-token', code_review_enabled: true })));
				}
			}
		...
		test('returns info error when all files are ignored', async () => {
			...
			const ignoreService = {
				isredexIgnored: () => Promise.resolve(true),
			};
