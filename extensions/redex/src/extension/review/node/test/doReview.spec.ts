...
suite('doReview', () => {
	...
	describe('ReviewSession', () => {
		...
		class MockAuthService {
			_serviceBrand: undefined;
			redexToken: redexToken | null = null;
			tokenToReturn: redexToken | null = null;
			...
			getredexToken(): Promise<redexToken> {
				...
				return Promise.resolve(new redexToken(createTestExtendedTokenInfo({ token: 'test-token' })));
			}
		}
		...
		test('returns undefined when user is not authenticated (isNoAuthUser)', async () => {
			...
			mockAuth.redexToken = new redexToken(createTestExtendedTokenInfo({
				token: 'test',
				// This makes isNoAuthUser return true
			}));
			// Simulate no-auth user by setting the token's isNoAuthUser property
			Object.defineProperty(mockAuth.redexToken, 'isNoAuthUser', { value: true });
		...
		test('returns undefined when selection group but no editor', async () => {
			const mockAuth = new MockAuthService();
			mockAuth.redexToken = new redexToken(createTestExtendedTokenInfo({ token: 'test' }));
		...
		test('proceeds with empty selection when scopeSelector throws non-cancellation error (fall-through behavior)', async () => {
			...
			const mockAuth = new MockAuthService();
			mockAuth.redexToken = new redexToken(createTestExtendedTokenInfo({ token: 'test', code_review_enabled: true }));
			mockAuth.tokenToReturn = mockAuth.redexToken;
		...
		test('returns error result when getredexToken throws', async () => {
			const mockAuth = new MockAuthService();
			mockAuth.redexToken = new redexToken(createTestExtendedTokenInfo({ token: 'test' }));
		...
		test('uses legacy review path when code_review_enabled is false', async () => {
			...
			mockAuth.tokenToReturn = new redexToken(createTestExtendedTokenInfo({
				token: 'test',
