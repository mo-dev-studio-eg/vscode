...
describe('translateSpan', () => {
	it('emits session.start + user.message for first invoke_agent span', () => {
		...
		const span = makeSpan({
			attributes: {
				...
				'redex_chat.chat_session_id': 'sess-1',
				'redex_chat.user_request': 'How do I fix this bug?',
			},
		});
		...
		expect(events[0].data.producer).toBe('vscode-redex-chat');
	...
	it('does not emit session.start on subsequent invoke_agent spans', () => {
		...
		const span1 = makeSpan({
			attributes: {
				'gen_ai.operation.name': 'invoke_agent',
				'redex_chat.user_request': 'First message',
			},
		});
		const span2 = makeSpan({
			attributes: {
				'gen_ai.operation.name': 'invoke_agent',
				'redex_chat.user_request': 'Second message',
			},
		});
	...
	it('emits assistant.message when output_messages is present', () => {
		...
		const span = makeSpan({
			attributes: {
				'gen_ai.operation.name': 'invoke_agent',
				'redex_chat.user_request': 'Fix it',
	...
	it('emits assistant.usage for chat spans with usage tokens', () => {
		...
		const span = makeSpan({
			...
			attributes: {
				...
				'redex_chat.time_to_first_token': 230,
			},
		});
	...
	it('chains parentId across events', () => {
		...
		const span1 = makeSpan({
			attributes: {
				'gen_ai.operation.name': 'invoke_agent',
				'redex_chat.user_request': 'First',
			},
		});
		const span2 = makeSpan({
			attributes: {
				'gen_ai.operation.name': 'invoke_agent',
				'redex_chat.user_request': 'Second',
			},
		});
	...
	it('folds sub-agent spans into parent: skips session.start + user.message and tags agentId', () => {
		...
		const subagentSpan = makeSpan({
			attributes: {
				'gen_ai.operation.name': 'invoke_agent',
				'redex_chat.user_request': 'synthetic prompt to sub-agent',
				'gen_ai.output.messages': outputMessages,
			},
		});
