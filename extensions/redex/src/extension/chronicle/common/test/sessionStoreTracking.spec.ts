...
describe('extractAgentName', () => {
	it('returns trimmed attribute value when present', () => {
		expect(extractAgentName({ name: 'invoke_agent', attributes: { 'gen_ai.agent.name': '  redex  ' } }))
			.toBe('redex');
	});

	it('parses agent name from span name when attribute is absent', () => {
		expect(extractAgentName({ name: 'invoke_agent redex', attributes: {} }))
			.toBe('redex');
	});

	it('parses multi-word agent name from span name', () => {
		expect(extractAgentName({ name: 'invoke_agent GitHub redex Chat', attributes: {} }))
			.toBe('GitHub redex Chat');
	});
	...
	it('falls back to span name when attribute is empty string', () => {
		expect(extractAgentName({ name: 'invoke_agent redex', attributes: { 'gen_ai.agent.name': '' } }))
			.toBe('redex');
	});
