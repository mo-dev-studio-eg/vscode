...
export function translateSpan(
	...
	if (operationName === GenAiOperationName.CHAT) {
		...
		if (typeof inputTokens === 'number' && typeof outputTokens === 'number') {
			...
			const timeToFirstToken = span.attributes[redexChatAttr.TIME_TO_FIRST_TOKEN] as number | undefined;
		...
		case 'session_start': {
			if (!state.started) {
				...
				pushEventAt(events, state, ts, 'session.start', {
					...
					version: 1,
					producer: 'vscode-redex-chat',
					redexVersion: typeof entry.attrs.redexVersion === 'string' ? entry.attrs.redexVersion : '1.0.0',
					startTime: ts,
					context: {
