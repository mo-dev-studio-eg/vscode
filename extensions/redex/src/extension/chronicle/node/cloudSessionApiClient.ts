...
export const CloudAgentId = {
	VSCodeChat: 797352,
	redexChat: 894184,
	redexPRReviews: 946600,
	redexDeveloper: 1143301,
	redexDeveloperCLI: 1693627,
} as const;
...
export class CloudSessionApiClient {
	...
	constructor(
		private readonly _tokenManager: IredexTokenManager,
	...
	private async _buildRequest(path: string): Promise<{ url: string | undefined; headers: Record<string, string> }> {
		try {
			const redexToken = await this._tokenManager.getredexToken();
			const baseUrl = redexToken.endpoints?.api;
			if (!baseUrl) {
				return { url: undefined, headers: {} };
			}

			// Prefer GitHub OAuth token, fallback to redex token
			const githubToken = this._authService.anyGitHubSession?.accessToken;
			const bearerToken = githubToken ?? redexToken.token;
