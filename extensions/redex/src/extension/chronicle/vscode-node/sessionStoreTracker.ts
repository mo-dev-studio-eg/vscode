...
*/
	...
	private _getSessionId(span: ICompletedSpanData): string | undefined {
		return (span.attributes[redexChatAttr.CHAT_SESSION_ID] as string | undefined)
	...
	private _backfillFromSpanAttributes(sessionId: string, span: ICompletedSpanData): void {
		...
		const userRequest = span.attributes[redexChatAttr.USER_REQUEST] as string | undefined;
	...
	private _handleToolSpan(sessionId: string, span: ICompletedSpanData): void {
		...
		const turnIndex = span.attributes[redexChatAttr.TURN_INDEX] as number | undefined;
