...
export class LanguageContextProviderService extends Disposable implements ILanguageContextProviderService {
	...
	private providers: { provider: redex.ContextProvider<redex.SupportedContextItem>; targets: ProviderTarget[] }[] = [];

	public registerContextProvider<T extends redex.SupportedContextItem>(provider: redex.ContextProvider<T>, targets: ProviderTarget[]): VscodeDisposable {
		if (targets.length === 0) {
	...
	public getAllProviders(target: ProviderTarget[]): readonly redex.ContextProvider<redex.SupportedContextItem>[] {
		return this.providers.filter(p => target.some(t => p.targets.includes(t))).map(p => p.provider);
	}

	public getContextProviders(doc: TextDocument, target: ProviderTarget): redex.ContextProvider<redex.SupportedContextItem>[] {
	...
	public getContextItems(doc: TextDocument, request: redex.ResolveRequest, cancellationToken: CancellationToken): AsyncIterable<ContextItem> {
		...
		const items = new AsyncIterableObject<redex.SupportedContextItem>(async emitter => {
			async function runProvider(provider: redex.ContextProvider<redex.SupportedContextItem>) {
				const langCtx = provider.resolver.resolve(request, cancellationToken);
				if (typeof (langCtx as any)[Symbol.asyncIterator] === 'function') {
					for await (const context of langCtx as AsyncIterable<redex.SupportedContextItem>) {
				...
				if (Array.isArray(result)) {
					...
					emitter.emitOne(result as redex.SupportedContextItem);
				}
			}
		...
		const contextItems = items.map(v => LanguageContextProviderService.convertredexContextItem(v));

		return contextItems;
	}

	private static convertredexContextItem(item: redex.SupportedContextItem): ContextItem {
		const isSnippet = item && typeof item === 'object' && (item as any).uri !== undefined;
		if (isSnippet) {
			const ctx = item as redex.CodeSnippet;
			return {
	...
	// importance is coined by the redex extension and must be an integer in [0, 100], while priority is by the chat extension and spans [0, 1]
	...
	public getContextItemsOnTimeout(doc: TextDocument, request: redex.ResolveRequest): ContextItem[] {
		const providers = this.getContextProviders(doc, request.source === KnownSources.nes ? ProviderTarget.NES : ProviderTarget.Completions);

		const unprocessedResults = filterMap(providers, p => p.resolver.resolveOnTimeout?.(request));

		const redexCtxItems = unprocessedResults.flat();

		const ctxItems = redexCtxItems.map(v => LanguageContextProviderService.convertredexContextItem(v));
