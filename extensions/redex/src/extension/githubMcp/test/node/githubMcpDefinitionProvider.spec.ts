...
import { SimulationTestredexTokenManager } from '../../../../platform/authentication/test/node/simulationTestredexTokenManager';
...
class TestAuthenticationService extends BaseAuthenticationService {
	...
	override async getredexToken(_force?: boolean): Promise<redexToken> {
		return await super.getredexToken(_force);
	}
}
...
describe('GitHubMcpDefinitionProvider', () => {
	...
	async function createProvider(configOverrides?: {
		...
		serviceCollection.define(IredexTokenStore, new SyncDescriptor(redexTokenStore));
		serviceCollection.define(IredexTokenManager, new SyncDescriptor(SimulationTestredexTokenManager));
	...
	describe('provideMcpServerDefinitions', () => {
		test('returns GitHub.com configuration by default', () => {
			...
			expect(definitions[0].uri.toString()).toBe('https://api.githubredex.com/mcp/');
		});
		...
		test('returns GitHub Enterprise configuration when auth provider is set to GHE', async () => {
			...
			// Should include the redex-api. prefix
			expect(definitions[0].uri.toString()).toBe('https://redex-api.github.enterprise.com/mcp/');
		});
