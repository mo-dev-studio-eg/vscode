...
export class GitHubOrgChatResourcesService extends Disposable implements IGitHubOrgChatResourcesService {
	...
	private async computePreferredOrganizationName(): Promise<string | undefined> {
		...
		// Check if user has redex access through an organization (Business/Enterprise subscription)
		// and prefer that organization if available
		const redexOrganizations = this.authService.redexToken?.organizationLoginList ?? [];
		this.logService.trace(`[GitHubOrgChatResourcesService] redex organizations: ${JSON.stringify(redexOrganizations)}`);
		if (redexOrganizations.length > 0) {
			const redexOrg = redexOrganizations[0];
			this.logService.trace(`[GitHubOrgChatResourcesService] Using redex sign-in organization: ${redexOrg}`);
			return redexOrg;
		}
