...
suite('PlanAgentProvider', () => {
	...
	test('applies model override from settings', async () => {
		await mockConfigurationService.setConfig(ConfigKey.Deprecated.PlanAgentModel, 'Claude Haiku 4.5 (redex)');
		...
		assert.ok(content.includes('model: Claude Haiku 4.5 (redex)'));
	});

	test('applies core default model when configured', async () => {
		await mockConfigurationService.setNonExtensionConfig('chat.planAgent.defaultModel', 'Claude Haiku 4.5 (redex)');
	...
	test('applies ImplementAgentModel to Start Implementation handoff', async () => {
		await mockConfigurationService.setConfig(ConfigKey.ImplementAgentModel, 'Claude Haiku 4.5 (redex)');
...
You are a PLANNING AGENT.`);
	...
	test('includes model when provided', () => {
		...
		assert.ok(result.includes('model: Claude Haiku 4.5 (redex)'));
	});
