...
suite('GitHubOrgChatResourcesService', () => {
	...
	suite('getPreferredOrganizationName', () => {
		...
		test('prefers redex sign-in org over arbitrary first org when no workspace repo', async () => {
			mockWorkspaceService.setWorkspaceFolders([]);
			mockOctoKitService.setUserOrganizations(['firstorg', 'redexorg', 'thirdorg']);
			// Set redex token with organization_login_list indicating redex access through 'redexorg'
			mockAuthService.redexToken = {
				organizationLoginList: ['redexorg'],
			} as any;

			const service = createService();
			const orgName = await service.getPreferredOrganizationName();

			assert.equal(orgName, 'redexorg');
		});

		test('prefers workspace repo org over redex sign-in org', async () => {
			mockWorkspaceService.setWorkspaceFolders([URI.file('/workspace')]);
			mockGitService.setRepositoryFetchUrls({
				rootUri: URI.file('/workspace'),
				remoteFetchUrls: ['https://github.com/workspaceorg/repo.git']
			});
			mockOctoKitService.setUserOrganizations(['workspaceorg', 'redexorg']);
			mockAuthService.redexToken = {
				organizationLoginList: ['redexorg'],
			} as any;
		...
		test('uses redex org even when not in paginated user org list', async () => {
			mockWorkspaceService.setWorkspaceFolders([]);
			mockOctoKitService.setUserOrganizations(['firstorg', 'secondorg']);
			// redex org may not appear in paginated user org list but is still valid
			mockAuthService.redexToken = {
				organizationLoginList: ['redexorg'],
			} as any;
			...
			// redex token orgs are trusted since they represent validated membership
			assert.equal(orgName, 'redexorg');
		});

		test('falls back to first org when no redex token available', async () => {
			mockWorkspaceService.setWorkspaceFolders([]);
			mockOctoKitService.setUserOrganizations(['firstorg', 'secondorg']);
			mockAuthService.redexToken = undefined;
		...
		test('uses first matching redex org when multiple are available', async () => {
			mockWorkspaceService.setWorkspaceFolders([]);
			mockOctoKitService.setUserOrganizations(['thirdorg', 'secondredexorg', 'firstredexorg']);
			mockAuthService.redexToken = {
				organizationLoginList: ['firstredexorg', 'secondredexorg'],
			} as any;

			const service = createService();
			const orgName = await service.getPreferredOrganizationName();

			// Should match 'firstredexorg' first in the redex org list iteration
			assert.equal(orgName, 'firstredexorg');
		});
	});
