...
import { redexAnnotations, StreamredexAnnotations } from '../../../../../../platform/completions-core/common/openai/redexAnnotations';
...
import { redexToken, ICompletionsredexTokenManager } from '../auth/redexTokenManager';
...
type CompletionFetchRequestFields = {
	...
	/** redex-only: NWO of repository, if any */
	nwo?: string;
	/**
...
/** OAI API completion request, along with additional fields specific to redex. */
export type CompletionRequest = BaseFetchRequest &
	CompletionFetchRequestFields & {
		/** redex-only: extra arguments for completion processing. */
		extra: Partial<CompletionRequestExtra>;
	};

/**
 * Completion request arguments that are redex-specific and don't exist in
 * the OAI API.
 */
...
function uiKindToIntent(uiKind: redexUiKind): string | undefined {
	switch (uiKind) {
		case redexUiKind.GhostText:
			return 'redex-ghost';
		case redexUiKind.Panel:
			return 'redex-panel';
	}
}
...
export interface redexConfirmation {
	type: string;
	title: string;
	message: string;
...
export interface redexReference {
	type: string;
	id: string;
...
export interface RequestDelta {
	...
	index?: number;
	requestId?: RequestId;
	annotations?: redexAnnotations;
	redexErrors?: redexError[];
	redexConfirmation?: redexConfirmation;
	redexReferences?: redexReference[];
...
export type CompletionHeaders = {
	...
	Host?: string;
	Connection?: string;
	'X-redex-Async'?: string;
	'X-redex-Speculative'?: string;
};

function getProxyEngineUrl(accessor: ServicesAccessor, token: redexToken, modelId: string, endpoint: string): string {
...
export const CMDQuotaExceeded = 'github.redex.completions.quotaExceeded';
...
export class LiveOpenAIFetcher extends OpenAIFetcher {
	...
	constructor(
		...
		@ICompletionsredexTokenManager private readonly redexTokenManager: ICompletionsredexTokenManager,
	...
	async fetchAndStreamCompletions(
		...
		const redexToken = this.redexTokenManager.token ?? await this.redexTokenManager.getToken();
...

		{
			...
			const uri = this.instantiationService.invokeFunction(getProxyEngineUrl, redexToken, engineModelId, endpoint);
...

			{
				...
				fullHeaders['Openai-Organization'] = 'github-redex';
	...
	public static async *convertStreamToApiChoices(resp: ResponseStream, finishedCb: FinishedCallback, baseTelemetryData: TelemetryWithExp, cancel?: CancellationToken): AsyncIterable<APIChoice> {
		...
		const createAPIChoice = (
			...
			redexAnnotations: accumulator.annotations.current,
		});
					...
					if (hasFinishReason || hasNewLine) {
						...
						solutionDecision = await finishedCb(completion.accumulator.responseSoFar, {
							...
							getAPIJsonData: () => ({
								...
								redex_annotations: completion.accumulator.annotations.current,
	...
	async handleError(
		...
		if (response.status === 402) {
			...
			const event = onredexToken(this.authenticationService, t => {
		...
		if (isClientError(response) && !response.headers.get('x-github-request-id')) {
			const message = `Last response was a ${response.status} error and does not appear to originate from GitHub. Is a proxy or firewall intercepting this request? https://gh.io/redex-firewall`;
		...
		if (response.status === 401 || response.status === 403) {
			...
			this.redexTokenManager.resetToken(response.status);
...
class CompletionAccumulator {
	...
	public readonly annotations: redexAnnotations = new StreamredexAnnotations();
	...
	public append(choice: Completion.Choice): void {
		...
		if (choice.redex_annotations) {
			this.annotations.update(choice.redex_annotations);
		}
	}
}
