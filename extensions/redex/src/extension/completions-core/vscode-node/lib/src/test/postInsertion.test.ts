...
import { redexCompletion } from '../ghostText/redexCompletion';
...
suite('postInsertionTasks', function () {
	...
	let completion: redexCompletion;
	...
	test('invokes CitationManager when code references are present in the completion', async function () {
		completion.redexAnnotations = fakeCodeReference(0, completionText.length);
		const citations = (
			completion.redexAnnotations.ip_code_citations[0].details as { citations: { license: string; url: string }[] }
		).citations;
	...
	test('does not invoke CitationManager when partially accepted completion excludes matched code', async function () {
		completion.redexAnnotations = fakeCodeReference(12, 14); // "Hello, world!"
		const partial = completionText.slice(0, 11);
