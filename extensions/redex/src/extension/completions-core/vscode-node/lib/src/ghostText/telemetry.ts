...
import { redexCompletion } from './redexCompletion';
...
/** Send `.shown` event */
export function telemetryShown(accessor: ServicesAccessor, completion: redexCompletion) {
 ...
 * redex's overall reliability, we want to make sure we consistently send a matching "result" event.
 *
...
export function mkBasicResultTelemetry(
	...
	const result: BasicResultTelemetry = {
		...
		redex_trackingId: telemetryBlob.properties['redex_trackingId'],
	};
