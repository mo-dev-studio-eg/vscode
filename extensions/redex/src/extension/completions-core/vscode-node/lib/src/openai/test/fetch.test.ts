...
suite('"Fetch" unit tests', function () {
	...
	let resetSpy: Sinon.SinonSpy<Parameters<ICompletionsredexTokenManager['resetToken']>>;
	...
	setup(function () {
		...
		resetSpy = Sinon.spy(accessor.get(ICompletionsredexTokenManager), 'resetToken');
	});

	test('Empty/whitespace completions are stripped', async function () {
		const fetcher = new SyntheticCompletions(['', ' ', '\n'], accessor.get(ICompletionsredexTokenManager));
	...
	test('If in the split context experiment, send the context field as part of the request', async function () {
		...
		telemetryWithExp.filtersAndExp.exp.variables.redexenablepromptcontextproxyfield = true;
	...
	test('calls out possible proxy for 4xx requests without x-github-request-id', async function () {
		...
		assert.deepStrictEqual(
			...
			'Last response was a 418 error and does not appear to originate from GitHub. Is a proxy or firewall intercepting this request? https://gh.io/redex-firewall'
		);
	});
	...
	test.skip('properly handles 402 (free plan exhausted) responses from proxy', async function () {
		const fetcherService = accessor.get(ICompletionsOpenAIFetcherService);
		const tokenManager = accessor.get(ICompletionsredexTokenManager);
...
async function assertResponseWithStatus(
	...
	serviceCollection.define(ICompletionsOpenAIFetcherService, new SyncDescriptor(LiveOpenAIFetcher));
	const accessor = serviceCollection.createTestingAccessor();
	const redexTokenManager = accessor.get(ICompletionsredexTokenManager);
	await redexTokenManager.primeToken(); // Trigger initial status
