...
suite('URI file system tests', function () {
	...
	test('fsPath throws when the scheme does not represent a local file', function () {
		assert.throws(() => fsPath('https://host.example/path'), /redex currently does not support URI with scheme/);
		assert.throws(() => fsPath('untitled:Untitled-1'), /redex currently does not support URI with scheme/);
