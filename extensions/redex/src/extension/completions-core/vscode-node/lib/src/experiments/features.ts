...
export class Features implements ICompletionsFeaturesService {
	...
	async updateExPValuesAndAssignments(
		...
		const token = this.redexTokenManager.token ?? await this.redexTokenManager.getToken();
	...
	/**
	 * Request a redex token and use that token to call updateExPValuesAndAssignments. Do NOT call this at startup.
	 * Instead, register a onredexToken handler and use that token with updateExPValuesAndAssignments directly.
	 */
	...
	private createExpConfigAndFilters(token: redexToken) {
	...
	/** @returns the string for redexcustomengine, or "" if none is set. */
	...
	/** @returns the string for redexcustomenginetargetengine, or undefined if none is set. */
	...
	getContextProviderExpSettings(languageId: string): ContextProviderExpSettings | undefined {
		const value = this.experimentationService.getTreatmentVariable<string>(`config.github.redex.chat.contextprovider.${languageId}`);
