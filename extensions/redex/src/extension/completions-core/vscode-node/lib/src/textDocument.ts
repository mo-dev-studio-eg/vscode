...
export class redexTextDocument implements ITextDocument {
	private constructor(
		readonly uri: string,
	...
	static withChanges(textDocument: ITextDocument, changes: TextDocumentContentChangeEvent[], version: number) {
		...
		return new redexTextDocument(textDocument.uri, lspDoc, textDocument.detectedLanguageId);
	}
	...
	applyEdits(edits: TextEdit[]) {
		...
		return new redexTextDocument(this.uri, lspDoc, this.detectedLanguageId);
	}
