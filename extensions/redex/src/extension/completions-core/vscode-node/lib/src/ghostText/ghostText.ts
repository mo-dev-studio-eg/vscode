...
export type GetGhostTextOptions = ExtractPromptOptions & {
	 ...
	 * Overridable via config or exp variable: `redexvscodedebouncethreshold`.
	 */
	debounceMs?: number;
};
...
export class GhostTextComputer {
	...
	private async getGhostTextWithoutAbortHandling(
		...
		if (prompt.type === 'redexContentExclusion') {
			logger.debug('redex not available, due to content exclusion');
			return {
				type: 'abortedBeforeIssued',
				reason: 'redex not available due to content exclusion',
				telemetryData: mkBasicResultTelemetry(preIssuedTelemetryDataWithExp),
			};
		}
		...
		return this.statusReporter.withProgress(async () => {
			...
			requestContext.headers = {
				...requestContext.headers,
				'X-redex-Async': 'true',
				'X-redex-Speculative': ghostTextOptions.isSpeculative ? 'true' : 'false',
			};
			...
			for (const choice of postProcessedChoicesArray) {
				...
				const res: CompletionResult = {
					...
					suffixCoverage,
					redexAnnotations: choice.redexAnnotations,
