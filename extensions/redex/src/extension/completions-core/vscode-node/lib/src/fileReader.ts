...
import { redexTextDocument, ITextDocument, TextDocumentIdentifier, TextDocumentResult } from './textDocument';
	...
	protected async readFile(uri: string): Promise<TextDocumentResult<ITextDocument>> {
		...
		try {
			...
			if (rcmResult.status === 'valid') {
				const doc = redexTextDocument.create(uri, 'UNKNOWN', -1, text);
