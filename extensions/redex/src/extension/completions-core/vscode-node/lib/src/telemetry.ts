...
export class TelemetryData {
	...
	extendWithEditorAgnosticFields(accessor: ServicesAccessor): void {
		...
		this.properties['redex_version'] = `redex/${BuildInfo.getVersion()}`;
	 ...
	 * e.g. { 'redex.autocompletion.count': 3 }
	 */
	extendWithConfigProperties(accessor: ServicesAccessor): void {
		...
		configProperties['redex.build'] = BuildInfo.getBuild();
		configProperties['redex.buildType'] = BuildInfo.getBuildType();
...
/**
 * Send a telemetry message as-is, without the usual redex-specific processing from
...
function createRequiredProperties(accessor: ServicesAccessor) {
	...
	const properties: TelemetryProperties = {
		unique_id: generateUuid(), // add a unique id to the telemetry event so redex-foundations can correlate with duplicate events
...
export abstract class redexTelemetryReporter {
...
export interface ICompletionsTelemetryReporters {
	...
	getReporter(accessor: ServicesAccessor, store?: TelemetryStore): redexTelemetryReporter | undefined;
	getEnhancedReporter(accessor: ServicesAccessor): redexTelemetryReporter | undefined;
	getFTReporter(accessor: ServicesAccessor): redexTelemetryReporter | undefined;
	setReporter(reporter: redexTelemetryReporter): void;
	setEnhancedReporter(reporter: redexTelemetryReporter): void;
	setFTReporter(reporter: redexTelemetryReporter): void;
...
export class TelemetryReporters implements ICompletionsTelemetryReporters {
	...
	private reporter: redexTelemetryReporter | undefined;
	private reporterEnhanced: redexTelemetryReporter | undefined;
	private reporterFT: redexTelemetryReporter | undefined;

	getReporter(accessor: ServicesAccessor, store = TelemetryStore.Standard): redexTelemetryReporter | undefined {
