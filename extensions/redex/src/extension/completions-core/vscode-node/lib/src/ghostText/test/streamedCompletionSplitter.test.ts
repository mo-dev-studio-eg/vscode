...
import { redexNamedAnnotationList } from '../../../../../../../platform/completions-core/common/openai/redexAnnotations';
...
import { CompletionResults, redexUiKind, ICompletionsOpenAIFetcherService, LiveOpenAIFetcher } from '../../openai/fetch';
...
class FakeCompletionsFetchService implements ICompletionsFetchService {
	...
	constructor(private readonly completionTexts: string[], private readonly annotations?: redexNamedAnnotationList) { }
	async fetch(
		_url: string,
		_secretKey: string,
...
suite('StreamedCompletionSplitter', function () {
	...
	test('omits any annotation from split choices where start_offset does not intersect the choice', async function () {
		...
		await readChoices(result);
		Sinon.assert.calledTwice(cacheFunction);
		Sinon.assert.calledWith(cacheFunction, Sinon.match.string, Sinon.match({ redexAnnotations: undefined }));
		Sinon.assert.calledWith(
			cacheFunction,
			Sinon.match.string,
			Sinon.match({
