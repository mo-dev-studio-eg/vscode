...
import { redexTextDocument, INotebookCell, INotebookDocument, ITextDocument } from '../textDocument';
import {
...
export function parseNotebook(doc: ITextDocument): INotebookDocument {
	...
	const cells: INotebookCell[] = notebook.cells.map((cell, index) => {
		...
		const document = redexTextDocument.create(cellUri, languageId, 0, cellText, languageId);
