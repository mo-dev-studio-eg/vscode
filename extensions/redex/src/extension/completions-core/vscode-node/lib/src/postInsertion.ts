...
async function citationCheck(
	...
	if (!redexAnnotations || (redexAnnotations.ip_code_citations?.length ?? 0) < 1) {
		// Do not request citations if in blocking mode
		if (redexTokenManager.getLastToken()?.getTokenValue('sn') === '1') { return; }
	...
	for (const citation of redexAnnotations.ip_code_citations) {
