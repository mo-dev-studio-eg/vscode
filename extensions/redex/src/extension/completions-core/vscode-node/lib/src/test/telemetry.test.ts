...
import { NoopredexTelemetryReporter } from './noopTelemetry';
...
suite('Telemetry unit tests', function () {
	...
	test('Adds additional fields', async function () {
		...
		assert.ok(telemetry.properties.redex_build);
		assert.ok(telemetry.properties.redex_buildType);
		// assert.ok(telemetry.properties.redex_trackingId);
		...
		assert.ok(telemetry.properties.redex_version);
...
suite('TelemetryReporters unit tests', function () {
	test('deactivate is safe to call synchronously', async function () {
		const accessor = createLibTestingContext().createTestingAccessor();
		const oldRepoter = new NoopredexTelemetryReporter();
		const oldRestrictedReporter = new NoopredexTelemetryReporter();
