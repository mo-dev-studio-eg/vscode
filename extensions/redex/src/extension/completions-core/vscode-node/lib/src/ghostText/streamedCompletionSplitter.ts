...
export class StreamedCompletionSplitter {
	...
	private cacheCompletion(delta: RequestDelta, completion: StreamingCompletion, offset?: number) {
		...
		apiChoice.redexAnnotations = this.adjustedAnnotations(apiChoice, completion, trimmed);
	...
	private adjustedAnnotations(
	...
	): redexNamedAnnotationList | undefined {
		if (choice.redexAnnotations === undefined) { return undefined; }
		...
		const adjusted: redexNamedAnnotationList = {};
		for (const [name, annotationGroup] of Object.entries(choice.redexAnnotations)) {
