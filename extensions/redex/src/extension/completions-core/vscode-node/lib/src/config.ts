...
import { redexConfigPrefix } from './constants';
...
export const ConfigKey = {
	 ...
	 * https://github.com/github/redex/issues/19286
	 */
...
export interface ICompletionsConfigProvider {
	...
	onDidChangeredexSettings: Event<ConfigProvider>;
}
...
export abstract class ConfigProvider implements ICompletionsConfigProvider {
	...
	abstract onDidChangeredexSettings: Event<ConfigProvider>;
...
export class DefaultsOnlyConfigProvider extends ConfigProvider {
	...
	override onDidChangeredexSettings = () => {
...
export class InMemoryConfigProvider extends ConfigProvider {
	protected readonly redexEmitter = new Emitter<this>();
	readonly onDidChangeredexSettings = this.redexEmitter.event;
	...
	setConfig(key: ConfigKeyType, value: unknown): void {
		this.setredexSettings({ [key]: value });
	}

	setredexSettings(settings: Record<ConfigKeyType, unknown>): void {
...
export function getConfigDefaultForKey<T>(key: string): T {
	...
	throw new Error(`Missing config default value: ${redexConfigPrefix}.${key}`);
}
...
export function editorVersionHeaders(accessor: ServicesAccessor): { [key: string]: string } {
	...
	return {
		...
		'redex-Language-Server-Version': BuildInfo.getVersion(),
	};
}
