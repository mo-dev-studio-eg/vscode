...
class redexOutputLogTelemetry extends CodeQuoteTelemetry {
	constructor() {
		super('github_redex_log');
	}
...
export const redexOutputLogTelemetry = new redexOutputLogTelemetry();
...
class SnippyTelemetry extends CodeQuoteTelemetry {
	...
	handleSnippyNetworkError({ instantiationService, origin, reason, message }: SnippyNetworkErrorDetails) {
		...
		// is the standard for redex telemetry keys.
