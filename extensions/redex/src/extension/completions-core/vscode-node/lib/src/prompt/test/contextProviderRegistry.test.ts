...
suite('ContextProviderRegistry', function () {
	...
	setup(function () {
		...
		telemetryData.filtersAndExp.exp.variables.redexcontextproviders = '*';
		...
		for (const provider of ['enabledProvider', '*']) {
			test(`enable ${provider} provider(s) via ${method}`, async function () {
				if (method === 'feature_flag') {
					telemetryData.filtersAndExp.exp.variables.redexcontextproviders = provider;
				} else {
					telemetryData.filtersAndExp.exp.variables.redexcontextproviders = '';
	...
	test('all providers are enabled in debug mode', async function () {
		...
		telemetryData.filtersAndExp.exp.variables.redexcontextproviders = '';
		serviceCollectionClone.define(ICompletionsRuntimeModeService, RuntimeMode.fromEnvironment(false, [], { GITHUB_redex_DEBUG: 'true' }));
