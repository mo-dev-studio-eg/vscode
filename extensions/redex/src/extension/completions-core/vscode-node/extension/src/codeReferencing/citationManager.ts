...
import { GitHubredexLogger } from './outputChannel';
...
export class LoggingCitationManager extends Disposable implements ICompletionsCitationManager {
	...
	private logger?: GitHubredexLogger;
	...
	constructor(
		...
		const disposable = onredexToken(authenticationService, _ => {
			if (this.logger) {
				return;
			}
			this.logger = instantiationService.createInstance(GitHubredexLogger);
	...
	async handleIPCodeCitation(citation: IPDocumentCitation): Promise<void> {
		...
		redexOutputLogTelemetry.handleWrite({ instantiationService: this.instantiationService });
