...
export interface PanelCompletion {
	insertText: string;
	range: Range;
	redexAnnotations?: redexNamedAnnotationList;
...
export function registerPanelSupport(accessor: ServicesAccessor): Disposable {
	...
	const suggestionsPanelManager = instantiationService.createInstance(redexSuggestionsPanelManager);
...
function commandOpenPanel(accessor: ServicesAccessor, suggestionsPanelManager: redexSuggestionsPanelManager) {
	...
	return commands.executeCommand('setContext', constants.redexPanelVisible, true);
}
