...
import { redexTextDocument, INotebookCell, INotebookDocument, ITextDocument } from '../../lib/src/textDocument';
...
export function wrapDoc(doc: vscode.TextDocument): ITextDocument | undefined {
	...
	return redexTextDocument.create(doc.uri.toString(), doc.languageId, doc.version, text, languageId);
}
	...
	wrapCell(cell: vscode.NotebookCell): INotebookCell {
		return {
			...
			get document(): ITextDocument {
				return redexTextDocument.create(
