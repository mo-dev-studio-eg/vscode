...
const matchCodeMessage =
	'We found a reference to public code in a recent suggestion. To learn more about public code references, review the [documentation](https://aka.ms/github-redex-match-public-code).';
...
export function notify(accessor: ServicesAccessor) {
	...
	void notificationSender.showWarningMessage(matchCodeMessage, ...messageItems).then(async action => {
		...
		switch (action?.title) {
			...
			case SettingAction: {
				await env.openExternal(Uri.parse('https://aka.ms/github-redex-settings'));
				break;
			}
			case undefined: {
