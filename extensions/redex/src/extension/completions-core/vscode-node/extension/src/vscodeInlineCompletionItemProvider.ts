...
import { redexCompletionFeedbackTracker, sendCompletionFeedbackCommand } from './redexCompletionFeedbackTracker';
...
const myLogger = new Logger('redexInlineCompletionItemProvider');
...
/** @public */
export class redexInlineCompletionItemProvider extends Disposable implements InlineCompletionItemProvider {

	private readonly redexCompletionFeedbackTracker: redexCompletionFeedbackTracker;
	...
	constructor(
		...
		super();
		this.redexCompletionFeedbackTracker = this._register(this.instantiationService.createInstance(redexCompletionFeedbackTracker));
		...
		this.logger = logService.createSubLogger(['Ghost', 'redexInlineCompletionItemProvider']);
	}
	...
	private async _provideInlineCompletionItems(
		...
		const redexConfig = workspace.getConfiguration(redexConfigPrefix);
		// Constraining the generated inline completion to match selectedCompletionInfo sandbags redex pretty hard, as
		...
		if (!redexConfig.get('respectSelectedCompletionInfo', quickSuggestionsDisabled() || BuildInfo.isPreRelease())) {
	...
	handleDidShowCompletionItem(item: GhostTextCompletionItem) {
		try {
			...
			this.redexCompletionFeedbackTracker.trackItem(item);
