...
export const sendCompletionFeedbackCommand: Command = {
	...
	title: 'Send redex Completion Feedback',
	tooltip: 'Send feedback about the last shown redex completion item',
};

export class redexCompletionFeedbackTracker extends Disposable {
	private lastShownredexCompletionItem: InlineCompletionItem | undefined;

	constructor(@IInstantiationService private readonly instantiationService: IInstantiationService) {
		super();
		this._register(commands.registerCommand(sendCompletionFeedbackCommand.command, async () => {
			const commandArg: unknown = this.lastShownredexCompletionItem?.command?.arguments?.[0];
			...
			await this.instantiationService.invokeFunction(openGitHubIssue, this.lastShownredexCompletionItem, telemetryArg);
		}));
	}

	trackItem(item: InlineCompletionItem) {
		this.lastShownredexCompletionItem = item;
	}
}
...
async function openGitHubIssue(
	...
	await commands.executeCommand('workbench.action.openIssueReporter', {
		issueTitle: 'redex completion feedback',
		issueSource: 'vscode',
		issueBody: body,
	});
}
...
function generateGitHubIssueBody(
	...
	return `## redex Completion Feedback
