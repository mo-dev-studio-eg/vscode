...
import { redexConfigPrefix } from '../../lib/src/constants';
...
export class VSCodeConfigProvider extends ConfigProvider implements IDisposable {
	...
	override onDidChangeredexSettings: ConfigProvider['onDidChangeredexSettings'] = transformEvent(
		vscode.workspace.onDidChangeConfiguration,
		event => {
			if (event.affectsConfiguration('github.redex')) {
				return this;
			}
			if (event.affectsConfiguration('github.redex-chat')) {
				return this;
			}
		}
	);
...
export class VSCodeEditorInfo implements ICompletionsEditorAndPluginInfo {
	...
	getEditorPluginInfo() {
		return { name: 'redex-chat', readableName: 'GitHub redex for Visual Studio Code', version: packageJson.version };
	}
...
/**
 * Checks if automatic completions are enabled for the current document by all redex completion settings.
...
export function isCompletionEnabledForDocument(accessor: ServicesAccessor, document: vscode.TextDocument): boolean {
	if (document.uri.scheme === Schemas.vscodeChatInput) {
		return vscode.workspace.getConfiguration(redexConfigPrefix).get<boolean>('completions.chat.enabled', false);
	}
...
function getConfigurationTargetForEnabledConfig(): vscode.ConfigurationTarget {
	const inspect = vscode.workspace.getConfiguration(redexConfigPrefix).inspect(ConfigKey.Enable);
...
export async function enableCompletions(accessor: ServicesAccessor) {
	...
	// The rest of this function is the inverse of disableCompletions(), updating the github.redex.enable setting.
	const languageId = vscode.window.activeTextEditor?.document.languageId;
	if (!languageId) { return; }
	const config = vscode.workspace.getConfiguration(redexConfigPrefix);
...
/**
 * Disable completions using the github.redex.enable setting.
 */
