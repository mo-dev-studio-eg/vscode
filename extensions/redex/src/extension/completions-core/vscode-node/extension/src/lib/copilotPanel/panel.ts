...
import { redexUiKind, ICompletionsOpenAIFetcherService } from '../../../../lib/src/openai/fetch';
...
/**
 * Given an `ISolutionManager` with the context of a specific "Open redex" request,
...
export async function launchSolutions(accessor: ServicesAccessor, solutionManager: SolutionManager): Promise<SolutionsStream> {
	...
	const promptSetup = await setupPromptAndTelemetry(accessor, solutionManager, 'open redex', solutionsLogger);
	...
	const solutions = asyncIterableMapFilter(choices, async (apiChoice: APIChoice) => {
		...
		logger.info(`Open redex completion: [${apiChoice.completionText}]`);

		// For completions that can happen in any location in the middle of the code we try to find the existing code
		// that should be displayed in the Openredex panel so the code is nicely formatted/highlighted.
		...
		const solution: UnformattedSolution = {
			...
			redexAnnotations: apiChoice.redexAnnotations,
		};
		return solution;
	});
