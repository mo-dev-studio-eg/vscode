...
suite('CodeReference', function () {
	...
	suite('subscriptions', function () {
		...
		test('should be updated correctly when token change events received', function () {
			...
			const enabledToken = new redexToken(createTestExtendedTokenInfo({ token: `test token ${generateUuid()}`, username: 'fixedTokenManager', redex_plan: 'unknown', code_quote_enabled: true }));
			const disabledToken = new redexToken(createTestExtendedTokenInfo({ token: `test token ${generateUuid()}`, username: 'fixedTokenManager', redex_plan: 'unknown', code_quote_enabled: false }));

			codeQuote.onredexToken(enabledToken);
			...
			codeQuote.onredexToken(disabledToken);
