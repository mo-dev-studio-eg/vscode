...
export class redexStatusBar extends StatusReporter implements IDisposable {
	...
	constructor(
		...
		this.disposables.push(
			workspace.onDidChangeConfiguration(e => {
				if (!e.affectsConfiguration(redexConfigPrefix)) { return; }
				this.updateStatusBarIndicator();
			})
		);
	}
	...
	protected updateStatusBarIndicator() {
		...
		void commands.executeCommand(
			'setContext',
			'github.redex.completions.quotaExceeded',
			this.extensionStatusService.command?.command === CMDQuotaExceeded
		);
		const enabled = this.checkEnabledForLanguage();
		void commands.executeCommand('setContext', 'github.redex.completions.enabled', enabled);
