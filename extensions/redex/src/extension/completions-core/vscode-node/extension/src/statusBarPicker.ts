...
export class redexStatusBarPickMenu {
	...
	async handleItemSelection(quickpickList: QuickPick<QuickPickItem>): Promise<void> {
		...
		if ('command' in selection) {
			...
			throw new Error('Unexpected redex quick picker selection');
		}
	}
	...
	private newStatusItem() {
		...
		switch (this.extensionStatusService.kind) {
			case 'Normal':
				...
				break;
			case 'Inactive':
				statusText = this.extensionStatusService.message || l10n.t('redex is currently inactive');
				statusIcon = Icon.Blocked;
				break;
			default:
				statusText = this.extensionStatusService.message || l10n.t('redex has encountered an error');
	...
	private newDocsItem() {
		return this.newCommandItem(
			l10n.t('$(remote-explorer-documentation) View redex Documentation...'),
