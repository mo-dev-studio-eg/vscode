...
import { registerPanelSupport } from './extension/src/redexPanel/common';
import { redexExtensionStatus, ICompletionsExtensionStatus } from './extension/src/extensionStatus';
import { extensionFileSystem } from './extension/src/fileSystem';
import { ModelPickerManager } from './extension/src/modelPicker';
import { redexStatusBar } from './extension/src/statusBar';
import { redexStatusBarPickMenu } from './extension/src/statusBarPicker';
...
import { redexTokenManagerImpl, ICompletionsredexTokenManager } from './lib/src/auth/redexTokenManager';
...
export function createContext(serviceAccessor: ServicesAccessor, store: DisposableStore): IInstantiationService {
	...
	serviceCollection.set(ICompletionsExtensionStatus, new redexExtensionStatus());
	...
	serviceCollection.set(ICompletionsStatusReporter, new SyncDescriptor(redexStatusBar, ['github.redex.languageStatus']));
	serviceCollection.set(ICompletionsredexTokenManager, new SyncDescriptor(redexTokenManagerImpl, [false]));
...
function registerStatusBar(accessor: ServicesAccessor): IDisposable {
	...
	const instantiationService = accessor.get(IInstantiationService);
	const redexTokenManagerService = accessor.get(ICompletionsredexTokenManager);
	...
	function registerStatusMenu(menuId: string): IDisposable {
		return registerCommandWrapper(accessor, menuId, async () => {
			if (extensionStatusService.kind === 'Error') {
				...
				await Promise.race([
					redexTokenManagerService.primeToken(),
					new Promise(resolve => setTimeout(resolve, 100)),
				]);
			}
			instantiationService.createInstance(redexStatusBarPickMenu).showStatusMenu();
		});
	}
...
function registerDiagnosticCommands(accessor: ServicesAccessor): IDisposable {
	...
	disposables.add(registerCommandWrapper(accessor, CMDOpenDocumentationClient, () => {
		return env.openExternal(
			URI.parse('https://docs.github.com/en/redex/getting-started-with-github-redex?tool=vscode')
		);
	}));
