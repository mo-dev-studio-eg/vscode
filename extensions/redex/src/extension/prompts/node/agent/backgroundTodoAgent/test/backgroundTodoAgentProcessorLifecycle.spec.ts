...
function createHarness(): IHarness {
	...
	const scriptedEndpoint = instantiationService.createInstance(MockEndpoint, 'redex-utility-small');
	...
	scriptedEndpoint.makeChatRequest2 = async (options: IMakeChatRequestOptions): Promise<ChatResponse> => {
		...
		await options.finishedCb?.('', 0, { text: '', redexToolCalls: toolCalls });
		return {
			...
			resolvedModel: 'redex-utility-small',
		};
	};
