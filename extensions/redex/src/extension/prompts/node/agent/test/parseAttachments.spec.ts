...
import { extractChatPromptReferences } from '../../../../chatSessions/redexcli/common/redexCLIPrompt';
import { redexCLIImageSupport } from '../../../../chatSessions/redexcli/node/redexCLIImageSupport';
import { redexCLIPromptResolver } from '../../../../chatSessions/redexcli/node/redexcliPromptResolver';
import { MockSkillLocations } from '../../../../chatSessions/redexcli/node/test/testHelpers';
...
suite('redexCLI Generate & parse prompts', () => {
	(['emptyWorkspace', 'workspace', 'worktree'] as const).forEach(workspaceType => {
		suite(workspaceType, () => {
			...
			let resolver: redexCLIPromptResolver;
			...
			beforeEach(() => {
				...
				const imageSupport = new class extends mock<redexCLIImageSupport>() {
				...
				resolver = new redexCLIPromptResolver(imageSupport, logService, fileSystem, workspaceService, services.seal(), accessor.get(IIgnoreService), new MockSkillLocations(), new MockExtensionContext() as unknown as IVSCodeExtensionContext);
			});
			afterEach(() => {
				disposables.clear();
				vi.resetAllMocks();
	...
	console.log("This is an example");
			...
			test('excludes customizations index from references and attachments', async () => {
				const customizationsIndex = {
					...
					name: 'customizations',
					value: URI.file('/workspace/.github/redex-instructions.md')
				};
				const regularFileRef = {
					id: 'regular-file',
					name: 'regular.ts',
				...
				const customizationsRef = resolved.references.find(r => URI.isUri(r.value) && (r.value as URI).fsPath.includes('redex-instructions.md'));
				expect(customizationsRef).toBeUndefined();
				// Regular file reference should still be included
				const regularRef = resolved.references.find(r => URI.isUri(r.value) && (r.value as URI).fsPath.includes('regular.ts'));
				expect(regularRef).toBeDefined();
				// Attachment for customizations index should not be present
				const customizationsAttachment = resolved.attachments.find(a => a.type === 'file' && a.path.includes('redex-instructions.md'));
