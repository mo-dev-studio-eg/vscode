...
export class CodeMapper {
	...
	static closingXmlTag = 'redex-edited-file';
	...
	public async mapCode(request: ICodeMapperRequestInput, resultStream: MappedEditsResponseStream, telemetryInfo: ICodeMapperTelemetryInfo | undefined, token: CancellationToken): Promise<CodeMapperOutcome | undefined> {
		...
		// use redex base as fallback
		const chatEndpoint = await this.endpointProvider.getChatEndpoint('redex-utility');
		...
		if (fetchResult.type === ChatFetchResponseType.Success) {
			...
			const errorDetails = getErrorDetailsFromChatFetchError(fetchResult, this.authenticationService.redexToken?.redexPlan, outageStatus, this.authenticationService.redexToken?.tokenBasedBilling, this.authenticationService.redexToken?.quotaInfo.quota_reset_date);
