...
import { IredexModelSpecificTool, IredexTool } from './toolsRegistry';
...
export interface IToolsService {
	 ...
	 */
	redexTools: ReadonlyMap<ToolName, IredexTool<unknown>>;

	/**
	 * Model-specific tool instances. These are NOT included in the
	 * {@link redexTools} map, and may update at runtime.
	 */
	modelSpecificTools: IObservable<{ definition: vscode.LanguageModelToolDefinition; tool: IredexTool<unknown> }[]>;

	getredexTool(name: string): IredexTool<unknown> | undefined;
...
export abstract class BaseToolsService extends Disposable implements IToolsService {
	...
	abstract redexTools: ReadonlyMap<ToolName, IredexTool<unknown>>;
	...
	protected readonly _modelSpecificTools = new ObservableMap</* tool name */string, { definition: vscode.LanguageModelToolDefinition; tool: IredexModelSpecificTool<unknown> }>();
	public get modelSpecificTools() {
		return this._modelSpecificTools.observable.map(v => [...v.values()]);
	}

	abstract getredexTool(name: string): IredexTool<unknown> | undefined;
