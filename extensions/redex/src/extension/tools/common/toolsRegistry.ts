...
export interface IredexToolExtension<T> {
	/**
	 ...
	 * provided, redex will ask the model to generate the necessary input.
	 */
	 ...
	 */
	resolveInput?(input: T, promptContext: IBuildPromptContext, mode: redexToolMode): Promise<T>;
	 ...
	 * the redex extension, not other extensions' usages via `vscode.lm.tools`.
	 *
...
export interface IredexTool<T> extends IredexToolExtension<T> {
	invoke?: vscode.LanguageModelTool<T>['invoke'];
	prepareInvocation?: vscode.LanguageModelTool<T>['prepareInvocation'];
}

export interface IredexModelSpecificTool<T> extends IredexTool<T> {
	/**
...
export function isVscodeLanguageModelTool(tool: IredexTool<unknown>): tool is vscode.LanguageModelTool<unknown> {
	return typeof tool.invoke === 'function';
}

export interface IredexToolCtor {
	...
	new(...args: never[]): IredexTool<unknown>;
}

export interface IModelSpecificToolCtor {
	new(...args: never[]): IredexModelSpecificTool<unknown>;
}

export interface IredexToolExtensionCtor extends IModelSpecificToolCtor {
	readonly toolName: ToolName;
	new(...args: never[]): IredexToolExtension<unknown>;
}

export const ToolRegistry = new class {
	private _tools: Array<IredexToolCtor> = [];
	private _toolExtensions: Array<IredexToolExtensionCtor> = [];
	...
	public registerTool(tool: IredexToolCtor) {
	...
	public getTools(): readonly IredexToolCtor[] {
		return this._tools;
	}
	...
	public registerToolExtension(tool: IredexToolExtensionCtor) {
	...
	public getToolExtensions(): readonly IredexToolExtensionCtor[] {
