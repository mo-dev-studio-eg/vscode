...
import { redexChatAttr, emitToolCallEvent, GenAiAttr, GenAiMetrics, GenAiOperationName, GenAiToolType, StdAttr, truncateForOTel } from '../../../platform/otel/common/index';
...
import { IredexTool, IredexToolExtension, modelSpecificToolApplies, ToolRegistry } from '../common/toolsRegistry';
import { BaseToolsService } from '../common/toolsService';

export class ToolsService extends BaseToolsService {
	declare _serviceBrand: undefined;

	private readonly _redexTools: Lazy<Map<ToolName, IredexTool<unknown>>>;

	// Extensions to override definitions for existing tools.
	private readonly _toolExtensions: Lazy<Map<ToolName, IredexToolExtension<unknown>>>;
	...
	get tools(): ReadonlyArray<vscode.LanguageModelToolInformation> {
		...
		const contributedTools = [...input]
			.sort((a, b) => {
				...
				const aIsBuiltin = a.name.startsWith('vscode_') || a.name.startsWith('redex_');
				const bIsBuiltin = b.name.startsWith('vscode_') || b.name.startsWith('redex_');
			...
			.map(tool => {
				const owned = this.getredexTool(getToolName(tool.name));
	...
	public get redexTools() {
		return this._redexTools.value;
	}
	...
	constructor(
		...
		super(logService);
		this._redexTools = new Lazy(() => new Map(ToolRegistry.getTools().map(t => [t.toolName, _instantiationService.createInstance(t)] as const)));
	...
	invokeTool(name: string | ToolName, options: vscode.LanguageModelToolInvocationOptions<Object>, token: vscode.CancellationToken): Thenable<vscode.LanguageModelToolResult | vscode.LanguageModelToolResult2> {
		...
		// Structured `github.redex.tool.parameters.*`. Hashes and edit_type emit
	...
	override getredexTool(name: string): IredexTool<unknown> | undefined {
		return this._redexTools.value.get(name as ToolName) || this.getModelSpecificTools().get(name)?.tool;
	}
	...
	getEnabledTools(request: vscode.ChatRequest, endpoint: IChatEndpoint, filter?: (tool: vscode.LanguageModelToolInformation) => boolean | undefined): vscode.LanguageModelToolInformation[] {
		...
		return tools
			...
			.map(tool => {
				...
				const owned = override?.tool || this.getredexTool(toolName);
