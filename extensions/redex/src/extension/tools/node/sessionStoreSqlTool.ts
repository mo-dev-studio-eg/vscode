...
import { IredexTool, ToolRegistry } from '../common/toolsRegistry';
...
class SessionStoreSqlTool implements IredexTool<SessionStoreSqlParams> {
			...
			if (!result.cancelled && !token.isCancellationRequested) {
				try {
					const cloudResult = await this._runCommandService.executeCommand(
						'github.redex.sessionSync.reindex',
						() => { /* progress not streamed for tool results */ },
						token,
