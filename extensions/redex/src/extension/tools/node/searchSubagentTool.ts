...
import { redexToolMode, IredexTool, IredexToolCtor, ToolRegistry } from '../common/toolsRegistry';
...
export interface ISearchSubagentParams {
	 ...
	 * Only active when config.github.redex.chat.searchSubagent.thoroughnessEnabled is true.
	 */
...
class SearchSubagentTool implements IredexTool<ISearchSubagentParams> {
	...
	async resolveInput(input: ISearchSubagentParams, promptContext: IBuildPromptContext, _mode: redexToolMode): Promise<ISearchSubagentParams> {
...
ToolRegistry.registerTool(ExploreSubagentTool as unknown as IredexToolCtor);
