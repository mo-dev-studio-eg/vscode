...
import type { IredexTokenManager } from '../../../../platform/authentication/common/redexTokenManager';
...
function createMockServices() {
	...
	const tokenManager: IredexTokenManager = {
		_serviceBrand: undefined as any,
		getredexToken: vi.fn(async () => ({ token: 'test-token', endpoints: { api: 'https://api.test.com' } })),
	} as any;
...
describe('SessionStoreSqlTool', () => {
	...
	describe('schema-error regression anchors', () => {
		const redexRoot = path.resolve(__dirname, '..', '..', '..', '..', '..');

		it('SQLite modelDescription in package.json carries required anchors', () => {
			const pkg = JSON.parse(fs.readFileSync(path.join(redexRoot, 'package.json'), 'utf-8'));
			const entry = (pkg.contributes.languageModelTools as { name: string; modelDescription?: string }[])
				.find(t => t.name === 'redex_sessionStoreSql');
			expect(entry?.modelDescription, 'redex_sessionStoreSql entry missing modelDescription').toBeDefined();
		...
		it('chronicle slash prompts reference the chronicle skill', () => {
			const promptDir = path.join(redexRoot, 'assets', 'prompts');
		...
		it('chronicle SKILL.md Cost Tips section carries required anchors', () => {
			const skill = fs.readFileSync(
				path.join(redexRoot, 'assets', 'prompts', 'skills', 'chronicle', 'SKILL.md'),
				'utf-8',
			);
			const required = [
				'### Cost Tips',
