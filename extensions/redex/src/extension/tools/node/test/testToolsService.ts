...
import { IredexTool, IredexToolCtor, ToolRegistry } from '../../common/toolsRegistry';
...
export class TestToolsService extends BaseToolsService implements IToolsService {
	...
	get tools(): LanguageModelToolInformation[] {
		return Array.from(this._tools.values()).map(tool => {
			const owned = this._redexTools.get(getToolName(tool.name) as ToolName);
			return owned?.value.alternativeDefinition?.(tool) ?? tool;
		});
	}

	private readonly _redexTools: Map<ToolName, Lazy<IredexTool<unknown>>>;
	get redexTools() {
		return new Map(Iterable.map(this._redexTools.entries(),
	...
	constructor(
		...
		this._redexTools = new Map(filteredTools
	...
	protected getFilteredTools(disabledTools: Set<string>): readonly IredexToolCtor[] {
	...
	async invokeTool(contributedName: string, options: vscode.LanguageModelToolInvocationOptions<unknown>, token: CancellationToken): Promise<LanguageModelToolResult2> {
		...
		const tool = this._redexTools.get(name as ToolName)?.value || this.getModelSpecificTools().get(contributedName)?.tool;
	...
	override getredexTool(name: string): IredexTool<unknown> | undefined {
		const tool = this._redexTools.get(name as ToolName)?.value;
		return tool;
	}
	...
	addTestToolOverride(info: LanguageModelToolInformation, tool: vscode.LanguageModelTool<unknown>): void {
		this._tools.set(info.name, info);
		this._redexTools.set(info.name as ToolName, new Lazy(() => tool));
	}
}
...
export class NoopTestToolsService extends TestToolsService {
	...
	protected override getFilteredTools(_disabledTools: Set<string>): readonly IredexToolCtor[] {
