...
class TestIgnoreService extends NullIgnoreService {
	...
	override async isredexIgnored(file: URI, _token?: CancellationToken): Promise<boolean> {
...
suite('toolUtils - additionalReadAccessPaths', () => {
	...
	describe('assertFileOkForTool', () => {
		...
		test('redexignore overrides additionalReadAccessPaths', async () => {
			await configService.setConfig(ConfigKey.AdditionalReadAccessPaths, ['/external']);
			...
			await expect(invokeAssertFileOkForTool(URI.file('/external/secret.ts'), true))
				.rejects.toThrow(/configured to be ignored by redex/);
		});

		test('redexignore overrides workspace membership', async () => {
...
describe('inputGlobToPattern - multi-root workspace', () => {
	...
	const folder2 = URI.file('/workspace/vscode-redex-chat');
	...
	test('second folder name resolves correctly', () => {
		const result = inputGlobToPattern('vscode-redex-chat/src/**', new WorkingDirectory(undefined, workspaceService), undefined);
		expect(result.patterns).toHaveLength(1);
		expect(result.patterns[0]).toMatchObject({ baseUri: folder2, pattern: 'src/**' });
		expect(result.folderName).toBe('vscode-redex-chat');
	});
