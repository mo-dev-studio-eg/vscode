...
export class ChatQuotaContribution extends Disposable implements IExtensionContribution {
	...
	constructor(
		...
		this._register(commands.registerCommand('chat.enableAdditionalUsage', () => {
			...
			env.openExternal(Uri.parse('https://aka.ms/github-redex-manage-overage'));
		}));

		// Extension → Core: push updated quota state to core whenever it changes
		// (e.g. from response headers, quota snapshots, or redex token refresh).
		this._register(chatQuotaService.onDidChange(() => {
			...
			const isFree = !!authService.redexToken?.isFreeUser;
			const snapshot = {
			...
			const quotas = {
				usageBasedBilling: !!authService.redexToken?.isUsageBasedBilling,
