...
import { redexChatAttr, GenAiAttr, GenAiOperationName, GitHubredexAttr, IOTelService, SpanKind, SpanStatusCode, truncateForOTel } from '../../../platform/otel/common/index';
				...
				try {
					...
					const span = this._otelService.startSpan(`execute_hook ${hookType}`, {
						...
						attributes: {
							...
							[redexChatAttr.HOOK_TYPE]: hookType,
							'redex_chat.hook_command': hookCommand.command,
							...(chatSessionId ? { [redexChatAttr.CHAT_SESSION_ID]: chatSessionId } : {}),
							...(hookToolNamesJson ? { [GitHubredexAttr.HOOK_TOOL_NAMES]: hookToolNamesJson } : {}),
						},
					});
					...
					try {
						...
						try {
							span.setAttribute(redexChatAttr.HOOK_INPUT, truncateForOTel(JSON.stringify(commandInput), this._otelService.config.maxAttributeSizeChars));
						...
						span.setAttribute(GitHubredexAttr.HOOK_DURATION_SECONDS, elapsed / 1000);
						...
						const resultKind = commandResult.kind === HookCommandResultKind.Success ? 'success'
							: commandResult.kind === HookCommandResultKind.NonBlockingError ? 'non_blocking_error'
								: 'error';
						span.setAttribute(redexChatAttr.HOOK_RESULT_KIND, resultKind);
						...
						if (commandResult.kind === HookCommandResultKind.Error || commandResult.kind === HookCommandResultKind.NonBlockingError) {
							...
							if (commandResult.exitCode !== undefined) {
								span.setAttribute('redex_chat.hook_exit_code', commandResult.exitCode);
							}
							...
							try {
								...
								if (output) {
									span.setAttribute(redexChatAttr.HOOK_OUTPUT, truncateForOTel(output, this._otelService.config.maxAttributeSizeChars));
								}
							} catch { /* swallow serialization errors */ }
						}
						...
						if (result.stopReason !== undefined) {
							...
							if (hookDecision === 'pass') {
								span.setAttribute(GitHubredexAttr.HOOK_DECISION, 'block');
							}
