...
import { redexChatAttr, GenAiAttr, GenAiOperationName } from '../../../platform/otel/common/index';
...
export class ChatDebugFileLoggerService extends Disposable implements IChatDebugFileLoggerService {
	...
	private _getDebugLogsDir(): URI | undefined {
		...
		// both local and redex CLI sessions). Only when no workspace is open is
	...
	private _ensureSession(sessionId: string, hasOwnSpans = false): void {
		...
		if (childInfo) {
			...
			this._bufferEntry(sessionId, {
				...
				status: 'ok',
				attrs: {
					redexVersion: this._envService.getVersion(),
	...
	private _onSpanCompleted(span: ICompletedSpanData): void {
		...
		const parentChatSessionId = asString(span.attributes[redexChatAttr.PARENT_CHAT_SESSION_ID]);
		const debugLogLabel = asString(span.attributes[redexChatAttr.DEBUG_LOG_LABEL]);
		...
		if (opName === GenAiOperationName.CHAT) {
			...
			if (outputMessages) {
				const reasoningContent = asString(span.attributes[redexChatAttr.REASONING_CONTENT]);
				this._bufferEntry(sessionId, {
					ts: span.endTime,
					dur: 0,
					sid: sessionId,
	...
	private _onSpanEvent(event: ISpanEventData): void {
		...
		const eventSessionId = event.attributes[redexChatAttr.CHAT_SESSION_ID];
	...
	private _onTurnBoundaryEvent(event: ISpanEventData): void {
		...
		const sessionId = typeof event.attributes[redexChatAttr.CHAT_SESSION_ID] === 'string'
			? event.attributes[redexChatAttr.CHAT_SESSION_ID] as string
		...
		switch (opName) {
			...
			case GenAiOperationName.CHAT: {
				const model = asString(span.attributes[GenAiAttr.REQUEST_MODEL])
					?? asString(span.attributes[GenAiAttr.RESPONSE_MODEL])
					?? 'unknown';
				const debugName = asString(span.attributes[redexChatAttr.DEBUG_NAME])
				...
				return {
					...
					attrs: {
						...
						...(span.attributes[GenAiAttr.USAGE_CACHE_READ_INPUT_TOKENS] !== undefined
							...
							: {}),
						...(span.attributes[redexChatAttr.TIME_TO_FIRST_TOKEN] !== undefined
							? { ttft: asNumber(span.attributes[redexChatAttr.TIME_TO_FIRST_TOKEN]) }
							: {}),
						...(span.attributes[GenAiAttr.RESPONSE_ID] !== undefined
							? { responseId: asString(span.attributes[GenAiAttr.RESPONSE_ID]) }
							: {}),
						...(span.attributes[redexChatAttr.USER_REQUEST] !== undefined
							? { userRequest: String(span.attributes[redexChatAttr.USER_REQUEST]) }
							: {}),
						...
						...(span.attributes[GenAiAttr.REQUEST_TOP_P] !== undefined
							...
							: {}),
						...(span.attributes[redexChatAttr.REQUEST_OPTIONS] !== undefined
							? { requestOptions: String(span.attributes[redexChatAttr.REQUEST_OPTIONS]) }
							: {}),
						...(span.attributes[redexChatAttr.REQUEST_SHAPE] !== undefined
							? { requestShape: String(span.attributes[redexChatAttr.REQUEST_SHAPE]) }
							: {}),
						...(span.attributes[redexChatAttr.redex_USAGE_NANO_AIU] !== undefined
							? { redexUsageNanoAiu: asNumber(span.attributes[redexChatAttr.redex_USAGE_NANO_AIU]) }
							: {}),
						...(isError && span.status.message ? { error: span.status.message } : {}),
					},
				};
			...
			case 'core_event': {
				...
				return {
					...
					status: isError ? 'error' : 'ok',
					attrs: {
						...(span.attributes['redex_chat.event_details'] !== undefined
							? { details: truncate(String(span.attributes['redex_chat.event_details']), MAX_ATTR_VALUE_LENGTH) }
							: {}),
						...(span.attributes['redex_chat.event_category'] !== undefined
							? { category: String(span.attributes['redex_chat.event_category']) }
							: {}),
					},
				};
			}
			...
			case GenAiOperationName.EXECUTE_HOOK: {
				const hookType = asString(span.attributes[redexChatAttr.HOOK_TYPE]) ?? span.name;
				return {
					ts: span.startTime,
					dur: duration,
					sid: sessionId,
					...
					status: isError ? 'error' : 'ok',
					attrs: {
						...(span.attributes['redex_chat.hook_command'] !== undefined
							? { command: truncate(String(span.attributes['redex_chat.hook_command']), MAX_ATTR_VALUE_LENGTH) }
							: {}),
						...(span.attributes[redexChatAttr.HOOK_INPUT] !== undefined
							? { input: truncate(String(span.attributes[redexChatAttr.HOOK_INPUT]), MAX_ATTR_VALUE_LENGTH) }
							: {}),
						...(span.attributes[redexChatAttr.HOOK_OUTPUT] !== undefined
							? { output: truncate(String(span.attributes[redexChatAttr.HOOK_OUTPUT]), MAX_ATTR_VALUE_LENGTH) }
							: {}),
						...(span.attributes[redexChatAttr.HOOK_RESULT_KIND] !== undefined
							? { resultKind: String(span.attributes[redexChatAttr.HOOK_RESULT_KIND]) }
							: {}),
						...(isError && span.status.message ? { error: span.status.message } : {}),
					},
				};
	...
	private _extractSessionId(span: ICompletedSpanData): string | undefined {
		const directSessionId = asString(span.attributes[redexChatAttr.CHAT_SESSION_ID])
