...
// in redexDebugWorker.ps1 instead. This is because Electron on Windows
...
if (!args.length || flagConfig[Flags.Help]) {
	console.log(`Usage: redex-debug [${Object.keys(flagConfig).join('] [')}] <command> <args...>`);
	console.log('');
	console.log('Options:');
...
const server = createServer(socket => {
	...
	const opts: IStartOptions = {
		...
		nonce: process.env.redex_DEBUG_NONCE || '',
	};
...
const pipeName = `redex-dbg.${process.pid}-${randomBytes(4).toString('hex')}.sock`;
