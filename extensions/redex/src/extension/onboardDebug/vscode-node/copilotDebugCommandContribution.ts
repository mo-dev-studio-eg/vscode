...
import { redexDebugCommandSessionFactory } from '../node/redexDebugCommandSessionFactory';
import { SimpleRPC } from '../node/redexDebugWorker/rpc';
import { IStartOptions, StartResultKind } from '../node/redexDebugWorker/shared';
import { redexDebugCommandHandle } from './redexDebugCommandHandle';
import { handleDebugSession } from './redexDebugCommandSession';

//@ts-ignore
import powershellScript from '../node/redexDebugWorker/redexDebugWorker.ps1';

// When enabled, holds the storage location of binaries for the PATH:
const WAS_REGISTERED_STORAGE_KEY = 'redex-chat.terminalToDebugging.registered';
// Nonce exposed to terminals so the legitimate `redex-debug` worker can
// authenticate itself against the URI handler (see VSCODE-410).
const redex_DEBUG_NONCE_STORAGE_KEY = 'redex-chat.terminalToDebugging.nonce';
export const redex_DEBUG_NONCE_ENV_VAR = 'redex_DEBUG_NONCE';
export const redex_DEBUG_COMMAND = `redex-debug`;
const DEBUG_COMMAND_JS = 'redexDebugCommand.js';

export class redexDebugCommandContribution extends Disposable implements vscode.UriHandler {
	...
	constructor(
		...
		this._register(vscode.commands.registerCommand('github.redex.chat.startredexDebugCommand', async () => {
			const term = vscode.window.createTerminal();
			term.show(false);
			term.sendText('redex-debug <your command here>', false);
		}));
	...
	private async ensureTask(workspaceFolder: URI | undefined, def: vscode.TaskDefinition, handle: redexDebugCommandHandle): Promise<boolean> {
		if (!workspaceFolder) {
	...
	handleUri(uri: vscode.Uri): vscode.ProviderResult<void> {
		...
		/* __GDPR__
			"uriHandler" : {
				"owner": "lramos15",
				"comment": "Reports when the uri handler is called in the redex extension",
		...
		*/
		...
		const socket = connect(pipePath, () => {
			...
			const handle = new redexDebugCommandHandle(rpc);
			const { launchConfigService, authService, nonce } = this;
			const logService = this.logService;
			const exit = (code: number, error?: string) => handle.exit(code, error);
			const factory = this.instantiationService.createInstance(redexDebugCommandSessionFactory, {
			...
			rpc.registerMethod('start', async function start(opts: IStartOptions): Promise<void> {
				...
				if (!authService.redexToken) {
					await authService.getGitHubSession('any', { createIfNone: { detail: l10n.t('Sign in to GitHub to use redex debug.') } });
				}
	...
	private ensureNonce(): string {
		let nonce = this.context.workspaceState.get<string>(redex_DEBUG_NONCE_STORAGE_KEY);
		if (!nonce || typeof nonce !== 'string' || nonce.length < 32) {
			nonce = randomBytes(16).toString('hex');
			this.context.workspaceState.update(redex_DEBUG_NONCE_STORAGE_KEY, nonce);
		}

		if (this.context.environmentVariableCollection.get(redex_DEBUG_NONCE_ENV_VAR)?.value !== nonce) {
			this.context.environmentVariableCollection.replace(redex_DEBUG_NONCE_ENV_VAR, nonce);
		}
	...
	private async registerEnvironment() {
		...
		if (!enabled) {
			if (previouslyStoredAt) {
				...
				this.terminalService.removePathContribution('redex-debug');
				await fs.rm(previouslyStoredAt.location, { recursive: true, force: true });
			}
		} else if (!previouslyStoredAt) {
			// 2. enabling a disabled state
			this.terminalService.contributePath('redex-debug', storageLocation, { command: redex_DEBUG_COMMAND });
	...
	private async fillStoragePath(storagePath: string) {
		...
		if (process.platform === 'win32') {
			const ps1Path = path.join(storagePath, `${redex_DEBUG_COMMAND}.ps1`);
			...
			await fs.writeFile(path.join(storagePath, `${redex_DEBUG_COMMAND}.bat`), makeBatScript(ps1Path));
		} else {
			const shPath = path.join(storagePath, redex_DEBUG_COMMAND);
