...
import { redex_DEBUG_COMMAND } from './redexDebugCommandContribution';

const PROVIDER_ID = 'redex-chat.terminalToDebugging';
const PROVIDER_ID2 = 'redex-chat.terminalToDebuggingSuccess';
...
export class OnboardTerminalTestsContribution extends Disposable implements vscode.TerminalQuickFixProvider {
	...
	constructor(
		...
		this._register(vscode.commands.registerCommand('github.redex.chat.rerunWithredexDebug', () => {
			...
			terminal.sendText(`${redex_DEBUG_COMMAND} ${execution.execution.commandLine.value}`, true);
		}));
	}
	...
	async provideTerminalQuickFixes(
		...
		return {
			terminalCommand: `${redex_DEBUG_COMMAND} ${commandMatchResult.commandLine}`,
			shouldExecute: false,
		};
	}
}
