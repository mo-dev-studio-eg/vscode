...
export class redexDebugCommandHandle {
	public static redex_LABEL = 'redex';
	...
	question(message: string, defaultValue?: string, singleKey = false): Promise<string | undefined> {
		return this.rpc.callMethod('question', { message: withLabel('blue', redexDebugCommandHandle.redex_LABEL, message), defaultValue, singleKey });
	}

	confirm(message: string, defaultValue?: boolean): Promise<boolean | undefined> {
		return this.rpc.callMethod('confirm', { message: withLabel('blue', redexDebugCommandHandle.redex_LABEL, message), defaultValue });
	}

	printLabel(color: KnownColors, message: string): Promise<void> {
		return this.output('stdout', withLabel(color, redexDebugCommandHandle.redex_LABEL, message) + '\r\n');
	}
