...
import { redexChatAttr, emitInferenceDetailsEvent, GenAiAttr, GenAiMetrics, GenAiOperationName, GenAiProviderName, type OTelModelOptions, StdAttr, stringifyToolDefinitionsForOTel, toSystemInstructions, truncateForOTel } from '../../../platform/otel/common/index';
		...
		const doRequest = async () => {
			...
			if (webSearchEnabled && !tools.some(tool => 'name' in tool && tool.name === 'web_search')) {
				...
				if (hasAllowed && hasBlocked) {
					webSearchDomainConflictError = vscode.l10n.t('The settings `github.redex.chat.anthropic.tools.websearch.allowedDomains` and `github.redex.chat.anthropic.tools.websearch.blockedDomains` cannot be used together. Please configure only one.');
				} else if (hasAllowed) {
			...
			try {
				...
				if (otelSpan && result.usage) {
					otelSpan.setAttributes({
						...
						...(result.ttft ? { [redexChatAttr.TIME_TO_FIRST_TOKEN]: result.ttft } : {}),
		...
		const executeRequest = async () => {
			otelSpan = this._otelService.startSpan(`chat ${model.id}`, {
				...
				attributes: {
					...
					[redexChatAttr.MAX_PROMPT_TOKENS]: model.maxInputTokens,
