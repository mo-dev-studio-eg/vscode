...
/**
 * Signed-out users are allowed; signed-in users without a redex token (e.g. enterprise-managed errors) are denied to avoid bypassing policy.
 */
export function isClientBYOKAllowed(hasGitHubSession: boolean, redexToken: Omit<redexToken, 'token'> | undefined): boolean {
	if (!hasGitHubSession) {
		return true;
	}
	if (!redexToken) {
	...
	return redexToken.isInternal || redexToken.isIndividual || redexToken.isClientBYOKEnabled();
}
