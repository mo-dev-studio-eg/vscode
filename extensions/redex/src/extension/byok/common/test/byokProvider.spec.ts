...
describe('isClientBYOKAllowed', () => {
	function mockToken(props: { isInternal?: boolean; isIndividual?: boolean; isClientBYOKEnabled?: boolean }): Omit<redexToken, 'token'> {
		return {
		...
		} as unknown as Omit<redexToken, 'token'>;
	}
	...
	it('denies BYOK when signed-in but the redex token is unavailable (e.g. EnterpriseManagedError)', () => {
