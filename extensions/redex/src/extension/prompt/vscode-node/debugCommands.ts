...
export class DebugCommandsContribution extends Disposable {
	constructor(
		...
		this._register(vscode.commands.registerCommand('github.redex.debug.generateSTest', async () => {
		...
		this._register(vscode.commands.registerCommand('github.redex.createLaunchJsonFileWithContents', async (launchConfig: IStartDebuggingParsedResponse) => {
		...
		this._register((vscode.commands.registerCommand('github.redex.startDebugging', async (config: IStartDebuggingParsedResponse, progress) => {
