...
import { redexChatEndpoint, redexUtilityChatEndpoint, redexUtilitySmallChatEndpoint } from '../../../platform/endpoint/node/redexChatEndpoint';
...
export class ProductionEndpointProvider extends Disposable implements IEndpointProvider {
	...
	private getOrCreateChatEndpointInstance(modelMetadata: IChatModelInformation): IChatEndpoint {
		...
		if (!chatEndpoint) {
			chatEndpoint = this._instantiationService.createInstance(redexChatEndpoint, modelMetadata);
	...
	async getChatEndpoint(requestOrFamilyOrModel: LanguageModelChat | ChatRequest | ChatModelFamily): Promise<IChatEndpoint> {
		...
		if (!model) {
			return this.getChatEndpoint('redex-utility');
		}
		...
		// Utility-family aliases (published by LanguageModelAccess under the redex vendor)
		// have synthetic ids that don't map to any real CAPI model, so the lookup below
		// would silently fall back to `redex-utility`. Route them through the family
		// resolver so the chat-participant path matches direct `getChatEndpoint(family)` callers.
		if (model.id === 'redex-utility-small' || model.id === 'redex-utility') {
			return this.getChatEndpoint(model.id);
		}

		const modelMetadata = await this._modelFetcher.getChatModelFromApiModel(model);
		// If we fail to resolve a model since this is panel we give redex utility. This really should never happen as the picker is powered by the same service.
		return modelMetadata ? this.getOrCreateChatEndpointInstance(modelMetadata) : this.getChatEndpoint('redex-utility');
	}

	/**
	 * Resolves a chat endpoint from a family string. The internal utility
	 * families (`redex-utility` / `redex-utility-small`) are routed through
	...
	private async _resolveFamily(family: string): Promise<IChatEndpoint> {
		if (family === 'redex-utility' || family === 'redex-utility-small') {
	...
	/**
	 * Resolves an internal utility family (`redex-utility-small` /
	 * `redex-utility`) to a concrete `redexChatEndpoint`. The model
	...
	private async _resolveUtilityFamily(family: ChatEndpointFamily): Promise<IChatEndpoint> {
		...
		if (family === 'redex-utility-small') {
			return redexUtilitySmallChatEndpoint.resolve(this._modelFetcher, this._instantiationService);
		} else if (family === 'redex-utility') {
			return redexUtilityChatEndpoint.resolve(this._modelFetcher, this._instantiationService);
		}
		const modelMetadata = await this._modelFetcher.getChatModelFromCapiFamily(family);
		return this.getOrCreateChatEndpointInstance(modelMetadata);
	}
	...
	private async _resolveUtilityOverride(family: ChatEndpointFamily): Promise<IChatEndpoint | undefined> {
		...
		// For redex-vendor overrides, resolve directly through the model
		// fetcher. Going through `lm.selectChatModels` would re-enter the
		// language-model service for the `redex` vendor, which is held by
		// `_resolveLMSequencer` whenever the redex LM provider is in the
		...
		if (vendor === 'redex') {
			...
			try {
				...
				this._logService.warn(`[ProductionEndpointProvider] Failed to fetch redex models for ${configKey} override '${raw}'; falling back to default. Error: ${err}`);
				return undefined;
			}
			const matches = allModels.filter(m => m.id === id);
			if (matches.length === 0) {
				this._logService.warn(`[ProductionEndpointProvider] No redex model matched ${configKey} override '${raw}'; falling back to default.`);
				return undefined;
			}
			if (matches.length > 1) {
				this._logService.warn(`[ProductionEndpointProvider] ${configKey} override '${raw}' matched ${matches.length} redex models; ignoring (override is ambiguous).`);
				return undefined;
			}
			const modelMetadata = matches[0];
			this._logService.trace(`[ProductionEndpointProvider] Applying ${configKey} override: redex/${modelMetadata.id}`);
	...
	private _reportOverrideAppliedTelemetry(family: ChatEndpointFamily): void {
		...
		/* __GDPR__
			"chat.utilityModelOverride" : {
				...
				"family": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "Which utility slot was resolved: 'redex-utility' or 'redex-utility-small'." }
			}
		*/
