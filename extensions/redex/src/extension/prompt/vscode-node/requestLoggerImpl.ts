...
class LoggedRequestInfo implements ILoggedRequestInfo {
	...
	toJSON(): object {
		...
		const metadata = {
			...
			redexUsageAic: this.entry.type === LoggedRequestKind.ChatMLSuccess && typeof this.entry.usage?.redex_usage?.total_nano_aiu === 'number'
				? this.entry.usage.redex_usage.total_nano_aiu / 1_000_000_000 : undefined,
...
`;
	...
	private _renderRequestToMarkdown(id: string, entry: LoggedRequest): string {
		...
		if (entry.type === LoggedRequestKind.ChatMLSuccess) {
			...
			if (typeof entry.usage?.redex_usage?.total_nano_aiu === 'number') {
				const aic = entry.usage.redex_usage.total_nano_aiu / 1_000_000_000;
				result.push(`redexUsage    : ${aic.toFixed(2)} AIC (${entry.usage.redex_usage.total_nano_aiu} nano-AIU)`);
			}
