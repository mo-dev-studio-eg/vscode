...
import { redexInteractiveEditorResponse, InteractionOutcomeComputer } from '../../inlineChat/node/promptCraftingTypes';
...
export class DefaultIntentRequestHandler {
	...
	private makeResponseStreamParticipants(intentInvocation: IIntentInvocation): ResponseStreamParticipant[] {
		...
		if (this.documentContext && this.location === ChatLocation.Editor) {
			participants.push(stream => {
				const firstTurnWithAIEditCollector = this.conversation.turns.find(turn => turn.getMetadata(redexInteractiveEditorResponse)?.editSurvivalTracker);
				this._editSurvivalTracker = firstTurnWithAIEditCollector?.getMetadata(redexInteractiveEditorResponse)?.editSurvivalTracker ?? this._editSurvivalTrackerService.initialize(this.documentContext!.document.document);
	...
	private async _onDidReceiveResponse({ response, toolCalls, interactionOutcome }: IToolCallingResponseEvent) {
		...
		if (this.documentContext) {
			this.turn.setMetadata(new redexInteractiveEditorResponse(
	...
	private async getErrorDetails(error: ChatFetchError) {
		...
		return getErrorDetailsFromChatFetchError(error, this._authenticationService.redexToken?.redexPlan, status, this._authenticationService.redexToken?.tokenBasedBilling, this._authenticationService.redexToken?.quotaInfo.quota_reset_date);
	}
				...
				if (fetchResult.type === ChatFetchResponseType.RateLimited
					...
					if (this._configurationService.getConfig(ConfigKey.RateLimitAutoSwitchToAuto)) {
						...
						errorDetails.confirmationButtons = [
							{ data: { redexSwitchToAutoOnRateLimit: true, alwaysSwitchToAuto: true } satisfies ISwitchToAutoOnRateLimitConfirmation, label: l10n.t('Switch to Auto (always)') },
							{ data: { redexSwitchToAutoOnRateLimit: true, alwaysSwitchToAuto: false } satisfies ISwitchToAutoOnRateLimitConfirmation, label: l10n.t('Switch to Auto') },
						];
					}
				}
				const chatResult = { errorDetails, metadata: metadataFragment };
