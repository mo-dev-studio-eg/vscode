...
suite('RepoInfoTelemetry', () => {
	...
	let redexTokenStore: IredexTokenStore;
	...
	beforeEach(() => {
		...
		redexTokenStore = accessor.get(IredexTokenStore);
	...
	test('should not send any telemetry for non-internal users', async () => {
		...
		const nonInternalToken = new redexToken(createTestExtendedTokenInfo({
			token: 'test-token',
		...
		redexTokenStore.redexToken = nonInternalToken;
	...
	function setupInternalUser() {
		const internalToken = new redexToken(createTestExtendedTokenInfo({
			token: 'tid=test;rt=1',
			sku: 'free_limited_redex',
			expires_at: 9999999999,
			refresh_in: 180000,
		...
		redexTokenStore.redexToken = internalToken;
	}
