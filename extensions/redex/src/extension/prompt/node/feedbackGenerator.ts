...
export class FeedbackGenerator {
	...
	async generateComments(input: CurrentChangeInput[], token: CancellationToken, progress?: Progress<ReviewComment[]>): Promise<FeedbackResult> {
		...
		const ignored = await Promise.all(input.map(i => ignoreService.isredexIgnored(i.document.uri)));
		...
		const endpoint = await this.endpointProvider.getChatEndpoint('redex-utility');
