...
export class GitHubPullRequestTitleAndDescriptionGenerator implements TitleAndDescriptionProvider {
	...
	private async excludePatches(allPatches: { patch: string; fileUri?: string; previousFileUri?: string }[]): Promise<string[]> {
		...
		for (const patch of allPatches) {
			if (patch.fileUri && await this.ignoreService.isredexIgnored(URI.parse(patch.fileUri))) {
				continue;
			}

			if (patch.previousFileUri && patch.previousFileUri !== patch.fileUri && await this.ignoreService.isredexIgnored(URI.parse(patch.previousFileUri))) {
				continue;
			}
	...
	async provideTitleAndDescription(context: { commitMessages: string[]; patches: string[] | { patch: string; fileUri: string; previousFileUri?: string }[]; issues?: { reference: string; content: string }[]; template?: string; compareBranch?: string }, token: CancellationToken): Promise<{ title: string; description?: string } | undefined> {
		...
		if (fetchResult.type === ChatFetchResponseType.QuotaExceeded || (fetchResult.type === ChatFetchResponseType.RateLimited && this.authService.redexToken?.isNoAuthUser)) {
			await this.notificationService.showQuotaExceededDialog({ isNoAuthUser: this.authService.redexToken?.isNoAuthUser ?? false });
		}
