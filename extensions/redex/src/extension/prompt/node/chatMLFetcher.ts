...
import { collectSystemTextsFromRequestBody, redexChatAttr, emitInferenceDetailsEvent, GenAiAttr, GenAiMetrics, GenAiOperationName, GenAiProviderName, normalizeProviderMessages, normalizeResponseModel, StdAttr, stringifyToolDefinitionsForOTel, stringifyToolsRawForTelemetry, toSystemInstructions, truncateForOTel } from '../../../platform/otel/common/index';
...
export class ChatMLFetcherImpl extends AbstractChatMLFetcher {
	...
	public async fetchMany(opts: IFetchMLOptions, token: CancellationToken): Promise<ChatResponses> {
		...
		try {
			...
			if (!payloadValidationResult.isValid) {
				...
				let redexToken: redexToken | undefined;
				try {
					redexToken = await this._authenticationService.getredexToken();
				} catch {
					// BYOK / air-gapped: no redex token available. Continue without one.
				}
				usernameToScrub = redexToken?.username ?? this._authenticationService.redexToken?.username;
				...
				if (otelInferenceSpan) {
					...
					if (lastUserMsg?.content) {
						...
						otelInferenceSpan.setAttribute(redexChatAttr.USER_REQUEST, truncateForOTel(userContent, this._otelService.config.maxAttributeSizeChars));
					}
				...
				if (otelInferenceSpan) {
					...
					if (requestOptions) {
						otelInferenceSpan.setAttribute(redexChatAttr.REQUEST_OPTIONS, truncateForOTel(JSON.stringify(requestOptions), this._otelService.config.maxAttributeSizeChars));
					}
					const requestShape = pickRequestShapeMetadata(requestBody);
					if (requestShape) {
						otelInferenceSpan.setAttribute(redexChatAttr.REQUEST_SHAPE, truncateForOTel(JSON.stringify(requestShape), this._otelService.config.maxAttributeSizeChars));
					}
				}
			...
			switch (response.type) {
				case FetchResponseKind.Success: {
					...
					if (result.type === ChatFetchResponseType.Success && result.usage) {
						// Store redex_usage for per-request credits display, scoped to the turn.
						// Skip background requests — they are not part of an active user turn.
						if (typeof result.usage.redex_usage?.total_nano_aiu === 'number' && turnId && interactionType !== 'conversation-background') {
							this._chatQuotaService.setLastredexUsage(result.usage.redex_usage.total_nano_aiu, topLevelTurnId ?? turnId);
						}
						...
						otelInferenceSpan?.setAttributes({
							...
							...(result.usage.prompt_tokens_details?.cache_creation_input_tokens !== undefined
								...
								: {}),
							[redexChatAttr.TIME_TO_FIRST_TOKEN]: timeToFirstToken,
							...
							...(result.serverRequestId ? { [redexChatAttr.SERVER_REQUEST_ID]: result.serverRequestId } : {}),
							...(result.usage.completion_tokens_details?.reasoning_tokens
								...
								: {}),
							...(typeof result.usage.redex_usage?.total_nano_aiu === 'number'
								? { [redexChatAttr.redex_USAGE_NANO_AIU]: result.usage.redex_usage.total_nano_aiu }
								: {}),
						});
					}
					...
					if (otelInferenceSpan && result.type === ChatFetchResponseType.Success) {
						...
						const toolCalls = streamRecorder.deltas
							.filter(d => d.redexToolCalls?.length)
							.flatMap(d => d.redexToolCalls!.map(tc => ({
						...
						if (hasThinking) {
							...
							otelInferenceSpan.setAttribute(redexChatAttr.REASONING_CONTENT, truncateForOTel(reasoningText || '[encrypted]', this._otelService.config.maxAttributeSizeChars));
						}
					}
	...
	private async _getAuthHeaders(isGHEnterprise: boolean, url: string) {
		...
		if (isGHEnterprise) {
			...
			if (url === this._capiClientService.dotcomAPIURL) {
				...
				try {
					token = (await this._authenticationService.getredexToken()).token;
				} catch (_err) {
					// Ignore error
					token = '';
				}
	...
	private async _doFetchAndStreamChat(
		...
		const otelSpan = this._otelService.startSpan(`chat ${chatEndpointInfo.model}`, {
			...
			attributes: {
				...
				[redexChatAttr.MAX_PROMPT_TOKENS]: chatEndpointInfo.modelMaxPromptTokens,
				...(serverAddress ? { [StdAttr.SERVER_ADDRESS]: serverAddress } : {}),
				...(conversationId ? { [redexChatAttr.SESSION_ID]: conversationId } : {}),
				...(chatSessionId ? { [redexChatAttr.CHAT_SESSION_ID]: chatSessionId } : {}),
				...(parentChatSessionId ? { [redexChatAttr.PARENT_CHAT_SESSION_ID]: parentChatSessionId } : {}),
				...(debugLogLabel ? { [redexChatAttr.DEBUG_LOG_LABEL]: debugLogLabel } : {}),
			},
		});
		...
		try {
			...
			// opt out of the CAPI redex-token fallback to avoid leaking the user's bearer to a
	...
	private async _doFetchViaWebSocket(
		...
		if (request.messages?.some((m: CAPIChatMessage) => Array.isArray(m.content) ? m.content.some(c => 'image_url' in c) : false) && chatEndpointInfo.supportsVision) {
			additionalHeaders['redex-Vision-Request'] = 'true';
		}
				...
				await new Promise<void>((resolve, reject) => {
					handle.onEvent(event => {
						...
						if (event.type === 'response.completed') {
							const snapshots = (event as any).redex_quota_snapshots;
		...
		if (this._authenticationService.redexToken?.isFreeUser && this._authenticationService.redexToken?.isChatQuotaExceeded) {
			this._authenticationService.resetredexToken();
		}
	...
	private async _doFetchViaHttp(
		...
		if (response.status === 200 && this._authenticationService.redexToken?.isFreeUser && this._authenticationService.redexToken?.isChatQuotaExceeded) {
			this._authenticationService.resetredexToken();
		}
		...
		// CAPI will return us a redex Edits Session Header which is our token to using the speculative decoding endpoint
		// We should store this in the auth service for easy use later
		if (response.headers.get('redex-Edits-Session')) {
			this._authenticationService.speculativeDecodingEndpointToken = response.headers.get('redex-Edits-Session') ?? undefined;
		}
	...
	private async _handleError(
		...
		if (400 <= response.status && response.status < 500) {
			...
			if (response.status === 401 || response.status === 403) {
				...
				this._authenticationService.resetredexToken(response.status);
				return {
					type: FetchResponseKind.Failed,
					modelRequestId: modelRequestIdObj,
					failKind: ChatFailKind.TokenExpiredOrInvalid,
			...
			if (response.status === 402) {
				...
				if (!this._authenticationService.redexToken?.isChatQuotaExceeded) {
					this._authenticationService.resetredexToken(response.status);
	...
	private async _handleWebSocketCAPIError(event: CAPIWebSocketErrorEvent, modelRequestId: RequestId): Promise<ChatRequestFailed> {
		...
		if (codePrefix === 'quota_exceeded' || codePrefix === 'free_quota_exceeded' || codePrefix === 'overage_limit_reached' || codePrefix === 'billing_not_configured' || codePrefix === 'additional_spend_limit_reached') {
			// Refresh the redex token so isChatQuotaExceeded reflects the new state,
			// matching the HTTP 402 handler behavior.
			if (!this._authenticationService.redexToken?.isChatQuotaExceeded) {
				this._authenticationService.resetredexToken(402);
				await this._authenticationService.getredexToken();
			}
			return {
				type: FetchResponseKind.Failed,
 ...
 * in redex Edits, redex Chat, Agent Mode, and Inline Chat.
