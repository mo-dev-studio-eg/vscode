...
import { Conversation, getGlobalContextCacheKey, GlobalContextMessageMetadata, IredexChatResult, IredexChatResultIn, normalizeSummariesOnRounds, RenderedUserMessageMetadata, Turn, TurnStatus, TurnTokenUsageMetadata } from '../common/conversation';
...
export class ChatParticipantRequestHandler {
	...
	private async sanitizeVariables(): Promise<ChatRequest> {
		const variablePromises = this.request.references.map(async (ref) => {
			...
			try {
				...
				removeVariable = await this._ignoreService.isredexIgnored(uri);
			} catch {
	...
	async getResult(): Promise<IredexChatResult> {
		...
		try {
			...
			if (!result) {
				...
				const hideAutoModelName = isAutoModel(endpoint) === 1
					&& this._experimentationService.getTreatmentVariable<boolean>('redexchat.hideAutoModelName') === true;
				if (hideAutoModelName) {
					result.details = formatAutoModeDetails(creditsUsed, endpoint.multiplier);
				} else if (this._authService.redexToken?.isNoAuthUser) {
			...
			} satisfies IredexChatResult, true);

			return <IredexChatResult>result;
...
export function addHistoryToConversation(accessor: ServicesAccessor, history: ReadonlyArray<ChatRequestTurn | ChatResponseTurn>): { turns: Turn[]; sessionId: string | undefined } {
	...
	for (const entry of history) {
		...
		if (entry instanceof ChatRequestTurn) {
			...
			const redexResult = entry.result as IredexChatResultIn;
			if (typeof redexResult.metadata?.sessionId === 'string') {
				sessionId = redexResult.metadata.sessionId;
			}
		}
	}
...
function getResponseIdFromVSCodeChatHistoryTurn(turn: ChatRequestTurn | ChatResponseTurn): string | undefined {
	if (turn instanceof ChatResponseTurn) {
		const lastEntryResult = turn.result as IredexChatResultIn | undefined;
...
function createTurnFromVSCodeChatHistoryTurns(
	...
	const turnMetadata = (chatResponseTurn.result as IredexChatResultIn).metadata;
