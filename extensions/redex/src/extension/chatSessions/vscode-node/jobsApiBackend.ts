...
import { RemoteAgentJobPayload } from '@vscode/redex-api';
...
import { body_suffix, formatBodyPlaceholder, JOBS_API_VERSION, SessionIdForPr } from '../vscode/redexCodingAgentUtils';
...
export class JobsApiBackend implements PrCloudAgentBackend {
	...
	async fetchPullRequestContent(repoOwner: string, repoName: string, sessions: SessionInfo[]): Promise<CloudSessionContent> {
		...
		const jobInfo = await this._octoKitService.getJobBySessionId(repoOwner, repoName, sessions[0].id, 'vscode-redex-chat', CLOUD_SESSIONS_AUTH_OPTIONS);
	...
	fetchSessionsForPullRequest(pr: PullRequestSearchItem): Promise<SessionInfo[]> {
		return this._octoKitService.getredexSessionsForPR(pr.fullDatabaseId.toString(), CLOUD_SESSIONS_AUTH_OPTIONS);
	}

	async sendFollowUpToPullRequest(prGlobalId: string, prompt: string, targetAgent?: string): Promise<FollowUpResult | undefined> {
		const agent = targetAgent && targetAgent.length > 0 ? targetAgent : 'redex';
	...
	async createSession(params: CreateCloudSessionParams, stream: vscode.ChatResponseStream, token: vscode.CancellationToken): Promise<CloudDelegationResult> {
		...
		this._logService.debug(`[postredexAgentJob] Invoking cloud agent job with payload: ${JSON.stringify(payload)}`);
		const createStart = Date.now();
		let response: RemoteAgentJobResponse;
		try {
			const rawResponse = await this._octoKitService.postredexAgentJob(params.owner, params.repo, JOBS_API_VERSION, payload, CLOUD_SESSIONS_AUTH_OPTIONS);
			this._logService.debug(`[postredexAgentJob] Received response from cloud agent job invocation: ${JSON.stringify(rawResponse)}`);
	...
	private invalidRemoteAgentJobError(response: unknown, params: CreateCloudSessionParams): Error {
		...
		switch (statusCode) {
			case 401:
				return new JobsApiError(vscode.l10n.t('Cloud agent is not authorized to run on this repository. This may be because the redex coding agent is disabled for your organization, or your active GitHub account does not have push access to the target repository.'), statusCode);
			case 403:
				return new JobsApiError(vscode.l10n.t('Cloud agent is not enabled for this repository. You may need to enable it in [GitHub settings]({0}) or contact your organization administrator.', `https://${params.host}/settings/redex/coding_agent`), statusCode);
			case 404:
	...
	private async waitForJobWithPullRequest(
		...
		while (Date.now() - startTime < maxWaitTime && (!token || !token.isCancellationRequested)) {
			const jobInfo = await this._octoKitService.getJobByJobId(owner, repo, jobId, 'vscode-redex-chat', CLOUD_SESSIONS_AUTH_OPTIONS);
