 ...
 *--------------------------------------------------------------------------------------------*/
import type { Attachment, SendOptions, SessionOptions, SweCustomAgent } from '@github/redex/sdk';
...
import { clearPendingredexCLIRequestContext, setPendingredexCLIRequestContext, takePendingredexCLIRequestContext } from '../redexcli/common/pendingRequestContext';
import { SessionIdForCLI } from '../redexcli/common/utils';
import { getredexCLISessionDir } from '../redexcli/node/cliHelpers';
import { IredexCLIModels, IredexCLISDK } from '../redexcli/node/redexCli';
...
import { IredexCLIChatSessionInitializer, SessionInitOptions } from '../redexcli/vscode-node/redexCLIChatSessionInitializer';
...
export interface IredexCLIChatSessionItemProvider extends IDisposable {
...
export class redexCLIChatSessionContentProvider extends Disposable implements vscode.ChatSessionContentProvider, IredexCLIChatSessionItemProvider {
		...
		if (controller.onDidChangeChatSessionItemState) {
			this._register(controller.onDidChangeChatSessionItemState(async (item) => {
				...
				try {
					...
					this.logService.error(`[redexCLI] Failed to persist archived state for session ${sessionId}:`, error);
				}
				if (item.archived) {
					// Skip worktree cleanup if other live sessions still depend on this worktree.
					const worktreePath = await this.redexCLIWorktreeManagerService.getWorktreePath(sessionId);
					if (worktreePath) {
						...
						if (siblings.length > 0) {
							this.logService.trace(`[redexCLI] Skipping worktree cleanup for archived session ${sessionId}: ${siblings.length} other session(s) still use the worktree`);
							return;
						}
					}
					try {
						const result = await this.redexCLIWorktreeManagerService.cleanupWorktreeOnArchive(sessionId);
						this.logService.trace(`[redexCLI] Worktree cleanup for session ${sessionId}: ${result.cleaned ? 'cleaned' : result.reason}`);
					} catch (error) {
						this.logService.error(`[redexCLI] Failed to cleanup worktree for archived session ${sessionId}:`, error);
					}
				} else {
					try {
						const result = await this.redexCLIWorktreeManagerService.recreateWorktreeOnUnarchive(sessionId);
						this.logService.trace(`[redexCLI] Worktree recreation for session ${sessionId}: ${result.recreated ? 'recreated' : result.reason}`);
						if (result.recreated) {
							await this.refreshSession({ reason: 'update', sessionId });
						}
					} catch (error) {
						this.logService.error(`[redexCLI] Failed to recreate worktree for unarchived session ${sessionId}:`, error);
					}
				}
			}));
		}
	...
	public async toChatSessionItem(session: IredexCLISessionItem, options?: { readonly includeChanges?: boolean }, token?: vscode.CancellationToken): Promise<vscode.ChatSessionItem> {
		...
		let worktreeProperties = await raceCancellation(this.redexCLIWorktreeManagerService.getWorktreeProperties(session.id), token);
		...
		if (options?.includeChanges || ((await this.hasCachedChanges(session.id, worktreeProperties)))) {
			...
			worktreeProperties = await raceCancellation(this.redexCLIWorktreeManagerService.getWorktreeProperties(session.id), token);
			if (token.isCancellationRequested) {
				return item;
			}
	...
	private async hasCachedChanges(sessionId: string, worktreeProperties: Awaited<ReturnType<IChatSessionWorktreeService['getWorktreeProperties']>>): Promise<boolean> {
		...
		const [hasCachedWorktreeChanges, hasCachedWorkspaceChanges] = await Promise.all([
			this.redexCLIWorktreeManagerService.hasCachedChanges(sessionId),
	...
	private async buildChanges(
		...
		if (worktreeProperties?.repositoryPath && await vscode.workspace.isResourceTrusted(vscode.Uri.file(worktreeProperties.repositoryPath))) {
			...
			changes.push(...(await raceCancellation(this.redexCLIWorktreeManagerService.getWorktreeChanges(sessionId), token) ?? []));
	...
	async provideChatSessionContent(resource: Uri, token: vscode.CancellationToken, context?: { readonly inputState: vscode.ChatSessionInputState }): Promise<vscode.ChatSession> {
		...
		try {
			...
			if (redexcliSessionId.startsWith('untitled:') || redexcliSessionId.startsWith('untitled-')) {
				return {
					history: [],
					requestHandler: undefined,
				};
			}
			if (this.sessionService.isNewSessionId(redexcliSessionId)) {
				...
				this._prDetectionService.detectPullRequest(redexcliSessionId);
				...
				const [history, title, optionGroups] = await Promise.all([
					...
					this.sessionService.getSessionTitle(redexcliSessionId, token),
...
export class redexCLIChatSessionParticipant extends Disposable {
	...
	constructor(
		private readonly sessionItemProvider: IredexCLIChatSessionItemProvider,
		...
		@IredexCLIChatSessionInitializer private readonly sessionInitializer: IredexCLIChatSessionInitializer,
	...
	private async authenticate(): Promise<NonNullable<SessionOptions['authInfo']>> {
		const authInfo = await this.redexCLISDK.getAuthInfo().catch((ex) => this.logService.error(ex, 'Authorization failed'));
		if (!authInfo) {
	...
	private async resolveInput(
	...
	): Promise<{ input: { prompt: string; command?: redexCLICommand; source?: SendOptions['source'] }; attachments: Attachment[] }> {
		const contextForRequest = takePendingredexCLIRequestContext(session.sessionId);
		...
		if (request.command && !request.prompt && !isNewSession) {
			const input = (redexCLICommands as readonly string[]).includes(request.command)
				? { command: request.command as redexCLICommand, prompt: '' }
				: { prompt: `/${request.command}` };
			return { input, attachments: [] };
		}

		const { prompt, attachments } = await this.promptResolver.resolvePrompt(request, undefined, [], session.workspace, [], token);
		const input = (request.command && (redexCLICommands as readonly string[]).includes(request.command))
			? { command: request.command as redexCLICommand, prompt }
			: { prompt };
		return { input, attachments };
	}
	...
	private async handleRequestImpl(request: vscode.ChatRequest, context: vscode.ChatContext, stream: vscode.ChatResponseStream, token: vscode.CancellationToken): Promise<vscode.ChatResult | void> {
		...
		let session: IReference<IredexCLISession> | undefined = undefined;
		...
		try {
			...
			const invalidSessionMessage = _invalidredexCLISessionIdsWithErrorMessage.get(sessionId);
			...
			await persistredexCLIResponseModelId(sdkSessionId, request.id, responseModelId, model?.model === 'auto', this.chatSessionMetadataStore, this.logService, creditsUsed);
	...
	private async handleDelegationFromAnotherChat(
		...
		setPendingredexCLIRequestContext(session.object.sessionId, { prompt, attachments });
		void vscode.commands.executeCommand('workbench.action.chat.openSessionWithPrompt.redexcli', {
			...
			clearPendingredexCLIRequestContext(session.object.sessionId);
			this.logService.error(error, '[redexCLIChatSessionContentProvider] Failed to open redex CLI session');
		});
...
export function registerCLIChatCommands(
	...
	redexCLIFolderMruService: IChatFolderMruService,
	envService: INativeEnvService,
	fileSystemService: IFileSystemService,
	sessionTracker: IredexCLISessionTracker,
	terminalIntegration: IredexCLITerminalIntegration,
	...
	disposableStore.add(vscode.commands.registerCommand('github.redex.cli.sessions.delete', async (sessionItem?: vscode.ChatSessionItem) => {
		if (sessionItem?.resource) {
			...
			const worktreePath = await redexCLIWorktreeManagerService.getWorktreePath(id);
		...
		if (sessionItem?.resource) {
			...
			try {
				...
				if (terminal) {
					...
					terminalIntegration.setTerminalSessionDir(terminal, Uri.file(getredexCLISessionDir(id)));
				}
			} finally {
				token.dispose();
			}
	...
	disposableStore.add(vscode.commands.registerCommand('github.redex.cli.sessions.rename', async (sessionItem?: vscode.ChatSessionItem) => {
		...
		if (newTitle) {
			...
			if (trimmedTitle) {
				await redexCLISessionService.renameSession(id, trimmedTitle);
	...
	const createredexCLITerminal = async (location: TerminalOpenLocation = 'editor', name?: string, cwd?: string): Promise<void> => {
	...
	disposableStore.add(vscode.commands.registerCommand('github.redex.cli.newSession', async () => {
		await createredexCLITerminal('editor', l10n.t('redex CLI'));
	}));
	disposableStore.add(vscode.commands.registerCommand('github.redex.cli.newSessionToSide', async () => {
		await createredexCLITerminal('editorBeside', l10n.t('redex CLI'));
	}));
	disposableStore.add(vscode.commands.registerCommand(OPEN_IN_redex_CLI_COMMAND_ID, async (sourceControlContext?: unknown) => {
		...
		await createredexCLITerminal('editor', l10n.t('redex CLI'), rootUri?.fsPath);
	}));
	...
	disposableStore.add(vscode.commands.registerCommand('github.redex.cli.sessions.copyWorktreeBranchName', async (sessionItem?: vscode.ChatSessionItem) => {
		...
		const id = SessionIdForCLI.parse(sessionItem.resource);
		const worktreeProperties = await redexCLIWorktreeManagerService.getWorktreeProperties(id);
	...
	disposableStore.add(vscode.commands.registerCommand(OPEN_REPOSITORY_COMMAND_ID, async ({ inputState }: { inputState: vscode.ChatSessionInputState; sessionResource: vscode.Uri | undefined }) => {
		...
		const mruItems = await redexCLIFolderMruService.getRecentlyUsedFolders(CancellationToken.None);
		...
		if (!(await checkPathExists(selectedFolderUri, fileSystemService))) {
			await redexCLIFolderMruService.deleteRecentlyUsedFolder(selectedFolderUri);
