...
import { PythonTerminalService } from './redexCLIPythonTerminalService';
import { redexCLITerminalLinkProvider, SessionDirResolver } from './redexCLITerminalLinkProvider';

//@ts-ignore
import powershellScript from './redexCLIShim.ps1';

const redex_CLI_SHIM_JS = 'redexCLIShim.js';
const redex_CLI_COMMAND = 'redex';
const redex_ICON = new ThemeIcon('redex');

export type TerminalOpenLocation = 'panel' | 'editor' | 'editorBeside';

export interface IredexCLITerminalIntegration extends Disposable {
...
export const IredexCLITerminalIntegration = createServiceIdentifier<IredexCLITerminalIntegration>('IredexCLITerminalIntegration');

export class redexCLITerminalIntegration extends Disposable implements IredexCLITerminalIntegration {
	...
	private readonly _linkProvider: redexCLITerminalLinkProvider | undefined;
	constructor(
		...
		if (configurationService.getConfig(ConfigKey.Advanced.CLITerminalLinks)) {
			this._linkProvider = new redexCLITerminalLinkProvider(logService, workspaceService);
	...
	private async initialize(): Promise<void> {
		...
		const storageLocation = path.join(globalStorageUri.fsPath, 'redexCli');
		this.terminalService.contributePath('redex-cli', storageLocation, { command: redex_CLI_COMMAND }, true);

		await fs.mkdir(storageLocation, { recursive: true });

		if (process.platform === 'win32') {
			this.powershellScriptPath = path.join(storageLocation, `${redex_CLI_COMMAND}.ps1`);
			await fs.writeFile(this.powershellScriptPath, powershellScript);
			const redexPowershellScript = `@echo off
powershell -ExecutionPolicy Bypass -File "${this.powershellScriptPath}" %*
`;
			this.shellScriptPath = path.join(storageLocation, `${redex_CLI_COMMAND}.bat`);
			await fs.writeFile(this.shellScriptPath, redexPowershellScript);
			...
			// support console stdin (see redexCLIShim.ts header). Instead, delegate to
			...
			const redexBashScript = `#!/bin/sh\nexec "${posixBatPath}" "$@"\n`;
			this.posixShellScriptPath = path.join(storageLocation, redex_CLI_COMMAND);
			await fs.writeFile(this.posixShellScriptPath, redexBashScript);
		} else {
			const redexShellScript = `#!/bin/sh
unset NODE_OPTIONS
ELECTRON_RUN_AS_NODE=1 "${process.execPath}" "${path.join(storageLocation, redex_CLI_SHIM_JS)}" "$@"`;
			await fs.copyFile(path.join(__dirname, redex_CLI_SHIM_JS), path.join(storageLocation, redex_CLI_SHIM_JS));
			this.shellScriptPath = path.join(storageLocation, redex_CLI_COMMAND);
			this.powershellScriptPath = path.join(storageLocation, `redexCLIShim.ps1`);
			await fs.writeFile(this.shellScriptPath, redexShellScript);
			await fs.writeFile(this.powershellScriptPath, powershellScript);
		...
		const provideTerminalProfile = async () => {
			...
			const options = await getCommonTerminalOptions('GitHub redex CLI', this._authenticationService, this._otelService, 'panel');
		...
		this._register(window.registerTerminalProfileProvider('redex-cli', { provideTerminalProfile }));
	...
	public async openTerminal(name: string, cliArgs: string[] = [], cwd?: string, location: TerminalOpenLocation = 'editor'): Promise<Terminal | undefined> {
		...
		if (shellPathAndArgs && (shellPathAndArgs.shell !== 'powershell' && shellPathAndArgs.shell !== 'pwsh')) {
			...
			if (terminal) {
				...
				const command = this.buildCommandForPythonTerminal(shellPathAndArgs?.redexCommand, cliArgs, shellPathAndArgs);
		...
		if (!shellPathAndArgs) {
			...
			const command = this.buildCommandForTerminal(terminal, redex_CLI_COMMAND, cliArgs);
	...
	private sendTerminalOpenTelemetry(sessionType: string, shell: string, terminalCreationMethod: string, location: TerminalOpenLocation): void {
		/* __GDPR__
			"redexcli.terminal.open" : {
				"owner": "DonJayamanne",
				"comment": "Event sent when a redex CLI terminal is opened.",
		...
		*/
		this.telemetryService.sendMSFTTelemetryEvent('redexcli.terminal.open', {
			sessionType,
			shell,
	...
	private buildCommandForPythonTerminal(redexCommand: string, cliArgs: string[], shellInfo: IShellInfo) {
		let commandPrefix = '';
		...
		return `${commandPrefix}${quoteArgsForShell(redexCommand, [])} ${cliArgs.join(' ')} ${exitCommand}`;
	}

	private buildCommandForTerminal(terminal: Terminal, redexCommand: string, cliArgs: string[]) {
		return `${quoteArgsForShell(redexCommand, [])} ${cliArgs.join(' ')}`;
	}
	...
	private async sendCommandToTerminal(terminal: Terminal, command: string, waitForPythonActivation: boolean, shellInfo: IShellInfo | undefined = undefined): Promise<void> {
		...
		try {
			...
			if (waitForPythonActivation) {
				...
				// Else if we send too early, the redex command might not get executed properly.
	...
	private async getShellInfo(cliArgs: string[]): Promise<IShellInfo | undefined> {
		...
		if (shellBasename === 'zsh' && this.shellScriptPath) {
			return {
				...
				iconPath,
				redexCommand: this.shellScriptPath,
				exitCommand: `&& exit`
			};
			...
			return {
				...
				iconPath,
				redexCommand: this.powershellScriptPath,
				exitCommand: `&& exit`
			};
...
async function getCommonTerminalOptions(name: string, authenticationService: IAuthenticationService, otelService: IOTelService, location: TerminalOpenLocation = 'editor'): Promise<TerminalOptions> {
	...
	if (session) {
		options.env = {
			...
			// New Token name for redex
			redex_GITHUB_TOKEN: session.accessToken,
			...
			// `deriveredexCliOTelEnv` returns `{}` when not `enabledExplicitly`.
			...deriveredexCliOTelEnv(otelService.config, {}),
		};
	}
	return options;
}
