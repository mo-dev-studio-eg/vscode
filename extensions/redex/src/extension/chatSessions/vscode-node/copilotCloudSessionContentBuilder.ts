...
import { AgentTaskGetResponse, AgentTaskSession, AgentTaskSessionEvent } from '@vscode/redex-api';
import type { SessionEvent } from '@github/redex/sdk';
...
import { CLI_TOOL_EVENT_HANDLERS } from '../redexcli/common/redexCLITools';
import { getAuthorDisplayName } from '../vscode/redexCodingAgentUtils';
		...
		const sessionResults = await Promise.all(
			sessions.map(async (session, sessionIndex) => {
				...
				if (sessionIndex === 0 && pullRequest.author && vscode.workspace.workspaceFolders && vscode.workspace.workspaceFolders.length > 0) {
					...
					const card = new vscode.ChatResponsePullRequestPart({ command: 'github.redex.chat.openPullRequestReroute', title: vscode.l10n.t('View Pull Request {0}', `#${pullRequest.number}`), arguments: [pullRequest.number] }, pullRequest.title, plaintextBody, getAuthorDisplayName(pullRequest.author), `#${pullRequest.number}`);
			...
			turns.forEach((turn, index) => {
				...
				if (index === 0 && pullRequest && pullRequest.author && vscode.workspace.workspaceFolders && vscode.workspace.workspaceFolders.length > 0) {
					const card = new vscode.ChatResponsePullRequestPart(
						{ command: 'github.redex.chat.openPullRequestReroute', title: vscode.l10n.t('View Pull Request {0}', `#${pullRequest.number}`), arguments: [pullRequest.number] },
						pullRequest.title,
						pullRequest.body,
