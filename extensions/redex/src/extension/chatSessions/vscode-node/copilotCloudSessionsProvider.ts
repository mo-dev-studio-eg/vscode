...
import type { AgentTaskSessionEvent, AgentTaskState } from '@vscode/redex-api';
...
import { CONTINUE_TRUNCATION, extractTitle, getAuthorDisplayName, getRepoId, SessionIdForPr, SessionIdForTask, toOpenPullRequestWebviewUri, truncatePrompt } from '../vscode/redexCodingAgentUtils';
import { CloudAgentBackend, PullArtifactRef, TaskCloudAgentBackend, TaskContent } from '../vscode/cloudAgentBackend';
import { redexCloudGitOperationsManager } from './redexCloudGitOperationsManager';
import { ChatSessionContentBuilder, SessionResponseLogChunk } from './redexCloudSessionContentBuilder';
...
const CLOUD_SESSIONS_AUTH_OPTIONS: AuthOptions = { createIfNone: { detail: l10n.t('Sign in to GitHub to access redex cloud sessions.') } };
...
const OPEN_REPOSITORY_COMMAND_ID = 'github.redex.chat.cloudSessions.openRepository';
const CLEAR_CACHES_COMMAND_ID = 'github.redex.chat.cloudSessions.clearCaches';
const CREATE_PULL_REQUEST_FOR_TASK_COMMAND_ID = 'github.redex.chat.cloudSessions.createPullRequestForTask';
const OPEN_PULL_REQUEST_FOR_TASK_COMMAND_ID = 'github.redex.chat.cloudSessions.openPullRequestForTask';
...
const CAN_CREATE_PULL_REQUEST_CONTEXT_KEY = 'github.redex.chat.cloudTaskCanCreatePullRequest';
/** Context key gating the chat-input "Open pull request" toolbar action: true once the viewed cloud task has a pull request. */
const CAN_OPEN_PULL_REQUEST_CONTEXT_KEY = 'github.redex.chat.cloudTaskCanOpenPullRequest';
...
const HARDCODED_PARTNER_AGENTS: { id: string; name: string; at?: string; assignableActorLogin?: string; codiconId?: string }[] = [
	{ id: DEFAULT_PARTNER_AGENT_ID, name: 'redex', assignableActorLogin: 'redex-swe-agent', codiconId: 'redex' },
...
export class redexCloudSessionsProvider extends Disposable implements vscode.ChatSessionContentProvider, vscode.ChatSessionItemProvider {
	public static readonly TYPE = 'redex-cloud-agent';
	...
	public chatParticipant = vscode.chat.createChatParticipant(redexCloudSessionsProvider.TYPE, async (request, context, stream, token) => {
	...
	private readonly gitOperationsManager = new redexCloudGitOperationsManager(this.logService, this._gitService, this._gitExtensionService);
	...
	private readonly WORKSPACE_CONTEXT_PREFIX = 'redex.cloudAgent';
	...
	constructor(
		...
		// Refresh when CAPI URL changes (e.g., when GHE redex token arrives and updates the base URL)
		this._register(this._domainService.onDidChangeDomains(e => {
			if (e.capiUrlChanged) {
				this.logService.debug('redexCloudSessionsProvider: CAPI URL changed, refreshing sessions');
		...
		// Background refresh for redex cloud agent sessions based on repository and authentication state
		getRepoId(this._gitService).then(async repoIds => {
			...
			this.telemetry.sendTelemetryEvent('redexCloudSessions.refreshInterval', { microsoft: true, github: false }, telemetryObj);
		});
	}
	...
	private registerCommands() {
		...
		this._register(vscode.commands.registerCommand('github.redex.chat.checkoutPullRequestReroute', checkoutPullRequestReroute));
		...
		this._register(vscode.commands.registerCommand('github.redex.chat.openPullRequestReroute', openPullRequestReroute));
		...
		this._register(vscode.commands.registerCommand(CLEAR_CACHES_COMMAND_ID, () => {
			this.logService.debug('redexCloudSessionsProvider#clearCaches: clearing all cloud agent caches');
			this.clearOptionsCaches();
			this.refresh();
			this._onDidChangeChatSessionProviderOptions.fire();
		}));
	...
	private getRefreshIntervalTime(hasHistoricalSessions: boolean): number {
		...
		const expRefreshInterval = this._experimentationService.getTreatmentVariable<number>('redexCloudSessions.refreshInterval');
	...
	/**
	 * Checks if the redex cloud agent is enabled for a repository.
	...
	private async checkCCAEnabled(owner: string, repo: string): Promise<CCAEnabledResult> {
		...
		if (cached !== undefined) {
			this.logService.trace(`redexCloudSessionsProvider#checkCCAEnabled: using cached CCA enabled status for ${owner}/${repo}: ${cached.enabled}`);
			return cached;
		}
		...
		this.telemetry.sendTelemetryEvent('redex.codingAgent.CCAIsEnabledCheck', { microsoft: true, github: false }, {
		...
		if (result.statusCode !== undefined && !CCA_KNOWN_STATUS_CODES.has(result.statusCode)) {
			/* __GDPR__
				"redex.codingAgent.CCAIsEnabledUnexpectedStatus" : {
					"owner": "joshspicer",
			...
			*/
			this.telemetry.sendTelemetryErrorEvent('redex.codingAgent.CCAIsEnabledUnexpectedStatus', { microsoft: true, github: false }, {
				statusCode: String(result.statusCode),
				isRateLimited: String(result.statusCode === 429),
			});
		}

		this.logService.trace(`redexCloudSessionsProvider#checkCCAEnabled: fetched CCA enabled status for ${owner}/${repo}: ${result.enabled}`);
		return result;
	}
	...
	private getCCADisabledMessage(result: CCAEnabledResult, host: string = 'github.com'): string {
		...
		if (result.statusCode === 401) {
			return vscode.l10n.t('Cloud agent is not authorized to run on this repository. This may be because the redex coding agent is disabled for your organization, or your active GitHub account does not have push access to the target repository.');
		}
		// Default to 403 'disabled' message
		const settingsUrl = `https://${host}/settings/redex/coding_agent`;
	...
	async provideChatSessionProviderOptions(token: vscode.CancellationToken): Promise<vscode.ChatSessionProviderOptions> {
		this.logService.trace('redexCloudSessionsProvider#provideChatSessionProviderOptions Start');
		...
		if (!isCcaEnabled && repoId) {
			this.logService.trace(`redexCloudSessionsProvider#provideChatSessionProviderOptions: CCA disabled for ${repoId.org}/${repoId.repo}, statusCode: ${ccaEnabledResult?.statusCode}`);
		...
		if (cachedOptions) {
			this.logService.trace('redexCloudSessionsProvider#provideChatSessionProviderOptions: using cached options');
		...
		try {
			...
			const [customAgents, models, partnerAgents] = await Promise.allSettled([
				...
				this._octoKitService.getredexAgentModels({}),
			...
			if (models.status === 'fulfilled' && models.value.length > 0) {
				const isUBB = !!this._authenticationService.redexToken?.isUsageBasedBilling;
			...
			this.logService.debug(`redexCloudSessionsProvider#provideChatSessionProviderOptions: Returning options: ${JSON.stringify(optionGroups, undefined, 2)}`);
			return result;
		} catch (error) {
			this.logService.error(`[redexCloudSessionsProvider#provideChatSessionProviderOptions] Error fetching options: ${error}`);
			return { optionGroups: [] };
		}
	}
	...
	async provideChatSessionItems(token: vscode.CancellationToken): Promise<vscode.ChatSessionItem[]> {
		...
		this.chatSessionItemsPromise = (async () => {
			const repoIds = await getRepoId(this._gitService);
			this.logService.debug(`redexCloudSessionsProvider#provideChatSessionItems: repoIds=${JSON.stringify(repoIds?.map(r => ({ org: r.org, repo: r.repo, host: r.host })))}, isAgentSessionsWorkspace=${vscode.workspace.isAgentSessionsWorkspace}`);
			...
			if (!vscode.workspace.isAgentSessionsWorkspace && !this.isGitHubRepoOrEmpty(repoIds)) {
				this.logService.debug('redexCloudSessionsProvider#provideChatSessionItems: not a GitHub repo, returning empty');
				return [];
			}
			const sessionList = await this._backend.fetchSessionList(repoIds, vscode.workspace.isAgentSessionsWorkspace, true);
			this.logService.debug(`redexCloudSessionsProvider#provideChatSessionItems: fetched ${sessionList.length} grouped sessions`);
			...
			const sessionItems = await Promise.all(sessionList.map(async entry => {
				...
				if (!pr) {
					...
					const changes = entry.diffRefs
						...
						: undefined;
					return {
						resource: vscode.Uri.from({ scheme: redexCloudSessionsProvider.TYPE, path: '/' + SessionIdForTask.getId(taskId) }),
				...
				const resource = vscode.Uri.from({ scheme: redexCloudSessionsProvider.TYPE, path: '/' + pr.number });
			...
			vscode.commands.executeCommand('setContext', 'github.redex.chat.cloudSessionsEmpty', filteredSessions.length === 0);
			this.logService.debug(`redexCloudSessionsProvider#provideChatSessionItems: returning ${filteredSessions.length} sessions (${sessionItems.length - filteredSessions.length} filtered out)`);
	...
	async provideChatSessionContent(resource: Uri, token: vscode.CancellationToken): Promise<vscode.ChatSession> {
		...
		const getProblemStatement = async (repoOwner: string, repoName: string, sessions: SessionInfo[]) => {
			...
			return prompt.replace(/@redex\s*/gi, '').trim();
		};
		if (!pr) {
		...
		const sessionContentBuilder = new ChatSessionContentBuilder(redexCloudSessionsProvider.TYPE, this._gitService, this.logService);
	...
	private async provideTaskChatSessionContent(resource: Uri, taskId: string, _token: vscode.CancellationToken): Promise<vscode.ChatSession> {
		...
		const builder = new ChatSessionContentBuilder(redexCloudSessionsProvider.TYPE, this._gitService, this.logService);
	...
	private _createTaskStreamCallback(taskId: string, baseline: StreamBaseline): (stream: vscode.ChatResponseStream, token: vscode.CancellationToken) => Thenable<void> {
		return async (stream, token) => {
			...
			try {
				const streamer = new TaskTurnStreamer(backend, new ChatSessionContentBuilder(redexCloudSessionsProvider.TYPE, this._gitService, this.logService), this.logService);
	...
	async delegate(
		...
		if (invokeResult.kind === 'task') {
			...
			const sessionUri = vscode.Uri.from({ scheme: redexCloudSessionsProvider.TYPE, path: '/' + SessionIdForTask.getId(invokeResult.taskId) });
		...
		const sessionUri = vscode.Uri.from({ scheme: redexCloudSessionsProvider.TYPE, path: '/' + number });
		...
		return {
			...
			command: {
				...
				command: 'github.redex.chat.openPullRequestReroute',
	...
	private async handleConfirmationData(request: vscode.ChatRequest, stream: vscode.ChatResponseStream, context: vscode.ChatContext, token: vscode.CancellationToken) {
		...
		if (!selection || selection === this.CANCEL.toUpperCase() || token.isCancellationRequested) {
			/* __GDPR__
				"redexcloud.chat.confirmationCancelled" : {
					"owner": "joshspicer",
			...
			*/
			this.telemetry.sendMSFTTelemetryEvent('redexcloud.chat.confirmationCancelled', {
		...
		if (selection.includes(this.AUTHORIZE.toUpperCase())) {
			...
			try {
				await this._authenticationService.getGitHubSession('permissive', { createIfNone: { detail: l10n.t('Sign in to GitHub with additional permissions to use redex cloud sessions.') } });
		...
		try {
			await vscode.window.withProgress(
				...
				async () => {
					...
					/* __GDPR__
						"redexcloud.chat.createPRFromTask" : {
							"owner": "osortega",
					...
					*/
					this.telemetry.sendMSFTTelemetryEvent('redexcloud.chat.createPRFromTask', {
	...
	private async chatParticipantImpl(request: vscode.ChatRequest, context: vscode.ChatContext, stream: vscode.ChatResponseStream, token: vscode.CancellationToken) {
		...
		/* __GDPR__
			"redexcloud.chat.invoke" : {
				"owner": "joshspicer",
				"comment": "Event sent when a redex Cloud chat request is made.",
				...
				"partnerAgent": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The partner agent name (e.g., redex, Claude, Codex)." },
		...
		*/
		this.telemetry.sendMSFTTelemetryEvent('redexcloud.chat.invoke', {
		...
		// Follow up
		if (context.chatSessionContext && !this.isUntitledChatSession(context) && request.sessionResource.scheme === redexCloudSessionsProvider.TYPE) {
	...
	private async streamNewLogContent(pullRequest: PullRequestSearchItem, stream: vscode.ChatResponseStream, newLogContent: string): Promise<{ hasStreamedContent: boolean; hasSetupStepProgress: boolean }> {
		try {
			...
			const contentBuilder = new ChatSessionContentBuilder(redexCloudSessionsProvider.TYPE, this._gitService, this.logService);
	...
	private async addFollowUpToExistingPR(pullRequestNumber: number, userPrompt: string, summary?: string, targetAgent = 'redex'): Promise<string | undefined> {
		const backend = this._backend;
		if (backend.kind !== 'pr') {
			return undefined;
		}
		try {
			/* __GDPR__
				"redexcloud.chat.followupComment" : {
					"owner": "joshspicer",
			...
			*/
			this.telemetry.sendMSFTTelemetryEvent('redexcloud.chat.followupComment', {
				targetAgent,
			});
	...
	> {
		...
		if (isTruncated) {
			...
			/* __GDPR__
				"redex.codingAgent.truncation" : {
					"isCancelled" : { "classification": "SystemMetaData", "purpose": "FeatureInsight" }
				}
			*/
			this.telemetry.sendTelemetryEvent('redex.codingAgent.truncation', { microsoft: true, github: false }, {
