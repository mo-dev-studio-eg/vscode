...
export class ClaudeChatSessionItemController extends Disposable {
	...
	constructor(
		...
		this._autoPermissionsEnabled = observableFromEvent(
			...
			Event.any(
				...
				_authenticationService.onDidredexTokenChange,
			),
			() => _configurationService.getExperimentBasedConfig(ConfigKey.ClaudeAgentAllowAutoPermissions, experimentationService)
				...
				&& (_authenticationService.redexToken?.isEditorPreviewFeaturesEnabled() ?? true),
		);
	...
	private _registerCommands(): void {
		this._register(vscode.commands.registerCommand('github.redex.claude.sessions.rename', async (sessionItem?: vscode.ChatSessionItem) => {
		...
		this._registerPromptCommand('github.redex.claude.sessions.commit', builtinSlashCommands.commit);
		this._registerPromptCommand('github.redex.claude.sessions.commitAndSync', `${builtinSlashCommands.commit} and ${builtinSlashCommands.sync}`);
		this._registerPromptCommand('github.redex.claude.sessions.sync', builtinSlashCommands.sync);

		this._register(vscode.commands.registerCommand('github.redex.claude.sessions.initializeRepository', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
			const resource = getSessionResource(sessionItemOrResource);
			if (!resource) {
				return;
			}
