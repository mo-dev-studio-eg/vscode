 ...
 * {@link IOTelService} (OTel metrics). Constructed once in `redexCloudSessionsProvider` with the
...
export class CloudBackendInstrumentation implements ICloudBackendInstrumentation {
	...
	sessionCreated(outcome: CloudBackendOutcome, durationMs: number, error?: unknown): void {
		if (outcome === 'failure') {
			...
			*/
			this._telemetryService.sendMSFTTelemetryErrorEvent('redexcloud.chat.sessionCreate', {
			...
			*/
			this._telemetryService.sendMSFTTelemetryEvent('redexcloud.chat.sessionCreate', {
				backendVersion: this.backendVersion,
				outcome,
			}, { durationMs });
		}
	...
	sessionActivated(durationMs: number): void {
		/* __GDPR__
			"redexcloud.chat.sessionActivated" : {
				"owner": "osortega",
		...
		*/
		this._telemetryService.sendMSFTTelemetryEvent('redexcloud.chat.sessionActivated', {
			backendVersion: this.backendVersion,
		}, { durationMs });
	...
	followUp(outcome: CloudBackendOutcome, error?: unknown): void {
		if (outcome === 'failure') {
			...
			*/
			this._telemetryService.sendMSFTTelemetryErrorEvent('redexcloud.chat.followup', {
				backendVersion: this.backendVersion,
				outcome,
				errorType: cloudErrorType(error),
				errorMessage: errorMessageOf(error),
			...
			*/
			this._telemetryService.sendMSFTTelemetryEvent('redexcloud.chat.followup', {
				backendVersion: this.backendVersion,
				outcome,
			});
		}
	...
	operationFailed(operation: CloudBackendOperation, error: unknown, status?: number): void {
		...
		/* __GDPR__
			"redexcloud.chat.operationError" : {
				"owner": "osortega",
		...
		*/
		this._telemetryService.sendMSFTTelemetryErrorEvent('redexcloud.chat.operationError', {
			backendVersion: this.backendVersion,
			operation,
			errorType,
			errorMessage: errorMessageOf(error),
	...
	legacyJobInvoke(hasHeadRef: boolean): void {
		/* __GDPR__
			"redexcloud.chat.remoteAgentJobInvoke" : {
				"owner": "joshspicer",
		...
		*/
		this._telemetryService.sendMSFTTelemetryEvent('redexcloud.chat.remoteAgentJobInvoke', {
	...
	legacyJobPullRequestReady(): void {
		/* __GDPR__
			"redexcloud.chat.remoteAgentJobPullRequestReady" : {
				"owner": "joshspicer",
				"comment": "Event sent when a remote agent job first returns pull request information."
			}
		*/
		this._telemetryService.sendMSFTTelemetryEvent('redexcloud.chat.remoteAgentJobPullRequestReady');
	}
}
