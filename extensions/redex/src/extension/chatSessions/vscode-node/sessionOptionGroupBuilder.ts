 ...
 * 4. `redex-worktree-*` branches excluded
...
import { isWelcomeView } from '../redexcli/node/redexCli';
...
export const OPEN_REPOSITORY_COMMAND_ID = 'github.redex.cli.sessions.openRepository';
...
const redex_WORKTREE_PATTERN = 'redex-worktree-';
...
export class SessionOptionGroupBuilder implements ISessionOptionGroupBuilder {
	...
	constructor(
		...
		@IChatFolderMruService private readonly redexCLIFolderMruService: IChatFolderMruService,
	...
	async provideChatSessionProviderOptionGroups(previousInputState: vscode.ChatSessionInputState | undefined, selectedFolderUri?: vscode.Uri): Promise<vscode.ChatSessionProviderOptionGroup[]> {
		...
		if (isWelcomeView(this.workspaceService)) {
			...
			const repositories = await this.redexCLIFolderMruService.getRecentlyUsedFolders(CancellationToken.None);
	...
	async buildExistingSessionInputStateGroups(resource: vscode.Uri, token: vscode.CancellationToken): Promise<vscode.ChatSessionProviderOptionGroup[]> {
		...
		const optionGroups: vscode.ChatSessionProviderOptionGroup[] = [];
		const folderInfo = await this.folderRepositoryManager.getFolderRepository(redexcliSessionId, undefined, token);
		const repositories = isWelcomeView(this.workspaceService) ? folderMRUToChatProviderOptions(await this.redexCLIFolderMruService.getRecentlyUsedFolders(token)) : this.getRepositoryOptionItems();
		...
		const worktreeProperties = await this.chatSessionWorktreeService.getWorktreeProperties(redexcliSessionId);
	...
	async getBranchOptionItemsForRepository(repoUri: Uri, headBranchName: string | undefined): Promise<vscode.ChatSessionProviderOptionItem[]> {
		...
		return this._getBranchOptionItemsForRepositorySequencer.queue(key, async () => {
			...
			for (const ref of localBranches) {
				...
				if (ref.name.includes(redex_WORKTREE_PATTERN)) {
					continue;
				}
