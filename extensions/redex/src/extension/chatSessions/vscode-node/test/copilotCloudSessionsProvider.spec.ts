...
import type { AgentTask, AgentTaskCreateRequest, AgentTaskGetResponse, AgentTaskListEventsResponse, AgentTaskListResponse, AgentTaskSessionEvent, AgentTaskState, AgentTaskSteerRequest, AgentTaskCreatePullRequestResponse } from '@vscode/redex-api';
...
import { ChatSessionContentBuilder } from '../redexCloudSessionContentBuilder';
import { normalizeInitialSessionOptions, parseSessionLogChunksSafely, taskStateToChatSessionStatus } from '../redexCloudSessionsProvider';
...
describe('redexCloudSessionsProvider helpers', () => {
	it('coerces object-shaped initialSessionOptions into option entries', () => {
		const logService = new RecordingLogService();
		const sessionResource = vscode.Uri.parse('redex-cloud-agent:/1');
...
describe('ChatSessionContentBuilder', () => {
	it('ignores malformed tool_calls payloads instead of throwing', async () => {
		const builder = new ChatSessionContentBuilder('redex-cloud-agent', new TestGitService(), new TestLogService());
		const logs = [
...
describe('ChatSessionContentBuilder Task API history', () => {
	const newBuilder = () =>
		new ChatSessionContentBuilder('redex-cloud-agent', new TestGitService(), new TestLogService());
...
describe('TaskApiBackend', () => {
	...
	it('fetchSessionList carries the raw task lifecycle state so settled tasks are not shown as in_progress', async () => {
		...
		const client = new FakeTaskApiClient({ repoTasks: [{ id: 't-idle', state: 'idle', created_at: '2026-03-27T00:00:00Z', creator: { id: 4242 }, agent_collaborators: [{ slug: 'redex-developer' }] } as unknown as AgentTask] });
		const octoKitService = new MockOctoKitService();
		octoKitService.getCurrentAuthedUser = async () => ({ id: 4242, login: 'octocat', name: 'The Octocat', avatar_url: '' });
		const backend = new TaskApiBackend(client, new TestLogService(), octoKitService, NullCloudBackendInstrumentation);
	...
	it('fetchSessionList shows only cloud coding agent tasks and excludes local-client tasks (CLI / VS Code / JetBrains)', async () => {
		const repoTasks = [
			{ id: 'cloud-dev', state: 'idle', created_at: '2026-03-27T00:00:00Z', creator: { id: 4242 }, agent_collaborators: [{ slug: 'redex-developer' }] },
			{ id: 'cloud-swe', state: 'idle', created_at: '2026-03-27T00:00:00Z', creator: { id: 4242 }, agent_collaborators: [{ slug: 'redex-swe-agent' }] },
			{ id: 'cli', state: 'idle', created_at: '2026-03-27T00:00:00Z', creator: { id: 4242 }, agent_collaborators: [{ slug: 'redex-developer-cli' }] },
...
describe('isCloudCodingAgentTask', () => {
	it('keeps cloud coding agent slugs and rejects local-client / missing / malformed slugs', () => {
		...
		expect({
			'redex-developer': classify([{ slug: 'redex-developer' }]),
			'redex-swe-agent': classify([{ slug: 'redex-swe-agent' }]),
			'redex-developer-cli': classify([{ slug: 'redex-developer-cli' }]),
			...
			'redex-developer': true,
			'redex-swe-agent': true,
			'redex-developer-cli': false,
			'vscode-chat': false,
