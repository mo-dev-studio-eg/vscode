...
import { redexCLITerminalLinkProvider } from '../redexCLITerminalLinkProvider';
...
const SESSION_DIR = `/Users/anthonykim/.redex/session-state/${SESSION_UUID}`;
...
describe('redexCLITerminalLinkProvider', () => {
	let provider: redexCLITerminalLinkProvider;
	...
	beforeEach(async () => {
		...
		provider = new redexCLITerminalLinkProvider(new TestLogService());
	...
	describe('relative paths', () => {
		...
		it('should resolve bare filename to files/<name> when root file does not exist', async () => {
			...
			const links = await provider.provideTerminalLinks(
				makeContext('| todo.md | /Users/anthonykim/.redex/session-state/id/files/todo.md | files/todo.md |', terminal),
				makeToken(),
			);
	...
	describe('tilde paths', () => {
		it('should expand ~/.redex/session-state/.../files/sample-summary.md', async () => {
			const links = await provider.provideTerminalLinks(
				makeContext(`  Absolute: ~/.redex/session-state/${SESSION_UUID}/files/sample-summary.md`, terminal),
				makeToken(),
			);
			expect(links).toHaveLength(1);
			expect(links[0].pathText).toContain('~/.redex/session-state');
			expect(links[0].uri?.fsPath).toBe(`/Users/anthonykim/.redex/session-state/${SESSION_UUID}/files/sample-summary.md`);
		});
	});

	describe('absolute paths', () => {
		it('should skip /Users/anthonykim/.redex/.../files/sample-summary.md', async () => {
			const links = await provider.provideTerminalLinks(
				makeContext(`  /Users/anthonykim/.redex/session-state/${SESSION_UUID}/files/sample-summary.md`, terminal),
				makeToken(),
			);
	...
	describe('session dir resolution', () => {
		...
		it('should not cache stale resolver result when session tracker learns the real session later', async () => {
			...
			const staleDir = '/Users/anthonykim/.redex/session-state/31830812-0221-4389-b6bf-b1d33fe556e2';
			const realDir = '/Users/anthonykim/.redex/session-state/278b1a81-eb86-4a81-bff0-ba68035c1b48';
		...
		it('should still consult resolver even when an explicit session dir is cached', async () => {
			...
			const oldDir = '/Users/anthonykim/.redex/session-state/old';
			const newDir = '/Users/anthonykim/.redex/session-state/new';

			// Explicitly cached (e.g. resumed session) but user then started a
			// new `redex` run in the same terminal.
		...
		it('should prefer active resolver dir over stale cached dir when file exists in both', async () => {
			...
			const staleDir = '/Users/anthonykim/.redex/session-state/ended-session';
			const activeDir = '/Users/anthonykim/.redex/session-state/active-session';
		...
		it('should prefer terminal-matched resolver dirs over unrelated sessions', async () => {
			...
			const sessionXDir = '/Users/anthonykim/.redex/session-state/session-x';
			const sessionYDir = '/Users/anthonykim/.redex/session-state/session-y';
	...
	describe('Windows paths', () => {
		...
		it('should expand tilde with backslash (~\\.redex\\...)', async () => {
			const links = await provider.provideTerminalLinks(
				makeContext('Create ~\\.redex\\session-state\\5d9e\\files\\sample-summary.md (+4)', terminal),
				makeToken(),
			);
			expect(links).toHaveLength(1);
			expect(links[0].uri?.fsPath).toContain('/Users/anthonykim');
			expect(links[0].uri?.fsPath).toContain('.redex');
		});

		it('should skip Windows absolute paths (C:\\...)', async () => {
			const links = await provider.provideTerminalLinks(
				makeContext('Absolute: C:\\Users\\antho\\.redex\\files\\sample-summary.md', terminal),
				makeToken(),
			);
