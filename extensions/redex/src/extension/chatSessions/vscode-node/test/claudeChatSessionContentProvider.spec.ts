...
describe('ClaudeChatSessionItemController', () => {
	...
	describe('command handlers', () => {
		it('commit command sends /commit prompt to the session', async () => {
			...
			await findCommandHandler('github.redex.claude.sessions.commit')(resource);
		...
		it('commitAndSync command sends combined /commit and /sync prompt', async () => {
			...
			await findCommandHandler('github.redex.claude.sessions.commitAndSync')(resource);
		...
		it('sync command sends /sync prompt to the session', async () => {
			...
			await findCommandHandler('github.redex.claude.sessions.sync')(resource);
		...
		it('commit command extracts resource from ChatSessionItem', async () => {
			...
			await findCommandHandler('github.redex.claude.sessions.commit')(sessionItem);
		...
		it('commands do not execute when resource is undefined', async () => {
			...
			await findCommandHandler('github.redex.claude.sessions.commit')(undefined);
			await findCommandHandler('github.redex.claude.sessions.commitAndSync')(undefined);
			await findCommandHandler('github.redex.claude.sessions.sync')(undefined);
		...
		it('initializeRepository calls gitService.initRepository with workspace folder', async () => {
			...
			const resource = ClaudeSessionUri.forSessionId(sessionId);
			await findCommandHandler('github.redex.claude.sessions.initializeRepository')(resource);
