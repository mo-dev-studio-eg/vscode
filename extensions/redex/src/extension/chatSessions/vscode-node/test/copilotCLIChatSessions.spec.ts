...
import { IredexCLIChatSessionInitializer } from '../../redexcli/vscode-node/redexCLIChatSessionInitializer';
import { IredexCLISessionTracker } from '../../redexcli/vscode-node/redexCLISessionTracker';
import { redexCLIChatSessionContentProvider, redexCLIChatSessionParticipant, resolveBranchLockState, resolveBranchSelection, resolveIsolationSelection, resolveSessionDirsForTerminal } from '../redexCLIChatSessions';
...
vi.mock('../redexCLIShim.ps1', () => ({ default: '# mock powershell script' }));
...
class TestSessionService extends mock<IredexCLISessionService>() {
	...
	override getAllSessions = vi.fn(async () => [] as IredexCLISessionItem[]);
	...
	override getSession = vi.fn(async () => ({
		...
		dispose: () => { },
	} as unknown as { object: IredexCLISession; dispose(): void }));
...
function createProvider() {
	...
	const provider = new redexCLIChatSessionContentProvider(
		sessionService,
		worktreeService,
...
describe('redexCLIChatSessionContentProvider', () => {
	beforeEach(() => {
		vi.restoreAllMocks();
	});
	...
	it('triggers pull request detection when opening an existing session', async () => {
		...
		await provider.provideChatSessionContent(
			URI.from({ scheme: 'redexcli', path: '/session-1' }),
...
describe('redexCLIChatSessionParticipant', () => {
	beforeEach(() => {
		vi.restoreAllMocks();
	});
	...
	it('keeps local /remote output visible for new sessions', async () => {
		...
		} as unknown as redexCLIPromptResolver;
		const logService = new class extends mock<ILogService>() {
			declare readonly _serviceBrand: undefined;
			override error = vi.fn();
		}();
		const participant = new redexCLIChatSessionParticipant(
			sessionItemProvider,
			promptResolver,
			undefined,
			undefined,
			...
			new class extends mock<IredexCLISDK>() {
				declare readonly _serviceBrand: undefined;
				override getAuthInfo = vi.fn(async () => ({ type: 'token' as const, token: 'token', host: 'https://github.com' }));
			}(),
			new class extends mock<IredexCLIChatSessionInitializer>() {
				declare readonly _serviceBrand: undefined;
			...
			new class extends mock<IredexCLIModels>() {
				declare readonly _serviceBrand: undefined;
...
// ─── Additional redexCLIChatSessionContentProvider tests ──────

describe('redexCLIChatSessionContentProvider (additional)', () => {
	beforeEach(() => {
		vi.restoreAllMocks();
	});
	...
	it('toChatSessionItem maps session to chat session item', async () => {
		...
		const sessionItem: IredexCLISessionItem = {
			id: 'session-1',
			label: 'Test Session',
			status: undefined,
			workingDirectory: undefined,
		} as unknown as IredexCLISessionItem;
