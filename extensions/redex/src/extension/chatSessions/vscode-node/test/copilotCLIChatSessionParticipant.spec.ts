...
import { Attachment, SessionOptions, SweCustomAgent } from '@github/redex/sdk';
...
import { type redexCLIModelInfo, type IredexCLIModels, type IredexCLISDK } from '../../redexcli/node/redexCli';
import { redexCLIPromptResolver } from '../../redexcli/node/redexcliPromptResolver';
import { redexCLISession, redexCLISessionInput, IredexCLISession } from '../../redexcli/node/redexcliSession';
import { redexCLISessionService, redexCLISessionWorkspaceTracker, IredexCLISessionService } from '../../redexcli/node/redexcliSessionService';
import { IredexCLIMCPHandler } from '../../redexcli/node/mcpHandler';
import { MockCliSdkSession, MockCliSdkSessionManager, MockSkillLocations, NullredexCLIAgents, NullIredexCLIImageSupport } from '../../redexcli/node/test/testHelpers';
import { IQuestion, IQuestionAnswer, IUserQuestionHandler } from '../../redexcli/node/userInputHelpers';
import { CustomSessionTitleService } from '../../redexcli/vscode-node/customSessionTitleServiceImpl';
import { redexCLIChatSessionContentProvider, redexCLIChatSessionItemProvider, redexCLIChatSessionParticipant } from '../redexCLIChatSessionsContribution';
import { redexCloudSessionsProvider } from '../redexCloudSessionsProvider';
import { redexCLIFolderRepositoryManager } from '../folderRepositoryManagerImpl';
...
vi.mock('../redexCLITerminalIntegration', () => {
	...
	class redexCLITerminalIntegration {
		dispose() { }
		openTerminal = vi.fn(async () => { });
	}
	return {
		IredexCLITerminalIntegration: createServiceIdentifier('IredexCLITerminalIntegration'),
		redexCLITerminalIntegration
	};
});
...
class FakeModels {
	...
	getModels = vi.fn(async () => [{ id: 'base', name: 'Base', maxContextWindowTokens: 128000, supportsVision: false }] as redexCLIModelInfo[]);
...
class FakeCloudProvider extends mock<redexCloudSessionsProvider>() {
	override delegate = vi.fn(async () => ({
		...
		author: 'Test Author',
		linkTag: '#1'
	})) as unknown as redexCloudSessionsProvider['delegate'];
}
...
function createChatContext(sessionId: string, isUntitled: boolean, ...requests: TestChatRequest[]): vscode.ChatContext {
	...
	const normalizedSessionId = isUntitled && !sessionId.startsWith('untitled-')
		...
		: sessionId;
	const resource = vscode.Uri.from({ scheme: 'redexcli', path: `/${normalizedSessionId}` });
...
class TestredexCLISession extends redexCLISession {
	public requests: Array<{ input: redexCLISessionInput; attachments: Attachment[]; model: { model: string; reasoningEffort?: string; contextTier?: 'default' | 'long_context' } | undefined; authInfo: NonNullable<SessionOptions['authInfo']>; token: vscode.CancellationToken }> = [];
	...
	public static handleRequestHook: ((request: { id: string; toolInvocationToken: vscode.ChatParticipantToolToken; sessionResource?: vscode.Uri }, input: redexCLISessionInput) => Promise<void>) | undefined;
	...
	override get status(): vscode.ChatSessionStatus | undefined {
		return TestredexCLISession.statusOverride;
	}
	override handleRequest(request: { id: string; toolInvocationToken: vscode.ChatParticipantToolToken; sessionResource?: vscode.Uri }, input: redexCLISessionInput, attachments: Attachment[], model: { model: string; reasoningEffort?: string; contextTier?: 'default' | 'long_context' } | undefined, authInfo: NonNullable<SessionOptions['authInfo']>, token: vscode.CancellationToken): Promise<void> {
		this.requests.push({ input, attachments, model, authInfo, token });
		if (TestredexCLISession.handleRequestHook) {
			return TestredexCLISession.handleRequestHook(request, input);
		}
		return TestredexCLISession.nextHandleRequestResult ?? Promise.resolve();
	}
	override attachStream(stream: vscode.ChatResponseStream): ReturnType<redexCLISession['attachStream']> {
	...
	override getLastResponseModelId(): string | undefined {
		return TestredexCLISession.lastResponseModelId;
	}
}


class FakeredexCLISessionService extends mock<IredexCLISessionService>() {
	private _sessionWorkingDirs = new Map<string, vscode.Uri>();
	override tryGetPartialSessionHistory: IredexCLISessionService['tryGetPartialSessionHistory'] = vi.fn(async () => undefined);
...
describe('redexCLIChatSessionParticipant.handleRequest', () => {
	const disposables = new DisposableStore();
	let promptResolver: redexCLIPromptResolver;
	let itemProvider: redexCLIChatSessionItemProvider;
	...
	let sessionService: redexCLISessionService;
	let telemetry: ITelemetryService;
	let tools: FakeToolsService;
	let participant: redexCLIChatSessionParticipant;
	let workspaceService: IWorkspaceService;
	let instantiationService: IInstantiationService;
	let logService: ILogService;
	let configurationService: InMemoryConfigurationService;
	let manager: MockCliSdkSessionManager;
	let mcpHandler: IredexCLIMCPHandler;
	let folderRepositoryManager: redexCLIFolderRepositoryManager;
	let cliSessionServiceForFolderManager: FakeredexCLISessionService;
	let contentProvider: redexCLIChatSessionContentProvider;
	let sdk: IredexCLISDK;
	let customSessionTitleService: CustomSessionTitleService;
	const cliSessions: TestredexCLISession[] = [];

	beforeEach(async () => {
		cliSessions.length = 0;
		TestredexCLISession.nextHandleRequestResult = undefined;
		TestredexCLISession.handleRequestHook = undefined;
		TestredexCLISession.statusOverride = undefined;
		TestredexCLISession.lastResponseModelId = undefined;
		// By default, simulate VS Code core opening the delegated session and
		// re-invoking handleRequest with the redexcli:// resource. This matches
		...
		mockExecuteCommand.mockImplementation(async (command: string, args: any) => {
			if (command === 'workbench.action.chat.openSessionWithPrompt.redexcli') {
		...
		} as unknown as IredexCLISDK;
		const services = disposables.add(createExtensionUnitTestingServices());
		const accessor = services.createTestingAccessor();
		disposables.add(accessor);
		promptResolver = new class extends mock<redexCLIPromptResolver>() {
		...
		itemProvider = new class extends mock<redexCLIChatSessionItemProvider>() {
			override swap = vi.fn();
		...
		cliSessionServiceForFolderManager = new FakeredexCLISessionService();
		...
		mcpHandler = new class extends mock<IredexCLIMCPHandler>() {
		...
		instantiationService = {
			...
			createInstance: (ctor: unknown, workspaceInfo: any, agentName: any, sdkSession: any) => {
				if (ctor === redexCLISessionWorkspaceTracker) {
					return new class extends mock<redexCLISessionWorkspaceTracker>() {
				...
				const session = new TestredexCLISession(workspaceInfo, agentName, sdkSession, [], undefined, logService, workspaceService, new MockChatSessionMetadataStore(), instantiationService, new NullRequestLogger(), new NullIredexCLIImageSupport(), new FakeToolsService(), new FakeUserQuestionHandler(), accessor.get(IConfigurationService), new NoopOTelService(resolveOTelConfig({ env: {}, extensionVersion: '0.0.0', sessionId: 'test' })), new FakeGitService(), { _serviceBrand: undefined } as any, { _serviceBrand: undefined, resetTurnCredits() { }, getCreditsForTurn() { return undefined; }, setLastredexUsage() { } } as any, new NullTelemetryService());
		...
		folderRepositoryManager = new redexCLIFolderRepositoryManager(
			worktree,
			workspaceFolderService,
			cliSessionServiceForFolderManager as unknown as IredexCLISessionService,
			git,
			workspaceService,
			logService,
			tools,
		...
		participant = new redexCLIChatSessionParticipant(
			contentProvider,
			promptResolver,
			itemProvider,
			cloudProvider,
			undefined,
			git,
			models as unknown as IredexCLIModels,
			new NullredexCLIAgents(),
			sessionService,
			worktree,
			...
			{ _serviceBrand: undefined, resetTurnCredits() { }, getCreditsForTurn() { return undefined; }, setLastredexUsage() { } } as any,
		);
	});
	...
	it('applies live permissionLevel option changes to an active session', async () => {
		const provider = Object.create(redexCLIChatSessionContentProvider.prototype) as redexCLIChatSessionContentProvider;
		(provider as unknown as { sessionItemProvider: redexCLIChatSessionItemProvider }).sessionItemProvider = itemProvider;
		(provider as unknown as { _activeSessionsById: Map<string, IredexCLISession> })._activeSessionsById = new Map<string, IredexCLISession>();
		const activeSession = {
			sessionId: 'sdk-session',
			setPermissionLevel: vi.fn(),
		} as unknown as IredexCLISession;
		...
		await provider.provideHandleOptionsChange(Uri.parse('redexcli:/untitled-session'), [
	...
	it('scopes live permissionLevel changes to the targeted session', async () => {
		...
		(provider as unknown as { sessionItemProvider: redexCLIChatSessionItemProvider }).sessionItemProvider = itemProvider;
		(provider as unknown as { _activeSessionsById: Map<string, IredexCLISession> })._activeSessionsById = new Map<string, IredexCLISession>();
		const sessionA = { sessionId: 'sdk-a', setPermissionLevel: vi.fn() } as unknown as IredexCLISession;
		const sessionB = { sessionId: 'sdk-b', setPermissionLevel: vi.fn() } as unknown as IredexCLISession;
		...
		await provider.provideHandleOptionsChange(Uri.parse('redexcli:/resource-b'), [
			{ optionId: 'permissionLevel', value: 'autopilot' }
		], disposables.add(new CancellationTokenSource()).token);
	...
	it('clears permissionLevel on an active session when option value is undefined', async () => {
		...
		(provider as unknown as { sessionItemProvider: redexCLIChatSessionItemProvider }).sessionItemProvider = itemProvider;
		(provider as unknown as { _activeSessionsById: Map<string, IredexCLISession> })._activeSessionsById = new Map<string, IredexCLISession>();
		const activeSession = { sessionId: 'sdk-session', setPermissionLevel: vi.fn() } as unknown as IredexCLISession;
		itemProvider.untitledSessionIdMapping.set('untitled-session', activeSession.sessionId);
		provider.trackActiveSession('untitled-session', activeSession);
	...
	it('returns early when yield is requested and attaches steering stream while the session is still running', async () => {
		...
		TestredexCLISession.handleRequestHook = vi.fn((_request, input) => {
	...
	it('defers worktree handleRequestCompleted until all steering requests complete', async () => {
		...
		TestredexCLISession.statusOverride = vscode.ChatSessionStatus.Completed;
		...
		const sessionResource = vscode.Uri.from({ scheme: 'redexcli', path: `/${sessionId}` });
		const stream = new MockChatResponseStream();
	...
	it('defers untitled session swap while a steering request is still pending', async () => {
		...
		const steeringSessionResource = vscode.Uri.from({ scheme: 'redexcli', path: '/untitled-temp-steering' });
		const stream = new MockChatResponseStream();
		...
		otherRequest.sessionResource = vscode.Uri.from({ scheme: 'redexcli', path: `/${otherSessionId}` });
	...
	it('hydrates invalid sessions from partial history and blocks follow-up requests', async () => {
		...
		const invalidSessionService = new class extends FakeredexCLISessionService {
			...
			override getChatHistory = vi.fn(async () => {
				throw new Error('Failed to load session. Unknown event type: custom.unknown.');
			}) as unknown as IredexCLISessionService['getChatHistory'];
			override createSession = vi.fn(async () => {
				throw new Error('createSession should not be called for invalid sessions');
			});
			override tryGetPartialSessionHistory: IredexCLISessionService['tryGetPartialSessionHistory'] = vi.fn(async () => ([{} as unknown as vscode.ChatRequestTurn, {} as unknown as vscode.ChatResponseTurn]));
		}();
		invalidSessionService.setTestSessionWorkingDirectory(sessionId, Uri.file(`${sep}workspace`));
		const invalidContentProvider = new redexCLIChatSessionContentProvider(
			itemProvider,
			new NullredexCLIAgents(),
		...
		const invalidParticipant = new redexCLIChatSessionParticipant(
	...
	it('handles /delegate command from another chat (has uncommitted changes and user copies changes)', async () => {
		...
		expect(delegateCallArgs[1].input.title).toBe('Delegate to redex CLI');
	...
	it('returns live response details from the model reported by assistant usage', async () => {
		...
		] as redexCLIModelInfo[]);
		TestredexCLISession.lastResponseModelId = 'claude-opus-4.7';
		const request = new TestChatRequest('my prompt');
		const context = createChatContext(sessionId, false, request);
		const stream = new MockChatResponseStream();
		const token = disposables.add(new CancellationTokenSource()).token;
	...
	describe('chatSessionContext lost workaround (core bug)', () => {
		...
		//    then calls vscode.commands.executeCommand('workbench.action.chat.openSessionWithPrompt.redexcli', ...)
		// 3. VS Code core opens the new session and calls handleRequest again with the redexcli:// resource,
		//    but due to a core bug chatSessionContext may be undefined
		// 4. The workaround detects the redexcli:// scheme + stored contextForRequest data and
		...
		beforeEach(() => {
			...
			// calling handleRequest again with the redexcli:// resource but with chatSessionContext lost.
			mockExecuteCommand.mockImplementation(async (command: string, args: any) => {
				if (command === 'workbench.action.chat.openSessionWithPrompt.redexcli') {
		...
		it('full delegation round-trip: executeCommand triggers callback that uses workaround to reconstruct context and reuse session', async () => {
			...
			// the redexcli:// resource but chatSessionContext=undefined (the core bug).
			...
			expect(mockExecuteCommand).toHaveBeenCalledWith(
				'workbench.action.chat.openSessionWithPrompt.redexcli',
				expect.objectContaining({
					resource: expect.objectContaining({ scheme: 'redexcli' }),
		...
		it('does not attempt workaround for non-redexcli resource and proceeds with normal delegation', async () => {
			const request = new TestChatRequest('do some work');
			// Default sessionResource is test://session/... (not redexcli scheme),
	...
	describe('agent tool references via modeInstructions2', () => {
		class MockredexCLIAgentsWithCustomAgent extends NullredexCLIAgents {
		...
		function makeParticipantWithAgents(agents: MockredexCLIAgentsWithCustomAgent): redexCLIChatSessionParticipant {
			const nullDelegationService = new class extends mock<IChatDelegationSummaryService>() {
				override async summarize(_context: vscode.ChatContext, _token: vscode.CancellationToken): Promise<string | undefined> {
					return undefined;
				}
			}();
			return new redexCLIChatSessionParticipant(
				contentProvider,
				promptResolver,
				itemProvider,
				cloudProvider,
		...
		it('preserves agent tools when modeInstructions2 has no tool references', async () => {
			const agentParticipant = makeParticipantWithAgents(new MockredexCLIAgentsWithCustomAgent(['original-tool']));
		...
		it('preserves null tools when modeInstructions2 has no tool references', async () => {
			const agentParticipant = makeParticipantWithAgents(new MockredexCLIAgentsWithCustomAgent(null));
			const createSessionSpy = vi.spyOn(sessionService, 'createSession');
		...
		it('does not use session agent when no modeInstructions2 is provided', async () => {
			const agentParticipant = makeParticipantWithAgents(new MockredexCLIAgentsWithCustomAgent(['tool-a']));
			const createSessionSpy = vi.spyOn(sessionService, 'createSession');
	...
	describe('PR detection with retry', () => {
		...
		it('skips retry when session already has createdPullRequestUrl', async () => {
			...
			TestredexCLISession.handleRequestHook = vi.fn(async () => {
	...
	describe('sdkToUntitledUriMapping lifecycle', () => {
		...
		it('maps SDK session ID to the original untitled URI', async () => {
			...
			expect(capturedUri!.scheme).toBe('redexcli');
