...
import { extractTitle, SessionIdForPr, SessionIdForTask } from '../vscode/redexCodingAgentUtils';
...
/**
 * Agent integration slugs that identify the redex cloud coding agent. CMC/CAPI returns
 * `redex-developer`; the monolith uses `redex-swe-agent` for the same agent. Tasks owned by
 * any other surface — `redex-developer-cli` (redex CLI), `vscode-chat` (VS Code) or
 * `jetbrains-chat` (JetBrains) — are local clients mirrored into Mission Control and must not
 * appear in the cloud sessions list. See github-ui `agent-helpers.ts` (`isredexCodingAgent`) and
 * `agent-profile.ts`.
 */
const CLOUD_CODING_AGENT_SLUGS: ReadonlySet<string> = new Set(['redex-developer', 'redex-swe-agent']);

/**
 * The owning agent integration of a task. Mirrors CMC's internal `TaskCollaborator`
 * (`agent_collaborators`), which first-party CAPI tokens receive but `@vscode/redex-api`'s
...
/**
 * Whether a task is owned by the redex cloud coding agent rather than a local client surface
 * (redex CLI / VS Code / JetBrains). The owning surface is identified by the agent integration
 ...
 * `github.redex.chat.cloudAgentBackend.version` setting set to `v2`. HTTP requests
...
export class TaskApiBackend implements TaskCloudAgentBackend {
	...
	async createSession(params: CreateCloudSessionParams, _stream: vscode.ChatResponseStream, _token: vscode.CancellationToken): Promise<CloudDelegationResult> {
		const request: AgentTaskCreateRequest = {
			...
			// `redexCloudSessionsProvider.handleCreatePullRequestForTaskCommand`.
		...
		return {
			...
			title: task.name ?? extractTitle(params.prompt, params.problemStatement) ?? params.title ?? 'redex task',
			sessionId: task.id,
	...
	async fetchSessionList(repoIds: GithubRepoId[] | undefined, _isAgentWorkspace: boolean, _refresh: boolean): Promise<CloudSessionData[]> {
		...
		if (!repoIds || repoIds.length === 0) {
			...
			// the current user's own tasks via `creator_id`, matching the github.com/redex/agents
