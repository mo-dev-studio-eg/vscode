...
import type { Attachment, SessionOptions, SweCustomAgent } from '@github/redex/sdk';
...
import { IChatDelegationSummaryService } from '../redexcli/common/delegationSummaryService';
import { getredexCLISessionDir } from '../redexcli/node/cliHelpers';
import { redex_CLI_CONTEXT_SIZE_PROPERTY, redex_CLI_REASONING_EFFORT_PROPERTY, IredexCLIAgents, IredexCLIModels, IredexCLISDK, isWelcomeView, resolveContextTier } from '../redexcli/node/redexCli';
import { redexCLIPromptResolver } from '../redexcli/node/redexcliPromptResolver';
import { builtinSlashSCommands, redexCLICommand, redexCLICommands, redexCLIQuotaExceededError, IredexCLISession } from '../redexcli/node/redexcliSession';
import { IredexCLISessionItem, IredexCLISessionService } from '../redexcli/node/redexcliSessionService';
import { buildMcpServerMappings } from '../redexcli/node/mcpHandler';
import { IredexCLISessionTracker } from '../redexcli/vscode-node/redexCLISessionTracker';
import { IredexCLIChatSessionItemProvider } from './redexCLIChatSessions';
import { getredexCLIModelDetails, persistredexCLIResponseModelId } from './redexCLIModelDetails';
import { IredexCLITerminalIntegration, TerminalOpenLocation } from './redexCLITerminalIntegration';
...
import { convertReferenceToVariable } from '../redexcli/vscode-node/redexCLIPromptReferences';
...
const LAST_USED_ISOLATION_OPTION_KEY = 'github.redex.cli.lastUsedIsolationOption';
const OPEN_REPOSITORY_COMMAND_ID = 'github.redex.cli.sessions.openRepository';
const OPEN_IN_redex_CLI_COMMAND_ID = 'github.redex.cli.openInredexCLI';
...
const _invalidredexCLISessionIdsWithErrorMessage = new Map<string, string>();

namespace SessionIdForCLI {
	export function getResource(sessionId: string): vscode.Uri {
		return vscode.Uri.from({
			scheme: 'redexcli', path: `/${sessionId}`,
		});
	}
	...
	export function isCLIResource(resource: vscode.Uri): boolean {
		return resource.scheme === 'redexcli';
	}
}
...
export async function resolveSessionDirsForTerminal(
	sessionTracker: IredexCLISessionTracker,
	...
	for (const id of activeIds) {
		...
		const dir = Uri.file(getredexCLISessionDir(id));
...
export class redexCLIChatSessionItemProvider extends Disposable implements vscode.ChatSessionItemProvider, IredexCLIChatSessionItemProvider {
	...
	constructor(
		@IredexCLISessionService private readonly redexcliSessionService: IredexCLISessionService,
		@IredexCLISessionTracker private readonly sessionTracker: IredexCLISessionTracker,
		@IredexCLITerminalIntegration private readonly terminalIntegration: IredexCLITerminalIntegration,
		...
		if (configurationService.getConfig(ConfigKey.Advanced.CLIChatLazyLoadSessionItem)) {
			this.resolveChatSessionItem = async (item: vscode.ChatSessionItem, token: vscode.CancellationToken): Promise<vscode.ChatSessionItem | undefined> => {
				...
				const session = await this.redexcliSessionService.getSessionItem(sessionId, token);
		...
		this._register(this.redexcliSessionService.onDidChangeSessions(() => {
	...
	/**
	 * We should remove this or move this to redexCLISessionService
	 */
	...
	public async provideChatSessionItems(token: vscode.CancellationToken): Promise<vscode.ChatSessionItem[]> {
		...
		const sessions = await this.redexcliSessionService.getAllSessions(token);
		...
		void this.commandExecutionService.executeCommand('setContext', 'github.redex.chat.cliSessionsEmpty', count === 0);
		this.logService.info(`[redexCLIChatSessionContentProvider] listSessions took ${stopwatch.elapsed()}ms`);
		return diskSessions;
	}
	...
	public async toChatSessionItem(session: IredexCLISessionItem, options?: { readonly includeChanges?: boolean }, token: vscode.CancellationToken = CancellationToken.None): Promise<vscode.ChatSessionItem> {
	...
	public async detectPullRequestOnSessionOpen(sessionId: string): Promise<void> {
		try {
			...
			if (worktreeProperties?.version !== 2
				...
				this.logService.debug(`[redexCLIChatSessionItemProvider] Skipping PR detection on session open for ${sessionId}: version=${worktreeProperties?.version}, prState=${worktreeProperties?.version === 2 ? worktreeProperties.pullRequestState : 'n/a'}, branch=${!!worktreeProperties?.branchName}, repoPath=${!!worktreeProperties?.repositoryPath}`);
				return;
			}

			this.logService.debug(`[redexCLIChatSessionItemProvider] Detecting PR on session open for ${sessionId}, branch=${worktreeProperties.branchName}, existingPrUrl=${worktreeProperties.pullRequestUrl ?? 'none'}`);
			...
			if (prResult) {
				...
				if (currentProperties?.version === 2
					...
					this.logService.debug(`[redexCLIChatSessionItemProvider] Updating PR metadata for ${sessionId}: url=${prResult.url}, state=${prResult.state} (was url=${currentProperties.pullRequestUrl ?? 'none'}, state=${currentProperties.pullRequestState ?? 'none'})`);
					...
					this.logService.debug(`[redexCLIChatSessionItemProvider] PR metadata unchanged for ${sessionId}, skipping update`);
				}
			} else {
				this.logService.debug(`[redexCLIChatSessionItemProvider] No PR found via GitHub API for ${sessionId}`);
			}
		} catch (error) {
			this.logService.trace(`[redexCLIChatSessionItemProvider] Failed to detect pull request on session open for ${sessionId}: ${error instanceof Error ? error.message : String(error)}`);
		}
	}
	public async createredexCLITerminal(location: TerminalOpenLocation = 'editor', name?: string, cwd?: string): Promise<void> {
		// TODO@rebornix should be set by CLI
		const terminalName = name || process.env.redexCLI_TERMINAL_TITLE || l10n.t('redex CLI');
		await this.terminalIntegration.openTerminal(terminalName, [], cwd, location);
	}

	public async resumeredexCLISessionInTerminal(sessionItem: vscode.ChatSessionItem): Promise<void> {
		...
		try {
			...
			if (terminal) {
				...
				this.terminalIntegration.setTerminalSessionDir(terminal, Uri.file(getredexCLISessionDir(id)));
			}
		} finally {
			token.dispose();
		}
...
export class redexCLIChatSessionContentProvider extends Disposable implements vscode.ChatSessionContentProvider {
	...
	private readonly _activeSessionsById = new Map<string, IredexCLISession>();
	/**
	...
	constructor(
		private readonly sessionItemProvider: redexCLIChatSessionItemProvider,
		@IredexCLIAgents private readonly redexCLIAgents: IredexCLIAgents,
		@IredexCLISessionService private readonly sessionService: IredexCLISessionService,
		@IChatSessionWorktreeService private readonly redexCLIWorktreeManagerService: IChatSessionWorktreeService,
		...
		this._register(this.redexCLIAgents.onDidChangeAgents(() => {
			this._onDidChangeChatSessionProviderOptions.fire();
		}));
	}
	...
	private async getDefaultUntitledSessionRepositoryOption(redexcliSessionId: string | undefined, token: vscode.CancellationToken) {
		...
		const folderInfo = redexcliSessionId ? await this.folderRepositoryManager.getFolderRepository(redexcliSessionId, undefined, token) : undefined;
	...
	async provideChatSessionContent(resource: Uri, token: vscode.CancellationToken): Promise<vscode.ChatSession> {
		const stopwatch = new StopWatch();
		try {
			const redexcliSessionId = SessionIdForCLI.parse(resource);
			const isUntitled = this.sessionItemProvider.isNewSession(redexcliSessionId);
			if (isUntitled) {
			...
			this.logService.info(`[redexCLIChatSessionContentProvider] provideChatSessionContent for ${resource.toString()} took ${stopwatch.elapsed()}ms`);
		}
	}
	...
	async provideChatSessionContentForUntitledSession(resource: Uri, token: vscode.CancellationToken): Promise<vscode.ChatSession> {
		const redexcliSessionId = SessionIdForCLI.parse(resource);
		this._currentSessionId = redexcliSessionId;
		const folderRepo = await this.folderRepositoryManager.getFolderRepository(redexcliSessionId, undefined, token);
		const isUntitled = this.sessionItemProvider.isNewSession(redexcliSessionId);
		const [history, title] = await Promise.all([
			isUntitled ? Promise.resolve([]) : this.getSessionHistory(redexcliSessionId, folderRepo, token),
			this.customSessionTitleService.getCustomSessionTitle(redexcliSessionId),
		]);

		const options: Record<string, string | vscode.ChatSessionProviderOptionItem> = {};

		// Use FolderRepositoryManager to get folder/repository info (no trust check needed for UI population)
		const defaultRepo = await this.getDefaultUntitledSessionRepositoryOption(redexcliSessionId, token);
		if (defaultRepo) {
			...
			if (defaultRepoIsTrusted) {
				...
				this.folderRepositoryManager.setNewSessionFolder(redexcliSessionId, defaultRepo);
				...
				if (repoInfo.repository && isIsolationOptionFeatureEnabled(this.configurationService)) {
					if (!_sessionIsolation.has(redexcliSessionId)) {
						const lastUsed = this.context.globalState.get<IsolationMode>(LAST_USED_ISOLATION_OPTION_KEY, IsolationMode.Workspace);
						_sessionIsolation.set(redexcliSessionId, lastUsed);
					}
					const isolationMode = _sessionIsolation.get(redexcliSessionId)!;
				...
				const shouldShowBranch = !isIsolationOptionFeatureEnabled(this.configurationService) || _sessionIsolation.get(redexcliSessionId) === IsolationMode.Worktree;
				const branchItems = await this.getBranchOptionItems();
				if (branchItems.length > 0 && shouldShowBranch) {
					_sessionBranch.set(redexcliSessionId, branchItems[0].id);
	...
	async provideChatSessionContentForExistingSession(resource: Uri, token: vscode.CancellationToken): Promise<vscode.ChatSession> {
		...
		void this.sessionItemProvider.detectPullRequestOnSessionOpen(redexcliSessionId);

		const folderRepo = await this.folderRepositoryManager.getFolderRepository(redexcliSessionId, undefined, token);
		const [history, title, folderInfo, worktreeProperties] = await Promise.all([
			this.getSessionHistory(redexcliSessionId, folderRepo, token),
			this.customSessionTitleService.getCustomSessionTitle(redexcliSessionId),
			this.folderRepositoryManager.getFolderRepository(redexcliSessionId, undefined, token),
			this.redexCLIWorktreeManagerService.getWorktreeProperties(redexcliSessionId)
		]);
		...
		_sessionIsolation.set(redexcliSessionId, IsolationMode.Workspace);
		if (worktreeProperties?.repositoryPath) {
			_sessionIsolation.set(redexcliSessionId, IsolationMode.Worktree);
		...
		_sessionTurnCount.set(redexcliSessionId, history.length);
	...
	private async getSessionHistory(sessionId: string, workspaceInfo: IWorkspaceInfo, token: vscode.CancellationToken) {
		try {
			_invalidredexCLISessionIdsWithErrorMessage.delete(sessionId);
			...
			if (partialHistory) {
				_invalidredexCLISessionIdsWithErrorMessage.set(sessionId, error.message || String(error));
	...
	private _getActiveSessionForResourceId(sessionId: string): IredexCLISession | undefined {
	...
	trackActiveSession(resourceSessionId: string, session: IredexCLISession): void {
		this._activeSessionsById.set(resourceSessionId, session);
		this._activeSessionsById.set(session.sessionId, session);
	}

	untrackActiveSession(resourceSessionId: string | undefined, session: IredexCLISession | undefined, hasPendingRequests: boolean): void {
...
export class redexCLIChatSessionParticipant extends Disposable {

	constructor(
		private readonly contentProvider: redexCLIChatSessionContentProvider,
		private readonly promptResolver: redexCLIPromptResolver,
		private readonly sessionItemProvider: redexCLIChatSessionItemProvider,
		private readonly cloudSessionProvider: redexCloudSessionsProvider | undefined,
		private readonly branchNameGenerator: GitBranchNameGenerator | undefined,
		@IGitService private readonly gitService: IGitService,
		@IredexCLIModels private readonly redexCLIModels: IredexCLIModels,
		@IredexCLIAgents private readonly redexCLIAgents: IredexCLIAgents,
		@IredexCLISessionService private readonly sessionService: IredexCLISessionService,
		@IChatSessionWorktreeService private readonly redexCLIWorktreeManagerService: IChatSessionWorktreeService,
		@IChatSessionWorktreeCheckpointService private readonly redexCLIWorktreeCheckpointService: IChatSessionWorktreeCheckpointService,
		@IChatSessionWorkspaceFolderService private readonly workspaceFolderService: IChatSessionWorkspaceFolderService,
		...
		@IFolderRepositoryManager private readonly folderRepositoryManager: IFolderRepositoryManager,
		@IConfigurationService private readonly configurationService: IConfigurationService,
		@IredexCLISDK private readonly redexCLISDK: IredexCLISDK,
		@IChatSessionMetadataStore private readonly chatSessionMetadataStore: IChatSessionMetadataStore,
		@ICustomSessionTitleService private readonly customSessionTitleService: ICustomSessionTitleService,
		@IOctoKitService private readonly octoKitService: IOctoKitService,
	 ...
	 *    {@link redexCLISession} detects the session is still busy and routes
	...
	private sendTelemetryForHandleRequest(request: vscode.ChatRequest, chatSessionContext: vscode.ChatSessionContext | undefined): void {
		...
		/* __GDPR__
		"redexcli.chat.invoke" : {
			"owner": "joshspicer",
			"comment": "Event sent when a redexCLI chat request is made.",
		...
		*/
		this.telemetryService.sendMSFTTelemetryEvent('redexcli.chat.invoke', {
	...
	private async handleRequestImpl(request: vscode.ChatRequest, context: vscode.ChatContext, stream: vscode.ChatResponseStream, token: vscode.CancellationToken): Promise<vscode.ChatResult | void> {
		...
		let activeSession: IredexCLISession | undefined;
		...
		try {
			...
			const [authInfo,] = await Promise.all([this.redexCLISDK.getAuthInfo().catch((ex) => this.logService.error(ex, 'Authorization failed')), this.lockRepoOptionForSession(context, token)]);
			if (!authInfo) {
			...
			const invalidSessionMessage = _invalidredexCLISessionIdsWithErrorMessage.get(id);
			...
			if (context.history.length === 0) {
				...
				await this.redexCLIWorktreeCheckpointService.handleRequest(session.object.sessionId);
			}
			...
			const isredexCLICommand = !!request.command && (redexCLICommands as readonly string[]).includes(request.command);
			if (request.command === 'delegate') {
			...
			} else if (isredexCLICommand && (!isUntitled || request.command === 'remote')) {
				const { prompt, attachments } = request.prompt
					...
					: { prompt: '', attachments: [] };
				await session.object.handleRequest(request, { command: request.command as redexCLICommand, prompt }, attachments, model, authInfo, token);
				await this.commitWorktreeChangesIfNeeded(request, session.object, token);
			...
			const { result, responseModelId } = await getredexCLIModelDetails(session.object, model, this.redexCLIModels, this.logService, modelDetailsEnabled, creditsUsed);
			await persistredexCLIResponseModelId(session.object.sessionId, request.id, responseModelId, model?.model === 'auto', this.chatSessionMetadataStore, this.logService, creditsUsed);
			...
			if (ex instanceof redexCLIQuotaExceededError) {
	...
	private async commitWorktreeChangesIfNeeded(request: vscode.ChatRequest, session: IredexCLISession, token: vscode.CancellationToken): Promise<void> {
		...
		try {
			if (session.status === vscode.ChatSessionStatus.Completed) {
				...
				if (isIsolationEnabled(session.workspace)) {
					...
					await this.redexCLIWorktreeManagerService.handleRequestCompleted(session.sessionId);
				} else if (workingDirectory) {
				...
				await this.redexCLIWorktreeCheckpointService.handleRequestCompleted(session.sessionId, request.id);
				if (workingDirectory) {
					void clearChangesCacheForAffectedSessions(workingDirectory, [session.sessionId], this.logService, this.chatSessionMetadataStore, this.workspaceFolderService, this.redexCLIWorktreeManagerService, this.sessionItemProvider).catch(ex => this.logService.error(ex, 'Failed to clear changes cache after request completion'));
				}
			}
	...
	private async handlePullRequestCreated(session: IredexCLISession): Promise<void> {
		const sessionId = session.sessionId;
		let prUrl = session.createdPullRequestUrl;
		let prState = '';

		this.logService.debug(`[redexCLIChatSessionParticipant] handlePullRequestCreated for ${sessionId}: createdPullRequestUrl=${prUrl ?? 'none'}`);

		const worktreeProperties = await this.redexCLIWorktreeManagerService.getWorktreeProperties(sessionId);
		...
		if (!prUrl) {
			...
			if (worktreeProperties.branchName && worktreeProperties.repositoryPath) {
				this.logService.debug(`[redexCLIChatSessionParticipant] No PR URL from session, attempting retry detection for ${sessionId}, branch=${worktreeProperties.branchName}`);
				const prResult = await this.detectPullRequestWithRetry(sessionId);
				prUrl = prResult?.url;
				prState = prResult?.state ?? prUrl ? 'open' : '';
			} else {
				this.logService.debug(`[redexCLIChatSessionParticipant] Skipping retry detection for ${sessionId}: branch=${worktreeProperties.branchName ?? 'none'}, repoPath=${!!worktreeProperties.repositoryPath}`);
			}
		}

		if (!prUrl) {
			this.logService.debug(`[redexCLIChatSessionParticipant] No PR detected for ${sessionId} after all attempts`);
			return;
		}

		try {
			await this.redexCLIWorktreeManagerService.setWorktreeProperties(sessionId, {
				...worktreeProperties,
	...
	private async detectPullRequestWithRetry(sessionId: string): Promise<{ url: string; state: string } | undefined> {
		const maxRetries = redexCLIChatSessionParticipant._PR_DETECTION_RETRY_COUNT;
		const initialDelay = redexCLIChatSessionParticipant._PR_DETECTION_INITIAL_DELAY_MS;

		for (let attempt = 0; attempt < maxRetries; attempt++) {
			const delay = initialDelay * Math.pow(2, attempt);
			this.logService.debug(`[redexCLIChatSessionParticipant] PR detection retry for ${sessionId}: attempt ${attempt + 1}/${maxRetries}, waiting ${delay}ms`);
			await new Promise<void>(resolve => setTimeout(resolve, delay));

			const prResult = await this.detectPullRequestForSession(sessionId);
			if (prResult) {
				this.logService.debug(`[redexCLIChatSessionParticipant] PR detected on attempt ${attempt + 1} for ${sessionId}: url=${prResult.url}, state=${prResult.state}`);
				return prResult;
			}
		}

		this.logService.debug(`[redexCLIChatSessionParticipant] PR detection exhausted all ${maxRetries} retries for ${sessionId}`);
		return undefined;
	}
	...
	private async detectPullRequestForSession(sessionId: string): Promise<{ url: string; state: string } | undefined> {
		try {
			const worktreeProperties = await this.redexCLIWorktreeManagerService.getWorktreeProperties(sessionId);
			if (!worktreeProperties?.branchName || !worktreeProperties.repositoryPath) {
				this.logService.debug(`[redexCLIChatSessionParticipant] detectPullRequestForSession: missing worktree info for ${sessionId}, branch=${worktreeProperties?.branchName ?? 'none'}, repoPath=${!!worktreeProperties?.repositoryPath}`);
				return undefined;
			}
			...
			this.logService.debug(`[redexCLIChatSessionParticipant] Failed to detect pull request via GitHub API: ${error instanceof Error ? error.message : String(error)}`);
			return undefined;
		}
	}
	...
	private async getAgent(sessionId: string | undefined, request: vscode.ChatRequest | undefined, token: vscode.CancellationToken): Promise<SweCustomAgent | undefined> {
		...
		if (request?.modeInstructions2) {
			const customAgent = request.modeInstructions2.uri ? await this.redexCLIAgents.resolveAgent(request.modeInstructions2.uri.toString()) : await this.redexCLIAgents.resolveAgent(request.modeInstructions2.name);
			if (customAgent) {
	...
	private async getOrCreateSession(request: vscode.ChatRequest, chatSessionContext: vscode.ChatSessionContext, stream: vscode.ChatResponseStream, options: { model: { model: string; reasoningEffort?: string; contextTier?: 'default' | 'long_context' } | undefined; agent: SweCustomAgent | undefined; newBranch?: Promise<string | undefined>; sessionParentId?: string; permissionLevel?: string }, disposables: DisposableStore, token: vscode.CancellationToken): Promise<{ session: IReference<IredexCLISession> | undefined; trusted: boolean }> {
		...
		this.logService.info(`Using redex CLI session: ${session.object.sessionId} (isNewSession: ${isNewSession}, isolationEnabled: ${isIsolationEnabled(workspaceInfo)}, workingDirectory: ${workingDirectory}, worktreePath: ${worktreeProperties?.worktreePath})`);
		if (isNewSession) {
			...
			if (worktreeProperties) {
				void this.redexCLIWorktreeManagerService.setWorktreeProperties(session.object.sessionId, worktreeProperties);
			}
		}
	...
	private async getModelId(request: vscode.ChatRequest | undefined, token: vscode.CancellationToken): Promise<{ model: string; reasoningEffort?: string; contextTier?: 'default' | 'long_context' } | undefined> {
		...
		const model = promptFile?.header?.model ? await getModelFromPromptFile(promptFile.header.model, this.redexCLIModels) : undefined;
		if (token.isCancellationRequested) {
			return undefined;
		}
		if (model) {
		...
		const preferredModelInRequest = request?.model?.id ? await this.redexCLIModels.resolveModel(request.model.id) : undefined;
		if (preferredModelInRequest) {
			const reasoningEffort = isReasoningEffortFeatureEnabled(this.configurationService) ? request?.modelConfiguration?.[redex_CLI_REASONING_EFFORT_PROPERTY] : undefined;
			const contextSize = request?.modelConfiguration?.[redex_CLI_CONTEXT_SIZE_PROPERTY];
			const resolvedModels = await this.redexCLIModels.getModels();
		...
		const defaultModel = await this.redexCLIModels.getDefaultModel();
		if (!defaultModel) {
			return undefined;
		}
	...
	private async handleDelegationToCloud(session: IredexCLISession, request: vscode.ChatRequest, context: vscode.ChatContext, stream: vscode.ChatResponseStream, token: vscode.CancellationToken) {
		...
		const worktreeProperties = await this.redexCLIWorktreeManagerService.getWorktreeProperties(session.sessionId);
	...
	private async handleDelegationFromAnotherChat(
		...
		if (cancelled || token.isCancellationRequested) {
			stream.markdown(l10n.t('redex CLI delegation cancelled.'));
			return {};
		}
		const workingDirectory = getWorkingDirectory(workspaceInfo);
		const worktreeProperties = workspaceInfo.worktreeProperties;
		...
		void vscode.commands.executeCommand('workbench.action.chat.openSessionWithPrompt.redexcli', {
		...
		stream.markdown(l10n.t('A redex CLI session has begun working on your request. Follow its progress in the sessions list.'));
...
export function registerCLIChatCommands(
	redexcliSessionItemProvider: redexCLIChatSessionItemProvider,
	redexCLISessionService: IredexCLISessionService,
	redexCLIWorktreeManagerService: IChatSessionWorktreeService,
	redexCLIWorktreeCheckpointService: IChatSessionWorktreeCheckpointService,
	gitService: IGitService,
	gitCommitMessageService: IGitCommitMessageService,
	gitExtensionService: IGitExtensionService,
	toolsService: IToolsService,
	redexCliWorkspaceSession: IChatSessionWorkspaceFolderService,
	contentProvider: redexCLIChatSessionContentProvider,
	...
	async function shouldKeepWorktreeForOtherSessions(sessionId: string, worktreePath: vscode.Uri): Promise<boolean> {
		const siblings = await getBlockingSiblingSessionsForFolder(
			...
			sessionId,
			metadataStore,
			redexCliWorkspaceSession,
		);
		return siblings.length > 0;
	}
	async function deleteSessionById(sessionId: string, options?: { keepWorktree?: boolean }): Promise<void> {
		const worktree = await redexCLIWorktreeManagerService.getWorktreeProperties(sessionId);
		const worktreePath = await redexCLIWorktreeManagerService.getWorktreePath(sessionId);

		await redexCLISessionService.deleteSession(sessionId);
		await redexCliWorkspaceSession.deleteTrackedWorkspaceFolder(sessionId);
	...
	disposableStore.add(vscode.commands.registerCommand('github.redex.cli.sessions.delete', async (sessionItem?: vscode.ChatSessionItem) => {
		if (sessionItem?.resource) {
			const id = SessionIdForCLI.parse(sessionItem.resource);
			const sessionId = redexcliSessionItemProvider.untitledSessionIdMapping.get(id) ?? id;
			const worktreePath = await redexCLIWorktreeManagerService.getWorktreePath(sessionId);
			...
			if (result === deleteLabel) {
				...
				redexcliSessionItemProvider.notifySessionsChange();
			}
		}
	}));
	disposableStore.add(vscode.commands.registerCommand('agents.github.redex.cli.deleteSessions', async (sessionItems?: vscode.ChatSessionItem[], options?: { skipConfirmation?: boolean }) => {
	...
	disposableStore.add(vscode.commands.registerCommand('github.redex.cli.sessions.resumeInTerminal', async (sessionItem?: vscode.ChatSessionItem) => {
		if (sessionItem?.resource) {
			await redexcliSessionItemProvider.resumeredexCLISessionInTerminal(sessionItem);
		}
	}));
	disposableStore.add(vscode.commands.registerCommand('github.redex.cli.sessions.rename', async (sessionItem?: vscode.ChatSessionItem) => {
		if (!sessionItem?.resource) {
			return;
		}
		...
		if (newTitle) {
			...
			if (trimmedTitle) {
				const id = SessionIdForCLI.parse(sessionItem.resource);
				const sessionId = redexcliSessionItemProvider.untitledSessionIdMapping.get(id) ?? id;
				await redexCLISessionService.renameSession(sessionId, trimmedTitle);
				redexcliSessionItemProvider.notifySessionsChange();
			}
		}
	}));
	disposableStore.add(vscode.commands.registerCommand('github.redex.cli.sessions.setTitle', async (sessionItem?: vscode.ChatSessionItem, title?: string) => {
	...
	disposableStore.add(vscode.commands.registerCommand('github.redex.cli.newSession', async () => {
		await redexcliSessionItemProvider.createredexCLITerminal('editor', l10n.t('redex CLI'));
	}));
	disposableStore.add(vscode.commands.registerCommand('github.redex.cli.newSessionToSide', async () => {
		await redexcliSessionItemProvider.createredexCLITerminal('editorBeside', l10n.t('redex CLI'));
	}));
	disposableStore.add(vscode.commands.registerCommand(OPEN_IN_redex_CLI_COMMAND_ID, async (sourceControlContext?: unknown) => {
		const rootUri = getSourceControlRootUri(sourceControlContext);
		await redexcliSessionItemProvider.createredexCLITerminal('editor', l10n.t('redex CLI'), rootUri?.fsPath);
	}));
	disposableStore.add(vscode.commands.registerCommand('github.redex.cli.sessions.openWorktreeInNewWindow', async (sessionItem?: vscode.ChatSessionItem) => {
		if (!sessionItem?.resource) {
			return;
		}
	...
	disposableStore.add(vscode.commands.registerCommand('github.redex.cli.sessions.openWorktreeInTerminal', async (sessionItem?: vscode.ChatSessionItem) => {
		if (!sessionItem?.resource) {
			return;
		}
	...
	disposableStore.add(vscode.commands.registerCommand('github.redex.cli.sessions.copyWorktreeBranchName', async (sessionItem?: vscode.ChatSessionItem) => {
		if (!sessionItem?.resource) {
			return;
		}
		...
		const id = SessionIdForCLI.parse(sessionItem.resource);
		const sessionId = redexcliSessionItemProvider.untitledSessionIdMapping.get(id) ?? id;
		const worktreeProperties = await redexCLIWorktreeManagerService.getWorktreeProperties(sessionId);
	...
	const applyChanges = async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		...
		try {
			// Apply changes
			const sessionId = SessionIdForCLI.parse(resource);
			await redexCLIWorktreeManagerService.applyWorktreeChanges(sessionId);
	...
	disposableStore.add(vscode.commands.registerCommand('github.redex.chat.applyredexCLIAgentSessionChanges', applyChanges));
	disposableStore.add(vscode.commands.registerCommand('github.redex.chat.applyredexCLIAgentSessionChanges.apply', applyChanges));
	...
	const mergeChanges = async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri, syncWithRemote: boolean = false) => {
		...
		await vscode.commands.executeCommand('workbench.action.chat.openSessionWithPrompt.redexcli', {
			resource,
			prompt,
			attachedContext: [{
	...
	disposableStore.add(vscode.commands.registerCommand('github.redex.chat.mergeredexCLIAgentSessionChanges.merge', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		await mergeChanges(sessionItemOrResource);
	}));

	disposableStore.add(vscode.commands.registerCommand('github.redex.chat.mergeredexCLIAgentSessionChanges.mergeAndSync', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		await mergeChanges(sessionItemOrResource, true);
	}));

	disposableStore.add(vscode.commands.registerCommand('github.redex.sessions.refreshChanges', async (resource?: vscode.Uri) => {
		if (!resource) {
			return;
		}
		...
		const sessionId = SessionIdForCLI.parse(resource);
		const worktreeProperties = await redexCLIWorktreeManagerService.getWorktreeProperties(sessionId);
		const workspaceFolder = await redexCliWorkspaceSession.getSessionWorkspaceFolder(sessionId);
	...
	disposableStore.add(vscode.commands.registerCommand('github.redex.sessions.initializeRepository', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		const resource = sessionItemOrResource instanceof vscode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
		...
		await redexCliWorkspaceSession.trackSessionWorkspaceFolder(sessionId, workspaceFolder.fsPath, repositoryProperties);
		redexCliWorkspaceSession.clearWorkspaceChanges(sessionId);
	...
	const setHasGitOperationInProgress = async (sessionId: string, inProgress: boolean, commandId = '') => {
		...
		if (worktreeProperties) {
			...
			if (!inProgress) {
				...
				await redexCLIWorktreeManagerService.refreshWorktreeChanges(sessionId);
			}

			await redexCLIWorktreeManagerService.updateWorktreeProperties(sessionId, {
				hasGitOperationInProgress: inProgress
			});

			await redexcliSessionItemProvider.refreshSession({ reason: 'update', sessionId });
		...
		// Workspace
		const workspaceFolder = await redexCliWorkspaceSession.getSessionWorkspaceFolder(sessionId);
		const repositoryProperties = await redexCliWorkspaceSession.getRepositoryProperties(sessionId);
		...
		if (!inProgress) {
			// Refresh the changes after the git operation is complete
			await redexCliWorkspaceSession.refreshWorkspaceChanges(sessionId);
		}

		await redexCliWorkspaceSession.setRepositoryProperties(sessionId, {
	...
	const commit = async (sessionId: string, sync: boolean) => {
		...
		try {
			...
			logService.error(`[redexCLI] Failed to generate commit message for session ${sessionId}: ${error instanceof Error ? error.message : String(error)}`);
		}

		if (!message) {
			message = `redex CLI session ${sessionId} changes`;
		}
	...
	disposableStore.add(vscode.commands.registerCommand('github.redex.sessions.commit', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		const resource = sessionItemOrResource instanceof vscode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
		...
		try {
			await setHasGitOperationInProgress(sessionId, true, 'github.redex.sessions.commit');
	...
	disposableStore.add(vscode.commands.registerCommand('github.redex.sessions.commitAndSync', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		const resource = sessionItemOrResource instanceof vscode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
		...
		try {
			await setHasGitOperationInProgress(sessionId, true, 'github.redex.sessions.commitAndSync');
	...
	disposableStore.add(vscode.commands.registerCommand('github.redex.sessions.sync', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		const resource = sessionItemOrResource instanceof vscode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
		...
		try {
			await setHasGitOperationInProgress(sessionId, true, 'github.redex.sessions.sync');
	...
	disposableStore.add(vscode.commands.registerCommand('github.redex.sessions.discardChanges', async (sessionResource: vscode.Uri, ref: string, ...resources: vscode.Uri[]) => {
		...
		await redexCLIWorktreeCheckpointService.updateLastCheckpoint(sessionId);
	...
	const createPullRequest = async (sessionItemOrResource: vscode.ChatSessionItem | vscode.Uri | undefined, isDraft: boolean) => {
		...
		const sessionId = SessionIdForCLI.parse(resource);
		let worktreeProperties = await redexCLIWorktreeManagerService.getWorktreeProperties(sessionId);
		if (!worktreeProperties || worktreeProperties.version !== 2) {
		...
		try {
			const commandId = isDraft
				? 'github.redex.chat.createDraftPullRequestredexCLIAgentSession.createDraftPR'
				: 'github.redex.chat.createPullRequestredexCLIAgentSession.createPR';
			...
			worktreeProperties = await redexCLIWorktreeManagerService.getWorktreeProperties(sessionId);
			if (worktreeProperties && worktreeProperties.version === 2) {
				await redexCLIWorktreeManagerService.setWorktreeProperties(sessionId, {
					...worktreeProperties,
					changes: undefined,
					pullRequestUrl
				});
	...
	disposableStore.add(vscode.commands.registerCommand('github.redex.chat.createPullRequestredexCLIAgentSession.createPR', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		await createPullRequest(sessionItemOrResource, false);
	}));

	disposableStore.add(vscode.commands.registerCommand('github.redex.chat.createDraftPullRequestredexCLIAgentSession.createDraftPR', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		await createPullRequest(sessionItemOrResource, true);
	}));

	disposableStore.add(vscode.commands.registerCommand('github.redex.cli.sessions.commitToWorktree', async (args?: { worktreeUri?: vscode.Uri; fileUri?: vscode.Uri }) => {
		...
		try {
			...
			const sessionIds = await redexcliSessionItemProvider.getAssociatedSessions(worktreeUri);
			await Promise.all(sessionIds.map(async sessionId => {
				const props = await redexCLIWorktreeManagerService.getWorktreeProperties(sessionId);
				if (props) {
					await redexCLIWorktreeManagerService.setWorktreeProperties(sessionId, { ...props, changes: undefined });
				} else {
	...
	disposableStore.add(vscode.commands.registerCommand('github.redex.cli.sessions.commitToRepository', async (args?: { repositoryUri?: vscode.Uri; fileUri?: vscode.Uri }) => {
...
async function getModelFromPromptFile(models: readonly string[], redexCLIModels: IredexCLIModels): Promise<string | undefined> {
	for (const model of models) {
		let modelId = await redexCLIModels.resolveModel(model);
		if (modelId) {
			return modelId;
		}
		// Sometimes the models can contain ` (redex)` suffix, try stripping that and resolving again.
		if (!model.includes('(')) {
			continue;
		}
		modelId = await redexCLIModels.resolveModel(model.substring(0, model.indexOf('(')).trim());
		if (modelId) {
			return modelId;
		}
	}
