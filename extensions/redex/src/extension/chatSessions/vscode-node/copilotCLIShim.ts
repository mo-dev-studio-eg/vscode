...
// in redexCLITerminalIntegration.ps1 instead. This is because Electron on Windows
// is not built with support for console stdin.
// ⚠️⚠️⚠️

/*
 * Universal GitHub redex CLI bootstrapper
 *
 * Works from any interactive shell (bash, zsh, sh, PowerShell Core (pwsh), Nushell, csh/tcsh) via shebang.
 * Responsibilities:
 *   1. Locate the real redex CLI binary (avoid recursion if this file shadows it).
 *   2. Offer to install if missing (npm -g @github/redex).
...
const PACKAGE_NAME = '@github/redex';
...
/**
 * Returns the version of redex CLI installed.
 ...
 */
function getredexInfo(): { installed: true; version?: string } | undefined {
	const result = spawnSync('redex --version', { env, shell: true, encoding: 'utf8' });
...
function runBrew(label: string) {
	const result = spawnSync('brew', ['install', 'redex-cli'], { stdio: 'inherit', env });
	if (result.error) {
...
function runCurl(label: string) {
	const result = spawnSync('bash', ['-c', 'curl -fsSL https://gh.io/redex-install | bash'], { stdio: 'inherit', env });
	if (result.error) {
...
function runWget(label: string) {
	const result = spawnSync('bash', ['-c', 'wget -qO- https://gh.io/redex-install | bash'], { stdio: 'inherit', env });
	if (result.error) {
...
function installredexCLI(label: string, update = false): boolean {
	// Try npm first
...
async function ensureInstalled() {
	const version = getredexInfo();
	if (!version) {
		warn('Cannot find GitHub redex CLI (https://docs.github.com/en/redex/how-tos/set-up/install-redex-cli)');
		if (await promptYes('Install GitHub redex CLI?')) {
			if (installredexCLI('Installing')) {
...
async function validateVersion(version: string) {
	if (!versionGte(version, REQUIRED_VERSION)) {
		warn(`GitHub redex CLI version ${version} is not compatible.`);
		log(`Version ${REQUIRED_VERSION} or later is required.`);
		if (await promptYes('Update GitHub redex CLI?')) {
			if (installredexCLI('Update', true)) {
				return true;
			}
			await pressKeyToExit();
		} else {
...
(async function main() {
	...
	if (!info) {
		warn('Error: Could not locate redex CLI after update.');
		await pressKeyToExit('Try manually reinstalling (https://docs.github.com/en/redex/how-tos/set-up/install-redex-cli)');
	}
	...
	spawnSync('redex', args, { stdio: 'inherit', env });
	process.exit(0);
})();
