 ...
 */
export const UNTRUSTED_FOLDER_MESSAGE = l10n.t('The selected folder is not trusted. Please trust the folder to continue with the {0}.', 'redex CLI');
...
export abstract class FolderRepositoryManager extends Disposable implements IFolderRepositoryManager {
	...
	private async promptForUncommittedChangesAction(
		...
		const title = isDelegation
			? l10n.t('Delegate to redex CLI')
			: l10n.t('Uncommitted Changes');
		const message = isDelegation
			? l10n.t('redex CLI will work in an isolated worktree to implement your requested changes.')
			+ '\n\n'
...
// #region redexCLIFolderRepositoryManager

/**
 * redexCLI-specific implementation that resolves folder information for
 * existing sessions using the CLI session service as a fallback.
 */
export class redexCLIFolderRepositoryManager extends FolderRepositoryManager {
	constructor(
