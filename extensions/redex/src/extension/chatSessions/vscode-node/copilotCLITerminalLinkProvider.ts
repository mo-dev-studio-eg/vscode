...
import { getredexHome } from '../redexcli/node/cliHelpers';

const UNTRUSTED_redex_HOME_MESSAGE = l10n.t('The redex home directory is not trusted. Please trust the directory to open this file.');
...
interface redexCLITerminalLink extends TerminalLink {
	uri?: Uri;
	terminal: Terminal;
	pathText: string;
	line?: number;
...
/**
 * Resolves relative file links in redex CLI terminal output.
 *
 * redex CLI paths are relative to the session-state directory, not the
 * workspace root. VS Code's built-in detector cannot resolve that context.
 */
export class redexCLITerminalLinkProvider implements TerminalLinkProvider<redexCLITerminalLink> {

	private readonly _redexTerminals = new WeakSet<Terminal>();
	...
	/**
	 * Marks a terminal as a redex CLI terminal.
	 */
	registerTerminal(terminal: Terminal): void {
		this._redexTerminals.add(terminal);
	}
	...
	async provideTerminalLinks(context: TerminalLinkContext, token: CancellationToken): Promise<redexCLITerminalLink[]> {
		...
		if (!this._redexTerminals.has(context.terminal) && sessionDirs.length === 0) {
			return [];
		}
		const links: redexCLITerminalLink[] = [];
	...
	async handleTerminalLink(link: redexCLITerminalLink): Promise<void> {
		try {
			...
			if (this.workspaceService && this._isInredexHome(uriToOpen)) {
				const trusted = await this.workspaceService.requestResourceTrust({
					uri: Uri.file(getredexHome()),
					message: UNTRUSTED_redex_HOME_MESSAGE,
				});
				if (!trusted) {
					return;
				}
	 ...
	 */
	private _isInredexHome(uri: Uri): boolean {
		return extUriBiasedIgnorePathCase.isEqualOrParent(uri, Uri.file(getredexHome()));
	}
	 ...
	 * 1. Each session-state directory (e.g., `~/.redex/session-state/{uuid}/`)
	 * 2. Workspace folders
