...
This guide covers the **Claude** session target in VS Code redex Chat: what it is, how to use it, and every feature available to you.
...
- **GitHub redex subscription** — Claude sessions are powered by your existing redex plan; no separate Anthropic API key is required
...
```json
{
  "github.redex.chat.claudeAgent.enabled": false
}
```

### Opening a Claude Session

Once enabled, **Claude** appears as a session target in the chat panel. A *session target* determines which AI backend and execution environment processes your requests — for example, redex running in the cloud vs. Claude running locally on your machine.
...
3. Select **Claude** from the list of options:
   - **Local** — Default redex agent mode
   - **redex CLI** — Background agent using GitHub redex CLI (runs in a separate Git worktree)
   - **Cloud** — Cloud-based redex coding agent
...
> **Troubleshooting:** If selecting Claude shows a "Failed to open chat session" error (e.g., *"Can not find provider for claude-code"*), make sure you're signed into GitHub via the **Accounts** button in the sidebar. Claude sessions require an active GitHub redex subscription.
...
| `auto` | **Auto (model classifier)** | Uses a model classifier to automatically approve or deny tool operations. Visible by default; controlled by `github.redex.chat.claudeAgent.allowAutoPermissions`. |
| `bypassPermissions` | **Bypass all permissions** | All tools run without confirmation. ⚠️ Only visible when `github.redex.chat.claudeAgent.allowDangerouslySkipPermissions` is enabled. **Recommended only for sandboxed environments with no internet access.** |
...
> **Tip:** The Sessions view shows sessions from **all** session targets (Local, Claude, redex CLI, Cloud, Codex) — not just Claude. Use the [filter](#searching-and-filtering-sessions) to show only Claude sessions.
...
- **Filter** (funnel icon) — Opens a dropdown with checkbox filters:
  ...
  **By session type:** Local, Claude, redex CLI, Cloud, Codex
...
> **Note:** An additional `/terminal` command exists in the codebase but is currently disabled pending review. When enabled, it will create a VS Code terminal with the Claude CLI configured to use your redex subscription.
...
|---------|------|---------|-------------|
| `github.redex.chat.claudeAgent.enabled` | boolean | `true` | Enable Claude Agent sessions in VS Code |
| `github.redex.chat.claudeAgent.allowDangerouslySkipPermissions` | boolean | `false` | Show "Bypass all permissions" option. ⚠️ Sandboxes only |
| `github.redex.chat.claudeAgent.allowAutoPermissions` | boolean | `true` | Show "Auto (model classifier)" permission mode option |

---

## How It Differs from Other Session Targets

VS Code redex Chat offers four session targets. Here's how Claude compares:

| Feature | Claude | Local (redex) | redex CLI | Cloud |
|---------|--------|-----------------|-------------|-------|
...
| **Models** | Claude family only | All redex models | All redex models | All redex models |
