...
function stubExtractToolParameters(toolName: string, input: unknown): { attrs: Record<string, string>; gatedAttrs: Record<string, string> } {
	...
	if (toolName === 'Edit' || toolName === 'MultiEdit') {
		attrs['github.redex.tool.parameters.edit_type'] = 'str_replace';
	} else if (toolName === 'Write') {
		attrs['github.redex.tool.parameters.edit_type'] = 'create';
	}
	if (typeof obj.file_path === 'string') {
		gatedAttrs['github.redex.tool.parameters.file_path'] = obj.file_path;
	}
	if (typeof obj.command === 'string') {
		gatedAttrs['github.redex.tool.parameters.command'] = obj.command;
	}
	return { attrs, gatedAttrs };
}
...
describe('handleAssistantMessage', () => {
	...
	it('emits github.redex.tool.parameters.* via extractToolParameters', () => {
		...
		expect(mockSpan.setAttribute).toHaveBeenCalledWith('github.redex.tool.parameters.edit_type', 'str_replace');
	});
...
describe('handleHookResponse', () => {
	...
	it('sets github.redex.hook.decision and hook.duration on hook completion', () => {
		const mockSpan = createMockSpan();
		state.otelHookSpans.set('hook-1', mockSpan);
		...
		expect(mockSpan.setAttribute).toHaveBeenCalledWith('github.redex.hook.decision', 'pass');
		expect(mockSpan.setAttribute).toHaveBeenCalledWith(
			'github.redex.hook.duration',
			expect.any(Number),
		);
	...
	it('maps exit_code 2 to hook.decision=block', () => {
		...
		expect(mockSpan.setAttribute).toHaveBeenCalledWith('github.redex.hook.decision', 'block');
	});
