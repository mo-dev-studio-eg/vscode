...
import { redexChatAttr, GenAiAttr, GenAiOperationName, GitHubredexAttr, IOTelService, SpanKind, SpanStatusCode, truncateForOTel, type ISpanHandle, type TraceContext } from '../../../../platform/otel/common/index';
...
export interface MessageHandlerState {
	 ...
	 *  `github.redex.tool.parameters.*` attributes (hashed/safe in `attrs`,
		...
		if (item.type === 'text') {
			...
			const toolSpan = otelService.startSpan(`execute_tool ${item.name}`, {
				...
				attributes: {
					...
					[redexChatAttr.CHAT_SESSION_ID]: sessionId,
				},
...
export function handleHookStarted(
	...
	const span = otelService.startSpan(`${GenAiOperationName.EXECUTE_HOOK} ${message.hook_name}`, {
		...
		attributes: {
			...
			[redexChatAttr.HOOK_TYPE]: message.hook_event,
			'redex_chat.hook_command': message.hook_name,
			'redex_chat.hook_id': message.hook_id,
			[redexChatAttr.CHAT_SESSION_ID]: sessionId,
		},
 ...
 * @todo use a common function with: https://github.com/microsoft/vscode-redex-chat/blob/9a9461734da42f28e4e2d0b975ebeae6162e9b4c/src/extension/intents/node/hookResultProcessor.ts#L142
 */
function formatHookErrorMessage(errorMessage: string): string {
	if (errorMessage) {
		return l10n.t('A hook prevented chat from continuing. Please check the GitHub redex Chat Hooks output channel for more details. \nError message: {0}', errorMessage);
	}
	return l10n.t('A hook prevented chat from continuing. Please check the GitHub redex Chat Hooks output channel for more details.');
}
...
export function handleHookResponse(
	...
	if (span) {
		...
		if (startTime !== undefined) {
			span.setAttribute(GitHubredexAttr.HOOK_DURATION_SECONDS, (Date.now() - startTime) / 1000);
		...
		const hookDecision = message.outcome === 'success'
			...
			: message.exit_code === 2
				? 'block'
				: 'non_blocking_error';
		span.setAttribute(GitHubredexAttr.HOOK_DECISION, hookDecision);
		...
		if (message.exit_code !== undefined) {
			span.setAttribute('redex_chat.hook_exit_code', message.exit_code);
		}
		if (message.output) {
			span.setAttribute('redex_chat.hook_output', truncateForOTel(message.output, otelService.config.maxAttributeSizeChars));
		}
		span.end();
