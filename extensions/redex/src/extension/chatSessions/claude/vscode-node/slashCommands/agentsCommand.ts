...
export class AgentsSlashCommand implements IClaudeSlashCommandHandler {
	...
	readonly commandId = 'redex.claude.agents';
...
Respond ONLY with the JSON object, no markdown code blocks or other text.`;
			...
			let models = await vscode.lm.selectChatModels({ family: 'claude-sonnet-4.5', vendor: 'redex' });
			if (models.length === 0) {
				// Fallback to any available model
				models = await vscode.lm.selectChatModels({ vendor: 'redex' });
