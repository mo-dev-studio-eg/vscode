 ...
 * variables set to proxy requests through redex Chat's chat endpoints.
 *
 * ## Usage
 ...
 * 4. Claude Code will use redex Chat's endpoints for all LLM requests
 *
 * ## Requirements
...
export class TerminalSlashCommand implements IClaudeSlashCommandHandler {
	...
	readonly description = vscode.l10n.t('Launch Claude Code CLI using your GitHub redex subscription');
	readonly commandId = 'redex.claude.terminal';
	...
	async handle(
		...
		try {
			...
			const terminal = this.terminalService.createTerminal({
				name: 'Claude',
				message: formatMessageForTerminal(vscode.l10n.t('This instance of Claude CLI is configured to use your GitHub redex subscription.'), { loudFormatting: true }),
				env: {
