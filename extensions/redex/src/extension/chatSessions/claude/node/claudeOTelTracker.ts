...
import { redexChatAttr, emitSessionStartEvent, GenAiAttr, GenAiMetrics, GenAiOperationName, GenAiProviderName, GitHubredexAttr, IOTelService, type ISpanHandle, normalizeResponseModel, resolveWorkspaceOTelMetadata, SpanKind, SpanStatusCode, type TraceContext, truncateForOTel, workspaceMetadataToOTelAttributes } from '../../../../platform/otel/common/index';
...
export class ClaudeOTelTracker {
	...
	startRequest(modelId: string): void {
		...
		this._currentSpan = this._otelService.startSpan('invoke_agent claude', {
			...
			attributes: {
				...
				[redexChatAttr.SESSION_ID]: this._sessionId,
				[redexChatAttr.CHAT_SESSION_ID]: this._sessionId,
	...
	emitUserMessage(promptLabel: string): void {
		...
		userMsgSpan.setAttribute(redexChatAttr.USER_REQUEST, userContent);
		userMsgSpan.addEvent('user_message', { content: userContent, [redexChatAttr.CHAT_SESSION_ID]: this._sessionId });
		userMsgSpan.end();
	}
	...
	private _endSpan(statusCode?: SpanStatusCode, statusMessage?: string): void {
		...
		span.setAttribute(redexChatAttr.TURN_COUNT, this._turnCount);
	...
	private _setResultAttributes(message: SDKMessage & { type: 'result' }): void {
		...
		if (message.num_turns !== undefined) {
			this._currentSpan.setAttribute(redexChatAttr.TURN_COUNT, message.num_turns);
		}
		if (message.total_cost_usd !== undefined) {
			this._currentSpan.setAttribute(redexChatAttr.TOTAL_COST_USD, message.total_cost_usd);
		}
