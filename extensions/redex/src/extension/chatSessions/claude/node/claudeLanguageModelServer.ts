...
export function processNonStreamingPassThroughResponse(
	...
	onUsage: ((usage: { promptTokens: number; completionTokens: number; redexCredits?: number }) => void) | undefined,
			...
			for await (const completion of inner) {
				...
				if (completion.usage && onUsage) {
					onUsage({
						...
						redexCredits: nanoAiuToCredits(completion.usage.redex_usage?.total_nano_aiu),
					});
				}
			}
		} catch (e) {
...
class ClaudeStreamingPassThroughEndpoint implements IChatEndpoint {
	...
	private _processNonStreamingResponse(
		...
		const onUsage = sessionId
			? (usage: { promptTokens: number; completionTokens: number; redexCredits?: number }) => {
