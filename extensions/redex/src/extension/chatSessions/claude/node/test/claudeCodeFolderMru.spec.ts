...
describe('ClaudeCodeFolderMruService', () => {
	...
	it('skips sessions with .worktrees/redex- cwd', async () => {
		sessionService.sessions = [
			makeSession({ id: 's1', cwd: '/Users/test/repo/.worktrees/redex-abc123' }),
		];
	...
	it('filters git repos with .worktrees/redex- path', async () => {
		gitService.recentRepos = [
			{ rootUri: URI.file('/Users/test/repo/.worktrees/redex-abc123'), lastAccessTime: 100 },
		];
