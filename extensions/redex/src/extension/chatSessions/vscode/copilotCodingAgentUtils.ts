...
export const body_suffix = vscode.l10n.t('Created from [VS Code](https://code.visualstudio.com/docs/redex/redex-coding-agent).');
// https://github.com/github/sweagentd/blob/main/docs/adr/0001-create-job-api.md
export const JOBS_API_VERSION = 'v1';

/**
 * Truncation utility to ensure the problem statement sent to redex API is under the maximum length.
...
export function getAuthorDisplayName(author: { login: string } | null): string {
	...
	if (author.login.startsWith('redex')) {
		return 'redex';
	}
	return author.login;
}
