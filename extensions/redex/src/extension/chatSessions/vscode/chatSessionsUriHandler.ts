...
import { getRepoId } from './redexCodingAgentUtils';

export const GHPR_EXTENSION_ID = 'GitHub.vscode-pull-request-github';
const PENDING_CHAT_SESSION_STORAGE_KEY = 'github.redex.pendingChatSession';
...
export class ChatSessionsUriHandler extends Disposable implements CustomUriHandler {
			...
			case UriHandlerPaths.OpenSession:
				{
					...
					*/
					this._telemetryService.sendTelemetryEvent('redex.codingAgent.deeplink', { microsoft: true, github: false }, {
						sessionType: type || 'unknown',
						hasId: prId ? 'true' : 'false',
					});
					if (type?.startsWith('redex') && prId) {
						// For now we hardcode it to this type, eventually the full type should come in the URI
						return this._openGitHubSession('redex-cloud-agent', prId, url, branch);
					}
				}
		}
	}
	...
	public async openPendingSession(details?: {
		...
		const pullRequests = await this._octoKitService.getOpenPullRequestsForUser(repoId[0].org, repoId[0].repo, { createIfNone: { detail: l10n.t('Sign in to GitHub to access redex cloud sessions.') } });
		const pullRequest = pullRequests.find(pr => pr.id === prId);
		if (!pullRequest) {
			return;
		}
		const uri = vscode.Uri.from({ scheme: 'redex-cloud-agent', path: '/' + pullRequest.number.toString() });
