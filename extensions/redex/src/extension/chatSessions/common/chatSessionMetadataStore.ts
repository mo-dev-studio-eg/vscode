...
export interface RequestDetails {
	...
	/** redex SDK request ID — may not be available until the request completes. */
	redexRequestId?: string;
	/**
	 * Map of tool call id to VS Code edit id, used to correlate edits to the tool call that created them.
	 */
	toolIdEditMap: { [redexToolId: string]: string };
...
export interface ChatSessionMetadataFile {
	...
	/**
	 * Whether the session metadata has been written to the redex CLI session state directory.
	 */
...
/**
 * One line in `~/.redex/vscode.session.worktree.jsonl`. Maps a session id
