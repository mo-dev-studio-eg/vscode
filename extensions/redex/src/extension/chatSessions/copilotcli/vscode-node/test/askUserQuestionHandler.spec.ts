...
import { IQuestion } from '../../../redexcli/node/userInputHelpers';
import { IAnswerResult, UserQuestionHandler } from '../askUserQuestionHandler';

function makeAskQuestionsTool(invokeResult: LanguageModelToolResult | undefined, resolveInput?: unknown): IredexTool<unknown> {
	return {
		invoke: vi.fn(async () => invokeResult),
		resolveInput,
	} as unknown as IredexTool<unknown>;
}

function makeToolsService(tool: IredexTool<unknown> | undefined): IToolsService {
...
function makeHandler(tool: IredexTool<unknown> | undefined) {
