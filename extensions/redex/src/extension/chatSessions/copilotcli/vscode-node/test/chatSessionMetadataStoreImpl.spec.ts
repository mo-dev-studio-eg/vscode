...
import { NullredexCLIAgents } from '../../../redexcli/node/test/testHelpers';
...
const jsonlPathHolder = vi.hoisted(() => {
	const p = '/mock/redex-home/worktree.jsonl';
	return { get: () => p };
});

vi.mock('../../../redexcli/node/cliHelpers', async (importOriginal) => {
	const actual = await importOriginal<typeof import('../../../redexcli/node/cliHelpers')>();
	return {
		...actual,
		getredexCLISessionDir: (sessionId: string) => `/mock/session-state/${sessionId}`,
		getredexCLISessionStateDir: () => '/mock/session-state',
		// New shared bulk + JSONL paths — all go through the mocked IFileSystemService.
		getredexBulkMetadataFile: () => '/mock/redex-home/vscode.session.metadata.cache.json',
		getredexWorktreeSessionsFile: () => jsonlPathHolder.get(),
	};
});
...
// New shared bulk file (top-N cache). Lives at `~/.redex/...` in production but is
// mocked above so the IFileSystemService mock can intercept reads/writes.
const BULK_METADATA_FILE = Uri.file('/mock/redex-home/vscode.session.metadata.cache.json');
// Legacy bulk file location in the per-install globalStorageUri — used only by the
// one-time migration in `initializeStorage()`.
const LEGACY_BULK_METADATA_FILE = Uri.joinPath(Uri.file('/mock/global/storage'), 'redexcli', 'redexcli.session.metadata.json');

function sessionDirectoryUri(sessionId: string): Uri {
	return Uri.file(getredexCLISessionDir(sessionId));
}
...
describe('ChatSessionMetadataStore', () => {
	...
	describe('initializeStorage - empty global state', () => {
		it('should not write bulk file when no data exists in either source', async () => {
			...
			const bulkWrites = writeSpy.mock.calls.filter(
				c => c[0].toString().includes('redexcli.session.metadata.json'),
			);
	...
	describe('updateGlobalStorageImpl - merge behavior', () => {
		...
		it('should handle bulk file read failure during debounced write gracefully', async () => {
			...
			vi.spyOn(mockFs, 'readFile').mockImplementation(async (uri) => {
				if (uri.toString().includes('redexcli.session.metadata.json')) {
					readCallAfterStore++;
	...
	describe('getRequestDetails / appendRequestDetails', () => {
		it('should append and retrieve request details', async () => {
			...
			await store.updateRequestDetails('session-1', [{ vscodeRequestId: 'request-1', redexRequestId: 'sdk-1', toolIdEditMap: { 'tool-1': 'edit-1' } }]);
			await store.updateRequestDetails('session-1', [{ vscodeRequestId: 'request-2', redexRequestId: 'sdk-2', toolIdEditMap: { 'tool-2': 'edit-2' } }]);

			const details = await store.getRequestDetails('session-1');
			expect(details).toEqual([
				{ vscodeRequestId: 'request-1', redexRequestId: 'sdk-1', toolIdEditMap: { 'tool-1': 'edit-1' } },
				{ vscodeRequestId: 'request-2', redexRequestId: 'sdk-2', toolIdEditMap: { 'tool-2': 'edit-2' } },
			]);
			store.dispose();
		});
		...
		it('should merge with existing request details on append', async () => {
			...
			await mockFs.writeFile(fileUri, new TextEncoder().encode(JSON.stringify([
				{ vscodeRequestId: 'request-existing', redexRequestId: 'sdk-existing', toolIdEditMap: { 'tool-1': 'edit-1' } },
			])));

			const store = await createStore();
			await store.updateRequestDetails(sessionId, [{ vscodeRequestId: 'request-new', redexRequestId: 'sdk-new', toolIdEditMap: { 'tool-2': 'edit-2' } }]);

			const raw = await mockFs.readFile(fileUri);
			const parsed = JSON.parse(new TextDecoder().decode(raw));
			expect(parsed).toEqual([
				{ vscodeRequestId: 'request-existing', redexRequestId: 'sdk-existing', toolIdEditMap: { 'tool-1': 'edit-1' } },
				{ vscodeRequestId: 'request-new', redexRequestId: 'sdk-new', toolIdEditMap: { 'tool-2': 'edit-2' } },
			]);
			store.dispose();
		});
		...
		it('should merge fields when appending with same vscodeRequestId', async () => {
			...
			expect(details[0].redexRequestId).toBe('sdk-1');
			store.dispose();
		});
		...
		it('should serialize concurrent appends to the same session', async () => {
			...
			await Promise.all([
				store.updateRequestDetails('session-1', [{ vscodeRequestId: 'request-1', redexRequestId: 'sdk-1', toolIdEditMap: { 'tool-1': 'edit-1' } }]),
				store.updateRequestDetails('session-1', [{ vscodeRequestId: 'request-2', redexRequestId: 'sdk-2', toolIdEditMap: { 'tool-2': 'edit-2' } }]),
				store.updateRequestDetails('session-1', [{ vscodeRequestId: 'request-3', redexRequestId: 'sdk-3', toolIdEditMap: { 'tool-3': 'edit-3' } }]),
			]);
	...
	describe('legacy bulk file migration (Step 0)', () => {
		...
		it('sets memento flag after successful merge so it does not re-run', async () => {
			...
			const store = await createStore();
			expect(extensionContext.globalState.get('github.redex.cli.legacyBulkMigrated')).toBe(true);
			store.dispose();
		});

		it('skips migration when memento flag is already set', async () => {
			extensionContext.globalState.seed('github.redex.cli.legacyBulkMigrated', true);
			mockFs.mockFile(LEGACY_BULK_METADATA_FILE, JSON.stringify({
				'legacy-session': { workspaceFolder: { folderPath: Uri.file('/legacy').fsPath, timestamp: 1 } },
			}));
			mockFs.mockFile(BULK_METADATA_FILE, JSON.stringify({}));
