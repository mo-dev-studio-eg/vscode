...
import { redexCLISessionTracker, getParentPid } from '../redexCLISessionTracker';
...
describe('redexCLISessionTracker', () => {
	let tracker: redexCLISessionTracker;

	beforeEach(() => {
		tracker?.dispose();
		tracker = new redexCLISessionTracker();
		mockTerminals.value = [];
		mockIsWindows.value = false;
	...
	describe('setSessionName and getSessionDisplayName', () => {
		it('should return sessionId when no name is set', () => {
			tracker.registerSession('session-1', { pid: 1234, ppid: 5678 });
			expect(tracker.getSessionDisplayName('session-1')).toBe('redex CLI Session');
		});
