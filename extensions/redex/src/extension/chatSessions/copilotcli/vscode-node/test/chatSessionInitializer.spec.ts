...
import { redexCLIModelInfo, IredexCLIAgents, IredexCLIModels } from '../../../redexcli/node/redexCli';
...
import { redexCLIChatSessionInitializer } from '../redexCLIChatSessionInitializer';
...
class TestSessionService extends mock<IredexCLISessionService>() {
	...
	override createSession = vi.fn(async (): Promise<IReference<IredexCLISession>> => ({
		object: makeSessionObject(),
		dispose: vi.fn(),
	}));
	override getSession = vi.fn(async (): Promise<IReference<IredexCLISession> | undefined> => ({
		object: makeSessionObject(),
		dispose: vi.fn(),
	}));
}
...
class TestModels extends mock<IredexCLIModels>() {
	declare readonly _serviceBrand: undefined;
	...
	override getModels = vi.fn(async (): Promise<redexCLIModelInfo[]> => []);
}

class TestAgents extends mock<IredexCLIAgents>() {
	declare readonly _serviceBrand: undefined;
...
function makeSessionObject(overrides?: Partial<IredexCLISession>): IredexCLISession {
	return {
...
function makeChatResource(sessionId: string = 'untitled:new-session'): vscode.Uri {
	return URI.from({ scheme: 'redexcli', path: `/${sessionId}` }) as unknown as vscode.Uri;
}
...
function createInitializer(overrides?: {
	...
	const initializer = new redexCLIChatSessionInitializer(
		sessionService,
		folderRepoManager,
		workspaceService,
		models,
