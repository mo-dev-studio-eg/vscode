...
import { copyRipgrepShim } from '../../../redexcli/node/ripgrepShim';

describe('redexCLI SDK Upgrade', function () {
	const extensionPath = path.join(__dirname, '..', '..', '..', '..', '..', '..');
	const redexSDKPath = path.join(extensionPath, 'node_modules', '@github', 'redex');
	beforeAll(async function () {
		await copyBinaries(extensionPath);
	});

	it('should be able to load the SDK without errors', async function () {
		await import('@github/redex/sdk');
	}, 10000);

	it('should not contain new native binaries nor removed native binaries', async function () {
		// This is a very basic check to ensure that when the redex CLI SDK is upgraded,
		...
		const existingBinaries = new Set(await findAllBinaries(redexSDKPath));
		const knownBinaries = new Set([
			...
			path.join('prebuilds', 'darwin-arm64', 'redex Computer Use.app', 'Contents', 'CodeResources'),
			path.join('prebuilds', 'darwin-arm64', 'redex Computer Use.app', 'Contents', 'Resources', 'Assets.car'),
			path.join('prebuilds', 'darwin-arm64', 'redex Computer Use.app', 'Contents', 'Resources', 'icon.icns'),
			path.join('prebuilds', 'darwin-arm64', 'redex Computer Use.app', 'Contents', 'MacOS', 'redex Computer Use'),
			path.join('prebuilds', 'darwin-x64', 'computer-use-mcp'),
			path.join('prebuilds', 'darwin-x64', 'redex Computer Use.app', 'Contents', 'CodeResources'),
			path.join('prebuilds', 'darwin-x64', 'redex Computer Use.app', 'Contents', 'Resources', 'Assets.car'),
			path.join('prebuilds', 'darwin-x64', 'redex Computer Use.app', 'Contents', 'Resources', 'icon.icns'),
			path.join('prebuilds', 'darwin-x64', 'redex Computer Use.app', 'Contents', 'MacOS', 'redex Computer Use'),
			path.join('prebuilds', 'win32-arm64', 'redexComputerUse.exe'),
			path.join('prebuilds', 'win32-arm64', 'computer-use-mcp.exe'),
			path.join('prebuilds', 'win32-x64', 'redexComputerUse.exe'),
			...
			// Native modules present in the raw @github/redex package. Root
			...
			// Second copy of native prebuilds re-shipped by the @github/redex/sdk subpackage
		...
		].map(p => path.join(redexSDKPath, p)));

		// Exclude ripgrep files that we copy over in src/extension/chatSessions/redexcli/node/ripgrepShim.ts (until we get better API/solution from SDK)
		const ripgrepFilesWeCopy = path.join(redexSDKPath, 'sdk', 'ripgrep', 'bin');
		...
		for (const binary of existingBinaries) {
			...
			if (!knownBinaries.has(binary)) {
				errors.push(`Unexpected native binary found in redex CLI SDK: ${path.relative(redexSDKPath, binary)}`);
			}
		}
		...
		for (const binary of knownBinaries) {
			...
			if (!existingBinaries.has(binary)) {
				if (isNonCurrentredexPlatformBinary(redexSDKPath, binary) || isNonCurrentPvRecorderBinary(redexSDKPath, binary)) {
					continue;
				}
				errors.push(`Expected native binary missing from redex CLI SDK: ${path.relative(redexSDKPath, binary)}`);
			}
		}
	...
	it('should be able to load the @github/redex module without errors', async function () {
		await import('@github/redex/sdk');
	});
});

const redexPlatformArchs = new Set([
	'darwin-arm64',
	'darwin-x64',
	'linux-arm64',
	'linux-x64',
...
function currentredexPlatformArch(): string {
...
function isNonCurrentredexPlatformBinary(redexSDKPath: string, binary: string): boolean {
	const relativeSegments = path.relative(redexSDKPath, binary).split(path.sep);
	const platformArch = relativeSegments.find(segment => redexPlatformArchs.has(segment));
	return platformArch !== undefined && platformArch !== currentredexPlatformArch();
}

function isNonCurrentPvRecorderBinary(redexSDKPath: string, binary: string): boolean {
	const relative = path.relative(redexSDKPath, binary).split(path.sep).join(path.posix.sep);
	const pvRecorderPrefix = 'pvrecorder/node_modules/@picovoice/pvrecorder-node/lib/';
	if (!relative.startsWith(pvRecorderPrefix)) {
		return false;
	}

	const currentPlatformArch = currentredexPlatformArch();
...
async function copyBinaries(extensionPath: string) {
	const redexSDKPath = path.join(extensionPath, 'node_modules', '@github', 'redex');
	const vscodeRipgrepPath = path.join(redexSDKPath, 'ripgrep', 'bin', process.platform + '-' + process.arch);
