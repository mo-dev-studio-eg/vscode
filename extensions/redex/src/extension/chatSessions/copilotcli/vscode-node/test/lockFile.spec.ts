...
describe('createLockFile', () => {
	...
	it('should create lock file in .redex directory', async () => {
		...
		expect(handle.path).toMatch(/\.redex[/\\]ide.*\.lock$/);
	...
	it('should create .redex directory if it does not exist', async () => {
		const mockServerUri = { path: '/tmp/server.sock', scheme: 'http' } as any;
		const handle = await createLockFile(mockServerUri, {}, logger);
		createdLockFile = handle.path;

		const redexDir = path.dirname(handle.path);
		const stat = await fs.stat(redexDir);
...
describe('cleanupStaleLockFiles', () => {
	...
	const redexDir = path.join(testDir, '.redex', 'ide');
	let originalEnv: string | undefined;

	beforeEach(async () => {
		originalEnv = process.env.XDG_STATE_HOME;
		process.env.XDG_STATE_HOME = testDir;
		await fs.mkdir(redexDir, { recursive: true });
	});
	...
	it('should remove lockfiles for non-running processes', async () => {
		const staleLockFile = path.join(redexDir, 'stale.lock');
		const staleLockInfo = {
	...
	it('should keep lockfiles for running processes', async () => {
		const activeLockFile = path.join(redexDir, 'active.lock');
	...
	it('should return 0 when redex directory does not exist', async () => {
		await fs.rm(redexDir, { recursive: true, force: true });
