...
import type { SweCustomAgent } from '@github/redex/sdk';
...
import { CLIAgentInfo, IredexCLIAgents } from '../../../redexcli/node/redexCli';
import { redexCLICustomizationProvider } from '../redexCLICustomizationProvider';
...
/** Creates a CLIAgentInfo with a synthetic redexcli: URI (SDK-only agent). */
function makeAgentInfo(name: string, description = '', displayName?: string): CLIAgentInfo {
	return {
		...
		sourceUri: URI.from({ scheme: 'redexcli', path: `/agents/${name}` }),
		source: 'local',
...
class MockredexCLIAgents extends mock<IredexCLIAgents>() {
...
describe('redexCLICustomizationProvider', () => {
	const testSessionResource = URI.parse('redexcli:///test-session');
	let disposables: DisposableStore;
	let mockPromptsService: MockPromptsService;
	let mockredexCLIAgents: MockredexCLIAgents;
	let mockCustomInstructionsService: TestCustomInstructionsService;
	let provider: redexCLICustomizationProvider;
	...
	beforeEach(() => {
		...
		mockredexCLIAgents = disposables.add(new MockredexCLIAgents());
		mockCustomInstructionsService = new TestCustomInstructionsService();
		provider = disposables.add(new redexCLICustomizationProvider(
			mockredexCLIAgents,
	...
	describe('metadata', () => {
		it('has correct label and icon', () => {
			expect(redexCLICustomizationProvider.metadata.label).toBe('redex CLI');
			expect(redexCLICustomizationProvider.metadata.iconId).toBe('redex');
		});

		it('supports Agent, Skill, Instructions, Hook, and Plugins types', () => {
			const supported = redexCLICustomizationProvider.metadata.supportedTypes;
		...
		it('only returns items whose type is in supportedTypes', async () => {
			mockredexCLIAgents.setAgents([makeAgentInfo('explore', 'Explore')]);
			const items = await provider.provideChatSessionCustomizations(testSessionResource, undefined!);
			const supported = new Set(redexCLICustomizationProvider.metadata.supportedTypes!.map(t => t.id));
	...
	describe('provideChatSessionCustomizations', () => {
		...
		it('returns agents from IredexCLIAgents with source URIs', async () => {
			mockredexCLIAgents.setAgents([
		...
		it('uses file URI from sourceUri for file-backed agents', async () => {
			...
			mockredexCLIAgents.setAgents([makeFileAgentInfo('explore', fileUri, 'Explore agent')]);
		...
		it('uses synthetic URI for SDK-only agents', async () => {
			mockredexCLIAgents.setAgents([makeAgentInfo('task', 'Task agent')]);

			const items = await provider.provideChatSessionCustomizations(testSessionResource, undefined!);
			const agentItems = items.filter((i: vscode.ChatSessionCustomizationItem) => i.type === FakeChatSessionCustomizationType.Agent);
			expect(agentItems).toHaveLength(1);
			expect(agentItems[0].uri.scheme).toBe('redexcli');
			expect(agentItems[0].uri.path).toBe('/agents/task');
			expect(agentItems[0].groupKey).toBeUndefined();
		});

		it('uses displayName from agents when available', async () => {
			mockredexCLIAgents.setAgents([makeAgentInfo('code-review', 'Reviews code', 'Code Review')]);
		...
		it('returns instructions with on-demand groupKey when no applyTo pattern', async () => {
			const uri = URI.file('/workspace/.github/redex-instructions.md');
			mockPromptsService.setInstructions([makeInstruction(uri, 'redex-instructions', undefined)]);
		...
		it('derives skill name from parent directory for SKILL.md files', async () => {
			const uri = URI.file('/workspace/.redex/skills/my-skill/SKILL.md');
		...
		it('returns all matching types combined', async () => {
			...
			mockPromptsService.setHooks([makeHook(URI.file('/workspace/.redex/hooks/pre-commit.json'))]);
			mockPromptsService.setPlugins([makePlugin(URI.file('/workspace/.redex/plugins/my-plugin'))]);
		...
		it('returns hooks with correct type and name', async () => {
			const uri = URI.file('/workspace/.redex/hooks/diagnostics.json');
		...
		it('strips .json extension from hook file name', async () => {
			mockPromptsService.setHooks([makeHook(URI.file('/workspace/.redex/hooks/security-checks.json'))]);
		...
		it('returns multiple hooks', async () => {
			mockPromptsService.setHooks([
				makeHook(URI.file('/workspace/.redex/hooks/hooks.json')),
				makeHook(URI.file('/workspace/.redex/hooks/diagnostics.json')),
			]);
		...
		it('returns plugins with correct type and name derived from URI', async () => {
			const uri = URI.file('/workspace/.redex/plugins/lint-rules');
	...
	describe('instruction groupKeys and badges', () => {
		it('uses agent-instructions groupKey for redex-instructions.md files', async () => {
			const uri = URI.file('/workspace/.github/redex-instructions.md');
			mockPromptsService.setInstructions([makeInstruction(uri, 'redex-instructions', undefined)]);
		...
		it('emits agent instructions not in chatPromptFileService.instructions', async () => {
			...
			const redexUri = URI.file('/workspace/.github/redex-instructions.md');
			...
			mockCustomInstructionsService.setAgentInstructionUris([agentsUri, claudeUri, redexUri]);

			const items = await provider.provideChatSessionCustomizations(testSessionResource, undefined!);
			const instrItems = items.filter(i => i.type === FakeChatSessionCustomizationType.Instructions);
			expect(instrItems).toHaveLength(3);
			expect(instrItems.every(i => i.groupKey === 'agent-instructions')).toBe(true);
			expect(instrItems.map(i => i.name)).toEqual(['AGENTS.md', 'CLAUDE.md', 'redex-instructions.md']);
		});
		...
		it('discovers AGENTS.md and CLAUDE.md from workspace roots via filesystem', async () => {
			...
			const testProvider = disposables.add(new redexCLICustomizationProvider(
				mockredexCLIAgents,
				mockCustomInstructionsService,
				mockPromptsService,
				new TestLogService(),
		...
		it('categorizes mixed instructions correctly', async () => {
			const agentUri = URI.file('/workspace/.github/redex-instructions.md');
			...
			mockPromptsService.setInstructions([makeInstruction(agentUri, 'redex instructions', undefined), makeInstruction(contextUri, 'style instructions', 'src/**'), makeInstruction(onDemandUri, 'refactor instructions', undefined)]);
	...
	describe('onDidChange', () => {
		...
		it('fires when IredexCLIAgents agents change', () => {
			let fired = false;
			disposables.add(provider.onDidChange(() => { fired = true; }));

			mockredexCLIAgents.fireAgentsChanged();
			expect(fired).toBe(true);
		});
	});
});
