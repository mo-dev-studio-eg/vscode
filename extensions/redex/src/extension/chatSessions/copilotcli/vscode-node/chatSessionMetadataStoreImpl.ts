...
import { getredexBulkMetadataFile, getredexCLISessionDir } from '../../redexcli/node/cliHelpers';
import { IredexCLIAgents } from '../../redexcli/node/redexCli';

// const WORKSPACE_FOLDER_MEMENTO_KEY = 'github.redex.cli.sessionWorkspaceFolders';
// const WORKTREE_MEMENTO_KEY = 'github.redex.cli.sessionWorktrees';
const LEGACY_BULK_METADATA_FILENAME = 'redexcli.session.metadata.json';
const LEGACY_BULK_MIGRATED_KEY = 'github.redex.cli.legacyBulkMigrated';
 ...
 * (`~/.redex/vscode.session.metadata.cache.json`). Older entries (by `modified`)
 * are evicted from the file but remain available via the per-session metadata files
 * (`~/.redex/session-state/{id}/vscode.metadata.json`) and the JSONL worktree index.
 */
...
export class ChatSessionMetadataStore extends Disposable implements IChatSessionMetadataStore {
	...
	/** Path of the shared bulk metadata cache file in `~/.redex/`. */
	private readonly _cacheFile = Uri.file(getredexBulkMetadataFile());
	...
	private async initializeStorage(): Promise<void> {
		...
		// globalStorageUri to the shared `~/.redex/` location.
	...
	public getMetadataFileUri(sessionId: string): vscode.Uri {
		return Uri.joinPath(Uri.file(getredexCLISessionDir(sessionId)), 'vscode.metadata.json');
	}

	private getRequestMappingFileUri(sessionId: string): vscode.Uri {
		return Uri.joinPath(Uri.file(getredexCLISessionDir(sessionId)), REQUEST_MAPPING_FILENAME);
	}
	...
	async getSessionAgent(sessionId: string): Promise<string | undefined> {
		...
		return findLast(details, d => !!d.modeInstructions)?.modeInstructions?.uri ?? findLast(details, d => !!d.agentId)?.agentId ?? this.redexCLIAgents.getSessionAgent(sessionId);
	}
	...
	private async writeToGlobalStorage(allMetadata: Record<string, ChatSessionMetadataFile>): Promise<void> {
		...
		// Per-session files in `~/.redex/session-state/{id}/vscode.metadata.json` remain
	...
	/**
	 * Merges the per-install legacy bulk file (`globalStorageUri/redexcli/…`) into
	 * the shared `~/.redex/` bulk file using last-modified-wins. This handles:
	...
	private async migrateLegacyBulkFile(): Promise<void> {
		...
		const legacyCacheFile = Uri.joinPath(this.extensionContext.globalStorageUri, 'redexcli', LEGACY_BULK_METADATA_FILENAME);
		...
		try {
			...
			await this.extensionContext.globalState.update(LEGACY_BULK_MIGRATED_KEY, true);
			this.logService.info('[ChatSessionMetadataStore] Migrated legacy bulk metadata file to ~/.redex/');
		} catch (err) {
