 ...
 */
export async function pickSession(logger: ILogger, httpServer: InProcHttpServer, sessionTracker: IredexCLISessionTracker): Promise<string | undefined> {
	const sessionIds = httpServer.getConnectedSessionIds();

	if (sessionIds.length === 0) {
		logger.debug('No connected CLI sessions');
		vscode.window.showWarningMessage(l10n.t('No redex CLI sessions are connected.'));
		return undefined;
	}
