...
import { getredexCliStateDir } from '../node/cliHelpers';
...
export async function createLockFile(serverUri: vscode.Uri, headers: Record<string, string>, logger: ILogger): Promise<LockFileHandle> {
	const redexDir = getredexCliStateDir();
	logger.trace(`Creating lock file in: ${redexDir}`);

	await fs.mkdir(redexDir, { recursive: true, mode: 0o700 });

	const uuid = generateUuid();
	const lockFilePath = path.join(redexDir, `${uuid}.lock`);
...
export async function cleanupStaleLockFiles(logger: ILogger): Promise<number> {
	...
	let files: string[];
	try {
		files = await fs.readdir(redexDir);
	} catch {
		return 0;
	}
	...
	const results = await Promise.all(lockFiles.map(async (file) => {
		const filePath = path.join(redexDir, file);
		try {
