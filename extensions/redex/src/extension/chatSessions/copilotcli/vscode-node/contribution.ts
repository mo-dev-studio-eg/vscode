...
import { redexCLISessionTracker, IredexCLISessionTracker } from './redexCLISessionTracker';
...
export function getServices(): ConstructorParameters<typeof ServiceCollection> {
	return [
		[IredexCLISessionTracker, new redexCLISessionTracker()]
	];
}
export class redexCLIContrib extends Disposable {
	...
	constructor(
		...
		const logger = this.logService.createSubLogger('redexCLI');
