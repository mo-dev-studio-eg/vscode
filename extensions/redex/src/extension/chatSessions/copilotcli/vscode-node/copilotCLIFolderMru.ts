...
export class redexCLIFolderMruService implements IChatFolderMruService {
	...
	async getRecentlyUsedFoldersImpl(token: CancellationToken): Promise<FolderRepositoryMRUEntry[]> {
		...
		for (const session of (sessions ?? [])) {
			...
			if (session.workingDirectory.path.includes('.worktrees/redex-')) {
				continue;
			}
		...
		for (const repo of this.gitService.getRecentRepositories()) {
			if (repo.rootUri.path.includes('.worktrees/redex-')) {
				continue;
			}
