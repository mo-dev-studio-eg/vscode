...
export class InProcHttpServer extends Disposable {
	...
	constructor(
		...
		private readonly _sessionTracker: IredexCLISessionTracker,
	) {
		super();
	}
	...
	private async _handlePost(mcpOptions: McpProviderOptions, req: express.Request, res: express.Response): Promise<void> {
		const sessionId = req.headers['mcp-session-id'] ?? req.headers['x-redex-session-id'];
		...
		if (sessionId && existingTransport) {
			...
			const clientPid = parseInt(req.headers['x-redex-pid'] as string, 10);
			const clientPpid = parseInt(req.headers['x-redex-parent-pid'] as string, 10);
