...
import type { SessionEvent, ToolExecutionCompleteEvent, ToolExecutionStartEvent } from '@github/redex/sdk';
...
import { IredexTool } from '../../../tools/common/toolsRegistry';
...
import { extractChatPromptReferences, getFolderAttachmentPath } from './redexCLIPrompt';
...
type FetchredexCliDocumentationTool = {
	toolName: 'fetch_redex_cli_documentation';
	arguments: Record<string, never>;
};
...
export type ToolInfo = StringReplaceEditorTool | EditTool | CreateTool | ViewTool | InsertTool |
	...
	ShowFileTool | FetchredexCliDocumentationTool | ProposeWorkTool | TaskCompleteTool |
...
function isInstructionAttachmentPath(path: string): boolean {
	...
	return normalizedPath.endsWith('/.github/redex-instructions.md')
		|| (normalizedPath.includes('/.github/instructions/') && normalizedPath.endsWith('.md'));
}

export function isredexCliEditToolCall(data: { toolName: string; arguments?: unknown }): boolean {
...
export function isredexCLIToolThatCouldRequirePermissions(event: ToolExecutionStartEvent): boolean {
	const toolCall = event.data as unknown as ToolCall;
	if (isredexCliEditToolCall(toolCall)) {
		return true;
	}
...
export function processToolExecutionStart(event: ToolExecutionStartEvent, pendingToolInvocations: Map<string, [ChatToolInvocationPart | ChatResponseMarkdownPart | ChatResponseThinkingProgressPart, toolData: ToolCall, parentToolCallId: string | undefined]>, workingDirectory?: URI): ChatToolInvocationPart | ChatResponseMarkdownPart | ChatResponseThinkingProgressPart | undefined {
	const toolInvocation = createredexCLIToolInvocation(event.data as ToolCall, undefined, workingDirectory);
	if (toolInvocation) {
...
export const CLI_TOOL_EVENT_HANDLERS: ToolEventHandlers<ToolCall> = {
	...
	isEditToolCall: isredexCliEditToolCall,
	getEditedUris: getAffectedUrisForEditTool,
};

/**
 * Creates a formatted tool invocation part for redexCLI tools
 */
export function createredexCLIToolInvocation(data: {
...
const ToolFriendlyNameAndHandlers: { [K in ToolCall['toolName']]: [title: string, pre: (invocation: ChatToolInvocationPart, toolCall: ToolCallFor<K>, editId?: string, workingDirectory?: URI) => void, post: (invocation: ChatToolInvocationPart, toolCall: ToolCallFor<K>, result: ToolCallResult, workingDirectory?: URI) => void] } = {
	...
	'fetch_redex_cli_documentation': [l10n.t('Fetch Documentation'), emptyInvocation, genericToolInvocationCompleted],
...
export class FakeToolsService implements IToolsService {
	...
	readonly redexTools = new Map<ToolName, IredexTool<unknown>>();
	...
	modelSpecificTools: IObservable<{ definition: LanguageModelToolDefinition; tool: IredexTool<unknown> }[]> = constObservable([]);
	...
	getredexTool(): IredexTool<unknown> | undefined {
		return undefined;
	}
