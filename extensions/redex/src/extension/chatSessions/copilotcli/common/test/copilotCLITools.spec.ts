...
import {
	buildChatHistoryFromEvents, createredexCLIToolInvocation, enrichToolInvocationWithSubagentMetadata, extractCdPrefix, FakeToolsService, getAffectedUrisForEditTool, isredexCliEditToolCall, isredexCLIToolThatCouldRequirePermissions, isTodoRelatedSqlQuery, processToolExecutionComplete, processToolExecutionStart, RequestIdDetails, stripReminders, ToolCall, updateTodoListFromSqlItems
} from '../redexCLITools';
...
describe('redexCLITools', () => {
	const logger = new TestLogService();
	describe('isredexCliEditToolCall', () => {
		it('detects StrReplaceEditor edit commands (non-view)', () => {
			expect(isredexCliEditToolCall({ toolName: 'str_replace_editor', arguments: { command: 'str_replace', path: '/tmp/a' } })).toBe(true);
			expect(isredexCliEditToolCall({ toolName: 'str_replace_editor', arguments: { command: 'insert', path: '/tmp/a', new_str: '' } })).toBe(true);
			expect(isredexCliEditToolCall({ toolName: 'str_replace_editor', arguments: { command: 'create', path: '/tmp/a' } })).toBe(true);
		});
		it('excludes StrReplaceEditor view command', () => {
			expect(isredexCliEditToolCall({ toolName: 'str_replace_editor', arguments: { command: 'view', path: '/tmp/a' } })).toBe(false);
		});
		it('always true for Edit & Create tools', () => {
			expect(isredexCliEditToolCall({ toolName: 'edit', arguments: { path: '' } })).toBe(true);
			expect(isredexCliEditToolCall({ toolName: 'create', arguments: { path: '' } })).toBe(true);
		});
	});
	...
	describe('buildChatHistoryFromEvents', () => {
		it('builds turns with user and assistant messages including PR metadata', () => {
			...
			if (prPart) {
				...
				expect((prPart as any).command.command).toBe('github.redex.chat.openPullRequestReroute');
			}
			if (markdownPart) {
		...
		it('createredexCLIToolInvocation formats str_replace_editor view with range', () => {
			const invocation = createredexCLIToolInvocation({ toolName: 'str_replace_editor', toolCallId: 'id3', arguments: { command: 'view', path: '/tmp/file.ts', view_range: [1, 5] } }) as ChatToolInvocationPart;
		...
		it('filters out instruction file attachments', () => {
			const events: any[] = [
				{
					type: 'user.message', data: {
						content: 'Hello',
						attachments: [
							{ type: 'file', path: '/workspace/.github/redex-instructions.md', displayName: 'redex-instructions.md' },
			...
			expect(paths).not.toContain('/workspace/.github/redex-instructions.md');
	...
	describe('createredexCLIToolInvocation', () => {
		it('returns undefined for report_intent', () => {
			expect(createredexCLIToolInvocation({ toolName: 'report_intent', toolCallId: 'id', arguments: { intent: '' } })).toBeUndefined();
		});
		it('creates thinking progress part for think tool', () => {
			const part = createredexCLIToolInvocation({ toolName: 'think', toolCallId: 'tid', arguments: { thought: 'Analyzing' } });
			expect(part).toBeInstanceOf(ChatResponseThinkingProgressPart);
		});
		it('formats bash tool invocation with description', () => {
			const part = createredexCLIToolInvocation({ toolName: 'bash', toolCallId: 'b1', arguments: { command: 'ls', description: 'List files' } });
		...
		it('formats str_replace_editor create', () => {
			const part = createredexCLIToolInvocation({ toolName: 'str_replace_editor', toolCallId: 'e1', arguments: { command: 'create', path: '/tmp/x.ts' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		...
		it.skip('formats show_file invocation with path', () => {
			const part = createredexCLIToolInvocation({ toolName: 'show_file', toolCallId: 'sf1', arguments: { path: '/tmp/file.ts' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		...
		it.skip('formats show_file invocation with diff mode', () => {
			const part = createredexCLIToolInvocation({ toolName: 'show_file', toolCallId: 'sf2', arguments: { path: '/tmp/file.ts', diff: true } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		...
		it.skip('formats show_file invocation with view_range', () => {
			const part = createredexCLIToolInvocation({ toolName: 'show_file', toolCallId: 'sf3', arguments: { path: '/tmp/file.ts', view_range: [10, 20] } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			const msg = getInvocationMessageText(part as ChatToolInvocationPart);
		...
		it('formats propose_work invocation with title', () => {
			const part = createredexCLIToolInvocation({ toolName: 'propose_work', toolCallId: 'pw1', arguments: { workType: 'code_change', workTitle: 'Refactor auth', workDescription: 'desc' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		...
		it('returns markdown part for task_complete invocation with summary', () => {
			const part = createredexCLIToolInvocation({ toolName: 'task_complete', toolCallId: 'tc1', arguments: { summary: 'Fixed the bug' } });
		...
		it('returns undefined for task_complete invocation without summary', () => {
			const part = createredexCLIToolInvocation({ toolName: 'task_complete', toolCallId: 'tc2', arguments: {} });
			expect(part).toBeUndefined();
		});
		it('formats ask_user invocation with question', () => {
			const part = createredexCLIToolInvocation({ toolName: 'ask_user', toolCallId: 'au1', arguments: { question: 'Which DB?' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		...
		it('formats ask_user invocation with structured message', () => {
			const part = createredexCLIToolInvocation({
				toolName: 'ask_user',
				toolCallId: 'au2',
				arguments: {
		...
		it('formats skill invocation', () => {
			const part = createredexCLIToolInvocation({ toolName: 'skill', toolCallId: 'sk1', arguments: { skill: 'pdf' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			expect(getInvocationMessageText(part as ChatToolInvocationPart)).toContain('pdf');
		});
		it('formats task invocation with description', () => {
			const part = createredexCLIToolInvocation({ toolName: 'task', toolCallId: 't1', arguments: { description: 'Run tests', prompt: 'Run all unit tests', agent_type: 'task' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			expect(getInvocationMessageText(part as ChatToolInvocationPart)).toContain('Run tests');
		});
		it('formats read_agent invocation', () => {
			const part = createredexCLIToolInvocation({ toolName: 'read_agent', toolCallId: 'ra1', arguments: { agent_id: 'agent-123' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			expect(getInvocationMessageText(part as ChatToolInvocationPart)).toContain('agent-123');
		});
		it('formats exit_plan_mode invocation', () => {
			const part = createredexCLIToolInvocation({ toolName: 'exit_plan_mode', toolCallId: 'ep1', arguments: { summary: 'Plan summary' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			expect(getInvocationMessageText(part as ChatToolInvocationPart)).toMatch(/plan/i);
		});
		it('formats sql invocation with description', () => {
			const part = createredexCLIToolInvocation({ toolName: 'sql', toolCallId: 'sq1', arguments: { description: 'Query todos', query: 'SELECT * FROM todos' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			expect(getInvocationMessageText(part as ChatToolInvocationPart)).toContain('Query todos');
		});
		it('formats lsp invocation with file', () => {
			const part = createredexCLIToolInvocation({ toolName: 'lsp', toolCallId: 'lsp1', arguments: { operation: 'goToDefinition', file: '/tmp/app.ts', line: 10, character: 5 } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			const msg = getInvocationMessageText(part as ChatToolInvocationPart);
		...
		it('formats lsp invocation without file', () => {
			const part = createredexCLIToolInvocation({ toolName: 'lsp', toolCallId: 'lsp2', arguments: { operation: 'workspaceSymbol', query: 'MyClass' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		...
		it('formats create_pull_request invocation', () => {
			const part = createredexCLIToolInvocation({ toolName: 'create_pull_request', toolCallId: 'pr1', arguments: { title: 'Fix auth flow', description: 'Summary of changes', draft: false } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		...
		it('formats search_code_subagent invocation', () => {
			const part = createredexCLIToolInvocation({ toolName: 'search_code_subagent', toolCallId: 'sc1', arguments: { query: 'find auth middleware' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		...
		it('formats store_memory invocation', () => {
			const part = createredexCLIToolInvocation({ toolName: 'store_memory', toolCallId: 'sm1', arguments: { subject: 'naming', fact: 'Use camelCase', citations: 'src/foo.ts:1', reason: 'consistency', category: 'general' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			expect(getInvocationMessageText(part as ChatToolInvocationPart)).toContain('naming');
		});
		it('creates invocation for fetch_redex_cli_documentation', () => {
			const part = createredexCLIToolInvocation({ toolName: 'fetch_redex_cli_documentation', toolCallId: 'fd1', arguments: {} });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		});
		it('creates invocation for list_agents', () => {
			const part = createredexCLIToolInvocation({ toolName: 'list_agents', toolCallId: 'la1', arguments: {} });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		});
		it('creates invocation for list_bash', () => {
			const part = createredexCLIToolInvocation({ toolName: 'list_bash', toolCallId: 'lb1', arguments: {} });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		});
		it('creates invocation for list_powershell', () => {
			const part = createredexCLIToolInvocation({ toolName: 'list_powershell', toolCallId: 'lp1', arguments: {} });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		});
		it('creates invocation for gh-advisory-database', () => {
			const part = createredexCLIToolInvocation({ toolName: 'gh-advisory-database', toolCallId: 'gh1', arguments: { dependencies: [{ name: 'lodash', version: '4.17.0', ecosystem: 'npm' }] } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		});
		it('creates invocation for parallel_validation', () => {
			const part = createredexCLIToolInvocation({ toolName: 'parallel_validation', toolCallId: 'pv1', arguments: {} });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		});
		it('formats apply_patch invocation', () => {
			const part = createredexCLIToolInvocation({ toolName: 'apply_patch', toolCallId: 'ap1', arguments: { input: '*** Begin Patch\n*** End Patch' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		...
		it('formats write_agent invocation with agent_id', () => {
			const part = createredexCLIToolInvocation({ toolName: 'write_agent', toolCallId: 'wa1', arguments: { agent_id: 'agent-42', message: 'Hello agent' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			expect(getInvocationMessageText(part as ChatToolInvocationPart)).toContain('agent-42');
		});
		it('creates invocation for mcp_reload', () => {
			const part = createredexCLIToolInvocation({ toolName: 'mcp_reload', toolCallId: 'mr1', arguments: {} });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		});
		it('formats mcp_validate invocation with path', () => {
			const part = createredexCLIToolInvocation({ toolName: 'mcp_validate', toolCallId: 'mv1', arguments: { path: '/home/user/.redex/config/mcp-config.json' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		...
		it('formats tool_search_tool_regex invocation with pattern', () => {
			const part = createredexCLIToolInvocation({ toolName: 'tool_search_tool_regex', toolCallId: 'ts1', arguments: { pattern: 'search.*file' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		...
		it('creates invocation for codeql_checker', () => {
			const part = createredexCLIToolInvocation({ toolName: 'codeql_checker', toolCallId: 'cq1', arguments: {} });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		});
	});
	...
	describe('isredexCLIToolThatCouldRequirePermissions', () => {
		...
		it('returns true for edit tool calls (create, edit)', () => {
			expect(isredexCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'create', toolCallId: '1', arguments: { path: '/tmp/a' } }))).toBe(true);
			expect(isredexCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'edit', toolCallId: '2', arguments: { path: '/tmp/b' } }))).toBe(true);
		});

		it('returns true for str_replace_editor non-view commands', () => {
			expect(isredexCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'str_replace_editor', toolCallId: '3', arguments: { command: 'str_replace', path: '/tmp/a' } }))).toBe(true);
		});

		it('returns true for bash and powershell', () => {
			expect(isredexCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'bash', toolCallId: '4', arguments: { command: 'echo hi' } }))).toBe(true);
			expect(isredexCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'powershell', toolCallId: '5', arguments: { command: 'echo hi' } }))).toBe(true);
		});

		it('returns true for view tool', () => {
			expect(isredexCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'view', toolCallId: '6', arguments: { path: '/tmp/a' } }))).toBe(true);
		});

		it('returns false for MCP tools even if tool name matches', () => {
			expect(isredexCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'bash', toolCallId: '7', mcpServerName: 'my-server', arguments: { command: 'echo' } }))).toBe(false);
			expect(isredexCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'view', toolCallId: '8', mcpServerName: 'my-server', arguments: { path: '/tmp' } }))).toBe(false);
		});

		it('returns false for non-permission tools like think, report_intent, glob', () => {
			expect(isredexCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'think', toolCallId: '9', arguments: { thought: 'hmm' } }))).toBe(false);
			expect(isredexCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'report_intent', toolCallId: '10', arguments: {} }))).toBe(false);
			expect(isredexCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'glob', toolCallId: '11', arguments: { pattern: '*.ts' } }))).toBe(false);
			expect(isredexCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'grep', toolCallId: '12', arguments: { pattern: 'foo' } }))).toBe(false);
		});

		it('returns false for str_replace_editor view command (not an edit)', () => {
			expect(isredexCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'str_replace_editor', toolCallId: '13', arguments: { command: 'view', path: '/tmp/a' } }))).toBe(false);
		});
	});
