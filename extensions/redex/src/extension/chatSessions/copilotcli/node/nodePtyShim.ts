...
/**
 * Copies the node-pty files from VS Code's installation into a @github/redex location.
 *
 * MUST be called before any `import('@github/redex/sdk')` or `import('@github/redex')`.
 *
 * @github/redex bundles the node-pty code and it is no longer possible to shim the package
...
export async function copyNodePtyFiles(extensionPath: string, sourceNodePtyPath: string, logService: ILogService): Promise<void> {
	const nodePtyDir = path.join(extensionPath, 'node_modules', '@github', 'redex', 'sdk', 'prebuilds', process.platform + '-' + process.arch);
