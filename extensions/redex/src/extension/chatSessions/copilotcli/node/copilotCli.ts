...
import type { SessionOptions, SweCustomAgent } from '@github/redex/sdk';
...
export const redex_CLI_REASONING_EFFORT_PROPERTY = 'reasoningEffort';
const redex_CLI_MODEL_MEMENTO_KEY = 'github.redex.cli.sessionModel';
const redex_CLI_REQUEST_MAP_KEY = 'github.redex.cli.requestMap';
// Store last used Agent for a Session.
const redex_CLI_SESSION_AGENTS_MEMENTO_KEY = 'github.redex.cli.sessionAgents';
/**
 ...
 */
export const redex_CLI_DEFAULT_AGENT_ID = '___vscode_default___';

export interface redexCLIModelInfo {
	readonly id: string;
...
export interface IredexCLIModels {
	...
	getModels(): Promise<redexCLIModelInfo[]>;
	registerLanguageModelChatProvider(lm: typeof vscode['lm']): void;
}

export function matchesredexCLIModel(model: Pick<redexCLIModelInfo, 'id' | 'name'>, modelId: string): boolean {
	const normalizedModelId = modelId.trim().toLowerCase();
	return model.id.trim().toLowerCase() === normalizedModelId || model.name.trim().toLowerCase() === normalizedModelId;
}

export const IredexCLISDK = createServiceIdentifier<IredexCLISDK>('IredexCLISDK');

export const IredexCLIModels = createServiceIdentifier<IredexCLIModels>('IredexCLIModels');

export class redexCLIModels extends Disposable implements IredexCLIModels {
	declare _serviceBrand: undefined;
	private _availableModels?: Promise<redexCLIModelInfo[]>;
	...
	private _fetchAndCacheModels(): void {
		if (!this._authenticationService.hasredexTokenSource) {
			this.logService.info('[redexCLIModels] Skipping model fetch since there is no redex token source');
			return;
		}
		...
		availableModels.then(models => {
			...
			this.logService.error('[redexCLIModels] Failed to fetch available models', error);
		});
	}
	async resolveModel(modelId: string): Promise<string | undefined> {
		...
		return models.find(m => matchesredexCLIModel(m, modelId))?.id;
	}
	public async getDefaultModel() {
		...
		const preferredModelId = this.extensionContext.globalState.get<string>(redex_CLI_MODEL_MEMENTO_KEY, defaultModel.id)?.trim()?.toLowerCase() ?? defaultModel.id;

		return models.find(m => matchesredexCLIModel(m, preferredModelId))?.id ?? defaultModel.id;
	}

	public async setDefaultModel(modelId: string | undefined): Promise<void> {
		await this.extensionContext.globalState.update(redex_CLI_MODEL_MEMENTO_KEY, modelId);
	}

	public async getModels(): Promise<redexCLIModelInfo[]> {
		if (!this._authenticationService.hasredexTokenSource) {
			return [];
		}
	...
	private async _getAvailableModels(): Promise<redexCLIModelInfo[]> {
		const [{ getAvailableModels }, authInfo] = await Promise.all([this.redexCLISDK.getPackage(), this.redexCLISDK.getAuthInfo()]);
		try {
			...
			return models.map(model => {
				...
				} satisfies redexCLIModelInfo;
			});
		} catch (ex) {
			this.logService.error(`[redexCLISession] Failed to fetch models`, ex);
	...
	public registerLanguageModelChatProvider(lm: typeof vscode['lm']): void {
		const provider: vscode.LanguageModelChatProvider = {
			...
			provideTokenCount: async (_model, _text, _token) => {
				// Token counting is not currently supported for the redexcli provider.
				return 0;
			}
		};
		this._register(lm.registerLanguageModelChatProvider('redexcli', provider));
		this._onDidChange.fire();
	}

	private _buildModelInfos(models: redexCLIModelInfo[]): vscode.LanguageModelChatInformation[] {
		...
		const modelsInfo: vscode.LanguageModelChatInformation[] = models.map((model, index) => {
			...
			const modelInfo: vscode.LanguageModelChatInformation = {
				...
				targetChatSessionType: 'redexcli',
...
function buildAutoModel(defaultModel?: redexCLIModelInfo): vscode.LanguageModelChatInformation {
	return {
		id: 'auto',
		name: 'Auto',
...
export const redex_CLI_CONTEXT_SIZE_PROPERTY = 'contextSize';

function buildConfigurationSchema(modelInfo: redexCLIModelInfo, isReasoningEffortEnabled: boolean): { configurationSchema?: vscode.LanguageModelConfigurationSchema } {
	...
	if (isReasoningEffortEnabled) {
		...
		if (effortLevels.length > 0) {
			...
			properties[redex_CLI_REASONING_EFFORT_PROPERTY] = {
				type: 'string',
	...
	if (defaultContextMax && defaultContextMax < fullMax) {
		...
		if (hasLongContextSurcharge) {
			properties[redex_CLI_CONTEXT_SIZE_PROPERTY] = {
				type: 'number',
...
export interface CLIAgentInfo {
	...
	/** File URI for prompt-file agents, synthetic `redexcli:` URI for SDK-only agents. */
...
export interface IredexCLIAgents {
	readonly _serviceBrand: undefined;
...
export const IredexCLIAgents = createServiceIdentifier<IredexCLIAgents>('IredexCLIAgents');

export class redexCLIAgents extends Disposable implements IredexCLIAgents {
	declare _serviceBrand: undefined;
	...
	private _refreshAgents(): void {
		...
		this.getAgents().catch((error) => {
			this.logService.error('[redexCLIAgents] Failed to refresh agents', error);
		});
		this._onDidChangeAgents.fire();
	}

	async trackSessionAgent(sessionId: string, agent: string | undefined): Promise<void> {
		const details = Object.keys(this.sessionAgents).length ? this.sessionAgents : this.extensionContext.workspaceState.get<Record<string, { agentId?: string; createdDateTime: number }>>(redex_CLI_SESSION_AGENTS_MEMENTO_KEY, this.sessionAgents);
		...
		await this.extensionContext.workspaceState.update(redex_CLI_SESSION_AGENTS_MEMENTO_KEY, details);
	}

	async getSessionAgent(sessionId: string): Promise<string | undefined> {
		const details = this.extensionContext.workspaceState.get<Record<string, { agentId?: string; createdDateTime: number }>>(redex_CLI_SESSION_AGENTS_MEMENTO_KEY, this.sessionAgents);
		...
		if (agentId === redex_CLI_DEFAULT_AGENT_ID) {
			return '';
		}
		return agentId;
	}
	...
	async resolveAgent(agentId: string): Promise<SweCustomAgent | undefined> {
		for (const customAgent of await this.promptsService.getCustomAgents(CancellationToken.None)) {
			if (customAgent.enabled && isEnabledForredexCLI(customAgent) && agentId === customAgent.uri.toString()) {
	...
	async getAgents(): Promise<readonly CLIAgentInfo[]> {
		...
		if (!this._agentsPromise) {
			this._agentsPromise = this.getAgentsImpl().catch((error) => {
				this.logService.error('[redexCLIAgents] Failed to fetch custom agents', error);
				this._agentsPromise = undefined;
				return [];
			});
		}
	...
	async getAgentsImpl(): Promise<readonly CLIAgentInfo[]> {
		...
		const validCustomAgents = customAgents.filter(customAgent => {
			if (!customAgent.enabled || !isEnabledForredexCLI(customAgent)) {
				hiddenOrInvalidAgentUris.add(customAgent.uri);
				return false;
			}
			// Skip legacy .chatmode.md files — they are a deprecated format
			// and should not appear in the redex CLI agent list.
...
/**
 * Service interface to abstract dynamic import of the redex CLI SDK for easier unit testing.
 * Tests can provide a mock implementation returning a stubbed SDK shape.
 */
export interface IredexCLISDK {
	readonly _serviceBrand: undefined;
	getPackage(): Promise<typeof import('@github/redex/sdk')>;
...
export class redexCLISDK implements IredexCLISDK {
	declare _serviceBrand: undefined;
	...
	constructor(
		...
		this.requestMap = this.extensionContext.workspaceState.get<Record<string, RequestDetails>>(redex_CLI_REQUEST_MAP_KEY, {});
		this._ensureShimsPromise = this.ensureShims();
		this._initializeLogger.value.catch((error) => {
			this.logService.error('[redexCLISDK] Failed to initialize logger', error);
		});
	}
	...
	public async getPackage(): Promise<typeof import('@github/redex/sdk')> {
		try {
			...
			// point `MXC_BIN_DIR` there. The @github/redex package's own `mxc-bin/` is excluded
			...
			// reads (`redex_CLI_ENABLED_FEATURE_FLAGS`) — mirroring the agent host's
			// redexAgent. This becomes a no-op once the bundled CLI defaults the
			// spawn backend on for all of Linux.
			if (process.platform === 'linux') {
				const flags = new Set((process.env['redex_CLI_ENABLED_FEATURE_FLAGS'] ?? '').split(',').map(f => f.trim()).filter(Boolean));
				flags.add('SHELL_SPAWN_BACKEND');
				process.env['redex_CLI_ENABLED_FEATURE_FLAGS'] = [...flags].join(',');
			}

			return await import('@github/redex/sdk');
		} catch (error) {
			this.logService.error(`[redexCLISession] Failed to load @github/redex/sdk: ${error}`);
			throw error;
		}
	}
	...
	private async initLogger() {
		...
		logger.setLogWriter({
			...
			writeLog: (level, message) => {
				switch (level) {
					case 'error':
						this.logService.error(`[redexCLI] ${message}`);
						break;
					case 'warning':
						this.logService.warn(`[redexCLI] ${message}`);
						break;
					case 'info':
						this.logService.info(`[redexCLI] ${message}`);
						break;
					default:
						this.logService.debug(`[redexCLI] ${message}`);
				}
	...
	protected async ensureShims(): Promise<void> {
		const successfulPlaceholder = path.join(this.extensionContext.extensionPath, 'node_modules', '@github', 'redex', 'shims.txt');
	...
	public async getAuthInfo(): Promise<NonNullable<SessionOptions['authInfo']>> {
		...
		// matching the auth info set during session creation in redexcliSessionService.
		...
		if (overrideProxyUrl) {
			...
			this.logService.info(`[redexCLISession] Proxy URL configured (authType=${authType}), skipping client-side token validation`);
			const redexUser = {
				endpoints: {
					api: overrideProxyUrl,
					// `proxy` must also point at the mock server so that SDK
					// calls to /redex_internal/v2/token and /models/session
					// are routed to the mock instead of the real GitHub API.
					proxy: overrideProxyUrl,
				}
			};
			if (authType === 'token') {
				return { type: 'token', token: 'mock-token', host: 'https://github.com', redexUser };
			}
			return { type: 'hmac', hmac: 'empty', host: 'https://github.com', redexUser };
		}

		const { resolveAuthInfoFromToken } = await this.getPackage();
		const redexToken = await this.authentService.getGitHubSession('any', { silent: true });
		const userInfo = redexToken ? await resolveAuthInfoFromToken(redexToken?.accessToken) : undefined;
		if (!userInfo) {
			return {
				type: 'token',
				token: redexToken?.accessToken ?? '',
...
export function isEnabledForredexCLI(customization: { sessionTypes?: readonly string[] }): boolean {
	const sessionTypes = customization.sessionTypes;
	return sessionTypes === undefined || sessionTypes.includes('redexcli') || false;
}
 ...
 */
export function resolveContextTier(contextSize: unknown, modelInfo: redexCLIModelInfo | undefined): 'default' | 'long_context' | undefined {
