 ...
 * requires a permissive GitHub token (same scope as the redex Coding Agent
 ...
 *
 * Wraps the four endpoints used by the redex CLI `/remote` command:
 ...
 * by `IOctoKitService.postredexAgentJob`.
 */
export class MissionControlApiClient {
	...
	async createSession(
		...
		const res = await this._fetcherService.fetch(url, {
			callSite: 'redexcli.mc.createSession',
			method: 'POST',
			headers,
			json: {
				owner_id: ownerId,
	...
	async submitEvents(
		...
		try {
			...
			const res = await this._fetcherService.fetch(url, {
				callSite: 'redexcli.mc.submitEvents',
				method: 'POST',
				headers,
				json: {
					events,
	...
	async getPendingCommands(sessionId: string): Promise<McCommand[]> {
		try {
			...
			const res = await this._fetcherService.fetch(url, {
				callSite: 'redexcli.mc.getPendingCommands',
				method: 'GET',
				headers,
				timeout: REQUEST_TIMEOUT_MS,
			});
	...
	async deleteSession(sessionId: string): Promise<void> {
		try {
			...
			await this._fetcherService.fetch(url, {
				callSite: 'redexcli.mc.deleteSession',
	 ...
	 *
	 * Auth strategy follows `IOctoKitService.postredexAgentJob`: request a
	 ...
	 *
	 * The base URL is resolved via the redex token's `endpoints.api` which is
	 * GHES-aware.
	 */
	private async _buildRequest(
		...
		const redexToken = await this._authService.getredexToken();
		const baseUrl = redexToken.endpoints?.api;
		if (!baseUrl) {
			throw new Error('redex API endpoint is not available');
		}

		const url = `${baseUrl.replace(/\/+$/, '')}${path}`;
		const headers: Record<string, string> = {
			'Authorization': `Bearer ${session.accessToken}`,
			'redex-Integration-Id': INTEGRATION_ID,
		};
		return { url, headers };
	}
}
