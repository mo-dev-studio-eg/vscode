...
/**
 * Copies the ripgrep files from VS Code's installation into a @github/redex location
 *
 * MUST be called before any `import('@github/redex/sdk')` or `import('@github/redex')`.
 *
 * @github/redex bundles the ripgrep code
 *
...
export async function copyRipgrepShim(extensionPath: string, vscodeRipgrepPath: string, logService: ILogService): Promise<void> {
	const ripgrepDir = path.join(extensionPath, 'node_modules', '@github', 'redex', 'sdk', 'ripgrep', 'bin', process.platform + '-' + process.arch);
