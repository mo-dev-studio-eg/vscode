...
import type { Attachment, PermissionRequestedEvent } from '@github/redex/sdk';
...
import { getAffectedUrisForEditTool, getCdPresentationOverrides, ToolCall } from '../common/redexCLITools';
import type { Session } from '../common/utils';
import { getredexCLISessionStateDir } from './cliHelpers';
...
export async function handleReadPermission(
	...
	imageSupport: IredexCLIImageSupport,
	...
	if (isFileFromSessionWorkspace(file, workspaceInfo)) {
		logService.trace(`[redexCLISession] Auto Approving request to read file in session workspace ${permissionRequest.path}`);
		return { kind: 'approve-once' };
	}

	if (workspaceService.getWorkspaceFolder(file)) {
		logService.trace(`[redexCLISession] Auto Approving request to read workspace file ${permissionRequest.path}`);
		return { kind: 'approve-once' };
	}

	// Auto-approve reads of internal session resources (e.g. plan.md).
	const sessionDir = Uri.joinPath(Uri.file(getredexCLISessionStateDir()), sessionId);
	if (extUriBiasedIgnorePathCase.isEqualOrParent(file, sessionDir)) {
		logService.trace(`[redexCLISession] Auto Approving request to read redex CLI session resource ${permissionRequest.path}`);
		return { kind: 'approve-once' };
	}
	...
	if (attachments.some(attachment => attachment.type === 'file' && isEqual(Uri.file(attachment.path), file))) {
		logService.trace(`[redexCLISession] Auto Approving request to read attached file ${permissionRequest.path}`);
		return { kind: 'approve-once' };
	}
...
export async function handleWritePermission(
	...
	if (workingDirectory && editFile) {
		...
		if (autoApprove) {
			logService.trace(`[redexCLISession] Auto Approving request ${editFile.fsPath}`);
	...
	if (editFile && extUriBiasedIgnorePathCase.isEqualOrParent(editFile, sessionDir)) {
		logService.trace(`[redexCLISession] Auto Approving request to write to redex CLI session resource ${editFile.fsPath}`);
		return { kind: 'approve-once' };
	}
...
async function invokeConfirmationTool(
	...
	try {
		...
		logService.error(error, `[redexCLISession] Permission request error`);
	}
...
export async function showInteractivePermissionPrompt(
	...
	const toolParams: CoreConfirmationToolParams = {
		tool: ToolName.CoreConfirmationTool,
		input: {
			title: 'redex CLI Permission Request',
...
async function trackEditIfNeeded(editTracker: ExternalEditTracker, toolCall: ToolCall | undefined, editFile: URI, stream: ChatResponseStream | undefined, logService: ILogService): Promise<void> {
	if (toolCall && stream) {
		try {
			...
			logService.error(error, `[redexCLISession] Failed to track edit for toolCallId ${toolCall.toolCallId}`);
		}
	}
}
...
async function getFileEditConfirmationToolParams(instaService: IInstantiationService, permissionRequest: PermissionRequest, toolCall?: ToolCall | undefined, workingDirectory?: URI): Promise<CoreConfirmationToolParams | undefined> {
	...
	// Extract file name from the toolCall, thats more accurate, (as recommended by redex cli sdk maintainers).
