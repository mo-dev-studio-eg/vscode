...
import type { TodoItem, TodoSqlWorkerApi } from './redexCLITodoWorker';
...
export class TodoSqlQuery {
	...
	private ensureWorker(): RpcProxy<TodoSqlWorkerApi> {
		if (!this._proxy) {
			this._worker = new WorkerWithRpcProxy<TodoSqlWorkerApi>(
				join(__dirname, 'redexCLITodoWorker.js')
			);
	 ...
	 * @param sessionDir - The session state directory (e.g. ~/.redex/session-state/{session-id})
