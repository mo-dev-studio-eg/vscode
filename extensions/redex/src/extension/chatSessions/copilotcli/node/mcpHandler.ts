...
export interface IredexCLIMCPHandler {
	readonly _serviceBrand: undefined;
	loadMcpConfig(sessionUri: URI): Promise<{ mcpConfig: Record<string, MCPServerConfig> | undefined; disposable: IDisposable }>;
}

export const IredexCLIMCPHandler = createServiceIdentifier<IredexCLIMCPHandler>('IredexCLIMCPHandler');

export class redexCLIMCPHandler implements IredexCLIMCPHandler {
	...
	private async loadMcpConfigWithGateway(sessionUri: URI): Promise<{ mcpConfig: Record<string, MCPServerConfig> | undefined; disposable: IDisposable }> {
		...
		try {
			...
			if (gateway) {
				...
				this.logService.warn('[redexCLIMCPHandler]   gateway failed to start');
				disposable.dispose();
			}
		} catch (error) {
			this.logService.warn(`[redexCLIMCPHandler]   gateway error: ${error}`);
		}
	...
	private normalizeServerName(originalName: string): string | undefined {
		...
		if (!normalized) {
			this.logService.error(`[redexCLIMCPHandler] Failed to normalize server name '${originalName}' - result is empty`);
			return undefined;
		}

		if (normalized !== originalName) {
			this.logService.trace(`[redexCLIMCPHandler] Normalized server '${originalName}' to '${normalized}'`);
		}
	...
	private async addBuiltInGitHubServer(config: Record<string, MCPServerConfig>): Promise<void> {
		try {
			...
			if (!definitions || definitions.length === 0) {
				this.logService.trace('[redexCLIMCPHandler] No GitHub MCP server definitions available.');
				return;
			}
			...
			this.logService.trace('[redexCLIMCPHandler] Added built-in GitHub MCP server.');
		} catch (error) {
			this.logService.warn(`[redexCLIMCPHandler] Failed to add built-in GitHub MCP server: ${error}`);
		}
	}
}
