...
import type { Attachment, SendOptions, SessionOptions, ToolExecutionCompleteEvent, ToolExecutionStartEvent } from '@github/redex/sdk';
...
import { redexChatAttr, GenAiAttr, GenAiOperationName, GenAiProviderName, IOTelService, ISpanHandle, resolveWorkspaceOTelMetadata, SpanKind, SpanStatusCode, TraceContext, truncateForOTel, workspaceMetadataToOTelAttributes } from '../../../../platform/otel/common/index';
...
import { clearTodoList, enrichToolInvocationWithSubagentMetadata, isredexCliEditToolCall, isredexCLIToolThatCouldRequirePermissions, isTodoRelatedSqlQuery, processToolExecutionComplete, processToolExecutionStart, stripReminders, ToolCall, updateTodoListFromSqlItems } from '../common/redexCLITools';
import { clearPendingredexCLIRequestContext, setPendingredexCLIRequestContext } from '../common/pendingRequestContext';
import { LocalSession, Session, SessionIdForCLI } from '../common/utils';
import { getredexCLISessionDir } from './cliHelpers';
import type { redexCliBridgeSpanProcessor } from './redexCliBridgeSpanProcessor';
import { IredexCLIImageSupport } from './redexCLIImageSupport';
...
/**
 * Known commands that can be sent to a redexCLI session instead of a free-form prompt.
 */
export type redexCLICommand = 'compact' | 'plan' | 'fleet' | 'remote';

/**
 * The set of all known redexCLI commands.  Used by callers that need to
 * distinguish a slash-command from a regular prompt at runtime.
 */
export const redexCLICommands: readonly redexCLICommand[] = ['compact', 'plan', 'fleet', 'remote'] as const;

export class redexCLIQuotaExceededError extends Error {
	constructor(message: string) {
		super(message);
		this.name = 'redexCLIQuotaExceededError';
	}
}

/**
 * Shared Mission Control state keyed by SDK session ID.
 * redexCLISession instances are recreated per request, so MC state
...
class redexCLIResponseStreamRouter {
	...
	private static _isClosedStreamError(error: unknown): boolean {
		...
		return message.toLowerCase().includes(redexCLIResponseStreamRouter._closedStreamErrorFragment);
	}
	...
	private _handleCallError(error: unknown, method: string, args: unknown[], stream: vscode.ChatResponseStream): unknown {
		if (redexCLIResponseStreamRouter._isClosedStreamError(error)) {
			if (this._stream === stream) {
				this._stream = undefined;
			}
			this._logService.trace(`[redexCLISession] Dropping ${method} for closed response stream in session ${this._sessionId}`);
			return this._fallback(method, args);
		}
		throw error;
	}
 ...
 */
export type redexCLISessionInput =
	| { readonly prompt: string; readonly source?: SendOptions['source'] }
	| { readonly prompt?: string; readonly command: redexCLICommand; readonly source?: SendOptions['source'] };

function getPromptLabel(input: redexCLISessionInput): string {
	if ('command' in input) {
...
function getRemoteControlArgs(input: redexCLISessionInput): string {
...
export interface IredexCLISession extends IDisposable {
...
export class redexCLISession extends DisposableStore implements IredexCLISession {
	...
	private readonly _streamRouter: redexCLIResponseStreamRouter;
	...
	private _bridgeProcessor: redexCliBridgeSpanProcessor | undefined;
	...
	setBridgeProcessor(bridge: redexCliBridgeSpanProcessor | undefined): void {
	...
	constructor(
		...
		@IredexCLIImageSupport private readonly _imageSupport: IredexCLIImageSupport,
		...
		this._streamRouter = new redexCLIResponseStreamRouter(this.logService, this.sessionId);
	...
	private _applyEffectiveSandboxConfig(bypassApprovals: boolean): void {
		...
		try {
			...
			this.logService.error(error, '[redexCLISession] Failed to update sandbox config for request');
		}
	}
	...
	public async handleRequest(
		...
		const capturingToken = new CapturingToken(`redex CLI | ${promptLabel}`, 'worktree', undefined, undefined, this.sessionId);
	...
	private async _handleRequestImpl(
		...
		return this._otelService.startActiveSpan(
			'invoke_agent redexcli',
			{
				kind: SpanKind.INTERNAL,
				attributes: {
					[GenAiAttr.OPERATION_NAME]: GenAiOperationName.INVOKE_AGENT,
					[GenAiAttr.AGENT_NAME]: 'redexcli',
					[GenAiAttr.PROVIDER_NAME]: GenAiProviderName.GITHUB,
					[GenAiAttr.CONVERSATION_ID]: this.sessionId,
					[redexChatAttr.SESSION_ID]: this.sessionId,
					[redexChatAttr.CHAT_SESSION_ID]: this.sessionId,
					...(modelId ? { [GenAiAttr.REQUEST_MODEL]: modelId } : {}),
					[redexChatAttr.USER_REQUEST]: truncateForOTel(promptLabel, this._otelService.config.maxAttributeSizeChars),
	...
	private async _handleRequestImplInner(
		...
		this.logService.info(`[redexCLISession] Invoking session ${this.sessionId}`);
		const disposables = new DisposableStore();
		const logStartTime = Date.now();
			...
			const abortHandle = setTimeout(() => {
				...
				if (!wroteResponseContent) {
					try {
						...
						this.logService.trace(`[redexCLISession] Unable to mark interrupted response: ${error instanceof Error ? error.message : String(error)}`);
					}
				}
				this._sdkSession.abort();
			}, 250);
		...
		// chat debug logs view for the in-process redex CLI experience.
		...
		// metrics, and the agent response for the in-process redex CLI experience (the SDK performs
		...
		clearTodoList(this._toolsService, request.toolInvocationToken, token).catch(err => {
			this.logService.error(err, '[redexCLISession] Failed to clear todo list at start of session');
		});
		/**
		...
		try {
			...
			disposables.add(toDisposable(this._sdkSession.on('permission.requested', async (event) => {
				...
				if (!isSandboxBypassShell && (effectivePermissionLevel === 'autoApprove' || effectivePermissionLevel === 'autopilot')) {
					this.logService.trace(`[redexCLISession] Auto Approving ${permissionRequest.kind} request (permission level: ${effectivePermissionLevel})`);
				...
				if (!isSandboxBypassShell && permissionRequest.kind === 'shell' && this._sandboxEnabled) {
					this.logService.trace(`[redexCLISession] Auto Approving shell request (sandbox is enabled)`);
					this._sdkSession.respondToPermission(requestId, { kind: 'approve-once' });
					return;
				}
				...
				try {
					...
					this.logService.error(error, `[redexCLISession] Error handling permission request of kind ${permissionRequest.kind}`);
					flushPendingInvocationMessageForToolCallId(permissionRequest.toolCallId);
			...
			if (shouldHandleExitPlanModeRequests) {
				disposables.add(toDisposable(this._sdkSession.on('exit_plan_mode.requested', async (event) => {
					...
					try {
						...
						this.logService.error(error, '[redexCLISession] Error handling exit plan mode');
			...
			disposables.add(toDisposable(this._sdkSession.on('assistant.usage', (event) => {
				...
				// Accumulate per-turn credits from SDK redexUsage data
				const redexUsage = (event.data as unknown as Record<string, unknown>).redexUsage;
				let redexUsageNanoAiu: number | undefined;
				if (redexUsage && typeof redexUsage === 'object') {
					const { totalNanoAiu } = redexUsage as { totalNanoAiu?: number };
					if (typeof totalNanoAiu === 'number') {
						redexUsageNanoAiu = totalNanoAiu;
						this._chatQuotaService.setLastredexUsage(totalNanoAiu, request.id);
					}
				}
				// Sync the live per-category quota state the SDK reports (internal-only field) so the
				// quota UI stays current without a separate `redex_internal/user` fetch. This mirrors
				// the extension-host chat path, which processes `redex_quota_snapshots` from CAPI.
			...
			disposables.add(toDisposable(this._sdkSession.on('tool.execution_start', (event) => {
				...
				if (isredexCliEditToolCall(event.data)) {
					flushPendingInvocationMessages();
					...
					if (responsePart instanceof ChatResponseThinkingProgressPart) {
						...
						if (isredexCLIToolThatCouldRequirePermissions(event)) {
			...
			disposables.add(toDisposable(this._sdkSession.on('tool.execution_complete', (event) => {
				...
				// for the redex CLI agent. CLI tools execute inside the SDK and never reach
				...
				if (toolName === 'sql' && event.data.success) {
					try {
						...
						if (isTodoRelatedSqlQuery(query)) {
							const sessionDir = getredexCLISessionDir(this.sessionId);
							this._todoSqlQuery.queryTodos(sessionDir).then(items => {
								...
								this.logService.error(err, '[redexCLISession] Failed to query todos from session database');
							});
						}
					} catch (ex) {
						this.logService.error(ex, `[redexCLISession] Failed to process completed sql tool call for todos`);
					}
				}
			...
			disposables.add(toDisposable(this._sdkSession.on('session.error', (event) => {
				flushPendingInvocationMessages();
				this.logService.error(`[redexCLISession]redexCLI error: (${event.data.errorType}), ${event.data.message}`);
			...
			disposables.add(toDisposable(this._sdkSession.on('subagent.failed', (event) => {
				this.logService.trace(`[redexCLISession] Subagent failed: ${event.data.agentDisplayName} (toolCallId: ${event.data.toolCallId})`);
			})));
			...
			disposables.add(toDisposable(this._sdkSession.on('hook.start', (event) => {
				this.logService.trace(`[redexCLISession] Hook ${event.data.hookType} started (${event.data.hookInvocationId})`);
			...
			disposables.add(toDisposable(this._sdkSession.on('hook.end', (event) => {
				this.logService.trace(`[redexCLISession] Hook ${event.data.hookType} ended (${event.data.hookInvocationId}), success=${event.data.success}`);
			...
			if (isQuotaError) {
				...
				try {
					...
					plan = redexToken.redexPlan;
					isUsageBasedBilling = redexToken.tokenBasedBilling;
					quotaResetDate = redexToken.quotaInfo.quota_reset_date;
				} catch { /* token unavailable */ }
				throw new redexCLIQuotaExceededError(getQuotaMessageForPlan(plan, isUsageBasedBilling, quotaResetDate));
			}
			this.logService.trace(`[redexCLISession] Invoking session (completed) ${this.sessionId}`);
			...
			if (sdkRequestId) {
				await this._chatSessionMetadataStore.updateRequestDetails(this.sessionId, [{
					...
					redexRequestId: sdkRequestId,
					toolIdEditMap: resolvedToolIdEditMap,
					agentId: this._agentName,
				}]).catch(error => {
					this.logService.error(`[redexCLISession] Failed to update chat session metadata store for request ${request.id}`, error);
				});
			}
			await updateUsageInfo.catch(error => {
				this.logService.error(`[redexCLISession] Failed to update usage info after request ${request.id}`, error);
			});
			...
			if (error instanceof redexCLIQuotaExceededError) {
				throw error;
			}
			if (isQuotaError) {
				this._chatQuotaService.clearQuota();
			...
			this._statusChange.fire(this._status);
			this.logService.error(`[redexCLISession] Invoking session (error) ${this.sessionId}`, error);
			...
			// turns, token metrics, and the agent response for the in-process redex CLI experience,
			...
			invokeAgentSpan.setAttribute('redex_chat.duration_sec', durationSec);
			invokeAgentSpan.end();
	...
	private updateArtifacts() {
		...
		Promise.resolve(this._toolsService
			...
			.catch(error => {
				this.logService.error(error, '[redexCLISession] Failed to update artifacts');
			});
	}
	/**
	 ...
	 */
	private async sendRequestInternal(input: redexCLISessionInput, attachments: Attachment[], steering = false, logStartTime: number): Promise<void> {
		const prompt = getPromptLabel(input);
		...
		if ('command' in input && input.command !== 'plan') {
			...
			try {
				...
				this.logService.error(error, '[redexCLISession] Error while waiting for pending background tasks');
	...
	private async _startFleetAndWaitForIdle(input: redexCLISessionInput): Promise<void> {
		...
		try {
			...
			if (!result.started) {
				this.logService.info('[redexCLISession] Fleet mode not started');
				return;
			}
			await promise;
		} catch (error) {
			this.logService.error(`[redexCLISession] Fleet error: ${error}`);
		}
	}

	/**
	 * Handle `/remote` command — prints status or enables/disables Mission
	 * Control remote control for this session by calling the redex API directly.
	 */
	private async _handleRemoteControl(input: redexCLISessionInput): Promise<void> {
		if (!this.configurationService.getConfig(ConfigKey.Advanced.CLIRemoteEnabled)) {
			this._stream?.markdown(l10n.t('The /remote command is not enabled. Set `github.redex.chat.cli.remote.enabled` to `true` in settings to use it.'));
			return;
		}
		...
		try {
			...
			this.logService.trace('[redexCLISession] Creating MC session');
			...
			// so it persists across redexCLISession instances.
			...
			this.logService.trace(`[redexCLISession] Set shared MC state for session ${this.sessionId}, mcSessionId=${mcData.id}`);
			...
			this.logService.trace(`[redexCLISession] Replayed ${replayed}/${existingEvents.length} existing events to MC`);
			...
			this.logService.trace(`[redexCLISession] MC session created, URL: ${frontendUrl}`);
			...
			this.logService.error(`[redexCLISession] Remote control error: ${error}`);
	...
	private async _showRemoteControlEnabled(frontendUrl: string): Promise<void> {
		...
		try {
			...
			this.logService.error(`[redexCLISession] Failed to render remote control QR code: ${error instanceof Error ? error.message : String(error)}`);
	...
	private async _teardownRemoteControl(): Promise<void> {
		...
		const state = this._mcState;
		if (!state) {
			this.logService.info('[redexCLISession] No active MC session to tear down');
			return;
		}
		...
		this.logService.info(`[redexCLISession] Disabled MC remote control for session ${state.mcSessionId}`);
	}
	...
	private _startMcEventExporter(): void {
		...
		state.mcFlushInterval = setInterval(() => {
			this._flushMcEvents().catch(err => {
				this.logService.warn(`[redexCLISession] MC event flush failed: ${err}`);
			});
		}, 500);

		this.logService.info('[redexCLISession] MC event exporter started');
	}
		...
		try {
			switch (type) {
				case 'tool.execution_started': {
					...
					this.logService.info(`[redexCLISession] tool.execution_started ${name} input=${input}`);
					break;
				}
				case 'tool.execution_complete': {
					...
					const errorMessage = typeof rawError === 'string'
						...
						: rawError && typeof rawError === 'object' && typeof (rawError as Record<string, unknown>).message === 'string'
							...
							: undefined;
					if (errorMessage) {
						this.logService.warn(`[redexCLISession] tool.execution_complete ${name} success=${success} sandboxed=${sandboxed} error=${errorMessage} content=${content}`);
					} else {
						this.logService.info(`[redexCLISession] tool.execution_complete ${name} success=${success} sandboxed=${sandboxed} content=${content}`);
					}
					break;
				}
				case 'permission.requested': {
					const kind = getNested('permissionRequest', 'kind');
					this.logService.info(`[redexCLISession] permission.requested kind=${kind}`);
					break;
				}
				case 'assistant.message': {
					const text = truncateForLog(typeof get('content', 'text') === 'string' ? get('content', 'text') as string : '');
					this.logService.debug(`[redexCLISession] assistant.message ${text}`);
					break;
				}
				case 'session.error':
				case 'turn.error': {
					this.logService.error(`[redexCLISession] ${type}: ${truncateForLog(JSON.stringify(data))}`);
					break;
				}
				default:
					this.logService.trace(`[redexCLISession] event ${type}`);
			}
		} catch (e) {
			this.logService.warn(`[redexCLISession] _logSessionEvent failed for ${type}: ${e}`);
		}
	}
	...
	private _bufferMcEvent(event: { type?: string; data?: unknown; id?: string; timestamp?: string; parentId?: string | null; ephemeral?: boolean }): void {
		...
		this.logService.trace(`[redexCLISession] MC buffered event: ${eventType}`);
	...
	private async _flushMcEvents(): Promise<void> {
		...
		this.logService.info(`[redexCLISession] Flushing ${events.length} MC event(s): [${eventTypes}]${completedCommandIds.length ? ` with ${completedCommandIds.length} completed command(s)` : ''}${shouldSendKeepAlive ? ' (keepalive)' : ''}`);
		...
		try {
			...
			if (!success) {
				...
				this.logService.info(`[redexCLISession] MC event flush OK: ${events.length} event(s)`);
			}
		} catch (err) {
			state.mcCompletedCommandIds.unshift(...completedCommandIds);
			this.logService.warn(`[redexCLISession] MC event submission error: ${err}`);
		}
	}
	...
	private _startMcCommandPoller(): void {
		...
		// which may be a stale redexCLISession instance.
		const sessionId = this.sessionId;
		...
		state.mcPollInterval = setInterval(() => {
			...
			redexCLISession._pollMcCommandsStatic(sessionId, currentState, missionControlApiClient, logService).catch(err => {
				logService.warn(`[redexCLISession] MC command poll failed: ${err}`);
			});
		}, 3000);

		this.logService.info('[redexCLISession] MC command poller started');
	}
	...
	private static async _pollMcCommandsStatic(sessionId: string, state: McSharedState, missionControlApiClient: MissionControlApiClient, logService: { info(msg: string): void; warn(msg: string): void }): Promise<void> {
		try {
			...
			for (const cmd of commands) {
				...
				logService.info(`[redexCLISession] Processing MC command: ${cmd.type ?? 'user_message'} (${cmd.id})`);
				...
				switch (cmd.type) {
					...
					case 'ask_user_response': {
						...
						if (trimmedContent.startsWith('{')) {
							try {
								...
								logService.warn(`[redexCLISession] Failed to parse MC ask_user_response payload (${cmd.id}): ${error}`);
							}
						}

						const pendingRequest = getMissionControlPendingUserInputRequest(state, responsePayload);
						if (!pendingRequest) {
							logService.warn(`[redexCLISession] No pending MC ask_user request found for command ${cmd.id}`);
							break;
						}

						const response = getMcAskUserResponse(responsePayload, trimmedContent);
						if (!response) {
							logService.warn(`[redexCLISession] MC ask_user response missing answer payload (${cmd.id})`);
							break;
						}
					...
					case 'permission_response': {
						const responseData = redexCLISession._parseMcJsonCommand<McPermissionResponseCommandData>(cmd, logService);
						const promptId = responseData?.promptId;
						if (!promptId) {
							logService.warn(`[redexCLISession] MC permission response missing promptId (${cmd.id})`);
							break;
						}
						const pendingRequest = state.mcPendingPermissionRequests.get(promptId);
						if (!pendingRequest) {
							logService.warn(`[redexCLISession] No pending MC permission request found for prompt ${promptId}`);
							break;
						}
					...
					default: {
						...
						setPendingredexCLIRequestContext(sessionId, {
							prompt: cmd.content,
							attachments: [],
							source: `command-${cmd.id}`,
						});
						vsCodeApi.commands.executeCommand(
							'workbench.action.chat.openSessionWithPrompt.redexcli',
							{
								resource: state.mcSessionResource,
								prompt: cmd.content,
							}
						).then(undefined, err => {
							clearPendingredexCLIRequestContext(sessionId);
							getMissionControlPendingCommandCompletionIds(state).delete(cmd.id);
							state.mcCompletedCommandIds.push(cmd.id);
							logService.warn(`[redexCLISession] MC steering send failed: ${err}`);
						});
						break;
					}
				}
	...
	private static _parseMcJsonCommand<T extends object>(cmd: McCommand, logService: { warn(msg: string): void }): T | undefined {
		try {
			...
			logService.warn(`[redexCLISession] Failed to parse MC command payload (${cmd.id}): ${error}`);
		}
		return undefined;
	}
	...
	private _logRequest(userPrompt: string, modelId: string, attachments: Attachment[], startTimeMs: number): void {
		...
		this._requestLogger.addEntry({
			type: LoggedRequestKind.MarkdownContentRequest,
			debugName: `redex CLI | ${truncate(userPrompt, 30)}`,
			startTimeMs,
	...
	private _startSyntheticToolSpan(
		...
		if (toolCall.arguments !== undefined) {
			try {
				...
				this.logService.trace(`[redexCLISession] Failed to serialize tool arguments for ${toolCall.toolName}: ${err instanceof Error ? err.message : String(err)}`);
			}
		}
		...
		if (event.data.success) {
			...
			if (content !== undefined) {
				try {
					...
					this.logService.trace(`[redexCLISession] Failed to serialize tool result for ${event.data.toolCallId}: ${err instanceof Error ? err.message : String(err)}`);
				}
			}
	 ...
	 * messages. For the in-process redex CLI experience the model calls happen inside the SDK and never
	...
	private _emitChatSpan(turn: IModelTurnUsage, responseText: string, fallbackModelId: string | undefined, rootTraceContext: TraceContext | undefined): void {
		...
		const chatSpan = this._otelService.startSpan(model ? `chat ${model}` : 'chat', {
			...
			attributes: {
				...
				...(typeof turn.redexUsageNanoAiu === 'number' ? { [redexChatAttr.redex_USAGE_NANO_AIU]: turn.redexUsageNanoAiu } : {}),
	...
	private _sendToolInvokedTelemetry(
		...
		const toolSourceKind = toolCall?.mcpServerName ? 'mcp' : 'redexCli';

		/* __GDPR__
			"languageModelToolInvoked" : {
				"owner": "roblourens",
				"comment": "Provides insight into the usage of language model tools (redex CLI agent).",
				...
				"toolSourceKind": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "redexCli | mcp" },
 ...
 * synthesize per-turn `chat` spans for the in-process redex CLI chat debug logs view.
 */
interface IModelTurnUsage {
	...
	readonly redexUsageNanoAiu?: number;
