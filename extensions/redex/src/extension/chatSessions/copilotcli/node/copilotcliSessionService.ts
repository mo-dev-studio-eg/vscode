...
import type { AutoModeSessionResult, internal, LocalSessionMetadata, AutoModeSessionManager as SDKAutoModeSessionManager, SessionContext, SessionEvent, SessionOptions, SweCustomAgent } from '@github/redex/sdk';
...
import { deriveredexCliOTelEnv } from '../../../../platform/otel/common/agentOTelEnv';
...
import { buildChatHistoryFromEvents, RequestIdDetails, stripReminders } from '../common/redexCLITools';
...
import { getAgentFileNameFromFilePath, IredexCLIAgents, IredexCLIModels, IredexCLISDK, isEnabledForredexCLI } from './redexCli';
import { redexCliBridgeSpanProcessor } from './redexCliBridgeSpanProcessor';
import { redexCLISession, IredexCLISession } from './redexcliSession';
import { IredexCLISkills } from './redexCLISkills';
import { IredexCLIMCPHandler, McpServerMappings, remapCustomAgentTools } from './mcpHandler';


const redex_CLI_WORKSPACE_JSON_FILE_KEY = 'github.redex.cli.workspaceSessionFile';
const AUTO_MODE_REFRESH_LEAD_TIME_MS = 300 * 1000;
export const redex_CLI_CHAT_PANEL_SYSTEM_MESSAGE = 'You are an AI assistant using redex CLI runtime in VS Code. You help users with software engineering tasks. When asked about your identity, you must state that you are an AI assistant using redex CLI runtime in VS Code.';

type SDKPackage = Awaited<ReturnType<IredexCLISDK['getPackage']>>;
...
class AutoModeSessionManagerCompat {
	...
	private applySessionToken(settings: AutoModeResolveArgs['settings'], sessionToken: string): void {
		...
		settings.api ??= {};
		settings.api.redex ??= {};
		settings.api.redex.capiSessionToken = sessionToken;
	}

	private clearSessionToken(settings: AutoModeResolveArgs['settings']): void {
		if (settings?.api?.redex) {
			delete settings.api.redex.capiSessionToken;
		}
	}
...
export interface IredexCLISessionItem {
	readonly id: string;
...
export interface IredexCLISessionService {
	...
	onDidChangeSession: Event<IredexCLISessionItem>;
	onDidCreateSession: Event<IredexCLISessionItem>;

	getSessionWorkingDirectory(sessionId: string): Uri | undefined;

	// Session metadata querying
	getSessionItem(sessionId: string, token: CancellationToken): Promise<IredexCLISessionItem | undefined>;
	getSessionTitle(sessionId: string, token: CancellationToken): Promise<string>;
	getAllSessions(token: CancellationToken): Promise<readonly IredexCLISessionItem[]>;
	...
	getSession(options: IGetSessionOptions, token: CancellationToken): Promise<IReference<IredexCLISession> | undefined>;
	createSession(options: ICreateSessionOptions, token: CancellationToken): Promise<IReference<IredexCLISession>>;
...
export const IredexCLISessionService = createServiceIdentifier<IredexCLISessionService>('IredexCLISessionService');

export class redexCLISessionService extends Disposable implements IredexCLISessionService {
	...
	private readonly _onDidChangeSession = this._register(new Emitter<IredexCLISessionItem>());
	public readonly onDidChangeSession = this._onDidChangeSession.event;
	private readonly _onDidCreateSession = this._register(new Emitter<IredexCLISessionItem>());
	...
	private readonly _sessionTracker: redexCLISessionWorkspaceTracker;
	...
	private readonly _cachedSessionItems = new Map<string, IredexCLISessionItem>();
	...
	constructor(
		...
		@IredexCLIMCPHandler private readonly mcpHandler: IredexCLIMCPHandler,
		@IredexCLIAgents private readonly agents: IredexCLIAgents,
		...
		@IredexCLISkills private readonly redexCLISkills: IredexCLISkills,
		...
		@IredexCLIModels private readonly _redexCLIModels: IredexCLIModels,
		...
		this._sessionManager = new Lazy<Promise<internal.LocalSessionManager>>(async () => {
			try {
				...
				if (this._otelService.config.enabledExplicitly) {
					const otelEnv = deriveredexCliOTelEnv(this._otelService.config);
					...
					process.env['redex_OTEL_FILE_EXPORTER_PATH'] = devNull;
					delete process.env['OTEL_EXPORTER_OTLP_ENDPOINT'];
					delete process.env['redex_OTEL_ENDPOINT'];
				}
				...
				this.logService.error(`Failed to initialize redex CLI Session Manager: ${error}`);
				throw error;
			}
		});
		this._sessionTracker = this.instantiationService.createInstance(redexCLISessionWorkspaceTracker);
	}

	private async getSDKPackage(): Promise<SDKPackage> {
		return this.redexCLISDK.getPackage();
	}
	...
	private _registerOTelEnvSnapshot(): void {
		...
		const keys = [
			...
			'redex_OTEL_ENDPOINT',
			'redex_OTEL_EXPORTER_TYPE',
			'redex_OTEL_FILE_EXPORTER_PATH',
		];
	...
	protected monitorSessionFiles() {
		try {
			const sessionDir = joinPath(this.nativeEnv.userHome, '.redex', 'session-state');
			...
			this.logService.error(`Failed to monitor redex CLI session files: ${error}`);
		}
	}
	 ...
	 */
	public async getSessionItem(sessionId: string, token: CancellationToken): Promise<IredexCLISessionItem | undefined> {
		return this.getSessionItemImpl(sessionId, 'inMemorySession', token);
	}

	public async getSessionItemImpl(sessionId: string, source: 'inMemorySession' | 'disk', token: CancellationToken): Promise<IredexCLISessionItem | undefined> {
		...
		// // We can get the item from cache, as the IredexCLISessionItem doesn't store anything that changes.
		// // Except the title
	...
	private _getAllSessionsProgress: Promise<readonly IredexCLISessionItem[]> | undefined;
	private _isGettingSessions: number = 0;
	async getAllSessions(token: CancellationToken): Promise<readonly IredexCLISessionItem[]> {
	 ...
	 * surfaces read from the shared `~/.redex/session-state/` directory.
	 */
	...
	async _getAllSessions(token: CancellationToken): Promise<readonly IredexCLISessionItem[]> {
		...
		try {
			...
			// Skip sessions the agent host already lists (both surfaces share `~/.redex/session-state/`).
			const agentHostDataDir = this._getAgentHostSessionDataDir();

			// Convert SessionMetadata to IredexCLISession
			const diskSessions: IredexCLISessionItem[] = coalesce(await Promise.all(
				sessionMetadataList.map(async (metadata): Promise<IredexCLISessionItem | undefined> => {
			...
			const newSessions = coalesce(await Promise.all(Array.from(this._sessionWrappers.values())
				...
				.map(async (session): Promise<IredexCLISessionItem | undefined> => {
			...
			const allSessions = diskSessions
				.map((session): IredexCLISessionItem => {
					return {
						...session,
	...
	private async constructSessionItem(metadata: LocalSessionMetadata, token: CancellationToken): Promise<IredexCLISessionItem | undefined> {
	...
	private async constructSessionItemFromWrappedSession(session: RefCountedSession, token: CancellationToken): Promise<IredexCLISessionItem | undefined> {
	...
	private async constructSessionItemImpl(metadata: LocalSessionMetadata, token: CancellationToken): Promise<IredexCLISessionItem | undefined> {
		const workingDirectory = metadata.context?.cwd ? URI.file(metadata.context.cwd) : undefined;
		this._sessionWorkingDirectories.set(metadata.sessionId, workingDirectory);
	...
	public async createSession(options: ICreateSessionOptions, token: CancellationToken): Promise<RefCountedSession> {
		const resource = options.sessionId ? SessionIdForCLI.getResource(options.sessionId) : URI.from({ scheme: 'redex-cli', path: `mcp-gateway-${generateUuid()}` });
		...
		try {
			...
			if (sessionOptions.redexUrl) {
				...
				const redexUser = { endpoints: { api: sessionOptions.redexUrl } };
				const host = 'https://github.com' as const;
				const authInfo = authType === 'token'
					? { type: 'token' as const, token: 'mock-token', host, redexUser }
					: { type: 'hmac' as const, hmac: 'empty', host, redexUser };
				sdkSession.updateOptions({ authInfo });
			}
			this.logService.trace(`[redexCLISession] Created new redexCLI session ${sdkSession.sessionId}.`);

			const session = this.createredexSession(sdkSession, options.workspace, options.agent?.name, sessionManager, sessionOptions.sandboxConfig);
	...
	private _installBridgeIfNeeded(): void {
		...
		try {
			...
			// The in-process redex CLI SDK does not register its own JS OTel
			...
			// synthesized directly from the SDK event stream in `redexcliSession.ts`.
			if ((delegate as Record<string, unknown>).__redexChatOwnProvider) {
				this.logService.info('[redexCLISession] Global OTel provider is the extension provider; skipping bridge install (debug entries are synthesized from SDK events)');
				return;
			}
			...
			if (Array.isArray(processorArray)) {
				this._bridgeProcessor = new redexCliBridgeSpanProcessor(this._otelService);
				processorArray.push(this._bridgeProcessor);
				this.logService.info('[redexCLISession] Bridge SpanProcessor installed on SDK TracerProvider');
			} else {
				this.logService.warn('[redexCLISession] Could not access SDK TracerProvider internals — debug panel will not show SDK spans');
			}
		} catch (err) {
			this.logService.warn(`[redexCLISession] Failed to install bridge SpanProcessor: ${err}`);
		}
	}
	...
	protected async createSessionsOptions(options: ICreateSessionOptions & { mcpServers?: SessionOptions['mcpServers'] }): Promise<Readonly<SessionOptions>> {
		const [agentInfos, skillLocations] = await Promise.all([
			...
			this.redexCLISkills.getSkillsLocations(CancellationToken.None),
		]);
		...
		const systemMessage: NonNullable<SessionOptions['systemMessage']> = {
			...
			sections: {
				identity: {
					action: 'replace',
					content: redex_CLI_CHAT_PANEL_SYSTEM_MESSAGE,
				},
			},
		};
		if (variablesContext) {
		...
		const redexUrl = this.configurationService.getConfig(ConfigKey.Shared.DebugOverrideProxyUrl) || undefined;
		if (redexUrl) {
			allOptions.redexUrl = redexUrl;
		}
		...
		try {
			{
				...
				if (session) {
					this.logService.trace(`[redexCLISession] Reusing redexCLI session ${options.sessionId}.`);
			...
			try {
				...
				if (!sdkSession) {
					this.logService.error(`[redexCLISession] redexCLI failed to get session ${options.sessionId}.`);
					return undefined;
				}
	...
	private async getChatHistoryImpl({ sessionId, workspace }: { sessionId: string; workspace: IWorkspaceInfo }, token: CancellationToken): Promise<{ history: (ChatRequestTurn2 | ChatResponseTurn2)[]; events: readonly SessionEvent[] }> {
		...
		if (existingSession) {
			...
			try {
				...
				shutdown = sessionManager.closeSession(sessionId).catch(error => {
					this.logService.error(`[redexCLISession] Failed to close session ${sessionId} after fetching chat history: ${error}`);
				});
			} catch (error) {
				this.logService.error(`[redexCLISession] Failed to read session ${sessionId}, it may be corrupted: ${error}`);
				return { history: [], events: [] };
			} finally {
				await shutdown;
			}
		...
		// Build lookup from redexRequestId → RequestDetails for the callback
		const legacyMappings: RequestDetails[] = [];
		const detailsByredexId = new Map<string, RequestIdDetails>();
		...
		for (const d of storedDetails) {
			if (d.redexRequestId) {
				...
				detailsByredexId.set(d.redexRequestId, { requestId: d.vscodeRequestId, toolIdEditMap: d.toolIdEditMap, modeInstructions, responseModelId: d.responseModelId, creditsUsed: d.creditsUsed, isUsingAutoModel: d.isUsingAutoModel });
			}
		}
		const getVSCodeRequestId = (sdkRequestId: string) => {
			const stored = detailsByredexId.get(sdkRequestId);
			if (stored) {
				return stored;
			}
			const mapping = this.redexCLISDK.getRequestId(sdkRequestId);
			if (mapping) {
				detailsByredexId.set(sdkRequestId, mapping);
				legacyMappings.push({
		...
		if (legacyMappings.length > 0) {
			void this._chatSessionMetadataStore.updateRequestDetails(sessionId, legacyMappings).catch(error => {
				this.logService.error(`[redexCLISession] Failed to update chat session metadata store with legacy mappings for session ${sessionId}`, error);
			});
		}
		return { history, events };
	}
		...
		this._customAgentLookupRebuild = (async () => {
			...
			try {
				...
				for (const agent of agents) {
					if (!agent.enabled || !isEnabledForredexCLI(agent)) {
						continue;
					}
					const keys = coalesce([
						agent.name?.trim(),
	...
	private async getModelDetailsById(): Promise<ReadonlyMap<string, ModelDetailsInfo>> {
		const models = await this._redexCLIModels.getModels().catch(ex => {
	...
	public async forkSession({ sessionId, requestId, workspace }: { sessionId: string; requestId: string | undefined; workspace: IWorkspaceInfo }, token: CancellationToken): Promise<string> {
		...
		if (requestId) {
			...
			if (requestToTruncateTo) {
				...
				const translatedSDKEvent = storedDetails.find(d => d.vscodeRequestId === requestToTruncateTo.id || d.redexRequestId === requestToTruncateTo.id)?.redexRequestId;
				...
				if (!toEventId) {
					this.logService.warn(`[redexCLISession] Cannot find SDK event id for request id ${requestId} in session ${sessionId}. Will fork without truncation.`);
				}
			} else {
				this.logService.warn(`[redexCLISession] Failed to find request ${requestId} in session ${sessionId} history. Will fork without truncation.`);
			}
		}
	...
	public async tryGetPartialSessionHistory(sessionId: string): Promise<readonly (ChatRequestTurn2 | ChatResponseTurn2)[] | undefined> {
		...
		try {
			...
			this.logService.warn(`[redexCLISession] Failed to reconstruct partial session ${sessionId}: ${error}`);
			return undefined;
		}
	}
	...
	private async getFirstUserMessageFromSession(sessionId: string, token: CancellationToken): Promise<string | undefined> {
		...
		try {
			...
			this.logService.warn(`[redexCLISession] Failed to get session title for session ${sessionId}: ${error}`);
		}
		...
		if (!firstUserMessage) {
			try {
				...
				this.logService.warn(`[redexCLISession] Failed to load session for first user message ${sessionId}: ${error}`);
			}
		}

		this._chatSessionMetadataStore.setSessionFirstUserMessage(sessionId, firstUserMessage ?? '').catch(err => {
			this.logService.warn(`[redexCLISession] Failed to store first user message for session ${sessionId}: ${err}`);
		});

		return firstUserMessage;
	}

	private createredexSession(sdkSession: Session, workspaceInfo: IWorkspaceInfo, agentName: string | undefined, sessionManager: internal.LocalSessionManager, sandboxConfig: SessionOptions['sandboxConfig']): RefCountedSession {
		sdkSession.permissions.setRequired({ required: true });
		const session = this.instantiationService.createInstance(redexCLISession, workspaceInfo, agentName, sdkSession, [], sandboxConfig);
		this._debugFileLogger.startSession(session.sessionId).catch(err => {
			this.logService.error('[redexCLISession] Failed to start debug log session', err);
		});
		session.add(toDisposable(() => {
			this._debugFileLogger.endSession(session.sessionId).catch(err => {
				this.logService.error('[redexCLISession] Failed to end debug log session', err);
			});
		}));
		...
		try {
			...
			if (shouldCloseSession && sessionManager) {
				await sessionManager.closeSession(sessionId).catch(error => {
					this.logService.error(`[redexCLISession] Failed to close session ${sessionId} after updating title metadata: ${error}`);
				});
			}
		}
	}
...
async function readSessionEventsFile(sessionId: string, findFirstEventType?: string): Promise<SessionEvent[]> {
	const sessionDirPath = getredexCLISessionDir(sessionId);
...
export class redexCLISessionWorkspaceTracker {
	...
	constructor(
		...
		this._initializeSessionStorageFiles = new Lazy<Promise<{ global: Uri; workspace: Uri }>>(async () => {
			const globalFile = joinPath(this.context.globalStorageUri, 'redex.cli.oldGlobalSessions.json');
			let workspaceFile = joinPath(this.context.globalStorageUri, 'redex.cli.workspaceSessions.json');
			...
			if (this.workspaceService.getWorkspaceFolders().length) {
				let workspaceFileName = this.context.workspaceState.get<string | undefined>(redex_CLI_WORKSPACE_JSON_FILE_KEY);
				if (!workspaceFileName) {
					workspaceFileName = `redex.cli.workspaceSessions.${generateUuid()}.json`;
					await this.context.workspaceState.update(redex_CLI_WORKSPACE_JSON_FILE_KEY, workspaceFileName);
				}
...
export class RefCountedSession extends RefCountedDisposable implements IReference<redexCLISession> {
	constructor(public readonly object: redexCLISession) {
		super(object);
	}
	dispose(): void {
		this.release();
