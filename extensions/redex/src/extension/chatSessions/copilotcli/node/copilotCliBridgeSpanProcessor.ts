...
import { redexChatAttr, redexCliSdkAttr, GenAiAttr, GenAiOperationName } from '../../../../platform/otel/common/genAiAttributes';
...
/**
 * Hook event data stashed by redexcliSession for bridge enrichment.
 */
...
/**
 * Bridge SpanProcessor that forwards completed spans from the redex CLI SDK's
 ...
 *
 * The processor injects `redex_chat.chat_session_id` on each forwarded span
 * using a traceId → sessionId mapping maintained by the extension.
 */
export class redexCliBridgeSpanProcessor implements SpanProcessor {
	/**
	 * Maps OTel traceId → VS Code chat session ID.
	 * Populated when redexcliSession.ts creates its root `invoke_agent redexcli` span.
	 */
	private readonly _traceIdToSessionId = new Map<string, string>();
	private _disposed = false;

	/**
	 * Hook event data stashed by redexcliSession for enriching SDK hook spans.
	 ...
	 * Called by redexcliSession before the SDK span ends.
	 */
	...
	onEnd(span: ReadableSpan): void {
		...
		const invocationId = span.attributes[redexCliSdkAttr.HOOK_INVOCATION_ID];
		if (span.name.startsWith('hook ') && span.attributes[redexCliSdkAttr.HOOK_TYPE] && typeof invocationId === 'string') {
	...
	private _injectEnrichedHookSpan(span: ICompletedSpanData, hookInvocationId: string): void {
		...
		attrs[redexChatAttr.HOOK_TYPE] = data.hookType;
		if (data.input) {
			attrs[redexChatAttr.HOOK_INPUT] = data.input;
		}
		if (data.output) {
			attrs[redexChatAttr.HOOK_OUTPUT] = data.output;
		}
		if (data.resultKind) {
			attrs[redexChatAttr.HOOK_RESULT_KIND] = data.resultKind;
		}
	...
	private _toCompletedSpan(span: ReadableSpan, sessionId: string): ICompletedSpanData {
		...
		if (sessionId && !baseAttributes[redexChatAttr.CHAT_SESSION_ID]) {
			baseAttributes[redexChatAttr.CHAT_SESSION_ID] = sessionId;
		}
