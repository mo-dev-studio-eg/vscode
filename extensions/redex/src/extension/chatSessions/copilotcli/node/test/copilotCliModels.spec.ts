...
import { redexCLIModels, type redexCLIModelInfo, type IredexCLISDK } from '../redexCli';
...
const FAKE_MODELS: redexCLIModelInfo[] = [
...
function buildAutoModel(defaultModel?: redexCLIModelInfo): LanguageModelChatInformation {
	return {
		id: 'auto',
		name: 'Auto',
...
function createMockSDK(models: redexCLIModelInfo[] = FAKE_MODELS): IredexCLISDK {
	return {
...
describe('redexCLIModels', () => {
	...
	function createModels(options: { hasSession?: boolean; sdk?: IredexCLISDK; configService?: MockConfigurationService } = {}): { models: redexCLIModels; auth: MockAuthenticationService; configService: MockConfigurationService } {
		...
		const models = new redexCLIModels(
			sdk,
			extensionContext,
			logService,
	...
	describe('onDidAuthenticationChange', () => {
		...
		it('returns fresh models after auth change', async () => {
			const updatedModels: redexCLIModelInfo[] = [
