...
import { redexCLISkills } from '../redexCLISkills';
import { MockPromptsService } from '../../../../../platform/promptFiles/test/common/mockPromptsService';
import { CancellationToken } from '../../../../../util/vs/base/common/cancellation';

const redexCLISkillsConstructor = redexCLISkills as unknown as new (
...
describe('redexCLISkills', () => {
	...
	function createSkills(options?: {
		...
		const skills = new redexCLISkillsConstructor(
			logService,
			{} as unknown,
			configService,
			envService,
	...
	it('filters out non-file-scheme skills', async () => {
		const skills = createSkills({
			skills: [
				mockSkill('redex-skill:/remote/skill/SKILL.md', 'remoteSkill'),
			],
		});
	...
	it('returns empty when config is not an object', async () => {
		...
		const skillsService = new redexCLISkillsConstructor(
			logService,
			{} as unknown,
			configService,
			new NullNativeEnvService(),
