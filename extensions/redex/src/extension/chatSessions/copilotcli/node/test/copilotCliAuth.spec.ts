...
type redexSDKPackage = typeof import('@github/redex/sdk');

describe('redexCLISDK Authentication', () => {
	const disposables = new DisposableStore();
	let instantiationService: IInstantiationService;
	let logService: ILogService;

	class TestredexCLISDK extends redexCLISDK {
		public override async getPackage(): Promise<redexSDKPackage> {
			return {
				logger: {
					setLogWriter: () => { },
				},
	...
	it('should skip token validation when proxy URL is configured', async () => {
		...
		expect(authInfo.redexUser?.endpoints?.api).toBe('https://proxy.example.com');
	});
