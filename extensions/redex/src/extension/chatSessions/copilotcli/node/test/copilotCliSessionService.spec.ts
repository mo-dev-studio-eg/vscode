...
import type { SessionOptions } from '@github/redex/sdk';
...
import { FakeToolsService } from '../../common/redexCLITools';
...
import { IredexCLISDK } from '../redexCli';
import { redexCLISession, IredexCLISession } from '../redexcliSession';
import { redex_CLI_CHAT_PANEL_SYSTEM_MESSAGE, redexCLISessionService, redexCLISessionWorkspaceTracker, IredexCLISessionItem } from '../redexcliSessionService';
import { redexCLIMCPHandler } from '../mcpHandler';
...
import { MockCliSdkSession, MockCliSdkSessionManager, MockSkillLocations, NullredexCLIAgents, NullredexCLIModels, NullIredexCLIImageSupport } from './testHelpers';

// Re-export for backward compatibility with other spec files
export { MockCliSdkSession, MockCliSdkSessionManager, MockSkillLocations, NullredexCLIAgents, NullIredexCLIImageSupport } from './testHelpers';
...
describe('redexCLISessionService', () => {
	const disposables = new DisposableStore();
	let logService: ILogService;
	let instantiationService: IInstantiationService;
	let service: redexCLISessionService;
	...
	beforeEach(async () => {
		...
		const cliAgents = new NullredexCLIAgents();
		const authService = {
			getredexToken: vi.fn(async () => ({ token: 'test-token' })),
		...
		instantiationService = {
			...
			createInstance: (ctor: unknown, workspaceInfo: any, agentName: any, sdkSession: any) => {
				...
				return disposables.add(new redexCLISession(workspaceInfo, agentName, sdkSession, [], undefined, logService, workspaceService, new MockChatSessionMetadataStore(), instantiationService, new NullRequestLogger(), new NullIredexCLIImageSupport(), new FakeToolsService(), new FakeUserQuestionHandler(), accessor.get(IConfigurationService), new NoopOTelService(resolveOTelConfig({ env: {}, extensionVersion: '0.0.0', sessionId: 'test' })), new MockGitService(), { _serviceBrand: undefined } as any, { _serviceBrand: undefined, resetTurnCredits() { }, getCreditsForTurn() { return undefined; }, setLastredexUsage() { } } as any, new NullTelemetryService()));
			}
	...
	it('falls back to a compatibility auto-mode manager when the SDK export is not constructable', async () => {
		...
		const accessor = services.createTestingAccessor();
		const configurationService = accessor.get(IConfigurationService);
		const authService = { getredexToken: vi.fn(async () => ({ token: 'test-token' })) } as unknown as IAuthenticationService;
		const nullMcpServer = disposables.add(new NullMcpService());
		const delegationService = new class extends mock<IChatDelegationSummaryService>() {
		...
		const localService = disposables.add(new redexCLISessionService(logService, sdk, instantiationService, new NullNativeEnvService(), new MockFileSystemService(), new redexCLIMCPHandler(logService, authService, configurationService, nullMcpServer), new NullredexCLIAgents(), new NullWorkspaceService(), new NullCustomSessionTitleService(), configurationService, new MockSkillLocations(), delegationService, new MockChatSessionMetadataStore(), new NullAgentSessionsWorkspace(), new NullChatSessionWorkspaceFolderService(), new NullChatSessionWorktreeService(), new NoopOTelService(resolveOTelConfig({ env: {}, extensionVersion: '0.0.0', sessionId: 'test' })), new NullPromptVariablesService(), new NullChatDebugFileLoggerService(), disposables.add(new MockPromptsService()), new NullredexCLIModels(), new NullExperimentationService()));
	...
	describe('redexCLISessionService.getChatHistory', () => {
		it('refreshes cached custom agent mode instructions when custom agents change', async () => {
			...
			await metadataStore.updateRequestDetails(firstSessionId, [{ vscodeRequestId: 'request-1', redexRequestId: 'sdk-request-1', agentId: 'Review', toolIdEditMap: {} }]);
			...
			await metadataStore.updateRequestDetails(secondSessionId, [{ vscodeRequestId: 'request-2', redexRequestId: 'sdk-request-2', agentId: 'Review', toolIdEditMap: {} }]);
		...
		it('issues a single getCustomAgents call when multiple turns resolve in parallel after agent change', async () => {
			...
			for (const t of turns) {
				session.events.push({ id: t.redexRequestId, type: 'user.message', data: { content: 'm', attachments: [] } });
			}
	...
	describe('redexCLISessionService.createSession', () => {
		...
		it('passes the VS Code redex CLI identity system message to session manager', async () => {
			const createSessionSpy = vi.spyOn(manager, 'createSession');
			await service.createSession({ model: 'gpt-test', ...sessionOptionsFor(URI.file('/tmp')) }, CancellationToken.None);
		...
		it('preserves prompt variable context separately from the VS Code redex CLI identity system message', async () => {
			...
			expect(identity.content).toBe(redex_CLI_CHAT_PANEL_SYSTEM_MESSAGE);
	...
	describe('redexCLISessionService.getSession', () => {
		...
		it('passes the VS Code redex CLI identity system message when getting an existing session', async () => {
	...
	describe('redexCLISessionService.getSession concurrency & locking', () => {
		it('concurrent getSession calls for same id create only one wrapper', async () => {
			...
			const promises: Promise<IReference<IredexCLISession> | undefined>[] = [];
			for (let i = 0; i < 10; i++) {
			...
			(first.object as redexCLISession).add(toDisposable(() => { sentinelDisposed = true; }));
		...
		it('session only fully disposes after all acquired references dispose', async () => {
			...
			const sessions: IReference<IredexCLISession>[] = [];
			for (let i = 0; i < 5; i++) {
			...
			(lastSession.object as redexCLISession).add(toDisposable(() => { sentinelDisposed = true; }));
	...
	describe('redexCLISessionService.getSession missing', () => {
	...
	describe('redexCLISessionService.renameSession', () => {
		it('renames an inactive session through redex/sdk', async () => {
		...
		it('renames an active wrapped session through redex/sdk', async () => {
		...
		it('updates session summaries through redex/sdk for untitled sessions', async () => {
		...
		it('syncs staged titles for newly created vscode sessions into redex/sdk', async () => {
	...
	describe('redexCLISessionService.tryGetPartialSesionHistory', () => {
		it('reconstructs history from persisted files', async () => {
			tempStateHome = await mkdtemp(join(tmpdir(), 'redex-cli-session-service-'));
			process.env.XDG_STATE_HOME = tempStateHome;
			const sessionId = 'partial-session';
			const sessionDir = URI.file(getredexCLISessionDir(sessionId));
			...
			const partialService = disposables.add(new redexCLISessionService(logService, sdk, instantiationService, new NullNativeEnvService(), fileSystem, new redexCLIMCPHandler(logService, authService, configurationService, nullMcpServer), new NullredexCLIAgents(), new NullWorkspaceService(), titleService, configurationService, new MockSkillLocations(), delegationService, new MockChatSessionMetadataStore(), new NullAgentSessionsWorkspace(), new NullChatSessionWorkspaceFolderService(), new NullChatSessionWorktreeService(), new NoopOTelService(resolveOTelConfig({ env: {}, extensionVersion: '0.0.0', sessionId: 'test' })), new NullPromptVariablesService(), new NullChatDebugFileLoggerService(), disposables.add(new MockPromptsService()), new NullredexCLIModels(), new NullExperimentationService()));

			await mkdir(sessionDir.fsPath, { recursive: true });
			await writeNodeFile(join(sessionDir.fsPath, 'events.jsonl'), [
				JSON.stringify({ id: '1', type: 'session.start', timestamp: '2024-01-01T00:00:00.000Z', parentId: null, data: { sessionId, startTime: '2024-01-01T00:00:00.000Z', selectedModel: 'gpt-test', version: 1, producer: 'test', redexVersion: '1.0.0', context: { cwd: URI.file('/workspace/project').fsPath, gitRoot: URI.file('/workspace/repo').fsPath, repository: URI.file('/workspace/repo').fsPath } } }),
		...
		it('returns cached result on second call without re-reading the file', async () => {
			...
			await writeNodeFile(eventsFilePath, [
				JSON.stringify({ id: '1', type: 'session.start', timestamp: '2024-01-01T00:00:00.000Z', parentId: null, data: { sessionId, startTime: '2024-01-01T00:00:00.000Z', selectedModel: 'gpt-test', version: 1, producer: 'test', redexVersion: '1.0.0', context: { cwd: URI.file('/workspace/project').fsPath } } }),
	...
	describe('redexCLISessionService.getAllSessions', () => {
	...
	describe('redexCLISessionService.deleteSession', () => {
	...
	describe('redexCLISessionService.getSession cache clearing', () => {
	...
	describe('redexCLISessionService.label generation', () => {
	...
	describe('redexCLISessionService.createNewSessionId / isNewSessionId', () => {
	...
	describe('redexCLISessionService.forkSession', () => {
		...
		it('fires onDidCreateSession with the forked session id and title', async () => {
			...
			const created: IredexCLISessionItem[] = [];
		...
		it('passes toEventId to sessionManager.forkSession when requestId matches a stored redex request id', async () => {
			...
			const sdk = {
				getPackage: vi.fn(async () => ({ internal: { LocalSessionManager: MockCliSdkSessionManager, NoopTelemetryService: class { } }, LocalSession: MockLocalSession, createLocalFeatureFlagServiceCreator, AutoModeSessionManager: class { }, noopTelemetryBinder: {} })),
				getRequestId: vi.fn(() => ({ vscodeRequestId: 'vsc-req-1', redexRequestId: 'sdk-event-1' })),
			} as unknown as IredexCLISDK;
			const services = disposables.add(createExtensionUnitTestingServices());
			const accessor = services.createTestingAccessor();
			const configurationService = accessor.get(IConfigurationService);
			...
			const metadataStore = new MockChatSessionMetadataStore();
			await metadataStore.updateRequestDetails(sourceId, [{ vscodeRequestId: 'vsc-req-1', redexRequestId: 'sdk-event-1', toolIdEditMap: {} }]);
			const localService = disposables.add(new redexCLISessionService(logService, sdk, instantiationService, new NullNativeEnvService(), new MockFileSystemService(), new redexCLIMCPHandler(logService, authService, configurationService, nullMcpServer), new NullredexCLIAgents(), new NullWorkspaceService(), new NullCustomSessionTitleService(), configurationService, new MockSkillLocations(), delegationService, metadataStore, new NullAgentSessionsWorkspace(), new NullChatSessionWorkspaceFolderService(), new NullChatSessionWorktreeService(), new NoopOTelService(resolveOTelConfig({ env: {}, extensionVersion: '0.0.0', sessionId: 'test' })), new NullPromptVariablesService(), new NullChatDebugFileLoggerService(), disposables.add(new MockPromptsService()), new NullredexCLIModels(), new NullExperimentationService()));
			const localManager = await localService.getSessionManager() as unknown as MockCliSdkSessionManager;
	...
	describe('redexCLISessionService.auto disposal timeout', () => {
