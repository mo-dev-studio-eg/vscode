...
import { redexCLIPromptResolver } from '../redexcliPromptResolver';
import { MockSkillLocations, NullIredexCLIImageSupport } from './testHelpers';

// Mock generateUserPrompt to avoid TSX rendering complexity in unit tests
vi.mock('../../../../prompts/node/agent/redexCLIPrompt', () => ({
...
describe('redexCLIPromptResolver', () => {
	...
	function createResolver(overrideSkillLocations?: MockSkillLocations, overrideExtensionContext?: IVSCodeExtensionContext) {
		const imageSupport = new NullIredexCLIImageSupport();
		...
		return new redexCLIPromptResolver(
			imageSupport as unknown as IredexCLIImageSupport,
			logService,
			fileSystemService,
			workspaceService,
