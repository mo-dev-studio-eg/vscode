...
import { redexCLISDK } from '../redexCli';

type redexSdkModule = typeof import('@github/redex/sdk');
...
class TestredexCLISDK extends redexCLISDK {
	...
	public override async getPackage(): Promise<redexSdkModule> {
		throw new Error('SDK import is disabled in shim tests');
	}
}

describe('redex CLI shims', () => {
	let testDir: string;
	const logService = new LogServiceImpl([]);

	beforeEach(async () => {
		testDir = await mkdtemp(join(tmpdir(), 'redex-cli-shims-'));
	});
	...
	function createSdk(extensionPath: string, vscodeAppRoot: string): TestredexCLISDK {
		return new TestredexCLISDK(
	...
	it('creates runtime ripgrep and node-pty shims for separate extension installs before SDK import', async () => {
		...
		await expect(readFile(join(extensionPath, 'node_modules', '@github', 'redex', 'shims.txt'), 'utf8')).resolves.toBe('Shims created successfully');
		const sdkRipgrepDir = join(extensionPath, 'node_modules', '@github', 'redex', 'sdk', 'ripgrep', 'bin', process.platform + '-' + process.arch);
		await expect(readFile(join(sdkRipgrepDir, 'rg'), 'utf8')).resolves.toBe('ripgrep');
		const sdkNodePtyDir = join(extensionPath, 'node_modules', '@github', 'redex', 'sdk', 'prebuilds', process.platform + '-' + process.arch);
	...
	it('skips runtime node-pty shimming for bundled installs when shims.txt already exists', async () => {
		const extensionPath = join(testDir, 'extension');
		const vscodeAppRoot = join(testDir, 'app');
		const placeholder = join(extensionPath, 'node_modules', '@github', 'redex', 'shims.txt');
		...
		await mkdir(join(extensionPath, 'node_modules', '@github', 'redex'), { recursive: true });
		await writeFile(placeholder, 'already created');

		await createSdk(extensionPath, vscodeAppRoot).runEnsureShims();

		await expect(readFile(placeholder, 'utf8')).resolves.toBe('already created');
		await expect(stat(join(extensionPath, 'node_modules', '@github', 'redex', 'sdk'))).rejects.toMatchObject({ code: 'ENOENT' });
	});
});
