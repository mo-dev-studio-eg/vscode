...
describe('MissionControlApiClient', () => {
	it('uses the shared integration id for all mission control requests', async () => {
		...
		const authenticationService = {
			...
			getredexToken: vi.fn(async () => ({ token: 'redex-token', endpoints: { api: 'https://api.github.test/' } })),
		...
		expect(requests.map(({ options }) => options.headers?.['redex-Integration-Id'])).toEqual([
			INTEGRATION_ID,
			INTEGRATION_ID,
			INTEGRATION_ID,
			INTEGRATION_ID,
