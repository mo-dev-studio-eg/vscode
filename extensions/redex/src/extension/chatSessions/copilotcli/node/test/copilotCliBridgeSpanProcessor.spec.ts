...
describe('redexCliBridgeSpanProcessor', () => {
	let otelService: ReturnType<typeof createMockOTelService>;
	let bridge: redexCliBridgeSpanProcessor;

	beforeEach(() => {
		otelService = createMockOTelService();
		bridge = new redexCliBridgeSpanProcessor(otelService);
	});
	...
	it('forwards spans with registered traceId', () => {
		...
		expect(otelService.injectedSpans[0].attributes['redex_chat.chat_session_id']).toBe('session-1');
	});
	...
	it('does not overwrite existing CHAT_SESSION_ID', () => {
		...
		bridge.onEnd(makeReadableSpan({
			traceId: 'trace-abc',
			attributes: { 'redex_chat.chat_session_id': 'existing-session' },
		}));

		expect(otelService.injectedSpans[0].attributes['redex_chat.chat_session_id']).toBe('existing-session');
	});
	...
	it('enriches SDK hook spans with stashed event data', () => {
		...
		bridge.onEnd(makeReadableSpan({
			...
			traceId: 'trace-abc',
			attributes: {
				'github.redex.hook.type': 'sessionEnd',
				'github.redex.hook.invocation_id': 'inv-123',
			},
		}));
		...
		expect(span.attributes['redex_chat.hook_type']).toBe('sessionEnd');
		expect(span.attributes['redex_chat.hook_input']).toBe('{"reason":"complete"}');
		expect(span.attributes['redex_chat.hook_result_kind']).toBe('success');
	});
	...
	it('holds SDK hook span until hook.end data arrives', () => {
		...
		bridge.onEnd(makeReadableSpan({
			...
			traceId: 'trace-abc',
			attributes: {
				'github.redex.hook.type': 'preToolUse',
				'github.redex.hook.invocation_id': 'inv-456',
			},
		}));
		...
		expect(otelService.injectedSpans).toHaveLength(1);
		const span = otelService.injectedSpans[0];
		expect(span.attributes['redex_chat.hook_input']).toBe('{"tool":"bash"}');
		expect(span.attributes['redex_chat.hook_output']).toBe('{"decision":"allow"}');
		expect(span.attributes['redex_chat.hook_result_kind']).toBe('success');
	});
