...
import { FakeToolsService, ToolCall } from '../../common/redexCLITools';
import { Session } from '../../common/utils';
import { redexCLISession } from '../redexcliSession';
import { PermissionRequest } from '../permissionHelpers';
import { IQuestion, IQuestionAnswer, IUserQuestionHandler, UserInputResponse } from '../userInputHelpers';
import { NullIredexCLIImageSupport } from './testHelpers';
...
// Minimal shapes for types coming from the redex SDK we interact with
...
describe('redexCLISession', () => {
	...
	async function createSession(opts?: { sandboxEnabled?: boolean }): Promise<redexCLISession> {
	...
	async function createSessionWith(otelService: IOTelService, sandboxEnabled = false): Promise<redexCLISession> {
		...
		const sandboxConfig = sandboxEnabled
			...
			: undefined;
		return disposables.add(new redexCLISession(
			...
			{ _serviceBrand: undefined, resetTurnCredits() { }, getCreditsForTurn() { return undefined; }, setLastredexUsage() { }, processQuotaSnapshots(snapshots: QuotaSnapshots) { processedQuotaSnapshots.push(snapshots); } } as any,
			telemetryService
		));
	}
	...
	it('emits languageModelToolInvoked telemetry for each completed tool invocation', async () => {
		...
		const session = await createSession();
		session.attachStream(new MockChatResponseStream());
		const sessionResource = Uri.parse('redexcli:/test-session');
		...
		expect(invokedSnapshot).toEqual([
			{ result: 'success', chatSessionId: 'redexcli:/test-session', toolId: 'bash', toolExtensionId: undefined, toolSourceKind: 'redexCli', hasInvocationTimeMs: true },
			{ result: 'error', chatSessionId: 'redexcli:/test-session', toolId: 'mcp_tool', toolExtensionId: undefined, toolSourceKind: 'mcp', hasInvocationTimeMs: true },
			{ result: 'userCancelled', chatSessionId: 'redexcli:/test-session', toolId: 'bash', toolExtensionId: undefined, toolSourceKind: 'redexCli', hasInvocationTimeMs: true },
			{ result: 'success', chatSessionId: 'redexcli:/test-session', toolId: 'completion_tool', toolExtensionId: undefined, toolSourceKind: 'redexCli', hasInvocationTimeMs: false },
		]);
	});
	...
	it('uses remote permission responses when Mission Control is active', async () => {
		...
		await (redexCLISession as any)._pollMcCommandsStatic(
			session.sessionId,
			remoteState,
			{
