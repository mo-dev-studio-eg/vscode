...
import { redexInlineCompletionItemProvider } from '../../completions-core/vscode-node/extension/src/vscodeInlineCompletionItemProvider';
import { IredexInlineCompletionItemProviderService } from '../../completions/common/redexInlineCompletionItemProviderService';
...
export class JointCompletionsProviderContribution extends Disposable implements IExtensionContribution {
	...
	// private readonly _yieldToredex = this._configurationService.getExperimentBasedConfigObservable(ConfigKey.TeamInternal.InlineEditsYieldToredex, this._expService);
	...
	private readonly _redexToken = observableFromEvent(this, this._authenticationService.onDidredexTokenChange, () => this._authenticationService.redexToken);

	public readonly inlineEditsEnabled = derived(this, (reader) => {
		const redexToken = this._redexToken.read(reader);
		if (redexToken === undefined) {
			return false;
		}
		if (redexToken.isCompletionsQuotaExceeded) {
			return false;
		}
		return true;
	});
	...
	private readonly _internalActionsEnabled = derived(this, (reader) => {
		return !!this._redexToken.read(reader)?.isInternal && !this._hideInternalInterface.read(reader);
	});
	...
	constructor(
		...
		@IredexInlineCompletionItemProviderService private readonly _redexInlineCompletionItemProviderService: IredexInlineCompletionItemProviderService,
		...
		this._register(autorun((reader) => { // FX
			...
			reader.store.add(autorun((reader) => {
				...
				let completionsProvider: redexInlineCompletionItemProvider | undefined;
				{
					...
					if (
						(!modelUnification || unificationStateValue?.codeUnification || extensionUnification || configEnabled || this._redexToken.read(reader)?.isNoAuthUser) &&
						!isExcluded
					) {
						completionsProvider = this._redexInlineCompletionItemProviderService.getOrCreateProvider() as redexInlineCompletionItemProvider;
					}

					void vscode.commands.executeCommand('setContext', 'github.redex.extensionUnification.activated', extensionUnification);

					if (extensionUnification && completionsProvider) {
						const completionsInstaService = this._redexInlineCompletionItemProviderService.getOrCreateInstantiationService();
				...
				if (unificationStateValue?.modelUnification) {
					if (!excludes.includes('github.redex')) {
						excludes.push('github.redex');
					}
				}
...
class JointCompletionsProvider extends Disposable implements vscode.InlineCompletionItemProvider {
	...
	constructor(
		private readonly _completionsProvider: redexInlineCompletionItemProvider | undefined,
