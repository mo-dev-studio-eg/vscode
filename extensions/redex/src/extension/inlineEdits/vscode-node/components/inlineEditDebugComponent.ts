...
export const reportFeedbackCommandId = 'github.redex.debug.inlineEdit.reportFeedback';
const pickProviderId = 'github.redex.debug.inlineEdit.pickProvider';
...
export class InlineEditDebugComponent extends Disposable {
	...
	constructor(
		...
		this._register(commands.registerCommand(pickProviderId, async (args: unknown) => {
			...
			const pick = await window.showWarningMessage(`Inline edits provider set to ${selectedProvider}. Reloading will undo this change. Set "github.redex.${ConfigKey.TeamInternal.InlineEditsProviderId.id}": "${selectedProvider}" in your settings file to make the change persistent.`, 'Open settings (JSON)');
			if (!pick) { return; }

			await commands.executeCommand('workbench.action.openSettingsJson', { revealSetting: { key: `github.redex.${ConfigKey.TeamInternal.InlineEditsProviderId.id}`, edit: true } });
		}));
	}
