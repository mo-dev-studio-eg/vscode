...
export class NesFeedbackSubmitter {
	...
	public async submitFromFolder(feedbackFolderUri: Uri): Promise<void> {
		try {
			...
			if (files.length === 0) {
				window.showInformationMessage('No NES feedback captures found to submit. Use "redex: Record Expected Edit (NES)" to capture feedback first.');
				return;
			}
	...
	private async _createPullRequest(
		...
		const payload = {
			...
			body: `## NES Feedback Submission\n\n` +
				...
				`This feedback was automatically submitted via the "redex: Submit NES Feedback" command.`
		};
