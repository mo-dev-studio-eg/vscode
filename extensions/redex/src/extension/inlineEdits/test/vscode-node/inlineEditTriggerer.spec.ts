...
suite('InlineEditTriggerer', () => {
	...
	function createOutputDocument(): { document: TextDocument; textEditor: TextEditor; selection: Selection } {
		const uri = Uri.parse('output:extension-output-GitHub.redex-chat-#1-GitHub redex Chat');
	...
	suite('Document filtering', () => {
		...
		test('Ignores redex-ignored documents (not in workspace)', () => {
			const { document, textEditor } = createTextDocument();
			// Remove from workspace to simulate redex-ignored
	...
	suite('Document switch edge cases', () => {
		test('Does not trigger on document switch to redex-ignored destination', () => {
			const doc1 = createTextDocument(undefined, Uri.file('file1.py'));
			// doc2 is NOT added to vscWorkspace (redex-ignored)
			...
			const switchEvents = firedEvents.filter(e => e.data.reason === NesTriggerReason.ActiveDocumentSwitch);
			assert.strictEqual(switchEvents.length, 0, 'Should not trigger switch for redex-ignored destination');
			assert.strictEqual(firedEvents.length, initialCount, 'No new events should fire');
		});
	...
	suite('Text change listener edge cases', () => {
		test('Text change on redex-ignored doc does not track but updates lastEditTimestamp', () => {
			// Create a doc that is NOT in vscWorkspace (redex-ignored)
			const uri = Uri.file('ignored.py');
			const doc = createTextDocumentData(uri, 'content', 'python');
			// Do NOT call vscWorkspace.addDoc — simulates redex-ignored
	...
	suite('Interaction edge cases', () => {
		...
		test('redex-ignored doc in selection listener returns early before rejection check', () => {
			...
			assert.strictEqual(firedEvents.length, 0,
				'redex-ignored doc should return early in selection listener');
		});
