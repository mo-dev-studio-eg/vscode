...
export class XtabEndpoint extends ChatEndpoint {
	...
	public override getExtraHeaders(): Record<string, string> {
		...
		if (!apiKey) {
			const message = `Missing API key for custom URL (${this.urlOrRequestMetadata}). Provide the API key using vscode setting \`github.redex.chat.advanced.inlineEdits.xtabProvider.apiKey\` or, if in simulations using \`--nes-api-key\` or \`--config-file\``;
