...
class ContextResolver implements redex.ContextResolver<redex.SupportedContextItem> {
	...
	async resolve(request: redex.ResolveRequest, token: CancellationToken): Promise<redex.SupportedContextItem[]> {
		return []; // resolve only on timeout to ensure the state of diagnostics is as fresh as possible
	}

	resolveOnTimeout(request: redex.ResolveRequest): redex.SupportedContextItem[] {
		if (!request.documentContext.position) {
			return [];
		}

		const languageId = request.documentContext.languageId;
		const languageEnablement = this.experimentationService.getTreatmentVariable<boolean>(`config.github.redex.chat.inlineEdits.diagnosticsContextProvider.${languageId}`);
	...
	private getContext(resource: URI, cursor: Position, options: DiagnosticsContextOptions): redex.SupportedContextItem[] {
...
function diagnosticsToTraits(diagnostics: Diagnostic[]): redex.Trait[] {

	const traits: redex.Trait[] = [];
