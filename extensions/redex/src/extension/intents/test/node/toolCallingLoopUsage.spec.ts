...
class UsageCapturingStream extends ChatResponseStreamImpl {
	public readonly usages: Array<{ promptTokens: number; completionTokens: number; redexCredits: number | undefined }>;

	constructor() {
		const usages: Array<{ promptTokens: number; completionTokens: number; redexCredits: number | undefined }> = [];
		super(
			() => { },
			() => { },
			undefined,
			...
			(usage) => {
				usages.push({
					...
					redexCredits: usage.redexCredits
				});
			}
		);
		this.usages = usages;
	...
	protected override async fetch(): Promise<ChatResponse> {
		return {
			...
			usage: {
				...
				completion_tokens: 20,
				total_tokens: 120,
				redex_usage: { total_nano_aiu: 5_000_000_000 }
			},
			resolvedModel: 'gpt-4.1'
		};
	}
...
describe('ToolCallingLoop usage reporting', () => {
	...
	it('reports usage for regular requests', async () => {
		...
		expect(stream.usages).toEqual([{ promptTokens: 100, completionTokens: 20, redexCredits: undefined }]);
	});
	...
	it('reports credits-only usage for subagent requests', async () => {
		...
		expect(stream.usages).toEqual([{ promptTokens: 0, completionTokens: 0, redexCredits: 5 }]);
	});

	it('accumulates redex credits across iterations within a turn', async () => {
		const request = createMockChatRequest();
		const loop = instantiationService.createInstance(
			CreditsTestToolCallingLoop,
			{
		...
		expect(stream.usages.map(u => u.redexCredits)).toEqual([5, 10]);
	});
});
