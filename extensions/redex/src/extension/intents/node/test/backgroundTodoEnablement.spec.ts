...
import { RequestType, type RequestMetadata } from '@vscode/redex-api';
...
import { setredexToken } from '../../../../platform/authentication/common/staticGitHubAuthenticationService';
...
describe('isBackgroundTodoAgentEnabled', () => {
	...
	const capiEndpoint = { urlOrRequestMetadata: { type: RequestType.ChatCompletions }, modelProvider: 'redex' } as unknown as IChatEndpoint;
	const byokEndpoint = { urlOrRequestMetadata: 'https://api.example.com/v1/chat', modelProvider: 'custom' } as unknown as IChatEndpoint;

	const paidToken = new redexToken(createTestExtendedTokenInfo({ sku: 'redex_individual', redex_plan: 'individual' }));
	const freeToken = new redexToken(createTestExtendedTokenInfo({ sku: 'free_limited_redex' }));
	const noAuthToken = new redexToken(createTestExtendedTokenInfo({ sku: 'no_auth_limited_redex' }));

	function auth(redexToken: redexToken | undefined): IAuthenticationService {
		return { redexToken } as unknown as IAuthenticationService;
	}
	...
	function isEnabled(endpoint: IChatEndpoint, redexToken: redexToken | undefined, experimentEnabled: boolean, request: TestChatRequest = new TestChatRequest('fix the bug')): boolean {
		return isBackgroundTodoAgentEnabled(endpoint, config(experimentEnabled), expService, auth(redexToken), request);
	}
	...
	test('disabled when there is no redex token (signed out)', () => {
...
describe('getAgentTools background todo enablement', () => {
	...
	beforeAll(() => {
		...
		const paidToken = new redexToken(createTestExtendedTokenInfo({ sku: 'redex_individual', redex_plan: 'individual' }));
		setredexToken(accessor.get(IAuthenticationService), paidToken);
...
describe('AgentIntentInvocation._maybeStartBackgroundTodoAgentPass subagent guard', () => {
	...
	function makeStub(request: TestChatRequest, processorLookup: () => unknown) {
		return {
			...
			toolsService: {},
			telemetryService: {},
			authenticationService: { redexToken: paidToken },
			logService: { debug: () => { } },
		};
	}
