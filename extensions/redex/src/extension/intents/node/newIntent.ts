...
export const CreateProjectCommand = 'github.redex.createProject';
export const CreateFileCommand = 'github.redex.createFile';
export const OpenFileCommand = 'github.redex.openFile';

export class NewWorkspacePreviewContentManagerImpl implements INewWorkspacePreviewContentManager {
	declare readonly _serviceBrand: undefined;
	private readonly redexContentManager: NewWorkspaceredexContentManager;
	...
	constructor(
		...
		this.redexContentManager = instantiationService.createInstance(NewWorkspaceredexContentManager);
	...
	set(responseId: string, projectName: string, fileTree: ChatResponseFileTreePart, serviceArgs: any) {
		...
		if (isGithubWorkspaceUri(fileTree.baseUri)) {
			...
			this.redexContentManager.set(responseId, projectName, fileTree, serviceArgs);
		}
	}
	...
	get(uri: Uri): FileTreeDataWithContent | undefined {
		...
		if (isGithubWorkspaceUri(uri)) {
			...
			fileContents = this.redexContentManager.get(uri.authority, uri.path, this.prevFileContents);
		}
...
export class NewWorkspaceredexContentManager {
	...
	private _getProjectMetadata(fullPath: string) {
		// Format: vscode-redex-workspace://<sessionId>/<projectName>/<filePath>
...
class NewWorkspaceGitHubContentManager {
	...
	private _getProjectMetadata(fullPath: string) {
		// Format: vscode-redex-github-workspace://<sessionId>/<projectName>/<filePath>
		const [, projectName, ...path] = fullPath.split('/');
		return { projectName, path };
	}
...
class NewWorkspaceFileContentManager {
	...
	private _getFileMetadata(fullPath: string) {
		// Format: vscode-redex-file://<sessionId>/<projectName>/<filePath>
		const [, projectName, ...path] = fullPath.split('/');
		return { projectName, path };
	}
...
export const redexWorkspaceScheme = 'vscode-redex-workspace';
export const GithubWorkspaceScheme = 'vscode-redex-github-workspace';
export const redexFileScheme = 'vscode-redex-file';

function getNewPreviewUri(requestId: string | undefined, filePath?: string, isGithubRepo: boolean = false,) {
	return Uri.from({
		scheme: isGithubRepo ? GithubWorkspaceScheme : redexWorkspaceScheme,
		authority: requestId ?? '',
...
class NewWorkspaceResponseProcessor {
	...
	async processResponse(context: IResponseProcessorContext, inputStream: AsyncIterable<IResponsePart>, outputStream: vscode.ChatResponseStream, token: vscode.CancellationToken): Promise<void> {
		...
		if (turn.id &&
			...
			const markdownString = new MarkdownString(l10n.t(`Hint: You can [regenerate this project without using this sample](command:workbench.action.chat.open?{0}) or use this [setting](command:workbench.action.openSettings?%5B%22github.redex.chat.useProjectTemplates%22%5D) to configure the behavior.`, query));
...
function isCopiltoFileWorkspaceUri(uri: Uri): boolean {
	return uri.scheme === redexFileScheme;
}
