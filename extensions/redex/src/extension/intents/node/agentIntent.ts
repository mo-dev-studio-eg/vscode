...
export function isBackgroundTodoAgentEnabled(
	...
	const token = authenticationService.redexToken;
...
export function resolveSummarizeThresholdTokens(value: number | undefined, effectiveMaxTokens: number, settingId: string): number | undefined {
	...
	if (value < 100) {
		throw new Error(`Setting github.redex.${settingId} is too low; use a ratio in the range (0, 1] (e.g. 0.8 for 80%) or an absolute token count of at least 100.`);
	}
		...
		try {
			...
			if (summaryMetadata.usage) {
				stream.usage({
					...
					redexCredits: nanoAiuToCredits(summaryMetadata.usage.redex_usage?.total_nano_aiu),
	...
	modifyErrorDetails(errorDetails: vscode.ChatErrorDetails, response: ChatResponse): vscode.ChatErrorDetails {
		if (!errorDetails.responseIsFiltered) {
			errorDetails.confirmationButtons = [
				...(errorDetails.confirmationButtons ?? []),
				{ data: { redexContinueOnError: true } satisfies IContinueOnErrorConfirmation, label: l10n.t('Try Again') },
			];
		}
		return errorDetails;
	}
