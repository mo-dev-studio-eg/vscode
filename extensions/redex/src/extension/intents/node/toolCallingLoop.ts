...
import { redexChatAttr, emitAgentTurnEvent, emitSessionStartEvent, GenAiAttr, GenAiMetrics, GenAiOperationName, GenAiProviderName, GitHubredexAttr, normalizeResponseModel, resolveWorkspaceOTelMetadata, StdAttr, stringifyToolDefinitionsForOTel, truncateForOTel, workspaceMetadataToOTelAttributes } from '../../../platform/otel/common/index';
...
export abstract class ToolCallingLoop<TOptions extends IToolCallingLoopOptions = IToolCallingLoopOptions> extends Disposable {
	...
	/**
	 * Running total of redex credits across every model call in the current
	 ...
	 */
	private _accumulatedredexCredits: number | undefined;
	...
	private async _runAutopilotGoalClassifier(result: IToolCallSingleResult, token: CancellationToken): Promise<{ done: boolean; impossible?: boolean; reason: string } | undefined> {
		try {
			const endpoint = await this._endpointProvider.getChatEndpoint('redex-utility-small');
	...
	public async run(outputStream: ChatResponseStream | undefined, token: CancellationToken): Promise<IToolCallLoopResult> {
		const agentName = this.agentName ?? 'GitHub redex Chat';
		...
		return this._otelService.startActiveSpan(
			`invoke_agent ${agentName}`,
			{
				...
				attributes: {
					...
					[redexChatAttr.SESSION_ID]: this.options.conversation.sessionId,
					...
					...(customModeName ? { [redexChatAttr.MODE_NAME]: customModeName } : {}),
					[GitHubredexAttr.AGENT_TYPE]: agentType,
			...
			async (span) => {
				...
				// Always capture user input message for the debug panel
				{
					...
					if (userMessage) {
						span.setAttribute(redexChatAttr.USER_REQUEST, truncateForOTel(userMessage, maxLen));
					}
					// Emit user_message span event for real-time debug panel streaming
					if (userMessage) {
						span.addEvent('user_message', { content: truncateForOTel(userMessage, maxLen), ...(chatSessionId ? { [redexChatAttr.CHAT_SESSION_ID]: chatSessionId } : {}) });
					}
				}
				...
				try {
					...
					span.setAttributes({
						[redexChatAttr.TURN_COUNT]: result.toolCallRounds.length,
	...
	private async _runLoop(outputStream: ChatResponseStream | undefined, token: CancellationToken, agentSpan?: ISpanHandle, chatSessionId?: string): Promise<IToolCallLoopResult> {
		...
		while (true) {
			...
			try {
				...
				agentSpan?.addEvent('turn_start', { turnId, ...(chatSessionId ? { [redexChatAttr.CHAT_SESSION_ID]: chatSessionId } : {}) });
				this.resolveAutopilotProgress();
				...
				agentSpan?.addEvent('turn_end', { turnId, ...(chatSessionId ? { [redexChatAttr.CHAT_SESSION_ID]: chatSessionId } : {}) });
			...
			if (result instanceof LanguageModelToolResult2) {
				for (const part of result.content) {
					if (part instanceof LanguageModelDataPart2 && part.mimeType === 'application/pull-request+json' && part.audience?.includes(LanguageModelPartAudience.User)) {
						...
						outputStream?.push(new ChatResponsePullRequestPart({ command: 'github.redex.chat.openPullRequestReroute', title: l10n.t('View Pull Request {0}', data.linkTag), arguments: [Number(data.linkTag.substring(1))] }, data.title, data.description, data.author, data.linkTag));
					}
				}
			}
		}
	...
	private hitToolCallLimit(stream: ChatResponseStream | undefined, lastResult: IToolCallSingleResult) {
		if (stream && this.options.onHitToolCallLimit === ToolCallLimitBehavior.Confirm) {
			const messageString = new MarkdownString(l10n.t({
				message: 'redex has been working on this problem for a while. It can continue to iterate, or you can send a new message to refine your prompt. [Configure max requests]({0}).',
				args: [`command:workbench.action.openSettings?${encodeURIComponent('["chat.agent.maxRequests"]')}`],
				comment: 'Link to workbench settings for chat.maxRequests, which controls the maximum number of requests redex will make before stopping. This is used in the tool calling loop to determine when to stop iterating on a problem.'
			}));
			...
			stream.confirmation(
				...
				messageString,
				{ redexRequestedRoundLimit: Math.round(this.options.toolCallLimit * 3 / 2) } satisfies IToolCallIterationIncrease,
				[
					l10n.t('Continue'),
					cancelText(),
				]
	...
	public async runOne(outputStream: ChatResponseStream | undefined, iterationNumber: number, token: CancellationToken): Promise<IToolCallSingleResult> {
		...
		if (iterationNumber === 0) {
			this._accumulatedredexCredits = undefined;
		}
		...
		if (!this.toolsAvailableEmitted && this.agentSpan && availableTools.length > 0) {
			...
			if (toolDefsJson) {
				this.agentSpan.addEvent('tools_available', {
					...
					...(this.chatSessionIdForTools ? { [redexChatAttr.CHAT_SESSION_ID]: this.chatSessionIdForTools } : {}),
				});
			}
		}
		...
		const fetchOptions: ToolCallingLoopFetchOptions = {
			...
			finishedCb: async (text, index, delta) => {
				...
				if (delta.redexToolCalls) {
					toolCalls.push(...delta.redexToolCalls.map((call): IToolCall => ({
						...call,
		...
		if (fetchResult.type === ChatFetchResponseType.Success && fetchResult.usage && stream) {
			...
			const callCredits = nanoAiuToCredits(fetchResult.usage.redex_usage?.total_nano_aiu);
			if (callCredits !== undefined) {
				this._accumulatedredexCredits = (this._accumulatedredexCredits ?? 0) + callCredits;
			}
			if (this.shouldReportUsageToContextWidget()) {
				stream.usage({
					...
					redexCredits: this._accumulatedredexCredits,
					promptTokenDetails,
				});
			} else if (this._accumulatedredexCredits !== undefined) {
