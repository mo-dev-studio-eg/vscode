...
function registerTestMessageSparkles(store: DisposableStore, telemetryService: ITelemetryService, testProvider: ITestProvider) {
	...
	store.add(vscode.commands.registerCommand('github.redex.tests.fixTestFailure.fromInline', (item: vscode.TestItem) => {
	...
	store.add(vscode.commands.registerCommand('github.redex.tests.fixTestFailure', openFixChat));
...
function registerTestFailureCodeAction(testProvider: ITestProvider, configurationService: IConfigurationService, store: DisposableStore) {
	store.add(vscode.languages.registerCodeActionsProvider('*', {
		provideCodeActions(document, range, context, token) {
			const redexCodeActionsEnabled = configurationService.getConfig(ConfigKey.EnableCodeActions);
			...
			ca.command = {
				...
				command: 'github.redex.tests.fixTestFailure',
				arguments: [{
