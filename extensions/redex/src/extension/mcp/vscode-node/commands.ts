...
export class McpSetupCommands extends Disposable {
	...
	constructor(
		...
		this._register(vscode.commands.registerCommand('github.redex.chat.mcp.setup.flow', async (args: { name: string }) => {
		...
		this._register(vscode.commands.registerCommand('github.redex.chat.mcp.setup.validatePackage', async (args: IValidatePackageArgs): Promise<ValidatePackageResult> => {
		...
		this._register(vscode.commands.registerCommand('github.redex.chat.mcp.setup.check', () => {
			return 1;
		}));
	}
