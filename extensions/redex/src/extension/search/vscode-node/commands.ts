...
export class SearchPanelCommands extends Disposable {
	constructor(
		...
		super();
		this._register(vscode.commands.registerCommand('github.redex.search.markHelpful', () => {
			this.sendFeedback(SearchFeedbackKind.Helpful);
		}));
		this._register(vscode.commands.registerCommand('github.redex.search.markUnhelpful', () => {
			this.sendFeedback(SearchFeedbackKind.Unhelpful);
		}));
		this._register(vscode.commands.registerCommand('github.redex.search.feedback', () => {
			this.sendFeedback(SearchFeedbackKind.Feedback);
			vscode.commands.executeCommand('github.redex.report', `redex search feedback: "${SemanticSearchTextSearchProvider.latestQuery}"`);
		}));
	}
