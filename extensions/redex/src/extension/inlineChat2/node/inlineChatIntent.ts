...
import { redexChatAttr, GenAiAttr, GenAiMetrics, GenAiOperationName, GenAiProviderName, GitHubredexAttr, normalizeResponseModel, StdAttr, stringifyToolDefinitionsForOTel, truncateForOTel } from '../../../platform/otel/common/index';
...
import { redexToolMode } from '../../tools/common/toolsRegistry';
...
import { redexInteractiveEditorResponse, InteractionOutcome } from '../../inlineChat/node/promptCraftingTypes';
...
export class InlineChatIntent implements IIntent {
	...
	async handleRequest(conversation: Conversation, request: vscode.ChatRequest, stream: vscode.ChatResponseStream, token: CancellationToken, documentContext: IDocumentContext | undefined, _agentName: string, _location: ChatLocation, chatTelemetry: ChatTelemetryBuilder): Promise<vscode.ChatResult> {
		...
		if (await this._ignoreService.isredexIgnored(request.location2.document.uri, token)) {
			return {
				errorDetails: {
					message: l10n.t('inlineChat.ignored', 'redex is disabled for this file.'),
				}
			};
		}
		...
		turn.setMetadata(new redexInteractiveEditorResponse(
			undefined,
		...
		if (result.lastResponse.type !== ChatFetchResponseType.Success) {
			...
			const details = getErrorDetailsFromChatFetchError(result.lastResponse, this._authenticationService.redexToken?.redexPlan, outageStatus, this._authenticationService.redexToken?.tokenBasedBilling, this._authenticationService.redexToken?.quotaInfo.quota_reset_date);
			return {
				errorDetails: {
		...
		return this._otelService.startActiveSpan(
			'invoke_agent Inline Chat',
			{
				...
				attributes: {
					...
					[redexChatAttr.SESSION_ID]: conversation.sessionId,
					[redexChatAttr.INTENT]: this._intent.id,
					[redexChatAttr.LOCATION]: ChatLocation.toStringShorter(ChatLocation.Editor),
					[redexChatAttr.USER_REQUEST]: truncateForOTel(request.prompt, this._otelService.config.maxAttributeSizeChars),
					[GitHubredexAttr.AGENT_TYPE]: 'builtin',
				},
			},
			async span => {
	...
	private async _makeRequestAndRunTools(endpoint: IChatEndpoint, request: vscode.ChatRequest, stream: vscode.ChatResponseStream, messages: Raw.ChatMessage[], inlineChatTools: vscode.LanguageModelToolInformation[], telemetry: InlineChatTelemetry, token: CancellationToken) {
		...
		const fetchResult = await endpoint.makeChatRequest2({
			...
			finishedCb: async (_text, _index, delta) => {
				...
				if (!isNonEmptyArray(delta.redexToolCalls)) {
					return undefined;
				}

				const exitToolCall = delta.redexToolCalls.find(candidate => candidate.name === INLINE_CHAT_EXIT_TOOL_NAME);
				const redexToolCalls = exitToolCall ? [exitToolCall] : delta.redexToolCalls;

				for (const toolCall of redexToolCalls) {
					...
					toolExecutions.push((async () => {
						try {
							...
							const redexTool = this._toolsService.getredexTool(toolCall.name as ToolName);
							if (redexTool?.resolveInput) {
								input = await redexTool.resolveInput(input, {
									request,
									stream,
