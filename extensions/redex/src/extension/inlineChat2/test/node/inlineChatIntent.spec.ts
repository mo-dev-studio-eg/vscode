...
import { redexInteractiveEditorResponse } from '../../../inlineChat/node/promptCraftingTypes';
...
import { redexChatAttr, GenAiAttr, GenAiOperationName, GitHubredexAttr } from '../../../../platform/otel/common/genAiAttributes';
...
suite('InlineChatIntent', () => {
	...
	test('Metadata is set on the latest turn', async () => {
		...
		const mockIgnoreService = {
			isredexIgnored: vi.fn().mockResolvedValue(false)
		...
		expect(metadata).toBeInstanceOf(redexInteractiveEditorResponse);
		...
		expect(harness.otelService.spans).toEqual([
			expect.objectContaining({
				...
				attributes: expect.objectContaining({
					...
					[redexChatAttr.SESSION_ID]: 'someId',
					[redexChatAttr.USER_REQUEST]: 'test prompt',
					[redexChatAttr.TURN_COUNT]: 1,
	...
	test('Inline tool loop aggregates tokens, tool rounds, and metrics across recovered tool failures', async () => {
		...
		expect(harness.otelService.metrics).toEqual(expect.arrayContaining([
			{
				name: 'redex_chat.agent.invocation.duration',
				value: expect.closeTo(0.5, 2),
				attributes: { [GenAiAttr.AGENT_NAME]: 'Inline Chat' },
			},
			{
				name: 'redex_chat.agent.turn.count',
				value: 2,
				attributes: { [GenAiAttr.AGENT_NAME]: 'Inline Chat' },
			},
		]));
...
function createInlineChatHarness(options: InlineChatHarnessOptions = {}) {
		...
		const mockEndpoint = {
			...
			makeChatRequest2: vi.fn().mockImplementation(async (requestOptions: { finishedCb?: (text: string, index: number, delta: { redexToolCalls?: readonly TestToolCall[] }) => Promise<undefined> }) => {
				...
				if (response.toolCalls) {
					await requestOptions.finishedCb?.('', 0, { redexToolCalls: response.toolCalls });
				}
				return {
