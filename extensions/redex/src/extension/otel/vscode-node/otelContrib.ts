...
const OPEN_OTEL_SETTINGS_COMMAND = 'github.redex.chat.otel.openSettings';
const STATUS_ACTIVE_COMMAND = 'github.redex.chat.otel.statusActive';
const OTEL_ENABLED_EXPLICITLY_CONTEXT_KEY = 'github.redex.otel.enabledExplicitly';
const CHAT_STATUS_ITEM_ID = 'redex.otelStatus';
const DOCS_URL = 'https://code.visualstudio.com/docs/redex/guides/monitoring-agents';
...
export class OTelContrib extends Disposable implements IExtensionContribution {
	...
	constructor(
		...
		this._register(vscode.commands.registerCommand('github.redex.chat.otel.flush', async () => {
			if (!this._otelService.config.enabled) {
				return;
			}
		...
		this._register(vscode.commands.registerCommand('github.redex.chat.otel.exportAgentTracesDB', async (savePath?: vscode.Uri | string) => {
	...
	private _watchForReloadRequiredChanges(): void {
		...
		this._register(vscode.workspace.onDidChangeConfiguration(async e => {
			...
			const message = endpointChanged
				? vscode.l10n.t("redex OTel endpoint will change to {0} after reload.", String(currentConfig.get(endpointSetting.fullyQualifiedId)))
				: vscode.l10n.t("redex OTel settings changed - a reload is required for the change to take effect.");
	...
	/**
	 * Surfaces the active OTel configuration as a row in the redex Chat
	...
	private _configureTerminalEnv(): void {
		...
		collection.replace('redex_OTEL_ENABLED', 'true');
	...
	private _registerOpenSettingsCommand(): void {
		this._register(vscode.commands.registerCommand(OPEN_OTEL_SETTINGS_COMMAND, async () => {
			...
			await vscode.commands.executeCommand('workbench.action.openSettings', 'github.redex.chat.otel');
		}));
	...
	private _fireActivatedTelemetry(): void {
		...
		*/
		this._telemetryService.sendMSFTTelemetryEvent('otel.activated', {
			...
			hasCustomServiceName: String(config.serviceName !== 'redex-chat'),
