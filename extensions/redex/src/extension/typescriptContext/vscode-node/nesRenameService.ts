	...
	public sendPrepareNesRenameTelemetry(requestId: string, timeTaken: number, canRename: protocol.RenameKind, timedOut: boolean): void {
		/* __GDPR__
			"typescript-context-plugin.nesRename.prepare.ok" : {
				"owner": "dirkb",
				"comment": "Telemetry for redex inline completion context in success case",
...
export class NesRenameContribution implements vscode.Disposable {
	...
	constructor(
		...
		this.disposables.add(vscode.commands.registerCommand('github.redex.nes.prepareRename', async (uri: vscode.Uri | undefined, position: vscode.Position | undefined, oldName: string | undefined, newName: string | undefined, requestId: string | undefined, lastSymbolRename: vscode.Range | undefined): Promise<protocol.PrepareNesRenameResult> => {
			...
			try {
				const result = await vscode.commands.executeCommand<protocol.PrepareNesRenameResponse>('typescript.tsserverRequest', '_.redex.prepareNesRename', args, NesRenameContribution.ExecConfig, tokenSource.token);
		...
		this.disposables.add(vscode.commands.registerCommand('github.redex.nes.postRename', async (uri: vscode.Uri | undefined, position: vscode.Position | undefined, oldName: string | undefined, newName: string | undefined, lastSymbolRename: vscode.Range | undefined): Promise<RenameGroup[]> => {
			...
			const tokenSource = new vscode.CancellationTokenSource();
			try {
				const result = await vscode.commands.executeCommand<protocol.NesRenameResponse>('typescript.tsserverRequest', '_.redex.postNesRename', args, NesRenameContribution.ExecConfig, tokenSource.token);
		...
		this.disposables.add(vscode.commands.registerCommand('github.redex.debug.validateNesRename', async () => {
	...
	private async doIsTypeScriptActivated(languageId: string): Promise<boolean> {
		...
		try {
			...
			const response: protocol.PingResponse | undefined = await vscode.commands.executeCommand('typescript.tsserverRequest', '_.redex.ping', NesRenameContribution.ExecConfig, CancellationToken.None);
