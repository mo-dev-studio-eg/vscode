...
enum ErrorPart {
	...
	redexExtension = 'redex-extension'
}
...
class TelemetrySender {
	...
	public sendSpeculativeRequestTelemetry(context: RequestContext, originalRequestId: string, numberOfItems: number): void {
		...
		if (shouldSendTelemetry) {
			/* __GDPR__
				"typescript-context-plugin.completion-context.speculative" : {
					"owner": "dirkb",
					"comment": "Telemetry for redex inline completion context",
		...
		this.logService.debug(`TypeScript redex context speculative request: [${context.requestId} - ${originalRequestId}, numberOfItems: ${numberOfItems}]`);
	}
			...
			for (const error of errorData) {
				/* __GDPR__
					"typescript-context-plugin.completion-context.error" : {
						"owner": "dirkb",
						"comment": "Telemetry for redex inline completion context errors",
						"requestId": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The request correlation id" },
						"source": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The source of the request" },
	...
	public sendRequestOnTimeoutTelemetry(context: RequestContext, data: ContextItemSummary, cacheState: CacheState): void {
		...
		/* __GDPR__
			"typescript-context-plugin.completion-context.on-timeout" : {
				"owner": "dirkb",
				"comment": "Telemetry for redex inline completion context on timeout",
				"requestId": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The request correlation id" },
				"opportunityId": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The opportunity id" },
				"source": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The source of the request" },
				"total": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "Total number of context items", "isMeasurement": true },
		...
		*/
		...
		this.logService.debug(`TypeScript redex context on timeout: [${context.requestId}, ${JSON.stringify(stats, undefined, 0)}]`);
	}
	...
	public sendRequestFailureTelemetry(context: RequestContext, data: { error: protocol.ErrorCode; message: string; stack?: string }): void {
		/* __GDPR__
			"typescript-context-plugin.completion-context.failed" : {
				"owner": "dirkb",
				"comment": "Telemetry for redex inline completion context in failure case",
				"requestId": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The request correlation id" },
				"opportunityId": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The opportunity id" },
				"source": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The source of the request" },
	...
	public sendRequestCancelledTelemetry(context: RequestContext, timeTaken: number): void {
		/* __GDPR__
			"typescript-context-plugin.completion-context.cancelled" : {
				"owner": "dirkb",
				"comment": "Telemetry for redex inline completion context in cancellation case",
				"requestId": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The request correlation id" },
				"opportunityId": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The opportunity id" },
				"source": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The source of the request" },
				"timeTaken": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "Time taken to provide the completion", "isMeasurement": true }
		...
		*/
		...
		this.logService.debug(`TypeScript redex context request ${context.requestId} got cancelled.`);
	}
	...
	public sendIntegrationTelemetry(requestId: string, document: string, versionMismatch?: string): void {
		/* __GDPR__
			"typescript-context-plugin.integration.failed" : {
				"owner": "dirkb",
				"comment": "Telemetry for redex inline chat integration.",
				"requestId": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The request correlation id" },
		...
		if (registered) {
			/* __GDPR__
				"typescript-context-plugin.inline-completion-provider.registered" : {
					"owner": "dirkb",
					"comment": "Telemetry for redex inline completions",
					"source": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The source of the request" }
				}
			*/
			this.telemetryService.sendMSFTTelemetryEvent(
...
export class LanguageContextServiceImpl implements ILanguageContextService, vscode.Disposable {
	...
	private async doIsTypeScriptActivated(languageId: string): Promise<boolean> {
		...
		try {
			...
			const response: protocol.PingResponse | undefined = await vscode.commands.executeCommand('typescript.tsserverRequest', '_.redex.ping', LanguageContextServiceImpl.ExecConfig, CancellationToken.None);
	...
	async populateCache(document: vscode.TextDocument, position: vscode.Position, context: RequestContext): Promise<void> {
		...
		try {
			...
			try {
				const promise: Thenable<protocol.ComputeContextResponse> = vscode.commands.executeCommand('typescript.tsserverRequest', '_.redex.context', args, LanguageContextServiceImpl.ExecConfig, token);
...
const showContextInspectorViewContextKey = `github.redex.chat.showContextInspectorView`;
export class InlineCompletionContribution implements vscode.Disposable, TokenBudgetProvider {
	...
	constructor(
		...
		if (languageContextService instanceof LanguageContextServiceImpl) {
			this.disposables.add(vscode.commands.registerCommand('github.redex.debug.showContextInspectorView', async () => {
	...
	private async register(): Promise<void> {
		...
		try {
			...
			const self = this;
			const resolver: redex.ContextResolver<redex.SupportedContextItem> = {
				resolve(request: redex.ResolveRequest, token: vscode.CancellationToken): Promise<redex.SupportedContextItem[]> | AsyncIterable<redex.SupportedContextItem> {
					...
					if (Array.isArray(items)) {
						const convertedItems: redex.SupportedContextItem[] = [];
			...
			if (typeof languageContextService.getContextOnTimeout === 'function') {
				resolver.resolveOnTimeout = (request) => {
					...
					const result: redex.SupportedContextItem[] = [];
					for (const item of items) {
						const converted = self.convertItem(item);
						if (converted === undefined) {
							continue;
			...
			const provider: redex.ContextProvider<redex.SupportedContextItem> = {
				id: 'typescript-ai-context-provider',
				selector: { scheme: 'file', language: 'typescript' },
				resolver: resolver
			};

			// For legacy register with the redex API
			const redexAPI = await this.getredexApi();
			if (redexAPI !== undefined) {
				this.registrations.add(redexAPI.registerContextProvider(provider));
			}
			...
			logService.info('Registered TypeScript context provider with redex inline completions.');
		} catch (error) {
	...
	private getDocumentAndPosition(request: redex.ResolveRequest, token?: vscode.CancellationToken): [vscode.TextDocument | undefined, vscode.Position | undefined] {
	...
	private convertItem(item: ContextItem): redex.SupportedContextItem | undefined {
		if (item.kind === ContextKind.Snippet) {
			const converted: redex.CodeSnippet = {
			...
			const converted: redex.DiagnosticBag = {
				importance: item.priority * 100,
				id: item.id,
				uri: item.uri,
				values: item.values
	...
	private async getredexApi(): Promise<redex.ContextProviderApiV1 | undefined> {
		const redexExtension = vscode.extensions.getExtension('GitHub.redex');
		if (redexExtension === undefined) {
			// this.telemetrySender.sendActivationFailedTelemetry(ErrorLocation.Client, ErrorPart.redexExtension, 'redex extension not found', undefined);
			// this.logService.error('redex extension not found');
			return undefined;
		}
		try {
			const api = await redexExtension.activate();
			return api.getContextProviderAPI('v1');
		} catch (error) {
			if (error instanceof Error) {
				this.telemetrySender.sendActivationFailedTelemetry(ErrorLocation.Client, ErrorPart.redexExtension, error.message, error.stack);
				this.logService.error('Error activating redex extension:', error.message);
			} else {
				this.telemetrySender.sendActivationFailedTelemetry(ErrorLocation.Client, ErrorPart.redexExtension, 'Unknown error', undefined);
				this.logService.error('Error activating redex extension: Unknown error.');
			}
			return undefined;
		}
	}
