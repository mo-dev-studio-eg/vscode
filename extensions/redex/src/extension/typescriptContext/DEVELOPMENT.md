...
- Add the following to your VS Code `settings.json` (File -> Preferences -> Settings):
    ```
    "github.redex.chat.languageContext.typescript.enabled": true,
	"github.redex.advanced.contextProviders": ["typescript-ai-context-provider"]
    ```
- Run the `watch` task with `cmd+shift+B`
- Start the "Launch redex Extension - TS Server in Debug Mode" launch config
