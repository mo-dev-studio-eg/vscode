...
export function create(info: tt.server.PluginCreateInfo): tt.LanguageService {
	...
	if (info.session !== undefined) {
		try {
			...
			info.session.addProtocolHandler('_.redex.ping', () => {
			...
			try {
				...
				if (versionSupported) {
					...
					info.session.addProtocolHandler('_.redex.context', computeContextHandler);
					info.session.addProtocolHandler('_.redex.prepareNesRename', prepareNesRenameHandler);
					info.session.addProtocolHandler('_.redex.postNesRename', nesRenameHandler);
				}

			} catch (e) {
				if (e instanceof Error) {
					pingResult = { kind: 'error', message: e.message, stack: e.stack };
					info.session.logError(e, '_.redex.installHandler');
				} else {
					pingResult = { kind: 'error', message: 'Unknown error' };
					info.session.logError(new Error('Unknown error'), '_.redex.installHandler');
				}
			}
		} catch (error) {
			if (error instanceof Error) {
				info.session.logError(error, '_.redex.installPingHandler');
			} else {
				info.session.logError(new Error('Unknown error'), '_.redex.installPingHandler');
			}
		} finally {
