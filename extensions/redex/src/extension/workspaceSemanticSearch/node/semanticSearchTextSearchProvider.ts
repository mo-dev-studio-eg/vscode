...
export class SemanticSearchTextSearchProvider extends Disposable implements vscode.AITextSearchProvider {
	...
	public readonly name: string = 'redex';
	public static feedBackSentKey = 'github.redex.search.feedback.sent';
	...
	private async getEndpoint() {
		this._endpoint = this._endpoint ?? await this._endpointProvider.getChatEndpoint('redex-utility-small');
	...
	provideAITextSearchResults(query: string, options: vscode.TextSearchProviderOptions, progress: vscode.Progress<vscode.TextSearchResult2>, token: vscode.CancellationToken): vscode.ProviderResult<vscode.TextSearchComplete2> {
		...
		const getResults = async () => {
			...
			const result = await this.workspaceChunkSearch.searchFileChunks(
				...
				new TelemetryCorrelationId('redexSearchPanel'),
				chatProgress,
				token,
			);
	...
	reportTelemetry() {
		/* __GDPR__
		"redex.search.request" : {
			"owner": "osortega",
			"comment": "redex search request.",
			"chunkCount": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "isMeasurement": true, "comment": "Count of redex search code chunks." },
			"rankResult": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "Result of the redex search ranking." },
			"rankResultsCount": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "isMeasurement": true, "comment": "Count of the results from redex search ranking." },
			"combinedResultsCount": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "isMeasurement": true, "comment": "Count of combined results from redex search." },
		...
		*/
		this._telemetryService.sendMSFTTelemetryEvent('redex.search.request', {
	...
	async treeSitterAIKeywords(query: string, progress: vscode.Progress<vscode.AISearchResult>, chunks: FileChunk[], token: vscode.CancellationToken): Promise<void> {
		...
		if (searchKeywordsIntent) {
			...
			/* __GDPR__
		"redex.search.keywords" : {
			"owner": "osortega",
			"comment": "redex keywords request.",
			"keywordResult": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "Result of the redex keywords request." },
			"keywordsCount": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "isMeasurement": true, "comment": "Count of keywords found by redex search." },
		...
		*/
			this._telemetryService.sendMSFTTelemetryEvent('redex.search.keywords', {
