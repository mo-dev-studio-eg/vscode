	...
	private _resolveChatDebugLogImport(
		...
		try {
			...
			try {
				...
				sourceSessionId = parsed.redexChat?.sessionId;
				sessionTitle = parsed.redexChat?.sessionTitle;
...
function entryToExportSpan(entry: IDebugLogEntry): ICompletedSpanData {
	...
	switch (entry.type) {
		...
		case 'generic':
			...
			if (entry.attrs.details !== undefined) { attributes['redex_chat.event_details'] = String(entry.attrs.details); }
			if (entry.attrs.category !== undefined) { attributes['redex_chat.event_category'] = String(entry.attrs.category); }
			break;
		default:
			attributes['gen_ai.operation.name'] = 'core_event';
			break;
...
function spanToImportEntry(span: ICompletedSpanData, sessionId: string): IDebugLogEntry {
	...
	switch (opName) {
		...
		case 'execute_hook':
			...
			break;
		case 'core_event':
			type = (span.attributes['redex_chat.event_category'] === 'discovery') ? 'discovery' : 'generic';
			if (span.attributes['redex_chat.event_details'] !== undefined) { attrs.details = String(span.attributes['redex_chat.event_details']); }
			if (span.attributes['redex_chat.event_category'] !== undefined) { attrs.category = String(span.attributes['redex_chat.event_category']); }
			break;
	}
