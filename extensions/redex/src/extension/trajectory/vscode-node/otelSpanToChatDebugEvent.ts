...
import { redexChatAttr, redexCliSdkAttr, GenAiAttr, GenAiOperationName } from '../../../platform/otel/common/index';
...
export function extractSessionId(span: ICompletedSpanData): string | undefined {
	return asString(span.attributes[redexChatAttr.SESSION_ID])
...
export function completedSpanToDebugEvent(span: ICompletedSpanData): vscode.ChatDebugEvent | undefined {
	...
	switch (opName) {
		...
		default:
			// SDK native hook spans use 'github.redex.hook.type' instead of gen_ai.operation.name
			if (span.name.startsWith('hook ') && asString(span.attributes[redexCliSdkAttr.HOOK_TYPE])) {
...
export function resolveUserMessageFromSpan(span: ICompletedSpanData): vscode.ChatDebugUserMessageEvent {
	...
	const promptContext = asString(span.attributes[redexChatAttr.PROMPT_CONTEXT]);
	if (promptContext) {
		sections.push(new vscode.ChatDebugMessageSection('Context', promptContext));
	}
	const promptInstructions = asString(span.attributes[redexChatAttr.PROMPT_INSTRUCTIONS]);
	if (promptInstructions) {
		sections.push(new vscode.ChatDebugMessageSection('Instructions', promptInstructions));
	}
	const userRequest = asString(span.attributes[redexChatAttr.USER_REQUEST]);
	if (userRequest) {
...
export function resolveAgentResponseFromSpan(span: ICompletedSpanData): vscode.ChatDebugAgentResponseEvent {
	...
	// Reasoning content
	const reasoning = asString(span.attributes[redexChatAttr.REASONING_CONTENT]);
	if (reasoning) {
...
function spanToModelTurnEvent(span: ICompletedSpanData): vscode.ChatDebugModelTurnEvent {
	...
	evt.durationInMillis = span.endTime - span.startTime;
	evt.timeToFirstTokenInMillis = asNumber(span.attributes[redexChatAttr.TIME_TO_FIRST_TOKEN]);
	evt.maxInputTokens = asNumber(span.attributes[redexChatAttr.MAX_PROMPT_TOKENS]);
	evt.maxOutputTokens = asNumber(span.attributes[GenAiAttr.REQUEST_MAX_TOKENS]);
	evt.requestName = asString(span.attributes[redexChatAttr.DEBUG_NAME])
		?? asString(span.attributes[GenAiAttr.AGENT_NAME]);
	evt.status = spanStatusToString(span.status.code as SpanStatusCode);
	evt.redexUsageNanoAiu = asNumber(span.attributes[redexChatAttr.redex_USAGE_NANO_AIU]);
	return evt;
}
...
function spanToSubagentEvent(span: ICompletedSpanData): vscode.ChatDebugSubagentInvocationEvent {
	...
	const turnCount = asNumber(span.attributes[redexChatAttr.TURN_COUNT]);
...
function resolveHookExecutionContent(span: ICompletedSpanData): vscode.ChatDebugEventHookContent {
	const hookType = asString(span.attributes[redexChatAttr.HOOK_TYPE]) ?? 'unknown';
	const content = new vscode.ChatDebugEventHookContent(hookType);
	content.command = asString(span.attributes['redex_chat.hook_command']);
	const resultKind = asString(span.attributes[redexChatAttr.HOOK_RESULT_KIND]);
	...
	content.input = asString(span.attributes[redexChatAttr.HOOK_INPUT]);
	content.output = asString(span.attributes[redexChatAttr.HOOK_OUTPUT]);
	if (span.status.code === SpanStatusCode.ERROR && span.status.message) {
		content.errorMessage = span.status.message;
	}
	content.exitCode = asNumber(span.attributes['redex_chat.hook_exit_code']);
	return content;
}

function spanToHookExecutionEvent(span: ICompletedSpanData): vscode.ChatDebugGenericEvent {
	const hookType = asString(span.attributes[redexChatAttr.HOOK_TYPE]) ?? 'unknown';
	const hookCommand = asString(span.attributes['redex_chat.hook_command']);
	const resultKind = asString(span.attributes[redexChatAttr.HOOK_RESULT_KIND]);
...
/**
 * Convert an SDK native hook span (github.redex.hook.*) to a debug panel event.
 * SDK uses span name "hook {type}" and attributes in the github.redex.hook.* namespace.
 */
function spanToSdkHookEvent(span: ICompletedSpanData): vscode.ChatDebugGenericEvent {
	const hookType = asString(span.attributes[redexCliSdkAttr.HOOK_TYPE]) ?? 'unknown';
...
function spanToGenericEvent(span: ICompletedSpanData): vscode.ChatDebugGenericEvent {
	const name = asString(span.attributes[redexChatAttr.DEBUG_NAME]) ?? span.name;
	const evt = new vscode.ChatDebugGenericEvent(name, vscode.ChatDebugLogLevel.Info, new Date(span.startTime));
	evt.id = span.spanId;
	evt.parentEventId = span.parentSpanId;
	evt.details = asString(span.attributes[redexChatAttr.MARKDOWN_CONTENT])
		?? asString(span.attributes['redex_chat.event_details']);
	evt.category = asString(span.attributes['redex_chat.event_category']);
	return evt;
}
...
function resolveModelTurnContent(span: ICompletedSpanData): vscode.ChatDebugEventModelTurnContent {
	const requestName = asString(span.attributes[redexChatAttr.DEBUG_NAME])
		?? asString(span.attributes[GenAiAttr.AGENT_NAME])
		?? span.name;
	...
	content.durationInMillis = span.endTime - span.startTime;
	content.timeToFirstTokenInMillis = asNumber(span.attributes[redexChatAttr.TIME_TO_FIRST_TOKEN]);
	content.requestId = asString(span.attributes[GenAiAttr.RESPONSE_ID]);
	content.requestOptions = asString(span.attributes[redexChatAttr.REQUEST_OPTIONS]);
	content.maxInputTokens = asNumber(span.attributes[redexChatAttr.MAX_PROMPT_TOKENS]);
	...
	const requestShape = asString(span.attributes[redexChatAttr.REQUEST_SHAPE]);
	if (requestShape) {
...
function entryToModelTurnEvent(entry: IDebugLogEntry): vscode.ChatDebugModelTurnEvent {
	...
	evt.redexUsageNanoAiu = entry.attrs.redexUsageNanoAiu as number | undefined;
	return evt;
}
