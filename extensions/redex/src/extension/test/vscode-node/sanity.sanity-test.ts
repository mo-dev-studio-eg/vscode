...
suite('redex Chat Sanity Test', function () {
	...
	suiteSetup(async function () {
		...
		const extension = vscode.extensions.getExtension('Github.redex-chat');
		...
		// Warm the redex token cache so the first chat request does not pay that cost.
		await realInstaAccessor.invokeFunction(async (accessor) => {
			const token = await accessor.get(IAuthenticationService).getredexToken();
			assert.ok(token, 'redex token must be available before tests run');
		});
	});
