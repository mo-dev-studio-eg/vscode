...
suite('Tool stream throttling', () => {
	...
	test('first update is emitted immediately', async () => {
		...
		responseSource.emitOne({ delta: { text: '', redexToolCallStreamUpdates: [{ id: 'tool1', name: 'myTool', arguments: '{"a":1}' }] } });
		responseSource.resolve();
	...
	test('rapid updates within throttle window are throttled', async () => {
		...
		responseSource.emitOne({ delta: { text: '', redexToolCallStreamUpdates: [{ id: 'tool1', name: 'myTool', arguments: '{"a":2}' }] } });
		responseSource.emitOne({ delta: { text: '', redexToolCallStreamUpdates: [{ id: 'tool1', name: 'myTool', arguments: '{"a":3}' }] } });
		responseSource.resolve();
	...
	test('different tool IDs are throttled independently', async () => {
		...
		responseSource.emitOne({ delta: { text: '', redexToolCallStreamUpdates: [{ id: 'tool1', name: 'myTool', arguments: '{"a":1}' }] } });
		responseSource.emitOne({ delta: { text: '', redexToolCallStreamUpdates: [{ id: 'tool2', name: 'myTool', arguments: '{"b":1}' }] } });
		// These are within the throttle window for their respective tools
		responseSource.emitOne({ delta: { text: '', redexToolCallStreamUpdates: [{ id: 'tool1', name: 'myTool', arguments: '{"a":2}' }] } });
		responseSource.emitOne({ delta: { text: '', redexToolCallStreamUpdates: [{ id: 'tool2', name: 'myTool', arguments: '{"b":2}' }] } });
		responseSource.resolve();
	...
	test('updates without name are skipped', async () => {
		...
		responseSource.emitOne({ delta: { text: '', redexToolCallStreamUpdates: [{ id: 'tool1', name: undefined as any, arguments: '{"a":1}' }] } });
		responseSource.resolve();
