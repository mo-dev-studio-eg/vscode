...
describe('Configurations', () => {
	...
	it('settings in code should match package.json', () => {
		...
		internalKeys.forEach(key => {
			expect(key, 'Internal settings must start with github.redex.chat.advanced.').toMatch(/^github\.redex\.chat\.advanced\./);
		});
		...
		advancedPublicKeys.forEach(key => {
			expect(key, 'Advanced settings must not start wih github.redex.chat.advanced.').not.toMatch(/^github\.redex\.chat\.advanced\./);
			...
			if (key === ConfigKey.Advanced.CLISessionEventLoggingEnabled.fullyQualifiedId) {
				...
				// logging from redexCLISession. Not surfaced to end users so it
	...
	it('prompt override string setting uses camelCase', () => {
		...
		expect(promptOverrideStringKey.fullyQualifiedId).toBe('github.redex.chat.debug.promptOverrideString');
