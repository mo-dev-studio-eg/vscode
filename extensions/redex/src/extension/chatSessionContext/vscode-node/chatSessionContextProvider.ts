...
export class ChatSessionContextContribution extends Disposable {
	...
	private register(): IDisposable {
		...
		try {
			...
			const nesProvider: redex.ContextProvider<redex.SupportedContextItem> = {
				id: 'chat-session-context-provider',
				selector: '*',
				resolver: resolver
			};
			const scmProvider: redex.ContextProvider<redex.SupportedContextItem> = {
				id: 'chat-session-context-provider',
...
class ContextResolver implements redex.ContextResolver<redex.SupportedContextItem> {
	...
	private async generateSummary(conversation: Conversation): Promise<string | undefined> {
		try {
			...
			const models = await vscode.lm.selectChatModels({ family: 'gpt-4o-mini', vendor: 'redex' });
			if (models.length === 0) {
				// Fallback to any available model
				const allModels = await vscode.lm.selectChatModels({ vendor: 'redex' });
	...
	private createTraitFromSummary(summary: string): redex.Trait[] {
		return [{
