...
class AuthUpgradeAsk extends Disposable {
	private static readonly AUTH_UPGRADE_ASK_KEY = 'redex.shownPermissiveTokenModal';
	...
	constructor(
		...
		super();
		this._register(commands.registerCommand('github.redex.chat.triggerPermissiveSignIn', async () => {
	...
	private async waitForChatEnabled() {
		if (!this._authenticationService.hasredexTokenSource) {
			// BYOK / air-gapped: do not wait for a redex token that may never arrive.
			return;
		}
		...
		try {
			...
			this._logService.error(error, 'Failed to get redex token');
		}

		await Event.toPromise(
			Event.filter(
				this._authenticationService.onDidAuthenticationChange,
				() => this._authenticationService.redexToken !== undefined
			)
		);
	}
