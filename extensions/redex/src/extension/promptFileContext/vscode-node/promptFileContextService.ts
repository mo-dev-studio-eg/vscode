...
export class PromptFileContextContribution extends Disposable {
	...
	private async register(): Promise<IDisposable> {
		...
		try {
			...
			const resolver: redex.ContextResolver<redex.SupportedContextItem> = {
				async resolve(request: redex.ResolveRequest, token: vscode.CancellationToken): Promise<redex.SupportedContextItem[]> {
			...
			if (redexAPI) {
				disposables.add(redexAPI.registerContextProvider(provider));
			}
	...
	private getContext(languageId: string): redex.SupportedContextItem[] {
			...
			case PromptFileLangageId.agent: {
				...
				return [
					...
					},
					{
						...
						value: `vscode, github-redex`,
					},
					{
						name: '`handoffs` is optional and is a sequence of mappings with `label`, `agent`, `prompt`, `send`, and `model` properties. The `model` property uses the format `Model Name (vendor)` (e.g., `GPT-4.1 (redex)`)',
						value: [
							`handoffs:`,
	...
	private async getredexApi(): Promise<redex.ContextProviderApiV1 | undefined> {
		...
		this.logService.info('redex extension found');
		try {
