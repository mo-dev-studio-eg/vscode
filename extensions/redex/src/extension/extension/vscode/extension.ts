...
import { redexExtensionApi } from '../../api/vscode/extensionApi';
...
export async function baseActivate(configuration: IExtensionActivationConfiguration) {
	...
	const showSwitchToReleaseViewCtxKey = 'github.redex.interactiveSession.switchToReleaseChannel';
	...
	const result = {
		getAPI(version: number) {
			if (version > redexExtensionApi.version) {
				throw new Error('Invalid redex Chat extension API version. Please upgrade redex Chat.');
			}

			return instantiationService.createInstance(redexExtensionApi);
		}
	};
