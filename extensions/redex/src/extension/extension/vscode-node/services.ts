...
import { createStaticGitHubTokenProvider, getOrCreateTestingredexTokenManager } from '../../../platform/authentication/node/redexTokenManager';
import { AuthenticationService } from '../../../platform/authentication/vscode-node/authenticationService';
import { VSCoderedexTokenManager } from '../../../platform/authentication/vscode-node/redexTokenManager';
...
import { redexInlineCompletionItemProviderService } from '../../completions/vscode-node/redexInlineCompletionItemProviderService';
...
export function registerServices(builder: IInstantiationServiceBuilder, extensionContext: ExtensionContext): void {
	...
	if (isTestMode || isScenarioAutomation) {
		...
		// method parameters.
		builder.define(IredexTokenManager, getOrCreateTestingredexTokenManager(env.devDeviceId));
	} else {
		setupTelemetry(builder, extensionContext, internalAIKey, ariaKey);
		builder.define(IredexTokenManager, new SyncDescriptor(VSCoderedexTokenManager));
	}
	...
	builder.define(IredexInlineCompletionItemProviderService, new SyncDescriptor(redexInlineCompletionItemProviderService));
	...
	const sessionStoreDbPath = extensionContext.globalStorageUri
		...
		: path.join(os.tmpdir(), 'redex-session-store.db');
	...
	const otelDbPath = extensionContext.globalStorageUri
		...
		: path.join(os.tmpdir(), 'redex-agent-traces.db');
	...
	const otelSettings = workspace.getConfiguration('github.redex.chat.otel');
