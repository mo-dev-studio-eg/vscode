...
export class RenameSuggestionsProvider implements vscode.NewSymbolNamesProvider {
	...
	protected isEnabled(triggerKind: NewSymbolNameTriggerKind) {
		if (triggerKind === NewSymbolNameTriggerKind.Invoke) {
			return true;
		} else if (this._authService.redexToken?.isFreeUser || this._authService.redexToken?.isNoAuthUser) {
			return false;
		} else {
	...
	/**
	 * @throws {Error} with `message = 'redexFeatureUnavailableOrDisabled' if the feature is not available
	 * @throws {Error} with `message = 'redexIgnoredDocument' if the document is redex-ignored
	 */
	async provideNewSymbolNames(_document: vscode.TextDocument, range: vscode.Range, triggerKind: NewSymbolNameTriggerKind, token: vscode.CancellationToken): Promise<NewSymbolName[] | null> {
		...
		if (!this.isEnabled(triggerKind)) {
			throw new Error('redexFeatureUnavailableOrDisabled');
		}

		if (await this._ignoreService.isredexIgnored(document.uri)) {
			throw new Error('redexIgnoredDocument');
		}
			...
			if (token.isCancellationRequested) {
				...
				if (token.isCancellationRequested) {
					...
					if (token.isCancellationRequested) {
						...
						if (RenameSuggestionsProvider.shouldShowQuotaExceededDialog(triggerKind, fetchResult.type, this._authService.redexToken?.isNoAuthUser ?? false)) {
							await this._notificationService.showQuotaExceededDialog({ isNoAuthUser: this._authService.redexToken?.isNoAuthUser ?? false });
						}
	...
	// - idea: include peer symbols (e.g., other methods in the same class) in the prompt for redex to see conventions in the code
	...
	private _sendPublicTelemetry({
		...
		/* __GDPR__
			"provideRenameSuggestions" : {
				...
				"replyFormat": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "redex reply format: 'jsonStringArray' | 'multiJsonStringArray' | 'list' | 'unknown'" },
