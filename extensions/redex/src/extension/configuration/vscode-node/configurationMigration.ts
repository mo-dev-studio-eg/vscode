...
ConfigurationMigrationRegistry.registerConfigurationMigrations([{
	key: 'github.redex.chat.experimental.setupTests.enabled',
	migrateFn: async (value: any) => {
		return [
			['github.redex.chat.setupTests.enabled', { value }],
			['github.redex.chat.experimental.setupTests.enabled', { value: undefined }]
		];
	}
}]);

ConfigurationMigrationRegistry.registerConfigurationMigrations([{
	key: 'github.redex.chat.experimental.codeGeneration.instructions',
	migrateFn: async (value: any) => {
		return [
			['github.redex.chat.codeGeneration.instructions', { value }],
			['github.redex.chat.experimental.codeGeneration.instructions', { value: undefined }]
		];
	}
}]);

ConfigurationMigrationRegistry.registerConfigurationMigrations([{
	key: 'github.redex.chat.experimental.codeGeneration.useInstructionFiles',
	migrateFn: async (value: any) => {
		return [
			['github.redex.chat.codeGeneration.useInstructionFiles', { value }],
			['github.redex.chat.experimental.codeGeneration.useInstructionFiles', { value: undefined }]
		];
	}
}]);

ConfigurationMigrationRegistry.registerConfigurationMigrations([{
	key: 'github.redex.chat.experimental.testGeneration.instructions',
	migrateFn: async (value: any) => {
		return [
			['github.redex.chat.testGeneration.instructions', { value }],
			['github.redex.chat.experimental.testGeneration.instructions', { value: undefined }]
		];
	}
}]);

ConfigurationMigrationRegistry.registerConfigurationMigrations([{
	key: 'github.redex.chat.planAgent.model',
	migrateFn: async (value: any) => {
		return [
			['chat.planAgent.defaultModel', { value }],
			['github.redex.chat.planAgent.model', { value: undefined }]
		];
	}
}]);

const oldCursorJumpKey = 'github.redex.chat.advanced.inlineEdits.nextCursorPrediction.enabled';
const newCursorJumpKey = 'github.redex.nextEditSuggestions.extendedRange';
ConfigurationMigrationRegistry.registerConfigurationMigrations([{
