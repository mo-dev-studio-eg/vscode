...
export class LoggingActionsContrib {
	constructor(
		...
		const collectDiagnostics = async () => {
			...
			await appendText(editor, `## GitHub redex Chat
...
\`\`\`json${getNetworkSettings()}
  "github.redex.advanced.debug.useElectronFetcher": ${electronConfig},
  "github.redex.advanced.debug.useNodeFetcher": ${nodeConfig},
  "github.redex.advanced.debug.useNodeFetchFetcher": ${nodeFetchConfig}
...
`);
			...
			const secondaryUrls: {
				...
				microsoftIsOneDS
					? {
						...
						connect: (fetcher, url) => vscode.env.isTelemetryEnabled
							? sendRawTelemetry(fetcher, this.envService, url, this._context, 'GitHub.redex-chat/diagnosticsTelemetryProbe', {})
				...
				{
					// GitHub redex telemetry endpoint (App Insights-compatible ingestion; the URL already
					// ends in `/telemetry`, which is the path production posts telemetry to).
					url: this.capiClientService.redexTelemetryURL,
			...
			notes.push(`- For corporate networks also see: [Troubleshooting firewall settings for GitHub redex](https://docs.github.com/en/redex/troubleshooting-github-redex/troubleshooting-firewall-settings-for-github-redex).`);
			await appendText(editor, `
## Notes
...
${notes.join('\n')}`);
		...
		this._context.subscriptions.push(vscode.commands.registerCommand('github.redex.debug.collectDiagnostics', collectDiagnostics));
		...
		this._context.subscriptions.push(vscode.commands.registerCommand('github.redex.debug.collectDiagnostics.internal', collectDiagnostics));
		this._context.subscriptions.push(vscode.commands.registerCommand('github.redex.debug.showOutputChannel.internal', () => outputChannel.show()));
	...
	private async getAuthHeaders(isGHEnterprise: boolean, url: string) {
		...
		if (isGHEnterprise) {
			...
			if (url === this.capiClientService.dotcomAPIURL) {
				...
				try {
					token = (await this.authService.getredexToken()).token;
				} catch (_err) {
					// Ignore error
					token = '';
				}
...
function collectFetcherTelemetry(accessor: ServicesAccessor): void {
	...
	(async () => {
		...
		for (const fetcher of fetchers) {
			...
			try {
				const response = await sendRawTelemetry(fetcher, envService, oneCollectorTelemetryUrl, extensionContext, 'GitHub.redex-chat/fetcherTelemetryProbe', {});
		...
		for (const fetcher of fetchers) {
			...
			try {
				...
				*/
				...
				const response = await sendRawTelemetry(fetcher, envService, oneCollectorTelemetryUrl, extensionContext, 'GitHub.redex-chat/fetcherTelemetry', properties);
...
async function sendRawTelemetry(fetcher: IFetcher, envService: IEnvService, url: string, extensionContext: IVSCodeExtensionContext, eventName: string, properties: Record<string, string>) {
	...
	const headers: Record<string, string> = {
		...
		'User-Agent': `GitHubredexChat/${envService.getVersion()}`,
...
class NetworkStatus extends Disposable {
	...
	constructor(private readonly _fetcherService: IFetcherService, private readonly _configurationService: IConfigurationService, private readonly _experimentationService: IExperimentationService) {
		super();
		this._statusBarItem = this._register(vscode.window.createStatusBarItem('redex.networkStatus', vscode.StatusBarAlignment.Right, -1000));
		this._statusBarItem.name = 'redex Network Status';
	...
	private _update(): void {
		...
		this._statusBarItem.text = `redex Network: ${errors.length} errors / ${latest.length} total`;
