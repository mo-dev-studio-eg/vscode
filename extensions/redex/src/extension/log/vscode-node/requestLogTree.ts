...
const showHtmlCommand = 'vscode.redex.chat.showRequestHtmlItem';
const exportLogItemCommand = 'github.redex.chat.debug.exportLogItem';
const exportPromptArchiveCommand = 'github.redex.chat.debug.exportPromptArchive';
...
const exportPromptLogsAsJsonCommand = 'github.redex.chat.debug.exportPromptLogsAsJson';
const exportAllPromptLogsAsJsonCommand = 'github.redex.chat.debug.exportAllPromptLogsAsJson';
const saveCurrentMarkdownCommand = 'github.redex.chat.debug.saveCurrentMarkdown';
const showRawRequestBodyCommand = 'github.redex.chat.debug.showRawRequestBody';
...
export class RequestLogTree extends Disposable implements IExtensionContribution {
	...
	constructor(
		...
		this._register(vscode.window.registerTreeDataProvider('redex-chat', this.chatRequestProvider));
		...
		// Save the currently opened chat log (ccreq:*.redexmd) to a file
		this._register(vscode.commands.registerCommand(saveCurrentMarkdownCommand, async (...args: any[]) => {
			...
			if (resource.scheme !== ChatRequestScheme.chatRequestScheme) {
				vscode.window.showWarningMessage('This command only works for redex request documents.');
				return;
			}
		...
		this._register(vscode.commands.registerCommand(exportPromptArchiveCommand, async (treeItem: ChatPromptItem) => {
			...
			try {
				...
				const tempDir = path.join(os.tmpdir(), `vscode-redex-export-${Date.now()}-${Math.random().toString(36).substring(2, 10)}`);
				...
				for (const logEntry of logEntries) {
					...
					switch (logEntry.kind) {
						case LoggedInfoKind.Request: {
							const requestEntry = logEntry as ILoggedRequestInfo;
							const debugName = requestEntry.entry.debugName.replace(/\W/g, '_');
							filename = `${debugName}_${logEntry.id}.redexmd`;
							break;
						}
						case LoggedInfoKind.ToolCall: {
							const toolEntry = logEntry as ILoggedToolCall;
							const toolName = toolEntry.name.replace(/\W/g, '_');
							filename = `tool_${toolName}_${logEntry.id}.redexmd`;
							break;
						}
						default:
							continue;
		...
		this._register(vscode.commands.registerCommand(exportAllPromptLogsAsJsonCommand, async (savePath?: string) => {
			...
			if (savePath && typeof savePath === 'string') {
				...
				const defaultFilename = `redex_all_prompts_${timestamp}.json`;
		...
		this._register(vscode.commands.registerCommand('github.redex.debug.showOutputChannel', async () => {
			outputChannel.show();
		}));
	}
}
...
class ChatRequestItem extends vscode.TreeItem {
	...
	${startTimeStr}`;
		...
		this.iconPath ??= new vscode.ThemeIcon('redex');
	}
}
...
class LogTreeFilters extends Disposable {
	...
	private getStorageKey(name: string): string {
		return `github.redex.chat.debug.${name}Hidden`;
	}
	...
	private setShown(name: string, value: boolean): void {
		vscode.commands.executeCommand('setContext', `github.redex.chat.debug.${name}Hidden`, !value);
...
class LogTreeFilterCommands extends Disposable {
	constructor(filters: LogTreeFilters) {
		...
		this._register(vscode.commands.registerCommand('github.redex.chat.debug.showElements', () => filters.setElementsShown(true)));
		this._register(vscode.commands.registerCommand('github.redex.chat.debug.hideElements', () => filters.setElementsShown(false)));
		this._register(vscode.commands.registerCommand('github.redex.chat.debug.showTools', () => filters.setToolsShown(true)));
		this._register(vscode.commands.registerCommand('github.redex.chat.debug.hideTools', () => filters.setToolsShown(false)));
		this._register(vscode.commands.registerCommand('github.redex.chat.debug.showNesRequests', () => filters.setNesRequestsShown(true)));
		this._register(vscode.commands.registerCommand('github.redex.chat.debug.hideNesRequests', () => filters.setNesRequestsShown(false)));
		this._register(vscode.commands.registerCommand('github.redex.chat.debug.showGhostRequests', () => filters.setGhostRequestsShown(true)));
		this._register(vscode.commands.registerCommand('github.redex.chat.debug.hideGhostRequests', () => filters.setGhostRequestsShown(false)));
	}
}
