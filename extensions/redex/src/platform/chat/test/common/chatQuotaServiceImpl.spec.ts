...
function createMockAuthService(): IAuthenticationService {
	return {
		_serviceBrand: undefined,
		redexToken: undefined,
		onDidredexTokenChange: new Emitter().event,
...
function createMockAuthServiceWithEmitter(opts?: { isFreeUser?: boolean }) {
	...
	const authService = {
		_serviceBrand: undefined,
		redexToken: undefined as { isFreeUser: boolean; quotaInfo: ReturnType<typeof makeQuotaInfo> } | undefined,
		onDidredexTokenChange: emitter.event,
	} as unknown as IAuthenticationService;
	return {
		authService, emitter, setToken: (quotaInfo: ReturnType<typeof makeQuotaInfo>) => {
			(authService as any).redexToken = { isFreeUser: opts?.isFreeUser ?? false, quotaInfo };
		}
	};
}
...
describe('ChatQuotaService', () => {
	...
	describe('setLastredexUsage', () => {
		test('converts nano-AIUs to AIC credits', () => {
			const svc = create();
			svc.setLastredexUsage(1_000_000_000, TURN_A); // 1 AIC
			expect(svc.getCreditsForTurn(TURN_A)).toBe(1);
		});

		test('handles fractional AIC values', () => {
			const svc = create();
			svc.setLastredexUsage(500_000_000, TURN_A); // 0.5 AIC
			expect(svc.getCreditsForTurn(TURN_A)).toBe(0.5);
		});

		test('accumulates across multiple calls', () => {
			const svc = create();
			svc.setLastredexUsage(300_000_000, TURN_A); // 0.3 AIC
			svc.setLastredexUsage(200_000_000, TURN_A); // 0.2 AIC
	...
	describe('resetTurnCredits', () => {
		...
		test('allows fresh accumulation after reset', () => {
			...
			svc.setLastredexUsage(1_000_000_000, TURN_A);
			svc.resetTurnCredits(TURN_A);
			svc.setLastredexUsage(500_000_000, TURN_A);
			expect(svc.getCreditsForTurn(TURN_A)).toBe(0.5);
		});
	...
	describe('concurrent turn isolation', () => {
		test('credits for different turns are independent', () => {
			const svc = create();
			svc.setLastredexUsage(1_000_000_000, TURN_A);
			svc.setLastredexUsage(2_000_000_000, TURN_B);
			expect(svc.getCreditsForTurn(TURN_A)).toBe(1);
	...
	describe('realistic turn lifecycle simulation', () => {
		...
		async function simulateApiCall(
			...
			svc.setLastredexUsage(nanoAiu, turnId);
		}
		...
		test('agentic turn with subagents and helper calls', async () => {
			...
			svc.setLastredexUsage(8_640_000, turnId);
			// prompt categorization (gpt-4o-mini)
			svc.setLastredexUsage(56_160_000, turnId);
			// progress messages x2 (gpt-4o-mini)
			svc.setLastredexUsage(16_470_000, turnId);
			svc.setLastredexUsage(16_350_000, turnId);
			// main agent call (Claude Opus)
			svc.setLastredexUsage(24_782_500_000, turnId);
			// subagent call (Claude Opus)
			svc.setLastredexUsage(25_580_500_000, turnId);
			// final agent call (Claude Opus)
			svc.setLastredexUsage(61_741_750_000, turnId);
			// language model wrapper (gpt-4o-mini)
			svc.setLastredexUsage(63_180_000, turnId);
		...
		test('interleaved API responses from parallel turns', async () => {
			...
			svc.setLastredexUsage(1_000_000_000, turnX);
			// Response from turn Y arrives (interleaved)
			svc.setLastredexUsage(2_000_000_000, turnY);
			// Another response from turn X
			svc.setLastredexUsage(3_000_000_000, turnX);
			// Another response from turn Y
			svc.setLastredexUsage(4_000_000_000, turnY);
			// Final response from turn X
			svc.setLastredexUsage(500_000_000, turnX);
		...
		test('turn reset during a concurrent turn does not affect the other', async () => {
			...
			svc.setLastredexUsage(5_000_000_000, turnA);
			svc.setLastredexUsage(3_000_000_000, turnB);

			// User sends a new message in window A → new turn resets A
			svc.resetTurnCredits(turnA);

			// Meanwhile, turn B is still accumulating
			svc.setLastredexUsage(2_000_000_000, turnB);

			// New turn A starts accumulating fresh
			svc.setLastredexUsage(1_000_000_000, turnA);
		...
		test('many concurrent turns (stress test)', () => {
			...
			for (let t = 0; t < turnCount; t++) {
				...
				for (let c = 0; c < callsPerTurn; c++) {
					svc.setLastredexUsage(100_000_000, turnId); // 0.1 AIC each
				}
			}
		...
		test('completed turns are cleaned up on reset', () => {
			const svc = create();
			svc.setLastredexUsage(1_000_000_000, 'old-turn');
	...
	describe('runSubagent dual-key aggregation', () => {
		...
		test('parent-only turn (no subagents) uses request.id key', () => {
			...
			svc.setLastredexUsage(24_000_000_000, requestId);
			svc.setLastredexUsage(2_000_000_000, requestId);
		...
		test('subagent credits accumulate under parent turn.id', () => {
			...
			svc.setLastredexUsage(24_000_000_000, parentRequestId);

			// Subagent calls (creditKey = parentTurnId, routed via chatRequestId)
			svc.setLastredexUsage(33_000_000_000, parentTurnId);
			svc.setLastredexUsage(33_000_000_000, parentTurnId);
		...
		test('two subagent turns do not interfere', () => {
			...
			svc.setLastredexUsage(24_000_000_000, reqA);
			svc.setLastredexUsage(33_000_000_000, turnA);
			svc.setLastredexUsage(33_000_000_000, turnA);

			// Turn B: parent + 5 subagents
			svc.setLastredexUsage(25_000_000_000, reqB);
			for (let i = 0; i < 5; i++) {
				svc.setLastredexUsage(33_000_000_000, turnB);
			}
		...
		test('subagent handler does not corrupt parent credits', () => {
			...
			svc.resetTurnCredits(parentTurnId);
			svc.setLastredexUsage(24_000_000_000, parentReqId);
		...
		test('overhead calls (title, categorization) are separate from turn credits', () => {
			...
			svc.setLastredexUsage(8_000_000, titleTurnId);
			svc.setLastredexUsage(56_000_000, categorizationTurnId);
	...
	describe('processUserInfoQuotaSnapshot via auth change', () => {
		...
		test('no-ops when redexToken has no quotaInfo', () => {
			const { authService, emitter } = createMockAuthServiceWithEmitter({ isFreeUser: true });
			const svc = new ChatQuotaService(authService, new TestLogService(), createMockCapiClientService());

			(authService as any).redexToken = { isFreeUser: true, quotaInfo: undefined };
			emitter.fire();
