...
export class FetchStreamRecorder {
	...
	constructor(
		...
		this.callback = async (text: string, index: number, delta: IResponseDelta): Promise<number | undefined> => {
			const isContentChunk = !!(delta.text || delta.beginToolCalls || (typeof delta.thinking?.text === 'string' && delta.thinking?.text || delta.thinking?.text?.length) || delta.redexToolCalls || delta.redexToolCallStreamUpdates);
			if (isContentChunk) {
				const now = Date.now();
