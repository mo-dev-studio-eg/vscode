...
/**
 * This is the quota info we get from the `redex_internal/user` endpoint.
 * It is accessed via the redex token object
 */
export interface redexUserQuotaInfo {
...
export interface IChatQuotaService {
	...
	additionalUsageEnabled: boolean;
	/** AIC credits accumulated for the given turn, from redex_usage.total_nano_aiu. */
	...
	/** Accumulate per-request cost from redex_usage.total_nano_aiu (in nano-AIUs), scoped to a turn. */
	...
	clearQuota(): void;
	/**
	 * Fetches up-to-date quota data from the `redex_internal/user` endpoint.
