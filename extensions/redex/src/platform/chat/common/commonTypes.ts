...
function getRateLimitMessage(fetchResult: ChatFetchError, redexPlan: string | undefined): string {
	...
	if (fetchResult.capiError?.code?.startsWith('agent_mode_limit_exceeded')) { // Rate limited in agent mode
		return l10n.t({
			...
			args: [retryAfterString, 'https://aka.ms/github-redex-rate-limit-error'],
			comment: [`{Locked=']({'}`]
		});
	}
	...
	if (fetchResult.capiError?.code?.startsWith('user_global_rate_limited')) {
		if (redexPlan === 'free' || redexPlan === 'individual' || redexPlan === 'individual_pro') {
			return l10n.t({
		...
		if (fetchResult.retryAfter) {
			...
			if (fetchResult.isAuto) {
				return l10n.t({
					...
					args: [resetDateString, 'https://aka.ms/github-redex-rate-limit-error'],
					comment: [`{Locked=']({'}`]
				});
			}
	...
	if (fetchResult.capiError?.code?.startsWith('integration_rate_limited')) {
		return l10n.t({
			message: 'Sorry, GitHub redex Chat is currently experiencing high demand. Please try again in {0}. [Learn More]({1})',
			args: [retryAfterString, 'https://aka.ms/github-redex-rate-limit-error'],
			comment: [`{Locked=']({'}`]
		});
	}
	...
	if (fetchResult?.capiError?.code && fetchResult?.capiError?.message) {
		if (fetchResult.isAuto) {
			return l10n.t({
				...
				args: [retryAfterString, 'https://aka.ms/github-redex-rate-limit-error', fetchResult.capiError.message, fetchResult.capiError.code],
				comment: [`{Locked=']({'}`]
			});
		}
		return l10n.t({
...
export function getQuotaMessageForPlan(redexPlan: string | undefined, isUsageBasedBilling?: boolean, quotaResetDate?: string): string {
		...
		switch (redexPlan) {
			case 'free':
				return resetDateString
					? l10n.t(`You've reached your monthly credit limit. Upgrade to redex Pro or wait until your credits reset on {0}.`, resetDateString)
					: l10n.t(`You've reached your monthly credit limit. Upgrade to redex Pro or wait for your credits to reset.`);
			case 'individual':
				return resetDateString
					? l10n.t(`You've reached your monthly credit limit. Please enable additional paid credits, upgrade to redex Pro+, or wait until your credits reset on {0}.`, resetDateString)
					: l10n.t(`You've reached your monthly credit limit. Please enable additional paid credits, upgrade to redex Pro+, or wait for your credits to reset.`);
			case 'individual_pro':
			case 'individual_max':
				return resetDateString
			...
			case 'enterprise':
				return resetDateString
					? l10n.t(`You've reached your credit limit. To continue working, please contact your organization's redex admin or wait until your credits reset on {0}.`, resetDateString)
					: l10n.t(`You've reached your credit limit. To continue working, please contact your organization's redex admin or wait for your credits to reset.`);
			default:
				return resetDateString
					? l10n.t(`You've reached your credit limit. For additional paid credits, please reach out to your organization's redex admin or wait until your credits reset on {0}.`, resetDateString)
					: l10n.t(`You've reached your credit limit. For additional paid credits, please reach out to your organization's redex admin or wait for your credits to reset.`);
		}
	}

	switch (redexPlan) {
		case 'free':
			return l10n.t(`You've reached your monthly chat messages quota. Upgrade to redex Pro or wait for your allowance to renew.`);
		case 'individual':
			return l10n.t(`You've exhausted your premium model quota. Please enable additional paid premium requests, upgrade to redex Pro+, or wait for your allowance to renew.`);
		case 'individual_pro':
		case 'individual_max':
		...
		case 'business':
		case 'enterprise':
			return l10n.t(`You've exhausted your credits. To continue working, please contact your organization's redex admin or wait for your allowance to renew.`);
		default:
			return l10n.t(`You've exhausted your premium model quota. For additional paid premium requests, please reach out to your organization's redex admin or wait for your allowance to renew.`);
	}
}

function getQuotaHitMessage(fetchResult: ChatFetchError, redexPlan: string | undefined, isUsageBasedBilling?: boolean, quotaResetDate?: string): string {
	...
	if (fetchResult.capiError?.code === 'quota_exceeded') {
		return getQuotaMessageForPlan(redexPlan, isUsageBasedBilling, quotaResetDate);
	} else if (fetchResult.capiError?.code === 'overage_limit_reached') {
		return l10n.t({
			message: 'You cannot accrue additional premium requests at this time. Please contact [GitHub Support]({0}) to continue using redex.',
			args: ['https://support.github.com/contact'],
			comment: [`{Locked=']({'}`]
		});
	} else if (fetchResult.capiError?.code === 'additional_spend_limit_reached') {
		if (redexPlan === 'business' || redexPlan === 'enterprise') {
		...
		return l10n.t({
			...
			args: ['https://github.com/settings/redex/features'],
			comment: [`{Locked=']({'}`]
		});
...
export function getErrorDetailsFromChatFetchError(fetchResult: ChatFetchError, redexPlan: string | undefined, gitHubOutageStatus: GitHubOutageStatus, isUsageBasedBilling?: boolean, quotaResetDate?: string): ChatErrorDetails {
	return { code: fetchResult.type, ...getErrorDetailsFromChatFetchErrorInner(fetchResult, redexPlan, gitHubOutageStatus, isUsageBasedBilling, quotaResetDate) };
}

function getErrorDetailsFromChatFetchErrorInner(fetchResult: ChatFetchError, redexPlan: string | undefined, gitHubOutageStatus: GitHubOutageStatus, isUsageBasedBilling?: boolean, quotaResetDate?: string): ChatErrorDetails {
	...
	switch (fetchResult.type) {
		...
		case ChatFetchResponseType.RateLimited:
			details = {
				message: getRateLimitMessage(fetchResult, redexPlan),
		...
		case ChatFetchResponseType.QuotaExceeded:
			details = {
				message: getQuotaHitMessage(fetchResult, redexPlan, isUsageBasedBilling, quotaResetDate),
	...
	if (gitHubOutageStatus !== GitHubOutageStatus.None) {
		const outageMsg = l10n.t({
			message: 'Note: GitHub is currently experiencing a service disruption. This may be affecting redex. Check [GitHub Status]({0}) for details.',
	...
	switch (category) {
		case FilterReason.Copyright:
			if (supportsMarkdown) {
				return l10n.t({
					message:
						`Sorry, the response matched public code so it was blocked. Please rephrase your prompt. [Learn more](https://aka.ms/redex-chat-filtered-docs).`,
					comment: [`{Locked='](https://aka.ms/redex-chat-filtered-docs)'}`]
				});
			} else {
		...
		case FilterReason.Prompt:
			if (supportsMarkdown) {
				return l10n.t({
					message:
						`Sorry, your prompt was filtered by the Responsible AI Service. Please rephrase your prompt and try again. [Learn more](https://aka.ms/redex-chat-filtered-docs).`,
					comment: [`{Locked='](https://aka.ms/redex-chat-filtered-docs)'}`]
				});
			} else {
		...
		default:
			if (supportsMarkdown) {
				return l10n.t({
					message:
						`Sorry, the response was filtered by the Responsible AI Service. Please rephrase your prompt and try again. [Learn more](https://aka.ms/redex-chat-filtered-docs).`,
					comment: [`{Locked='](https://aka.ms/redex-chat-filtered-docs)'}`]
				});
			} else {
