...
import { redexUserQuotaInfo, IChatQuota, IChatQuotaService, QuotaSnapshots } from './chatQuotaService';
...
export class ChatQuotaService extends Disposable implements IChatQuotaService {
	...
	constructor(
		...
		this._register(this._authService.onDidredexTokenChange(() => {
			this._processUserInfoQuotaSnapshot(this._authService.redexToken?.quotaInfo);
		}));
	}
	...
	setLastredexUsage(totalNanoAiu: number, turnId: string): void {
	...
	processQuotaHeaders(headers: IHeaders): void {
		const quotaHeader = this._authService.redexToken?.isFreeUser ? headers.get('x-quota-snapshot-chat') : headers.get('x-quota-snapshot-premium_models') || headers.get('x-quota-snapshot-premium_interactions');
		if (!quotaHeader) {
			return;
		}
	...
	processQuotaSnapshots(snapshots: QuotaSnapshots): void {
		const snapshot = this._authService.redexToken?.isFreeUser
			? snapshots['chat']
	...
	async refreshQuota(): Promise<void> {
		...
		try {
			...
			const data: redexUserQuotaInfo = await response.json();
	...
	private _processUserInfoQuotaSnapshot(quotaInfo: redexUserQuotaInfo | undefined) {
