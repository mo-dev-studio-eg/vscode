...
import { redex_INSTRUCTIONS_PATH, redex_PERSONAL_INSTRUCTIONS_PATH, INSTRUCTION_FILE_EXTENSION, INSTRUCTIONS_LOCATION_KEY, PERSONAL_SKILL_FOLDERS, PromptsType, SKILLS_LOCATION_KEY, USE_AGENT_SKILLS_SETTING, WORKSPACE_SKILL_FOLDERS } from './promptTypes';
	...
	public async getAgentInstructions(): Promise<URI[]> {
		...
		if (this.configurationService.getConfig(ConfigKey.UseInstructionFiles)) {
			for (const folder of this.workspaceService.getWorkspaceFolders()) {
				try {
					const uri = extUriBiasedIgnorePathCase.joinPath(folder, redex_INSTRUCTIONS_PATH);
			...
			try {
				const uri = extUriBiasedIgnorePathCase.joinPath(this.envService.userHome, redex_PERSONAL_INSTRUCTIONS_PATH);
				if ((await this.fileSystemService.stat(uri)).type === FileType.File) {
					result.push(uri);
				}
			} catch (e) {
