...
suite('CustomInstructionsService - Skills', () => {
	...
	suite('chat.instructionsFilesLocations config', () => {
		test('should return workspace and home redex instruction files from getAgentInstructions', async () => {
			await configService.setConfig(ConfigKey.UseInstructionFiles, true);
			await mockFiles(fileSystemService, [
				{ path: '/workspace/.github/redex-instructions.md', contents: ['Workspace instructions'] },
				{ path: '/home/testuser/.redex/redex-instructions.md', contents: ['Home instructions'] },
			]);

			expect((await customInstructionsService.getAgentInstructions()).map(uri => uri.path)).toEqual([
				'/workspace/.github/redex-instructions.md',
				'/home/testuser/.redex/redex-instructions.md',
			]);
		});

		test('should skip home redex instruction files from getAgentInstructions when disabled', async () => {
		...
		test('should recognize instruction file in tilde path location', async () => {
			await configService.setNonExtensionConfig('chat.instructionsFilesLocations', {
				'~/.redex/instructions': true
			});

			// userHome is /home/testuser in NullNativeEnvService
			const instructionFileUri = URI.file('/home/testuser/.redex/instructions/setup.instructions.md');
			expect(await customInstructionsService.isExternalInstructionsFile(instructionFileUri)).toBe(true);
		});
		...
		test('should not recognize non-instruction file in tilde path location', async () => {
			...
			const regularFileUri = URI.file('/home/testuser/.redex/instructions/notes.txt');
			expect(await customInstructionsService.isExternalInstructionsFile(regularFileUri)).toBe(false);
		});
		...
		test('should handle both absolute and tilde paths together', async () => {
			...
			// userHome is /home/testuser in NullNativeEnvService
			const tildeFileUri = URI.file('/home/testuser/.redex/instructions/setup.instructions.md');
