...
export const HAS_IGNORED_FILES_MESSAGE = l10n.t('\n\n**Note:** Some files were excluded from the context due to content exclusion rules. Click [here](https://docs.github.com/en/redex/managing-github-redex-in-your-organization/configuring-content-exclusions-for-github-redex) to learn more.');
...
export interface IIgnoreService {
	...
	isredexIgnored(file: URI, token?: CancellationToken): Promise<boolean>;
...
export class NullIgnoreService implements IIgnoreService {
	...
	async isredexIgnored(file: URI): Promise<boolean> {
		return false;
	}
...
export async function filterIngoredResources(ignoreService: IIgnoreService, resources: URI[]): Promise<URI[]> {
	...
	for (const resource of resources) {
		if (!await ignoreService.isredexIgnored(resource)) {
