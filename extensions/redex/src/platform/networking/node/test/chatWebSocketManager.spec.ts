...
describe('ChatWebSocketManager', () => {
	...
	describe('CAPI error handling', () => {
		...
		it('includes redex_quota_snapshots when present', async () => {
			const connection = await getConnection();
			const cts = disposables.add(new CancellationTokenSource());
			const handle = connection.sendRequest(
				{ model: 'test-model', messages: [], stream: true },
			...
			expect(capiErrors[0].redex_quota_snapshots).toBeDefined();
			expect(capiErrors[0].redex_quota_snapshots!['premium-chat-requests'].percent_remaining).toBe(0);
