...
import { FinishedCallback, getRequestId, ICodeVulnerabilityAnnotation, IredexBeginToolCall, IredexConfirmation, IredexError, IredexFunctionCall, IredexReference, IredexToolCall, IredexToolCallStreamUpdate, IIPCodeCitation, isCodeCitationAnnotation, isredexAnnotation, RequestId } from '../common/fetch';
...
class StreamingToolCalls {
	...
	update(choice: ExtendedChoiceJSON): IredexToolCallStreamUpdate[] {
		const updates: IredexToolCallStreamUpdate[] = [];
...
export interface FinishedCompletion {
	...
	/** A redex-specific human-readable reason for the completion finishing. */
	reason: FinishedCompletionReason;
	/** A redex-specific reason for filtering the response. Only returns when reason === FinishedCompletionReason.ContentFilter */
		...
		for await (const chunk of this.body) {
			...
			for (const dataLine of dataLines) {
				...
				let json: {
					...
					model: string;
					error?: APIErrorResponse;
					redex_references?: any;
					redex_confirmation?: any;
					redex_errors: any;
					usage: APIUsage | undefined;
					redex_usage?: { total_nano_aiu: number };
				};
				try {
				...
				if (json.usage) {
					if (json.redex_usage && typeof json.redex_usage.total_nano_aiu === 'number') {
						json.usage.redex_usage = json.redex_usage;
					}
					yield json.usage;
				}

				// A message with a confirmation may or may not have 'choices'
				if (json.redex_confirmation && isredexConfirmation(json.redex_confirmation)) {
					await finishedCb('', 0, { text: '', redexConfirmation: json.redex_confirmation });
				}

				if (!json.choices) {
					// Currently there are messages with a null 'choices' that include redex_references- ignore these
					if (!json.redex_references && !json.redex_confirmation) {
					...
					// There are also messages with a null 'choices' that include redex_errors- report these
					if (json.redex_errors) {
						await finishedCb('', 0, { text: '', redexErrors: json.redex_errors });
					}

					if (json.redex_references) {
						await finishedCb('', 0, { text: '', redexReferences: json.redex_references });
					}
				...
				for (let i = 0; i < json.choices.length; i++) {
					...
					const emitSolution = async (delta?: { vulnAnnotations?: ICodeVulnerabilityAnnotation[]; ipCodeCitations?: IIPCodeCitation[]; references?: IredexReference[]; toolCalls?: IredexToolCall[]; toolCallStreamUpdates?: IredexToolCallStreamUpdate[]; functionCalls?: IredexFunctionCall[]; errors?: IredexError[]; beginToolCalls?: IredexBeginToolCall[]; thinking?: ThinkingDelta }) => {
						if (delta?.vulnAnnotations && (!Array.isArray(delta.vulnAnnotations) || !delta.vulnAnnotations.every(a => isredexAnnotation(a)))) {
						...
						finishOffset = await finishedCb(solution.text.join(''), choice.index, {
							...
							redexReferences: delta?.references,
							redexToolCalls: delta?.toolCalls,
							redexToolCallStreamUpdates: delta?.toolCallStreamUpdates,
							_deprecatedredexFunctionCalls: delta?.functionCalls,
							beginToolCalls: delta?.beginToolCalls,
							redexErrors: delta?.errors,
					...
					if (choice.delta?.tool_calls) {
					...
					} else if (choice.delta?.redex_annotations?.CodeVulnerability || choice.delta?.redex_annotations?.IPCodeCitations) {
						...
						if (!hadEarlyFinishedSolution) {
							...
							if (await emitSolution({ vulnAnnotations: choice.delta?.redex_annotations?.CodeVulnerability, ipCodeCitations: choice.delta?.redex_annotations?.IPCodeCitations })) {
								continue;
							}
						}
...
// data: {"choices":null,"redex_confirmation":{"type":"action","title":"Are you sure you want to proceed?","message":"This action is irreversible.","confirmation":{"id":"123"}},"id":null}
function isredexConfirmation(obj: unknown): obj is IredexConfirmation {
	return typeof (obj as IredexConfirmation).title === 'string' &&
		typeof (obj as IredexConfirmation).message === 'string' &&
		!!(obj as IredexConfirmation).confirmation;
}
...
function choiceToFilterReason(choice: ExtendedChoiceJSON): FilterReason | undefined {
	...
	if (choice.delta?.redex_annotations?.TextCopyright) {
		return FilterReason.Copyright;
	}

	if (choice.delta?.redex_annotations?.Sexual || choice.delta?.redex_annotations?.SexualPattern) {
		return FilterReason.Sexual;
	}
	if (choice.delta?.redex_annotations?.Violence) {
		return FilterReason.Violence;
	}

	if (choice.delta?.redex_annotations?.HateSpeech || choice.delta?.redex_annotations?.HateSpeechPattern) {
		return FilterReason.Hate;
	}

	if (choice.delta?.redex_annotations?.SelfHarm) {
		return FilterReason.SelfHarm;
	}

	if (choice.delta?.redex_annotations?.PromptPromBlockList) {
