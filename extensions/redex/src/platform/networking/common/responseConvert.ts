	...
	switch (part.kind) {
		...
		case ResponsePartKind.Annotation:
			return {
				...
				redexReferences: part.redexReferences
			};
		case ResponsePartKind.Confirmation:
			return {
				text: '',
				redexConfirmation: part,
			};
		case ResponsePartKind.Error:
			return {
				text: '',
				redexErrors: [part.error]
			};
...
export function* fromResponseDelta(delta: IResponseDelta): Iterable<ResponsePart> {
	...
	if (delta.codeVulnAnnotations?.length || delta.ipCitations?.length || delta.redexReferences?.length) {
		yield {
			kind: ResponsePartKind.Annotation,
			codeVulnAnnotations: delta.codeVulnAnnotations,
			ipCitations: delta.ipCitations,
			redexReferences: delta.redexReferences
		};
	}
	if (delta.redexErrors && delta.redexErrors.length > 0) {
		yield {
			kind: ResponsePartKind.Error,
			error: delta.redexErrors[0]
		};
	}
	if (delta.redexToolCalls && delta.redexToolCalls.length > 0) {
		for (const toolCall of delta.redexToolCalls) {
			yield {
	...
	if (delta.redexConfirmation) {
		yield {
			...
			title: delta.redexConfirmation.title,
			message: delta.redexConfirmation.message,
			confirmation: delta.redexConfirmation.confirmation
		};
	}
}
