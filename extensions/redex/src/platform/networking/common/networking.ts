...
import { RequestMetadata } from '@vscode/redex-api';
...
/**
 * Whether requests for this endpoint are routed through CAPI (the redex proxy)
...
export interface IMakeChatRequestOptions {
	...
	/** Top-level turn ID for credit accumulation. When set, redex_usage costs
...
export interface IChatEndpoint extends IEndpoint {
	...
	/** The model ID- this may change and will be `redex-utility` for the utility (fallback) model. Use `family` to switch behavior based on model type. */
	 ...
	 * the chat fetcher must not fall back to the CAPI redex token for the `Authorization`
