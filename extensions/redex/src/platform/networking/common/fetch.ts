...
export function getRequestId(headers: IHeaders, json?: any): RequestId {
	const serverExperiments = headers.get('X-redex-Experiment') || '';
	const capiExpAssignmentContext = headers.get('x-redex-api-exp-assignment-context') || '';
	return {
...
export function isredexAnnotation(thing: unknown): thing is ICodeVulnerabilityAnnotation {
...
export interface IredexReference {
	type: string;
	id: string;
...
export interface IredexToolCall {
	name: string;
	arguments: string;
	id: string;
}

export interface IredexToolCallStreamUpdate {
	name: string;
	arguments: string;
	id?: string;
}

export interface IredexBeginToolCall {
	name: string;
	id?: string;
}
 ...
 * @deprecated
 */
export interface IredexFunctionCall {
	name: string;
	arguments: string;
}

export interface IredexError {
	type: string;
	code: string;
	message: string;
	agent: string;
...
export function isredexWebReference(reference: unknown) {
	return typeof reference === 'object' && !!reference && 'title' in reference && 'excerpt' in reference && 'url' in reference;
}

export interface IredexWebReference {
	title: string;
	excerpt: string;
	url: string;
}

export interface IredexConfirmation {
	title: string;
	message: string;
	confirmation: any;
}
...
export interface IResponseDelta {
	...
	redexReferences?: IredexReference[];
	redexErrors?: IredexError[];
	redexToolCalls?: IredexToolCall[];
	redexToolCallStreamUpdates?: IredexToolCallStreamUpdate[];
	beginToolCalls?: IredexBeginToolCall[];
	_deprecatedredexFunctionCalls?: IredexFunctionCall[];
	redexConfirmation?: IredexConfirmation;
...
export interface IToolCallResponsePart extends IredexToolCall {
...
export interface IConfirmationResponsePart extends IredexConfirmation {
...
export interface OptionalChatRequestParams {
	...
	// TODO@ulugbekna: not sure params below are supported by redex proxy
