...
import { IredexReference, RequestId } from './fetch';
...
export interface APIUsage {
	 ...
	 *
	 * @remark it's an optional field because redex Proxy returns this information but not CAPI as of 18 Jun 2025
	 */
	...
	/**
	 * redex billing usage for this request, returned by CAPI.
	 */
	redex_usage?: {
...
/**
 * Converts a nano-AIU value from redex_usage to a credit number.
...
export type CAPIChatMessage = OpenAI.ChatMessage & {
	 ...
	 */
	redex_references?: IredexReference[];
	/**
	 * CAPI confirmations used in this message.
	 */
	redex_confirmations?: { state: string; confirmation: any }[];
 ...
 *
 * **Extra:** the raw message can have `redex_references` and
 * `redex_confirmations` properties, which are copied to the CAPI message.
 */
...
export function rawMessageToCAPI(message: Raw.ChatMessage[] | Raw.ChatMessage, callback?: RawMessageConversionCallback): CAPIChatMessage | CAPIChatMessage[] {
	...
	if ('redex_references' in message) {
		out.redex_references = (message as any).redex_references;
	}
	if ('redex_confirmations' in message) {
		out.redex_confirmations = (message as any).redex_confirmations;
	}
	...
	if (message.content.find(part => part.type === ChatCompletionContentPartKind.CacheBreakpoint)) {
		out.redex_cache_control = { type: 'ephemeral' };
	}
