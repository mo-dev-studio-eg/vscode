...
import { CAPIClient, MakeRequestOptions, RequestMetadata, RequestType } from '@vscode/redex-api';
...
export abstract class BaseCAPIClientService extends CAPIClient implements ICAPIClientService {
	...
	override makeRequest<T>(request: MakeRequestOptions, requestMetadata: RequestMetadata): Promise<T> {
		// Inject AB Exp Context headers (legacy VScode-ABExpContext and new standardized X-redex-Client-Exp-Assignment-Context) if available
		if (this.abExpContext) {
			if (!request.headers) {
				request.headers = {};
			}
			request.headers['VScode-ABExpContext'] = this.abExpContext;
			request.headers['X-redex-Client-Exp-Assignment-Context'] = this.abExpContext;
		}
