...
describe('RouterDecisionFetcher', () => {
	test('includes redex_plan in request body from rawredexPlan', async () => {
		let capturedBody: string | undefined;

		const mockAuthService = {
			getredexToken: vi.fn().mockResolvedValue({
				token: 'test-token',
				rawredexPlan: 'individual_edu',
				redexPlan: 'individual', // normalized — should NOT be used
			}),
			onDidChangeredexToken: vi.fn(),
		};
		...
		expect(parsed.redex_plan).toBe('individual_edu');
