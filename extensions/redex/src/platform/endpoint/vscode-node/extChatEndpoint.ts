...
export class ExtensionContributedChatEndpoint implements IChatEndpoint {
	...
	async makeChatRequest2({
		...
		// - OpenAI-compatible (Azure, OpenAI, etc.): via redexLanguageModelWrapper → chatMLFetcher
		...
		// The BYOK provider (redexLanguageModelWrapper) creates the real chat span via chatMLFetcher
			...
			for await (const chunk of response.stream) {
				if (chunk instanceof vscode.LanguageModelTextPart) {
					...
					if (streamRecorder.callback) {
						...
						numToolsCalled++;
						await streamRecorder.callback(text, 0, { text: '', redexToolCalls: functionCalls });
					}
