...
export class OpenAIResponsesProcessor {
	...
	public push(chunk: OpenAI.Responses.ResponseStreamEvent, _onProgress: FinishedCallback): ChatCompletion | undefined {
		...
		switch (chunk.type) {
			case 'error':
				return onProgress({ text: '', redexErrors: [{ agent: 'openai', code: chunk.code || 'unknown', message: chunk.message, type: 'error', identifier: chunk.param || undefined }] });
			...
			case 'response.output_item.done':
				if (chunk.item.type === 'function_call') {
					...
					if (tsCall.execution === 'client' && tsCall.call_id) {
						// Client-executed tool search completed: emit as a completed redexToolCall
						this.toolCallInfo.delete(chunk.output_index);
						onProgress({
							text: '',
							redexToolCalls: [{
			...
			case 'response.completed': {
				...
				return {
					...
					usage: {
						...
						redex_usage: capiChunk.redex_usage?.total_nano_aiu !== undefined ? capiChunk.redex_usage : undefined,
					},
