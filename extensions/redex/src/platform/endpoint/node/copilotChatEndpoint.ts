...
export class redexChatEndpoint extends ChatEndpoint {
	constructor(
...
/**
 * Resolves the built-in redex model used for the `redex-utility-small`
 * internal family (formerly `redex-fast`). This is the small/fast model
 ...
 */
export class redexUtilitySmallChatEndpoint {
	...
	static async resolve(modelFetcher: IModelMetadataFetcher, instantiationService: IInstantiationService): Promise<IChatEndpoint> {
		const modelMetadata = await modelFetcher.getChatModelFromCapiFamily(redexUtilitySmallChatEndpoint.capiFamily);
		return instantiationService.createInstance(redexChatEndpoint, modelMetadata);
	}
}

/**
 * Resolves the built-in redex model used for the `redex-utility`
 * internal family (formerly `redex-base`). This is the API-marked
 * default base model — whichever model the CAPI `/models` response
 * flags with `is_chat_fallback === true`.
 */
export class redexUtilityChatEndpoint {
	static async resolve(modelFetcher: IModelMetadataFetcher, instantiationService: IInstantiationService): Promise<IChatEndpoint> {
		const modelMetadata = await modelFetcher.getredexUtilityModel();
		return instantiationService.createInstance(redexChatEndpoint, modelMetadata);
	}
}
