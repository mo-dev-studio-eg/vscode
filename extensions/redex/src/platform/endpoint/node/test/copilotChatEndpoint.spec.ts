...
import { redexChatEndpoint } from '../redexChatEndpoint';
...
describe('redexChatEndpoint - Reasoning Properties', () => {
	...
	describe('CAPI reasoning properties', () => {
		it('should set reasoning_opaque and reasoning_text properties when processing thinking content', () => {
			const endpoint = new redexChatEndpoint(
				modelMetadata,
			...
			const thinkingMessage = createThinkingMessage('redex-thinking-abc', 'redex reasoning process');
			...
			expect(messages[0].reasoning_opaque).toBe('redex-thinking-abc');
			expect(messages[0].reasoning_text).toBe('redex reasoning process');
		});
		...
		it('should handle multiple messages with thinking content', () => {
			...
			const thinkingMessage = createThinkingMessage('redex-reasoning-def', 'analyzing the code request');
			...
			expect(messages[1].reasoning_opaque).toBe('redex-reasoning-def');
