...
describe('OpenAIResponsesProcessor tool search events', () => {
	...
	it('handles client tool_search_call as redexToolCall', () => {
		...
		// Second delta: completed redexToolCall
		expect(deltas[1].redexToolCalls).toBeDefined();
		expect(deltas[1].redexToolCalls![0]).toMatchObject({
			id: 'call_abc',
			name: 'tool_search',
			arguments: '{"query":"Find shipping tools"}',
		});
