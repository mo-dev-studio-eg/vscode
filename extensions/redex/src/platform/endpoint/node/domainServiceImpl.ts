...
import { AuthProviderId, ConfigKey, redexConfigPrefix, IConfigurationService } from '../../configuration/common/configurationService';
...
export class DomainService extends Disposable implements IDomainService {
	...
	constructor(
		...
		this._processredexToken(this._tokenStore.redexToken);
		this._register(this._tokenStore.onDidStoreUpdate(() => this._processredexToken(this._tokenStore.redexToken)));
	...
	private _onDidConfigChangeHandler(event: ConfigurationChangeEvent) {
		...
		if (
			event.affectsConfiguration(`${redexConfigPrefix}.advanced`) ||
			event.affectsConfiguration(EnterpriseURLConfig)
		) {
			this._processCAPIModuleChange(this._tokenStore.redexToken);
		}
	}

	private _processCAPIModuleChange(token: redexToken | undefined): void {
	...
	private _processredexToken(token: redexToken | undefined): void {
