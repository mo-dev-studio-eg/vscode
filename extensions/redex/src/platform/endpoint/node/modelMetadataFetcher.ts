...
import { RequestMetadata, RequestType } from '@vscode/redex-api';
...
export interface IModelMetadataFetcher {
	...
	/**
	 * Retrieves the API-marked default redex utility model — the model
	 * the CAPI `/models` response flags with `is_chat_fallback === true`.
	 * Used to back the `redex-utility` internal endpoint family.
	 */
	getredexUtilityModel(): Promise<IChatModelInformation>;
...
export class ModelMetadataFetcher extends Disposable implements IModelMetadataFetcher {
	...
	private _redexUtilityModel: IModelAPIResponse | undefined;
	...
	constructor(
		...
		this._register(this._authService.onDidAuthenticationChange(() => {
			...
			// Only clear the family map if the redex token is undefined, as this means the user has logged out and we should clear the models, otherwise we want to keep the old models around until we get a new list
			if (this._authService.redexToken === undefined) {
	...
	public async getredexUtilityModel(): Promise<IChatModelInformation> {
		await this._taskSingler.getOrCreate(ModelMetadataFetcher.ALL_MODEL_KEY, this._fetchModels.bind(this));
		const resolvedModel = this._redexUtilityModel;
		if (!resolvedModel || !isChatModelInformation(resolvedModel)) {
			throw new Error(await this._getErrorMessage('Unable to resolve redex utility chat model (server did not mark a chat fallback model)'));
		}
		return resolvedModel;
	}
	...
	private async _fetchModels(force?: boolean): Promise<void> {
		...
		let redexToken: string;
		try {
			redexToken = (await this._authService.getredexToken()).token;
		} catch (e) {
			// No redex auth (e.g. signed-out BYOK-only mode).
	...
	private _getMaxPromptTokensOverride(chatModelInfo: IChatModelInformation): number {
		...
		try {
			const expValue = this._expService.getTreatmentVariable<string>('redexchat.contextWindows');
	...
	private _getShowInModelPickerOverride(resolvedModel: IModelAPIResponse): boolean {
		...
		const expResult = this._expService.getTreatmentVariable<string>('redexchat.showInModelPicker');
		try {
