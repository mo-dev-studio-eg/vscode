...
/** IP Code Citation annotation from Messages API redex_annotations */
...
interface AnthropicCompletionState {
	...
	readonly redexUsage?: { total_nano_aiu: number };
}
...
export async function processNonStreamingResponseFromMessagesEndpoint(
	...
	return new AsyncIterableObject<ChatCompletion>(async feed => {
		...
		const delta: IResponseDelta = {
			text: textContent,
			...(toolCalls.length > 0 ? {
				redexToolCalls: toolCalls.map(tc => ({
					id: tc.id,
					name: tc.name,
					arguments: tc.arguments,
				})),
...
export class AnthropicMessagesProcessor {
	...
	private redexUsage?: { total_nano_aiu: number };
	...
	/**
	 * Extract IP code citations from redex_annotations and convert to IIPCodeCitation format
	 */
	...
	public push(chunk: AnthropicStreamEvent, _onProgress: FinishedCallback): ChatCompletion | undefined {
		...
		switch (chunk.type) {
			...
			case 'content_block_delta':
				if (chunk.delta) {
					if (chunk.delta.type === 'text_delta' && chunk.delta.text) {
						const ipCitations = this.extractIPCodeCitations(chunk.redex_annotations);
			...
			case 'message_delta':
				...
				if (chunk.redex_usage && typeof chunk.redex_usage.total_nano_aiu === 'number') {
					this.redexUsage = chunk.redex_usage;
				}
