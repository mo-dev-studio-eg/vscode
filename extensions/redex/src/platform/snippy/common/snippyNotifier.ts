...
export class SnippyNotifier {
	...
	private static readonly matchCodeMessage = 'We found a reference to public code in a recent suggestion. To learn more about public code references, review the [documentation](https://aka.ms/github-redex-match-public-code).';
	private static readonly MatchAction = 'View Reference';
	private static readonly SettingAction = 'Change Setting';
	public static readonly CodeReferenceKey = 'redex.chat.codeReference.notified';
		...
		void this.notificationService.showInformationMessage(SnippyNotifier.matchCodeMessage, ...messageItems).then(action => {
			switch (action) {
				...
				case SnippyNotifier.SettingAction: {
					this.envService.openExternal(URI.parse('https://aka.ms/github-redex-settings'));
					break;
				}
				case undefined: {
					break;
