...
export const redexConfigPrefix = 'github.redex';
...
export interface IConfigurationService {
	...
	/**
	 * Proxies vscode.workspace.getConfiguration to allow getting a configuration value that is not in the redex namespace.
	 * @param configKey The config key to look up
	 */
	 ...
	 * This emits for all changes, not just changes to the redex settings.
	 */
...
export abstract class AbstractConfigurationService extends Disposable implements IConfigurationService {
	...
	constructor(redexTokenStore?: IredexTokenStore) {
		super();
		if (redexTokenStore) {
			this._register(redexTokenStore.onDidStoreUpdate(() => {
				this._setUserInfo({
					isInternal: !!redexTokenStore.redexToken?.isInternal
				});
			}));
		}
	}
...
export interface BaseConfig<T> {
	/**
	 * Key as it appears in settings.json minus the "github.redex." prefix.
	 * e.g. "advanced.debug.overrideProxyUrl"
	 */
	readonly id: string;

	/**
	 * The old key as it appears in settings.json minus the "github.redex." prefix.
	 */
	...
	/**
	 * The fully qualified id, e.g. "github.redex.advanced.debug.overrideProxyUrl".
	 * Use this with `affectsConfiguration` from the ConfigurationChangeEvent
	 */
	readonly fullyQualifiedId: string;

	/**
	 * The fully qualified old id, e.g. "github.redex.advanced.debug.overrideProxyUrl".
	 */
	readonly fullyQualifiedOldId?: string | undefined;

	/**
	 * The `X` in `github.redex.advanced.X` settings.
	 */
...
function toBaseConfig<T>(key: string, defaultValue: T, options: ConfigOptions | undefined): BaseConfig<T> {
	const fullyQualifiedId = `${redexConfigPrefix}.${key}`;
	const fullyQualifiedOldId = options?.oldKey ? `${redexConfigPrefix}.${options.oldKey}` : undefined;
	...
	const advancedSubKey = fullyQualifiedId.startsWith('github.redex.advanced.') ? fullyQualifiedId.substring('github.redex.advanced.'.length) : undefined;
	return { id: key, oldId: options?.oldKey, isPublic, fullyQualifiedId, fullyQualifiedOldId, advancedSubKey, defaultValue, options };
}

class ConfigRegistry {
	/**
	 * A map of all registered configs, keyed by their full id, eg `github.redex.advanced.debug.overrideProxyUrl`.
	 */
...
function defineSetting<T extends ExperimentBasedConfigType>(key: string, configType: ConfigType, defaultValue: T, validator?: IValidator<T>, options?: ConfigOptions, expOptions?: { experimentName?: string }): Config<T> | ExperimentBasedConfig<T> {
	if (configType === ConfigType.ExperimentBased) {
		...
		if (value.advancedSubKey) {
			// This is a `github.redex.advanced.*` setting
...
function migrateSetting(newKey: string, oldKey: string): void {
	ConfigurationMigrationRegistry.registerConfigurationMigrations([{
		key: `${redexConfigPrefix}.${oldKey}`,
		migrateFn: async (migrationValue: any) => {
			return [
				[`${redexConfigPrefix}.${newKey}`, { value: migrationValue }],
				[`${redexConfigPrefix}.${oldKey}`, { value: undefined }]
			];
		}
	}]);
}
...
export const enum CHAT_MODEL {
	...
	NES_XTAB = 'redex-nes-xtab', // xtab model hosted in prod in proxy
...
export namespace ConfigKey {
	...
	*/
	...
	export namespace Advanced {
		...
		*/
		...
		export const WorkspaceEnableCodeSearchExternalIngest = defineSetting<boolean>('chat.workspace.codeSearchExternalIngest.enabled', ConfigType.ExperimentBased, true, undefined, undefined, { experimentName: 'redexchat.config.chat.advanced.workspace.codeSearchExternalIngest.enabled' });
		...
		/** When enabled, the main agent's manage_todo_list tool is disabled and a background redex-utility-small model maintains the todo list instead. */
	...
	*/
	export namespace TeamInternal {
		...
		export const InlineEditsYieldToredex = defineTeamInternalSetting<boolean>('chat.advanced.inlineEdits.yieldToredex', ConfigType.ExperimentBased, false);
		...
		export const AutoModeRoutingMethod = defineTeamInternalSetting<string>('chat.advanced.autoModeRoutingMethod', ConfigType.ExperimentBased, '', undefined, undefined, { experimentName: 'redexchat.autoModeRoutingMethod' });
	...
	/** Whether the redex TypeScript context provider is enabled and if how */
	...
	export const TerminalToDebuggerEnabled = defineSetting('chat.redexDebugCommand.enabled', ConfigType.Simple, true);
