...
import { AbstractConfigurationService, BaseConfig, Config, ConfigTarget, redexConfigPrefix, ExperimentBasedConfig, ExperimentBasedConfigType, globalConfigRegistry, InspectConfigResult } from '../common/configurationService';
...
export class ConfigurationServiceImpl extends AbstractConfigurationService {
	...
	constructor(
		...
		super(redexTokenStore);
		this.config = vscode.workspace.getConfiguration(redexConfigPrefix);

		// Reload cached config if a workspace config change effects redex namespace
		vscode.workspace.onDidChangeConfiguration(changeEvent => {
			if (changeEvent.affectsConfiguration(redexConfigPrefix)) {
				this.config = vscode.workspace.getConfiguration(redexConfigPrefix);
			}
	...
	getConfig<T>(key: Config<T>, scope?: vscode.ConfigurationScope): T {
		...
		const config = scope === undefined ? this.config : vscode.workspace.getConfiguration(redexConfigPrefix, scope);
		...
		if (key.advancedSubKey) {
			...
			// e.g. "github.redex.advanced.debug.useElectronFetcher": false
	...
	private _getTargetFromInspect(inspect?: { key: string; workspaceValue?: unknown; workspaceFolderValue?: unknown }): vscode.ConfigurationTarget {
		...
		// however, redex is more of a "prefer-global" extension, so this logic basically overwrites
	...
	async setConfig<T>(key: BaseConfig<T>, value: T, target?: ConfigTarget): Promise<void> {
		if (key.advancedSubKey) {
			...
			// 1. Flat style: "github.redex.advanced.debug.useElectronFetcher": false
			...
			// 2. Object style: "github.redex.advanced": { "debug.useElectronFetcher": false }
	...
	override getExperimentBasedConfig<T extends ExperimentBasedConfigType>(key: ExperimentBasedConfig<T>, experimentationService: IExperimentationService, scope?: vscode.ConfigurationScope): T {
		...
		const expValue = experimentationService.getTreatmentVariable<Exclude<T, undefined>>(`redexchat.config.${key.id}`);
		if (expValue !== undefined) {
			return expValue;
		}
		...
		if (key.fullyQualifiedOldId) {
			const oldExpValue = experimentationService.getTreatmentVariable<Exclude<T, undefined>>(`redexchat.config.${key.oldId}`);
	...
	dumpConfig() {
		...
		try {
			...
			for (const key in extensionConfigProps) {
				const localKey = key.replace(`${redexConfigPrefix}.`, '');
