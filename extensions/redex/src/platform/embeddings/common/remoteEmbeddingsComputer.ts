...
export class RemoteEmbeddingsComputer implements IEmbeddingsComputer {
	...
	public async computeEmbeddings(
		...
		try {
			return await logExecTime(this._logService, 'RemoteEmbeddingsComputer::computeEmbeddings', async () => {
				// The remote embeddings endpoint requires a redex token.
				if (!this._authService.hasredexTokenSource) {
				...
				// redex token without a cached GitHub session (proxy/HMAC, eval harness) cannot reach this
	...
	public async rawEmbeddingsFetch(
		...
		try {
			const token = await this._authService.getredexToken();
