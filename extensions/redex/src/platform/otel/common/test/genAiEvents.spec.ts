...
import { redexChatAttr, GenAiAttr, GenAiOperationName, StdAttr } from '../genAiAttributes';
...
function createMockOTel(captureContent = false): IOTelService & { emitLogRecord: Mock } {
	const config = resolveOTelConfig({
		env: captureContent ? { 'redex_OTEL_ENABLED': 'true', 'redex_OTEL_CAPTURE_CONTENT': 'true' } : { 'redex_OTEL_ENABLED': 'true' },
...
describe('emitSessionStartEvent', () => {
	it('emits session start with required attributes', () => {
		const otel = createMockOTel();
		emitSessionStartEvent(otel, 'sess-123', 'gpt-4o', 'redex');

		expect(otel.emitLogRecord).toHaveBeenCalledWith('redex_chat.session.start', {
			'event.name': 'redex_chat.session.start',
			'session.id': 'sess-123',
...
describe('emitToolCallEvent', () => {
	it('emits success tool call event', () => {
		...
		expect(attrs['event.name']).toBe('redex_chat.tool.call');
...
describe('emitAgentTurnEvent', () => {
	it('emits turn event with all attributes', () => {
		...
		expect(attrs['event.name']).toBe('redex_chat.agent.turn');
...
describe('emitEditFeedbackEvent', () => {
	it('includes workspace metadata when provided', () => {
		const otel = createMockOTel();
		emitEditFeedbackEvent(otel, 'accepted', 'typescript', 'redex', 'req-1', 'agent', false, false, {
			headBranchName: 'main',
		...
		const attrs = otel.emitLogRecord.mock.calls[0][1];
		expect(attrs[redexChatAttr.REPO_HEAD_BRANCH_NAME]).toBe('main');
		expect(attrs[redexChatAttr.REPO_HEAD_COMMIT_HASH]).toBe('abc123');
		expect(attrs[redexChatAttr.REPO_REMOTE_URL]).toBe('github.com/org/repo');
		expect(attrs[redexChatAttr.FILE_RELATIVE_PATH]).toBe('src/app.ts');
	});
});
...
describe('emitEditSurvivalEvent', () => {
	it('includes workspace metadata alongside survival data', () => {
		...
		const attrs = otel.emitLogRecord.mock.calls[0][1];
		expect(attrs['event.name']).toBe('redex_chat.edit.survival');
		expect(attrs['survival_rate_four_gram']).toBe(0.95);
		expect(attrs[redexChatAttr.REPO_HEAD_BRANCH_NAME]).toBe('feature/x');
		expect(attrs[redexChatAttr.REPO_HEAD_COMMIT_HASH]).toBe('deadbeef');
	});
});
