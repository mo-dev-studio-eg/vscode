	...
	it('span is created with model and conversation ID in _doFetch phase', () => {
		...
		const span = otel.startSpan('chat gpt-4o', {
			...
			attributes: {
				...
				[redexChatAttr.MAX_PROMPT_TOKENS]: 128000,
			},
		});
		...
		span.setAttributes({
			...
			[redexChatAttr.TIME_TO_FIRST_TOKEN]: 450,
			[GenAiAttr.REQUEST_STREAM]: true,
			[GenAiAttr.RESPONSE_TIME_TO_FIRST_CHUNK]: 0.45,
			[redexChatAttr.redex_USAGE_NANO_AIU]: 3_500_000_000,
		});
		...
		expect(s.attributes[redexChatAttr.redex_USAGE_NANO_AIU]).toBe(3_500_000_000);
