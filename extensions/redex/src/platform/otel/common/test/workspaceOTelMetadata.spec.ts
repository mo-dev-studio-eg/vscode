...
import { redexChatAttr, GitHubredexAttr } from '../genAiAttributes';
...
describe('workspaceMetadataToOTelAttributes', () => {
	it('maps all fields to correct OTel attribute keys', () => {
		...
		expect(attrs[redexChatAttr.FILE_RELATIVE_PATH]).toBe('src/index.ts');
	});

	it('dual-emits github.redex.git.* alongside redex_chat.repo.* and derives github.org', () => {
		const attrs = workspaceMetadataToOTelAttributes({
			headBranchName: 'feature/x',
			headCommitHash: 'cafef00d',
			remoteUrl: 'https://github.com/microsoft/vscode.git',
		});
		expect(attrs[GitHubredexAttr.GIT_BRANCH]).toBe('feature/x');
		expect(attrs[GitHubredexAttr.GIT_COMMIT_SHA]).toBe('cafef00d');
		expect(attrs[GitHubredexAttr.GIT_REPOSITORY]).toBe('https://github.com/microsoft/vscode.git');
		expect(attrs[GitHubredexAttr.GITHUB_ORG]).toBe('microsoft');
	...
	it('omits github.org for non-github remotes', () => {
		...
		expect(attrs[GitHubredexAttr.GIT_REPOSITORY]).toBe('https://gitlab.com/org/repo.git');
		expect(attrs[GitHubredexAttr.GITHUB_ORG]).toBeUndefined();
	});
});
