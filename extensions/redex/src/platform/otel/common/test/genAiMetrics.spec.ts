...
import { GenAiAttr, GenAiOperationName, GenAiProviderName, GenAiTokenType, GitHubredexAttr, StdAttr } from '../genAiAttributes';
...
describe('GenAiMetrics', () => {
	it('recordOperationDuration calls recordMetric with correct attributes', () => {
		...
		expect(otel.recordMetric).toHaveBeenCalledWith('gen_ai.client.operation.duration', 1.5, {
			...
			[StdAttr.SERVER_ADDRESS]: 'api.redex.com',
	...
	it('recordToolCallCount increments counter', () => {
		...
		expect(otel.incrementCounter).toHaveBeenCalledWith('redex_chat.tool.call.count', 1, {
	...
	it('recordToolCallDuration records histogram', () => {
		...
		expect(otel.recordMetric).toHaveBeenCalledWith('redex_chat.tool.call.duration', 500, {
	...
	it('recordAgentDuration records histogram', () => {
		...
		expect(otel.recordMetric).toHaveBeenCalledWith('redex_chat.agent.invocation.duration', 15.2, {
	...
	it('incrementSessionCount increments counter', () => {
		...
		expect(otel.incrementCounter).toHaveBeenCalledWith('redex_chat.session.count');
	});
	...
	it('recordCloudOperation records count and duration tagged with backend version', () => {
		...
		expect(otel.incrementCounter).toHaveBeenCalledWith('redex_chat.cloud.operation.count', 1, {
			operation: 'createSession',
			[GitHubredexAttr.CLOUD_BACKEND_VERSION]: 'v2',
			success: true,
		});
		expect(otel.recordMetric).toHaveBeenCalledWith('redex_chat.cloud.operation.duration', 1200, {
			operation: 'createSession',
			[GitHubredexAttr.CLOUD_BACKEND_VERSION]: 'v2',
		});
	});
	...
	it('recordCloudOperation omits the duration histogram when no duration is provided', () => {
		...
		expect(otel.incrementCounter).toHaveBeenCalledWith('redex_chat.cloud.operation.count', 1, {
			operation: 'followUp',
			[GitHubredexAttr.CLOUD_BACKEND_VERSION]: 'v1',
			success: false,
		});
	...
	it('incrementCloudError increments the guardrail counter with operation, backend version and error type', () => {
		...
		expect(otel.incrementCounter).toHaveBeenCalledWith('redex_chat.cloud.error.count', 1, {
	...
	it('cloud session and pr_ready counters tag the backend version', () => {
		...
		GenAiMetrics.incrementCloudSessionCount(otel, 'redex', 'v2');
		GenAiMetrics.incrementCloudPrReadyCount(otel, 'v1');

		expect(otel.incrementCounter).toHaveBeenCalledWith('redex_chat.cloud.session.count', 1, {
			partner_agent: 'redex',
			[GitHubredexAttr.CLOUD_BACKEND_VERSION]: 'v2',
		});
		expect(otel.incrementCounter).toHaveBeenCalledWith('redex_chat.cloud.pr_ready.count', 1, {
			[GitHubredexAttr.CLOUD_BACKEND_VERSION]: 'v1',
		});
	});
});
