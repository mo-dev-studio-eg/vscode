...
import { redexChatAttr, GenAiAttr, GenAiOperationName, GenAiProviderName } from '../genAiAttributes';
...
describe('Agent Trace Hierarchy', () => {
	...
	it('emits session start event and agent metrics', async () => {
		...
		emitSessionStartEvent(otel, 'sess-abc', 'gpt-4o', 'redex');
		GenAiMetrics.incrementSessionCount(otel);
		GenAiMetrics.recordAgentDuration(otel, 'redex', 15.2);
		GenAiMetrics.recordAgentTurnCount(otel, 'redex', 4);
		emitAgentTurnEvent(otel, 0, 1500, 250, 2);

		// Session event
		expect(otel.logRecords).toHaveLength(2); // session.start + agent.turn
		expect(otel.logRecords[0].attributes?.['event.name']).toBe('redex_chat.session.start');

		// Agent turn event
		expect(otel.logRecords[1].attributes?.['event.name']).toBe('redex_chat.agent.turn');
		expect(otel.logRecords[1].attributes?.['turn.index']).toBe(0);

		// Metrics
		expect(otel.counters).toHaveLength(1);
		expect(otel.counters[0].name).toBe('redex_chat.session.count');
		expect(otel.metrics).toHaveLength(2);
		expect(otel.metrics[0].name).toBe('redex_chat.agent.invocation.duration');
		expect(otel.metrics[1].name).toBe('redex_chat.agent.turn.count');
	});
	...
	it('records edit acceptance and survival metrics', () => {
		...
		expect(otel.counters[0].name).toBe('redex_chat.edit.acceptance.count');
		expect(otel.counters[0].attributes?.[redexChatAttr.EDIT_SOURCE]).toBe('inline_chat');
		expect(otel.counters[0].attributes?.[redexChatAttr.EDIT_OUTCOME]).toBe('accepted');
		expect(otel.counters[0].attributes?.[redexChatAttr.LANGUAGE_ID]).toBe('typescript');

		expect(otel.counters[1].name).toBe('redex_chat.edit.acceptance.count');
		expect(otel.counters[1].attributes?.[redexChatAttr.EDIT_OUTCOME]).toBe('rejected');

		// Chat edit outcome counter
		expect(otel.counters[2].name).toBe('redex_chat.chat_edit.outcome.count');
		expect(otel.counters[2].attributes?.[redexChatAttr.EDIT_SOURCE]).toBe('chat_editing');
		expect(otel.counters[2].attributes?.[redexChatAttr.EDIT_OUTCOME]).toBe('accepted');
		expect(otel.counters[2].attributes?.[redexChatAttr.HAS_REMAINING_EDITS]).toBe(false);

		// Survival histograms
		expect(otel.metrics).toHaveLength(2);
		expect(otel.metrics[0].name).toBe('redex_chat.edit.survival.four_gram');
		expect(otel.metrics[0].value).toBe(0.85);
		expect(otel.metrics[0].attributes?.[redexChatAttr.EDIT_SOURCE]).toBe('inline_chat');
		expect(otel.metrics[0].attributes?.[redexChatAttr.TIME_DELAY_MS]).toBe(30000);

		expect(otel.metrics[1].name).toBe('redex_chat.edit.survival.no_revert');
	...
	it('omits optional attributes when undefined', () => {
		...
		expect(otel.counters[0].attributes?.[redexChatAttr.LANGUAGE_ID]).toBeUndefined();
		expect(otel.counters[1].attributes?.[redexChatAttr.LANGUAGE_ID]).toBeUndefined();
		expect(otel.counters[1].attributes?.[redexChatAttr.HAS_REMAINING_EDITS]).toBeUndefined();
	});
});
