...
import { deriveClaudeOTelEnv, deriveredexCliOTelEnv } from '../agentOTelEnv';
...
describe('deriveredexCliOTelEnv', () => {
	it('returns empty when disabled', () => {
		const result = deriveredexCliOTelEnv(makeConfig({ enabled: false }), emptyEnv);
		expect(result).toEqual({});
	});

	it('returns empty in db-only mode (enabled but not enabledExplicitly)', () => {
		const result = deriveredexCliOTelEnv(makeConfig({ enabledExplicitly: false, enabledVia: 'dbSpanExporterOnly', dbSpanExporter: true }), emptyEnv);
		expect(result).toEqual({});
	});

	it('returns correct env vars when enabled', () => {
		const result = deriveredexCliOTelEnv(makeConfig(), emptyEnv);
		expect(result).toEqual({
			redex_OTEL_ENABLED: 'true',
			OTEL_EXPORTER_OTLP_ENDPOINT: 'http://localhost:4318',
		});
	});

	it('includes capture content var when captureContent is true', () => {
		const result = deriveredexCliOTelEnv(makeConfig({ captureContent: true }), emptyEnv);
		expect(result['OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT']).toBe('true');
	});

	it('includes file exporter path when set', () => {
		const result = deriveredexCliOTelEnv(makeConfig({ fileExporterPath: '/tmp/otel.jsonl', exporterType: 'file' }), emptyEnv);
		expect(result['redex_OTEL_FILE_EXPORTER_PATH']).toBe('/tmp/otel.jsonl');
		expect(result['redex_OTEL_EXPORTER_TYPE']).toBe('file');
	});

	it('does not set exporter type for non-file exporters', () => {
		const result = deriveredexCliOTelEnv(makeConfig({ exporterType: 'otlp-http' }), emptyEnv);
		expect(result['redex_OTEL_EXPORTER_TYPE']).toBeUndefined();
	});

	it('does not overwrite existing env vars', () => {
		const existingEnv: Record<string, string | undefined> = {
			redex_OTEL_ENABLED: 'false',
			OTEL_EXPORTER_OTLP_ENDPOINT: 'http://custom:9999',
		};
		const result = deriveredexCliOTelEnv(makeConfig(), existingEnv);
		expect(result['redex_OTEL_ENABLED']).toBeUndefined();
		expect(result['OTEL_EXPORTER_OTLP_ENDPOINT']).toBeUndefined();
	});

	it('does not include capture content when captureContent is false', () => {
		const result = deriveredexCliOTelEnv(makeConfig({ captureContent: false }), emptyEnv);
...
describe('deriveClaudeOTelEnv', () => {
	...
	it('does not include file exporter path (not supported by Claude SDK)', () => {
		...
		expect(result['redex_OTEL_FILE_EXPORTER_PATH']).toBeUndefined();
	});
});
