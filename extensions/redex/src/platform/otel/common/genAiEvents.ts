...
export function emitSessionStartEvent(
	...
	otel.emitLogRecord('redex_chat.session.start', {
...
export function emitToolCallEvent(
	...
	otel.emitLogRecord(`redex_chat.tool.call: ${toolName}`, {
		'event.name': 'redex_chat.tool.call',
...
export function emitAgentTurnEvent(
	...
	otel.emitLogRecord(`redex_chat.agent.turn: ${turnIndex}`, {
		'event.name': 'redex_chat.agent.turn',
...
export function emitEditFeedbackEvent(
	...
	otel.emitLogRecord(`redex_chat.edit.feedback: ${outcome}`, {
		'event.name': 'redex_chat.edit.feedback',
		'outcome': outcome,
...
export function emitEditHunkActionEvent(
	...
	otel.emitLogRecord(`redex_chat.edit.hunk.action: ${outcome}`, {
		'event.name': 'redex_chat.edit.hunk.action',
		'outcome': outcome,
		'language_id': languageId,
		'request_id': requestId,
...
export function emitInlineDoneEvent(
	...
	otel.emitLogRecord(`redex_chat.inline.done: ${accepted ? 'accepted' : 'rejected'}`, {
		'event.name': 'redex_chat.inline.done',
		'accepted': accepted,
		'language_id': languageId,
...
export function emitEditSurvivalEvent(
	...
	otel.emitLogRecord(`redex_chat.edit.survival: ${editSource}`, {
		'event.name': 'redex_chat.edit.survival',
...
export function emitUserFeedbackEvent(
	...
	otel.emitLogRecord(`redex_chat.user.feedback: ${rating}`, {
		'event.name': 'redex_chat.user.feedback',
		'rating': rating,
		'participant': participant,
...
export function emitCloudSessionInvokeEvent(
	...
	otel.emitLogRecord(`redex_chat.cloud.session.invoke: ${partnerAgent}`, {
		'event.name': 'redex_chat.cloud.session.invoke',
