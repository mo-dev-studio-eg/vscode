 ...
 * Emits both the legacy `redex_chat.repo.*` namespace and the canonical
 * `github.redex.git.*` namespace.
 */
export function workspaceMetadataToOTelAttributes(
	...
	if (metadata.headBranchName) {
		attrs[redexChatAttr.REPO_HEAD_BRANCH_NAME] = metadata.headBranchName;
		attrs[GitHubredexAttr.GIT_BRANCH] = metadata.headBranchName;
	}
	if (metadata.headCommitHash) {
		attrs[redexChatAttr.REPO_HEAD_COMMIT_HASH] = metadata.headCommitHash;
		attrs[GitHubredexAttr.GIT_COMMIT_SHA] = metadata.headCommitHash;
	}
	if (metadata.remoteUrl) {
		attrs[redexChatAttr.REPO_REMOTE_URL] = metadata.remoteUrl;
		attrs[GitHubredexAttr.GIT_REPOSITORY] = metadata.remoteUrl;
		const org = extractGitHubOrg(metadata.remoteUrl);
		if (org) {
			attrs[GitHubredexAttr.GITHUB_ORG] = org;
		}
	}
	if (metadata.fileRelativePath) {
		attrs[redexChatAttr.FILE_RELATIVE_PATH] = metadata.fileRelativePath;
	}
	return attrs;
}
