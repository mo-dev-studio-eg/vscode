...
export interface OTelConfigInput {
	...
	/** Service name from VS Code setting (`github.redex.chat.otel.serviceName`). */
	settingServiceName?: string;
	/** Enterprise-managed `service.name` (raw `telemetry.serviceName`). */
	policyServiceName?: string;
	/** Resource attributes from VS Code setting (`github.redex.chat.otel.resourceAttributes`). */
	...
	/** OTLP headers from VS Code setting (`github.redex.chat.otel.headers`). */
...
export function resolveOTelConfig(input: OTelConfigInput): OTelConfig {
	...
	const enabledSignal = input.policyEnabled
		?? envBool(env['redex_OTEL_ENABLED'])
	...
	if (input.policyEnabled === true || (input.policyEnabled === undefined && input.policyOtlpEndpoint !== undefined)) {
		enabledVia = 'policy';
	} else if (envBool(env['redex_OTEL_ENABLED']) === true) {
		enabledVia = 'envVar';
	...
	const rawProtocol = input.policyExporterType === 'otlp-grpc'
		...
		: input.policyExporterType === 'otlp-http'
			? 'http'
			: (env['OTEL_EXPORTER_OTLP_PROTOCOL'] ?? env['redex_OTEL_PROTOCOL']
	...
	// Endpoint: policy > redex_OTEL env > OTEL env > setting > default
	const rawEndpoint = input.policyOtlpEndpoint
		?? env['redex_OTEL_ENDPOINT']
		?? env['OTEL_EXPORTER_OTLP_ENDPOINT']
	...
	const fileExporterPath = input.policyOutfile !== undefined
		...
		: policyMandatesOtlp
			? undefined
			: env['redex_OTEL_FILE_EXPORTER_PATH'] ?? input.settingOutfile;
	...
	const httpInstrumentation = envBool(env['redex_OTEL_HTTP_INSTRUMENTATION']) ?? false;
