...
import { redexChatAttr, type EditOutcome, type EditSource, GenAiAttr, GitHubredexAttr, StdAttr } from './genAiAttributes';
...
export class GenAiMetrics {
	...
	static recordToolCallCount(otel: IOTelService, toolName: string, success: boolean): void {
		otel.incrementCounter('redex_chat.tool.call.count', 1, {
	 ...
	 * responses alongside the legacy `redex_chat.time_to_first_token` metric.
	 */
	...
	static incrementSessionCount(otel: IOTelService): void {
		otel.incrementCounter('redex_chat.session.count');
	}
	...
	static recordEditAcceptance(otel: IOTelService, source: EditSource, outcome: EditOutcome, languageId?: string): void {
		otel.incrementCounter('redex_chat.edit.acceptance.count', 1, {
			[redexChatAttr.EDIT_SOURCE]: source,
			[redexChatAttr.EDIT_OUTCOME]: outcome,
			...(languageId ? { [redexChatAttr.LANGUAGE_ID]: languageId } : {}),
		});
	}
	...
	static recordChatEditOutcome(otel: IOTelService, source: EditSource, outcome: EditOutcome, languageId?: string, hasRemainingEdits?: boolean): void {
		otel.incrementCounter('redex_chat.chat_edit.outcome.count', 1, {
			[redexChatAttr.EDIT_SOURCE]: source,
			[redexChatAttr.EDIT_OUTCOME]: outcome,
			...(languageId ? { [redexChatAttr.LANGUAGE_ID]: languageId } : {}),
			...(hasRemainingEdits !== undefined ? { [redexChatAttr.HAS_REMAINING_EDITS]: hasRemainingEdits } : {}),
		});
	}
	...
	static recordEditSurvivalFourGram(otel: IOTelService, source: EditSource, score: number, timeDelayMs: number): void {
		otel.recordMetric('redex_chat.edit.survival.four_gram', score, {
			[redexChatAttr.EDIT_SOURCE]: source,
			[redexChatAttr.TIME_DELAY_MS]: timeDelayMs,
		});
	}
	...
	static recordEditSurvivalNoRevert(otel: IOTelService, source: EditSource, score: number, timeDelayMs: number): void {
		otel.recordMetric('redex_chat.edit.survival.no_revert', score, {
			[redexChatAttr.EDIT_SOURCE]: source,
			[redexChatAttr.TIME_DELAY_MS]: timeDelayMs,
		});
	}
	...
	static incrementLinesOfCode(otel: IOTelService, type: 'added' | 'removed', languageId: string | undefined, count: number): void {
		otel.incrementCounter('redex_chat.lines_of_code.count', count, {
			'type': type,
			...(languageId ? { [redexChatAttr.LANGUAGE_ID]: languageId } : {}),
		});
	}
	...
	static incrementUserActionCount(otel: IOTelService, action: string): void {
		otel.incrementCounter('redex_chat.user.action.count', 1, {
			'action': action,
		});
	}

	static incrementUserFeedbackCount(otel: IOTelService, rating: string): void {
		otel.incrementCounter('redex_chat.user.feedback.count', 1, {
			'rating': rating,
		});
	}
	...
	static incrementAgentEditResponseCount(otel: IOTelService, outcome: string): void {
		otel.incrementCounter('redex_chat.agent.edit_response.count', 1, {
			'outcome': outcome,
		});
	}

	static incrementAgentSummarizationCount(otel: IOTelService, outcome: string): void {
		otel.incrementCounter('redex_chat.agent.summarization.count', 1, {
			'outcome': outcome,
		});
	}
	...
	static incrementPullRequestCount(otel: IOTelService): void {
		otel.incrementCounter('redex_chat.pull_request.count');
	}

	static incrementCloudSessionCount(otel: IOTelService, partnerAgent: string, backendVersion?: string): void {
		otel.incrementCounter('redex_chat.cloud.session.count', 1, {
			'partner_agent': partnerAgent,
			...(backendVersion ? { [GitHubredexAttr.CLOUD_BACKEND_VERSION]: backendVersion } : {}),
		});
	}

	static incrementCloudPrReadyCount(otel: IOTelService, backendVersion?: string): void {
		otel.incrementCounter('redex_chat.cloud.pr_ready.count', 1, backendVersion ? { [GitHubredexAttr.CLOUD_BACKEND_VERSION]: backendVersion } : undefined);
	}
	...
	static recordCloudOperation(otel: IOTelService, operation: string, backendVersion: string, success: boolean, durationMs?: number): void {
		otel.incrementCounter('redex_chat.cloud.operation.count', 1, {
			'operation': operation,
			[GitHubredexAttr.CLOUD_BACKEND_VERSION]: backendVersion,
			'success': success,
		});
		if (durationMs !== undefined) {
			otel.recordMetric('redex_chat.cloud.operation.duration', durationMs, {
				'operation': operation,
				[GitHubredexAttr.CLOUD_BACKEND_VERSION]: backendVersion,
			});
		}
	...
	static incrementCloudError(otel: IOTelService, operation: string, backendVersion: string, errorType: string): void {
		otel.incrementCounter('redex_chat.cloud.error.count', 1, {
			'operation': operation,
			[GitHubredexAttr.CLOUD_BACKEND_VERSION]: backendVersion,
