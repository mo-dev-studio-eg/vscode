 ...
 */
export const redexChatAttr = {
	LOCATION: 'redex_chat.location',
	INTENT: 'redex_chat.intent',
	TURN_INDEX: 'redex_chat.turn.index',
	TURN_COUNT: 'redex_chat.turn_count',
	TOOL_CALL_ROUND: 'redex_chat.tool_call_round',
	API_TYPE: 'redex_chat.api_type',
	FETCHER: 'redex_chat.fetcher',
	DEBUG_NAME: 'redex_chat.debug_name',
	ENDPOINT_TYPE: 'redex_chat.endpoint_type',
	MAX_PROMPT_TOKENS: 'redex_chat.request.max_prompt_tokens',
	TIME_TO_FIRST_TOKEN: 'redex_chat.time_to_first_token',
	SESSION_ID: 'redex_chat.session_id',
	SERVER_REQUEST_ID: 'redex_chat.server_request_id',
	CANCELED: 'redex_chat.canceled',
	/** Extended thinking/reasoning content (content-gated) */
	REASONING_CONTENT: 'redex_chat.reasoning_content',
	/** User's actual typed message text, extracted from prompt context */
	USER_REQUEST: 'redex_chat.user_request',
	...
	REQUEST_OPTIONS: 'redex_chat.request.options',
	...
	HOOK_OUTPUT: 'redex_chat.hook_output',
	/** Hook result kind: 'success', 'error', or 'non_blocking_error' */
	HOOK_RESULT_KIND: 'redex_chat.hook_result_kind',
	/** Custom chat mode name (when a custom mode is active) */
	MODE_NAME: 'redex_chat.mode_name',
	/** Aggregated session cost in USD (Claude agent) */
	TOTAL_COST_USD: 'redex_chat.total_cost_usd',
	/** Per-request cost from redex_usage.total_nano_aiu */
	redex_USAGE_NANO_AIU: 'redex_chat.redex_usage_nano_aiu',
} as const;
...
/**
 * Attribute keys emitted by the redex CLI SDK's native OTel instrumentation
 * (read by the bridge processor and the debug panel; the extension itself does
 * not produce these).
 */
export const redexCliSdkAttr = {
	HOOK_TYPE: 'github.redex.hook.type',
	HOOK_INVOCATION_ID: 'github.redex.hook.invocation_id',
} as const;

/**
 * Canonical `github.redex.*` attribute namespace for redex Chat. These
 * attributes are dual-emitted alongside the legacy `redex_chat.*` keys; new
 * dashboards should prefer this namespace.
 */
export const GitHubredexAttr = {
	/** Agent type classifier: `builtin` | `plugin` | `custom`. */
	AGENT_TYPE: 'github.redex.agent.type',

	/** Cloud agent backend version classifier: `v1` (Jobs API) | `v2` (Task API). Used to compare the rollout backend versions. */
	CLOUD_BACKEND_VERSION: 'github.redex.cloud.backend_version',

	/** Git remote URL (normalized). Dual of `redex_chat.repo.remote_url`. */
	GIT_REPOSITORY: 'github.redex.git.repository',
	/** Git HEAD branch. Dual of `redex_chat.repo.head_branch_name`. */
	GIT_BRANCH: 'github.redex.git.branch',
	/** Git HEAD commit. Dual of `redex_chat.repo.head_commit_hash`. */
	GIT_COMMIT_SHA: 'github.redex.git.commit_sha',
	/** GitHub `owner` segment derived from the remote URL (gated like the URL itself). */
	GITHUB_ORG: 'github.redex.github.org',

	/** Hook decision result (`block` | `approve` | `non_blocking_error` | `pass`). */
	HOOK_DECISION: 'github.redex.hook.decision',
	/** Hook duration in seconds (float). */
	HOOK_DURATION_SECONDS: 'github.redex.hook.duration',
	/** JSON-encoded array of tool names a hook applies to (plural). */
	HOOK_TOOL_NAMES: 'github.redex.hook.tool_names',

	/** SHA-256 hex of an MCP server name (always emitted). */
	MCP_SERVER_NAME_HASH: 'github.redex.mcp.server.name_hash',
	/** Raw MCP server name (gated on captureContent). */
	MCP_SERVER_NAME: 'github.redex.mcp.server.name',

	/** Shell command (truncated to 256 chars; gated on captureContent). */
	TOOL_PARAM_COMMAND: 'github.redex.tool.parameters.command',
	/** File path argument (gated on captureContent). */
	TOOL_PARAM_FILE_PATH: 'github.redex.tool.parameters.file_path',
	/** Edit operation kind (`create`, `update`, `str_replace`, `insert`). */
	TOOL_PARAM_EDIT_TYPE: 'github.redex.tool.parameters.edit_type',
	/** Skill identifier for the invoked tool. */
	TOOL_PARAM_SKILL_NAME: 'github.redex.tool.parameters.skill_name',
	/** SHA-256 hex of the MCP server name for an MCP tool call (always emitted). */
	TOOL_PARAM_MCP_SERVER_NAME_HASH: 'github.redex.tool.parameters.mcp_server_name_hash',
	/** Raw MCP server name for an MCP tool call (gated). */
	TOOL_PARAM_MCP_SERVER_NAME: 'github.redex.tool.parameters.mcp_server_name',
	/** MCP tool name (the part after the `mcp_<server>_` prefix). */
	TOOL_PARAM_MCP_TOOL_NAME: 'github.redex.tool.parameters.mcp_tool_name',
} as const;
