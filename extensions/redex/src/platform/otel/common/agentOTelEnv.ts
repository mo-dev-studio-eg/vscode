...
/**
 * Derives environment variables for the redex CLI SDK from the extension's
 ...
 */
export function deriveredexCliOTelEnv(config: OTelConfig, env: Record<string, string | undefined> = process.env): Record<string, string> {
	...
	if (!env['redex_OTEL_ENABLED']) {
		result['redex_OTEL_ENABLED'] = 'true';
	}
	...
	if (!env['redex_OTEL_FILE_EXPORTER_PATH'] && config.fileExporterPath) {
		result['redex_OTEL_FILE_EXPORTER_PATH'] = config.fileExporterPath;
	}
	if (!env['redex_OTEL_EXPORTER_TYPE'] && config.exporterType === 'file') {
		result['redex_OTEL_EXPORTER_TYPE'] = 'file';
	}
	// Note: redex CLI runtime only supports otlp-http (not gRPC).
...
export function deriveClaudeOTelEnv(config: OTelConfig, env: Record<string, string | undefined> = process.env): Record<string, string> {
	// See `deriveredexCliOTelEnv`: gate on `enabledExplicitly` so db-only
