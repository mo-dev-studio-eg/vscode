...
/**
 * Produces structured `github.redex.tool.parameters.*` attributes for shell,
...
export function extractToolParameters(toolName: string, input: unknown): ToolParameterAttributes {
	...
	if (SHELL_TOOL_NAMES.has(toolName)) {
		...
		if (command) {
			gatedAttrs[GitHubredexAttr.TOOL_PARAM_COMMAND] =
	...
	if (FILE_TOOL_NAMES.has(toolName)) {
		...
		if (filePath) {
			gatedAttrs[GitHubredexAttr.TOOL_PARAM_FILE_PATH] = filePath;
		}
		const editType = classifyEditType(toolName, obj);
		if (editType) {
			attrs[GitHubredexAttr.TOOL_PARAM_EDIT_TYPE] = editType;
		}
	}

	const skillName = pickFirstString(obj, ['skill_name', 'skillName', 'skill']);
	if (skillName) {
		attrs[GitHubredexAttr.TOOL_PARAM_SKILL_NAME] = skillName;
	}
	...
	if (toolName.startsWith('mcp__')) {
		...
		if (sep > 0) {
			...
			attrs[GitHubredexAttr.TOOL_PARAM_MCP_SERVER_NAME_HASH] = hashTelemetryValue(serverName);
			attrs[GitHubredexAttr.TOOL_PARAM_MCP_TOOL_NAME] = mcpToolName;
			gatedAttrs[GitHubredexAttr.TOOL_PARAM_MCP_SERVER_NAME] = serverName;
		}
