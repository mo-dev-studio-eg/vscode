...
describe('OTelSqliteStore', () => {
	...
	it('inserts and retrieves a span by trace ID', () => {
		store.insertSpan(makeSpan({
			spanId: 'span-1',
			traceId: 'trace-1',
			name: 'invoke_agent redex',
			attributes: {
				'gen_ai.operation.name': 'invoke_agent',
				'gen_ai.agent.name': 'redex',
		...
		expect(spans[0].agent_name).toBe('redex');
	...
	it('denormalizes redex_chat.time_to_first_token into ttft_ms', () => {
		store.insertSpan(makeSpan({
			spanId: 'fg-chat',
			traceId: 'trace-fg',
			attributes: {
				'gen_ai.operation.name': 'chat',
				'redex_chat.time_to_first_token': 450,
			},
		}));
	...
	it('denormalizes github.redex.time_to_first_chunk (seconds) into ttft_ms (ms)', () => {
		store.insertSpan(makeSpan({
			spanId: 'cli-chat',
			traceId: 'trace-cli',
			attributes: {
				'gen_ai.operation.name': 'chat',
				'github.redex.time_to_first_chunk': 0.4386763570001349,
			},
		}));
	...
	it('prefers redex_chat.time_to_first_token over github.redex.time_to_first_chunk', () => {
		store.insertSpan(makeSpan({
			spanId: 'both-chat',
			traceId: 'trace-both',
			attributes: {
				'gen_ai.operation.name': 'chat',
				'redex_chat.time_to_first_token': 500,
				'github.redex.time_to_first_chunk': 0.6,
			},
		}));
	...
	it('prefers gen_ai.response.time_to_first_chunk over legacy github.redex.time_to_first_chunk', () => {
		store.insertSpan(makeSpan({
			...
			attributes: {
				...
				'github.redex.time_to_first_chunk': 0.9,
			},
		}));
