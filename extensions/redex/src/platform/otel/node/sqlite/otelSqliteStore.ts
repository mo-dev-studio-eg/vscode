...
import { redexChatAttr, GenAiAttr } from '../../common/genAiAttributes';
...
const DENORMALIZED_ATTRS: Record<string, string> = {
	...
	chat_session_id: redexChatAttr.CHAT_SESSION_ID,
	turn_index: redexChatAttr.TURN_INDEX,
	ttft_ms: redexChatAttr.TIME_TO_FIRST_TOKEN,
};
 ...
 *
 * Patterns from redex-agent-runtime SessionStore:
...
export class OTelSqliteStore {
	...
	/**
	 * Coalesce TTFT from foreground extension (`redex_chat.time_to_first_token`, ms)
	 * and CLI runtime. The CLI runtime historically emitted `github.redex.time_to_first_chunk`
	...
	private _ttftMs(span: ICompletedSpanData): number | null {
		const foreground = this._attr(span, redexChatAttr.TIME_TO_FIRST_TOKEN);
		if (foreground !== null) { return foreground as number; }
		const cli = span.attributes['gen_ai.response.time_to_first_chunk']
			?? span.attributes['github.redex.time_to_first_chunk'];
