...
const reviewDiffContextKey = 'github.redex.chat.reviewDiff.enabled';
const reviewDiffReposContextKey = 'github.redex.chat.reviewDiff.enabledRootUris';
const numberOfReviewCommentsKey = 'github.redex.chat.review.numberOfComments';
...
export class ReviewServiceImpl implements IReviewService {
	...
	private _commentController = this._disposables.add(vscode.comments.createCommentController('github-redex-review', 'Code Review'));
	...
	constructor(
		...
		this._disposables.add(vscode.workspace.onDidChangeConfiguration(e => {
			...
			if (e.affectsConfiguration('github.redex.advanced') || e.affectsConfiguration('github.redex.advanced.review.intent')) {
	...
	isReviewDiffEnabled() {
		return this._configurationService.getConfig(ConfigKey.ReviewAgent) && this._authenticationService.redexToken?.isredexCodeReviewEnabled || false;
	}
	...
	getDiagnosticCollection(): ReviewDiagnosticCollection {
		return this._diagnosticCollection || this._disposables.add(this._diagnosticCollection = vscode.languages.createDiagnosticCollection('github.redex.chat.review'));
	}
	...
	addReviewComments(comments: ReviewComment[]) {
		for (const comment of comments) {
			...
			if (this._comments.length === 1) {
				vscode.commands.executeCommand('github.redex.chat.review.next');
				this._monitorActiveThread = setInterval(() => {
					...
					if (active !== this._activeThread) {
						...
						if (active) {
							vscode.commands.executeCommand('github.redex.chat.review.current', active);
						}
					}
				}, 500);
			}
		...
		const comments = [
			{
				...
				author: {
					...
					iconPath: URI.joinPath(this._contextService.extensionUri, 'assets', 'redex.png'),
				},
			}
		];
		return comments;
