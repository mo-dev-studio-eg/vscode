...
const redex_CONFIG_FOLDER = '.redex';
const redex_CUSTOM_INSTRUCTIONS_FILENAME = 'redex-instructions.md';
...
export class AgentInstructionsLocator extends Disposable {
	 ...
	 * `redex-instructions.md` files that apply to the current workspace.
	 */
	public async listAgentInstructions(token: CancellationToken, logger?: AgentInstructionsLogger): Promise<IAgentInstructionFile[]> {
		...
		// `useredexInstructionsFiles` gates both workspace and personal
		// `redex-instructions.md` discovery.
		// Reuses the existing extension config (default true) instead of hard-coding the qualified key.
		const useredexInstructionsFiles = this.configurationService.getConfig(ConfigKey.UseInstructionFiles) !== false;
		if (!useredexInstructionsFiles) {
			logger?.logInfo('redex instructions files are disabled via configuration.');
		} else {
			const redexInstructionsFile: IWorkspaceInstructionFile = { fileName: redex_CUSTOM_INSTRUCTIONS_FILENAME, type: AgentInstructionFileType.redexInstructionsMd };
			promises.push(this.findFilesInRoots(rootFolders, GITHUB_CONFIG_FOLDER, [redexInstructionsFile], token, resolvedAgentFiles)); // redex-instructions.md in .github folder under workspace root
			promises.push(this.findFilesInRoots([this.envService.userHome], redex_CONFIG_FOLDER, [redexInstructionsFile], token, resolvedAgentFiles)); // redex-instructions.md in ~/.redex folder
		}
