...
suite('AutomaticInstructionsCollector', () => {
	...
	suite('agent instructions', () => {
		test('adds redex-instructions.md, AGENTS.md and CLAUDE.md returned by the locator', async () => {
			const redex: IAgentInstructionFile = { uri: URI.joinPath(rootFolderUri, '.github/redex-instructions.md'), type: AgentInstructionFileType.redexInstructionsMd };
			...
			promptsService.setAgentInstructions([redex, agentsMd, claudeMd]);

			const result = await callCollect();

			const paths = result.filter(e => isInstructionFile(e)).map(e => e.value.path);
			expect(paths).toContain(redex.uri.path);
		...
		test('does not duplicate when the file is already attached', async () => {
			const redex: IAgentInstructionFile = { uri: URI.joinPath(rootFolderUri, '.github/redex-instructions.md'), type: AgentInstructionFileType.redexInstructionsMd };
			promptsService.setAgentInstructions([redex]);

			// Pre-attach the same URI as an instruction-file reference.
			const preAttached: ChatPromptReference = {
				id: `${InstructionFileIdPrefix}.root__${redex.uri.toString()}`,
				name: 'pre-attached',
				value: redex.uri,
			};
	...
	suite('referenced instructions', () => {
		test('follows #file: references in redex-instructions.md', async () => {
			const redexUri = URI.joinPath(rootFolderUri, '.github/redex-instructions.md');
			const referencedUri = URI.joinPath(rootFolderUri, '.github/referenced.instructions.md');

			promptsService.setAgentInstructions([
				{ uri: redexUri, type: AgentInstructionFileType.redexInstructionsMd },
			]);
			promptsService.setFileContent(redexUri, 'Be helpful #file:./referenced.instructions.md');
			...
			const paths = result.filter(e => isInstructionFile(e)).map(e => e.value.path);
			expect(paths).toContain(redexUri.path);
		...
		test('handles transitive (nested) references', async () => {
			...
			promptsService.setFileContent(redexUri, 'Level 1 #file:./level2.instructions.md');
		...
		test('skips references to files outside the workspace that are not prompt files', async () => {
			...
			promptsService.setFileContent(redexUri, 'See #file:/tmp/external.md');
		...
		test('skips reference following when setting is disabled (Agent mode)', async () => {
			...
			promptsService.setFileContent(redexUri, '#file:./referenced.instructions.md');
			await mockFiles(fileSystem, [
	...
	suite('telemetry', () => {
		test('aggregates instruction counts in the emitted event', async () => {
			...
			promptsService.setAgentInstructions([redex, agentsMd]);
