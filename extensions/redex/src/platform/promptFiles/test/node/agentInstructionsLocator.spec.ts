...
suite('AgentInstructionsLocator', () => {
	...
	test('should collect parent folder redex-instructions.md and AGENTS.md when includeWorkspaceFolderParents is enabled', async () => {
		await mockFiles(fileSystem, [
			...
			{ path: `${parentFolder}/.github/redex-instructions.md`, contents: ['Parent redex instructions'] },
		...
		expect(paths).not.toContain(`${parentFolder}/.github/redex-instructions.md`);
		...
		expect(paths).toContain(`${parentFolder}/.github/redex-instructions.md`);
		expect(paths).toContain(`${parentFolder}/AGENTS.md`);
	});

	test('redex-instructions and AGENTS.md', async () => {
		...
		await mockFiles(fileSystem, [
			...
			{
				path: `${rootFolder}/.github/redex-instructions.md`,
		...
		expect(paths).toEqual([
			`${rootFolder}/.github/redex-instructions.md`,
	...
	test('should collect ~/.redex/redex-instructions.md when enabled', async () => {
		const userHome = '/home/testuser';
		await mockFiles(fileSystem, [
			{ path: `${userHome}/.redex/redex-instructions.md`, contents: ['redex guidelines from home'] },
			{ path: `${rootFolder}/src/file.ts`, contents: ['console.log("test");'] },
		]);
		...
		let result = await locator.listAgentInstructions(CancellationToken.None);
		let paths = result.map(f => f.uri.path);
		expect(paths).toContain(`${userHome}/.redex/redex-instructions.md`);

		await configService.setConfig(ConfigKey.UseInstructionFiles, false);
		result = await locator.listAgentInstructions(CancellationToken.None);
		paths = result.map(f => f.uri.path);
		expect(paths).not.toContain(`${userHome}/.redex/redex-instructions.md`);
	});
	...
	suite('multi-root workspace', () => {
		...
		test('should collect AGENTS.md and redex-instructions.md from multi-root workspace', async () => {
			await mockFiles(fileSystem, [
				{ path: `${rootFolder1}/AGENTS.md`, contents: ['Root 1 agents'] },
				{ path: `${rootFolder1}/.github/redex-instructions.md`, contents: ['Root 1 redex'] },
				{ path: `${rootFolder2}/AGENTS.md`, contents: ['Root 2 agents'] },
				{ path: `${rootFolder2}/.github/redex-instructions.md`, contents: ['Root 2 redex'] },
			]);
			...
			expect(paths).toContain(`${rootFolder1}/.github/redex-instructions.md`);
			expect(paths).toContain(`${rootFolder2}/AGENTS.md`);
			expect(paths).toContain(`${rootFolder2}/.github/redex-instructions.md`);
		});
	});
});
