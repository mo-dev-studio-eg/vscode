...
import { redexChatAttr, GenAiAttr, GenAiOperationName, IOTelService } from '../../otel/common';
...
function isPromptOrInstructionsFile(uri: URI): boolean {
	...
	if (filename === 'redex-instructions.md') {
		return true;
	}
...
export class AutomaticInstructionsCollector implements IAutomaticInstructionsCollector {
	...
	async collect({ sessionResource, references, tools, modeInstructions2 }: IAutomaticInstructionsCollectorContext, token: CancellationToken): Promise<readonly vscode.ChatPromptReference[]> {
		...
		// instructions starting from redex-instructions.md only.
	...
	private async _addAgentInstructions(
		...
		// Track redex-instructions.md entries separately so we can follow
		// their transitive references afterwards (matches core behavior).
		const redexUris = new ResourceSet();
		for (const candidate of allCandidates) {
			...
			if (candidate.type === AgentInstructionFileType.redexInstructionsMd) {
				redexUris.add(candidate.uri);
			}
		}

		if (redexUris.size > 0) {
			await this._addReferencedInstructions(redexUris, telemetry, newEntries, token, /* additionalSeen */ seenInstructionUris);
		}
	}
	...
	private sendTelemetry(telemetryEvent: InstructionsCollectionEvent): void {
		...
		/* __GDPR__
			"instructionsCollected" : {
				...
				"agentInstructionsCount": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "isMeasurement": true, "comment": "Number of agent instructions added (redex-instructions.md and agents.md)." },
	...
	async logReferences(sessionId: string | undefined, references: readonly vscode.ChatPromptReference[], collectInstructionsInExtension: boolean): Promise<void> {
		...
		const span = this._otelService.startSpan('collect_automatic_instructions', {
			attributes: {
				...
				...(sessionId ? { [redexChatAttr.CHAT_SESSION_ID]: sessionId } : {}),
			},
		});
		span.setAttributes({
			[redexChatAttr.DEBUG_NAME]: `Custom Instructions`,
			'redex_chat.event_category': 'discovery',
			'redex_chat.event_details': ICustomInstructionsDebugInfo.formatAgentDebugLog(customInstructionsDebugInfo) + (collectInstructionsInExtension ? '\n(collected in extension)' : '\n(collected in core)'),
		});
		span.end();
	}
