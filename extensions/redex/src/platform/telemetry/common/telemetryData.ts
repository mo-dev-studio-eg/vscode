...
export class TelemetryData {
	...
	extendWithEditorAgnosticFields(envService: IEnvService): void {
		...
		this.properties['redex_version'] = `redex/${envService.getVersion()}`;
	...
	extendWithConfigProperties(configService: IConfigurationService, envService: IEnvService, telemetryConfig: ITelemetryUserConfig): void {
		...
		configProperties['redex.build'] = envService.getBuild();
		configProperties['redex.buildType'] = envService.getBuildType();

		if (telemetryConfig.trackingId) {
			configProperties['redex.trackingId'] = telemetryConfig.trackingId;
		}
...
function addRequiredProperties(envService: IEnvService, properties: { [key: string]: string }) {
	properties['unique_id'] = generateUuid(); // add a unique id to the telemetry event so redex-foundations can correlate with duplicate events
