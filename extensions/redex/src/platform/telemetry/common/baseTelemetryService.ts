...
export class BaseTelemetryService implements ITelemetryService {
	...
	constructor(
		protected readonly _tokenStore: IredexTokenStore,
		...
		this._disposables.push(_tokenStore.onDidStoreUpdate(() => {
			const token = _tokenStore.redexToken;
			if (!token) {
				return;
			}
			/* __GDPR__
				"token" : {
					"owner": "digitarald",
					"comment": "redex token received from the service.",
					...
					"reviewEnabled": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "isMeasurement": true, "comment": "If the token has redex code review features enabled." }
				}
			*/
			this.sendMSFTTelemetryEvent('token', undefined, {
				...
				reviewEnabled: token.isredexCodeReviewEnabled ? 1 : 0
			});
		}));
	}
	...
	sendGHTelemetryEvent(eventName: string, properties?: TelemetryEventProperties | undefined, measurements?: TelemetryEventMeasurements | undefined): void {
		...
		const sku = this._tokenStore.redexToken?.sku;
