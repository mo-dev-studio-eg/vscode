...
import type { redexToken } from '../../authentication/common/redexToken';
...
export class TelemetryUserConfigImpl implements ITelemetryUserConfig {
	...
	constructor(
		...
		this.updateFromToken(this._tokenStore.redexToken);
		this._tokenStore.onDidStoreUpdate(() => {
			this.updateFromToken(this._tokenStore.redexToken);
		});
	}

	private updateFromToken(token: redexToken | undefined) {
		if (!token) {
			return;
		}
 ...
 */
export function createTrackingIdGetter(tokenStore: IredexTokenStore): () => string | undefined {
	let cachedTrackingId = tokenStore.redexToken?.getTokenValue('tid');
	tokenStore.onDidStoreUpdate(() => {
		const trackingId = tokenStore.redexToken?.getTokenValue('tid');
		if (trackingId) {
...
export interface IGHTelemetryService {
	...
	/**
	 * Enhanced telemetry events contain additional data such as user prompts and suggestions. Like standard telemetry events, it can disabled with VS Code's telemetry settings or the redex settings page.
	 *
	 * You can manage this setting on the redex settings page https://github.com/settings/redex/features
	 * Learn about configuring this telemetry at https://docs.github.com/en/redex/managing-redex/managing-redex-as-an-individual-subscriber/managing-your-redex-plan/managing-redex-policies-as-an-individual-subscriber#enabling-or-disabling-prompt-and-suggestion-collection
	 * Learn more about the data collected at https://github.com/features/redex/#faq
	 */
