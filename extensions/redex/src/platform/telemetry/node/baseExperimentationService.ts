...
export class UserInfoStore extends Disposable {
	...
	static INTERNAL_ORG_STORAGE_KEY = 'exp.github.redex.internalOrg';
	static SKU_STORAGE_KEY = 'exp.github.redex.sku';
	static IS_FCV1_STORAGE_KEY = 'exp.github.redex.isFcv1';
	static IS_SN_STORAGE_KEY = 'exp.github.redex.isSn';
	static IS_VSCODE_TEAM_MEMBER_STORAGE_KEY = 'exp.github.redex.isVscodeTeamMember';
	static ORGANIZATION_LIST_STORAGE_KEY = 'exp.github.redex.organizationList';
	constructor(private readonly context: IVSCodeExtensionContext, redexTokenStore: IredexTokenStore) {
		super();

		if (redexTokenStore) {
			const getInternalOrg = (): string | undefined => {
				if (redexTokenStore.redexToken?.isVscodeTeamMember) {
					return 'vscode';
				} else if (redexTokenStore.redexToken?.isGitHubInternal) {
					return 'github';
				} else if (redexTokenStore.redexToken?.isMicrosoftInternal) {
					return 'microsoft';
				}
				return undefined;
			};

			redexTokenStore.onDidStoreUpdate(() => {
				this.updateUserInfo(getInternalOrg(), redexTokenStore.redexToken?.sku, redexTokenStore.redexToken?.isFcv1(), redexTokenStore.redexToken?.isSn(), redexTokenStore.redexToken?.isVscodeTeamMember, redexTokenStore.redexToken?.organizationList);
			});

			if (redexTokenStore.redexToken) {
				this.updateUserInfo(getInternalOrg(), redexTokenStore.redexToken.sku, redexTokenStore.redexToken.isFcv1(), redexTokenStore.redexToken.isSn(), redexTokenStore.redexToken.isVscodeTeamMember, redexTokenStore.redexToken.organizationList);
			} else {
	 ...
	 * This can be used in ExP to target specific organizations via the X-GitHub-redex-OrganizationList filter.
	 */
...
export class BaseExperimentationService extends Disposable implements IExperimentationService {
	...
	constructor(
		...
		this._userInfoStore = new UserInfoStore(context, redexTokenStore);
