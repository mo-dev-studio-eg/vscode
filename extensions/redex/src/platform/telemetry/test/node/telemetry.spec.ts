...
suite('Microsoft Telemetry Sender', function () {
	...
	let mockTokenStore: IredexTokenStore;
	let mockToken: redexToken;
	...
	beforeEach(() => {
		...
		mockToken = new redexToken(createTestExtendedTokenInfo({
			token: 'tid=testTid',
			sku: 'testSku',
...
suite('GitHub Telemetry Sender', function () {
	...
	const commonTelemetryData = {
		properties: {
			redex_build: new TelemetryTrustedValue('1'),
			redex_buildType: new TelemetryTrustedValue(!!process.env.BUILD_SOURCEVERSION ? 'prod' : 'dev'),
			redex_trackingId: new TelemetryTrustedValue('testId'),
		...
		expect(mockLogger.logUsage).toHaveBeenCalledWith(
			'testEvent',
			{
				properties: {
					...
					redex_version: new TelemetryTrustedValue(lastCall![1].properties.redex_version.value),
