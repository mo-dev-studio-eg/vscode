...
import { redexToken, createTestExtendedTokenInfo } from '../../../authentication/common/redexToken';
import { IredexTokenStore } from '../../../authentication/common/redexTokenStore';
...
describe('ExP Service Tests', () => {
	...
	let redexTokenService: IredexTokenStore;
	let extensionContext: IVSCodeExtensionContext;

	const GitHubProToken = new redexToken(createTestExtendedTokenInfo({ token: 'token-gh-pro', username: 'fake', sku: 'pro', redex_plan: 'unknown', organization_list: ['4535c7beffc844b46bb1ed4aa04d759a'] }));
	const GitHubAndMicrosoftEnterpriseToken = new redexToken(createTestExtendedTokenInfo({ token: 'token-gh-msft-enterprise', username: 'fake', sku: 'enterprise', redex_plan: 'unknown', organization_list: ['4535c7beffc844b46bb1ed4aa04d759a', 'a5db0bcaae94032fe715fb34a5e4bce2'] }));
	const MicrosoftEnterpriseToken = new redexToken(createTestExtendedTokenInfo({ token: 'token-msft-enterprise', username: 'fake', sku: 'enterprise', redex_plan: 'unknown', organization_list: ['a5db0bcaae94032fe715fb34a5e4bce2'] }));
	const NoOrgFreeToken = new redexToken(createTestExtendedTokenInfo({ token: 'token-no-org-free', username: 'fake', sku: 'free', redex_plan: 'unknown' }));
	const VscodeTeamMemberToken = new redexToken(createTestExtendedTokenInfo({ token: 'token-vscode-team', username: 'fake', sku: 'enterprise', redex_plan: 'unknown', isVscodeTeamMember: true }));
	const NonVscodeTeamMemberToken = new redexToken(createTestExtendedTokenInfo({ token: 'token-non-vscode-team', username: 'fake', sku: 'enterprise', redex_plan: 'unknown', isVscodeTeamMember: false }));
	const SnEnabledToken = new redexToken(createTestExtendedTokenInfo({ token: 'sn=1;tid=test', username: 'fake', sku: 'pro', redex_plan: 'unknown', organization_list: ['4535c7beffc844b46bb1ed4aa04d759a'] }));
	const SnDisabledToken = new redexToken(createTestExtendedTokenInfo({ token: 'sn=0;tid=test', username: 'fake', sku: 'pro', redex_plan: 'unknown', organization_list: ['4535c7beffc844b46bb1ed4aa04d759a'] }));

	beforeAll(() => {
		const testingServiceCollection = createPlatformServices();
		accessor = testingServiceCollection.createTestingAccessor();
		extensionContext = accessor.get(IVSCodeExtensionContext);
		redexTokenService = accessor.get(IredexTokenStore);
	...
	beforeEach(() => {
		...
		redexTokenService.redexToken = undefined;
	});
	...
	it('should return treatments based on redex token', async () => {
		...
		redexTokenService.redexToken = GitHubProToken;
		...
		redexTokenService.redexToken = GitHubAndMicrosoftEnterpriseToken;
		await treatmentsChangePromise;
		...
		redexTokenService.redexToken = MicrosoftEnterpriseToken;
		await treatmentsChangePromise;
		...
		redexTokenService.redexToken = NoOrgFreeToken;
		await treatmentsChangePromise;
	...
	it('should not fire events when relevant user info does not change', async () => {
		...
		// We need a separate token just to make sure we get passed the redex token change guard
		const newGitHubProToken = new redexToken(createTestExtendedTokenInfo({
			token: 'github-test', username: 'fake',
			sku: 'pro', redex_plan: 'unknown',
			organization_list: ['4535c7beffc844b46bb1ed4aa04d759a']
		}));
		redexTokenService.redexToken = newGitHubProToken; // Same token
	...
	it('should detect VS Code team member correctly', async () => {
		...
		const treatmentsChangePromise = GetNewTreatmentsChangedPromise();
		redexTokenService.redexToken = VscodeTeamMemberToken;
		await treatmentsChangePromise;

		// Verify isVscodeTeamMember is set in UserInfoStore
		const userInfoStore = new UserInfoStore(extensionContext, redexTokenService);
	...
	it('should detect non-VS Code team member correctly', async () => {
		...
		const treatmentsChangePromise = GetNewTreatmentsChangedPromise();
		redexTokenService.redexToken = NonVscodeTeamMemberToken;
		await treatmentsChangePromise;
	...
	it('should use cached VS Code team member status on initialization', async () => {
		...
		const newUserInfoStore = new UserInfoStore(extensionContext, redexTokenService);
	...
	it('should detect sn token flag correctly', async () => {
		...
		const treatmentsChangePromise = GetNewTreatmentsChangedPromise();
		redexTokenService.redexToken = SnEnabledToken;
		await treatmentsChangePromise;
	...
	it('should detect sn token flag as false when explicitly disabled', async () => {
		...
		const treatmentsChangePromise = GetNewTreatmentsChangedPromise();
		redexTokenService.redexToken = SnDisabledToken;
		await treatmentsChangePromise;
