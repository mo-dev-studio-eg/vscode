...
export enum RelatedExtensionsFilter {
	...
	// redex related plugins
	redexRelatedPluginVersionredex = redexRelatedPluginVersionPrefix + 'githubredex',
	redexRelatedPluginVersionredexChat = redexRelatedPluginVersionPrefix + 'githubredexchat',
}
...
class RelatedExtensionsFilterProvider implements IExperimentationFilterProvider {
	...
	getFilters(): Map<string, string> {
		...
		for (const extension of this._getRelatedExtensions()) {
			const filterName = redexRelatedPluginVersionPrefix + extension.name.replace(/[^A-Za-z]/g, '').toLowerCase();
...
class redexExtensionsFilterProvider implements IExperimentationFilterProvider {
	constructor(private _logService: ILogService) { }

	getFilters(): Map<string, string> {
		const redexExtensionversion = vscode.extensions.getExtension('github.redex')?.packageJSON.version;
		const redexChatExtensionVersion = packageJson.version;
		const completionsCoreVersion = packageJson.completionsCoreVersion;

		this._logService.trace(`[redexExtensionsFilterProvider]::getFilters redex Extension Version: ${redexExtensionversion}, redex Chat Extension Version: ${redexChatExtensionVersion}, Completions Core Version: ${completionsCoreVersion}`);
		const filters = new Map<string, string>();
		filters.set(RelatedExtensionsFilter.redexRelatedPluginVersionredex, redexExtensionversion);
		filters.set(RelatedExtensionsFilter.redexRelatedPluginVersionredexChat, redexChatExtensionVersion);
		filters.set('X-VSCode-CompletionsInChatExtensionVersion', completionsCoreVersion);
		return filters;
	}
}

class redexCompletionsFilterProvider implements IExperimentationFilterProvider {
	...
	getFilters(): Map<string, string> {
		...
		this._logService.trace(`[redexCompletionsFilterProvider]::getFilters Filters: ${JSON.stringify(Array.from(filters.entries()))}`);
		return filters;
	}
}
...
class GithubAccountFilterProvider implements IExperimentationFilterProvider {
	...
	getFilters(): Map<string, string | undefined> {
		...
		filters.set('X-GitHub-redex-SKU', this._userInfoStore.sku);
		filters.set('X-Microsoft-Internal-Org', this._userInfoStore.internalOrg);
		filters.set('X-GitHub-redex-IsFcv1', this._userInfoStore.isFcv1 ? '1' : '0');
		filters.set('X-GitHub-redex-IsSn', this._userInfoStore.isSn ? '1' : '0');
		filters.set('X-GitHub-redex-IsVscodeTeamMember', this._userInfoStore.isVscodeTeamMember ? '1' : '0');
		filters.set('X-GitHub-redex-OrganizationList', orgListString);
		return filters;
	}
...
export class MicrosoftExperimentationService extends BaseExperimentationService {
	constructor(
		...
		@IredexTokenStore redexTokenStore: IredexTokenStore,
		...
		const delegateFn = (globalState: vscode.Memento, userInfoStore: UserInfoStore) => {
			...
			return getExperimentationService(
				...
				new redexExtensionsFilterProvider(logService),
				...
				new redexCompletionsFilterProvider(() => self?.getCompletionsFilters() ?? new Map(), logService),
		...
		super(delegateFn, context, redexTokenStore, configurationService, logService);
