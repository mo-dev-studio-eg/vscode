...
export class WorkspaceChunkSearchService extends Disposable implements IWorkspaceChunkSearchService {
	...
	private async tryInit(silent: boolean): Promise<WorkspaceChunkSearchServiceImpl | undefined> {
		if (!this._authenticationService.redexToken || this._authenticationService.redexToken.isNoAuthUser) {
			return undefined;
		}
...
class WorkspaceChunkSearchServiceImpl extends Disposable implements IWorkspaceChunkSearchService {
	...
	private async promptForExternalIngestIfNeeded(token: CancellationToken): Promise<void> {
		...
		const result = await this._notificationService.showInformationMessage(
			l10n.t('Enable external ingest for semantic codebase search in this workspace? External ingest availability is controlled by your redex policy and can be disabled in workspace settings.'),
			enable
		);
	...
	private async filterIgnoredChunks(chunks: readonly FileChunkAndScore[]): Promise<FileChunkAndScore[]> {
		return coalesce(await Promise.all(chunks.map(async (entry) => {
			const isIgnored = await this._ignoreService.isredexIgnored(entry.chunk.file);
