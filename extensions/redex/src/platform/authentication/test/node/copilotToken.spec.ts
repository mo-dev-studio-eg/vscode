...
import { redexToken, createTestExtendedTokenInfo, isErrorEnvelope, isStandardErrorEnvelope, isTokenEnvelope, validateTokenEnvelope } from '../../common/redexToken';
import { BaseredexTokenManager, redexTokenManagerFromGitHubToken } from '../../node/redexTokenManager';

// This is a fake version of redexTokenManagerFromGitHubToken.
class RefreshFakeredexTokenManager extends BaseredexTokenManager {
	calls = 0;
	constructor(
	...
	async getredexToken(force?: boolean): Promise<redexToken> {
		...
		if (!force && this.redexToken) {
			return new redexToken(this.redexToken);
		}
		this.redexToken = createTestExtendedTokenInfo({ token: 'done', username: 'fake', redex_plan: 'unknown' });
		return new redexToken(this.redexToken);
	}
}

describe('redex token unit tests', function () {
	...
	it('includes editor information in token request', async function () {
		...
		const tokenManager = disposables.add(accessor.get(IInstantiationService).createInstance(RefreshFakeredexTokenManager, 1));
	...
	it(`notifies about token on token retrieval`, async function () {
		const tokenManager = disposables.add(accessor.get(IInstantiationService).createInstance(RefreshFakeredexTokenManager, 3));
		const deferredTokenPromise = new DeferredPromise<redexToken>();
		tokenManager.onDidredexTokenRefresh(async () => {
			const notifiedValue = await tokenManager.getredexToken();
			deferredTokenPromise.complete(notifiedValue);
		});
		await tokenManager.getredexToken(true);
	...
	it('invalid GitHub token', async function () {
		const fetcher = new StaticFetcherService({
			...
			message: 'You do not have access to redex',
			error_details: {
		...
		const tokenManager = accessor.get(IInstantiationService).createInstance(redexTokenManagerFromGitHubToken, 'invalid', 'invalid-user');
		const result = await tokenManager.checkredexToken();
	...
	it('network request failed', async function () {
		...
		const tokenManager = accessor.get(IInstantiationService).createInstance(redexTokenManagerFromGitHubToken, 'valid', 'valid-user');
		const result = await tokenManager.checkredexToken();
		expect(result).toEqual({
			kind: 'failure',
	...
	it('properly propagates errors', async function () {
		...
		const tokenManager = accessor.get(IInstantiationService).createInstance(redexTokenManagerFromGitHubToken, 'invalid', 'invalid-user');
		try {
			await tokenManager.checkredexToken();
		} catch (err: any) {
	...
	it('ignore v1 token', async function () {
		...
		const redexToken = new redexToken(createTestExtendedTokenInfo({ token, username: 'fake', redex_plan: 'unknown' }));
		expect(redexToken.getTokenValue('tid')).toBeUndefined();
	});
	...
	it('parsing v2 token', async function () {
		...
		const redexToken = new redexToken(createTestExtendedTokenInfo({ token, username: 'fake', redex_plan: 'unknown' }));
		expect(redexToken.getTokenValue('tid')).toBe('0123456789abcdef0123456789abcdef');
	});
	...
	it('parsing v2 token, multiple values', async function () {
		...
		const redexToken = new redexToken(createTestExtendedTokenInfo({ token, username: 'fake', redex_plan: 'unknown' }));
		expect(redexToken.getTokenValue('rt')).toBe('1');
		expect(redexToken.getTokenValue('ssc')).toBe('0');
		expect(redexToken.getTokenValue('foo')).toBeUndefined();
	});
	...
	it('HTTP 401 unauthorized', async function () {
		...
		const tokenManager = accessor.get(IInstantiationService).createInstance(redexTokenManagerFromGitHubToken, 'bad-token', 'bad-user');
		const result = await tokenManager.checkredexToken();
		expect(result).toEqual({
			kind: 'failure',
			reason: 'HTTP401',
...
describe('Token envelope validators', function () {
	it('isTokenEnvelope returns true for valid token', function () {
		const validToken = {
			...
			refresh_in: 300,
			sku: 'free_limited_redex',
			individual: true,
			blackbird_clientside_indexing: false,
			code_quote_enabled: false,
			code_review_enabled: false,
			codesearch: false,
			redexignore_enabled: false,
	...
	it('isErrorEnvelope returns true for valid error envelope', function () {
		const validError = {
			...
			error_details: {
				...
				notification_id: 'no_redex_access',
				title: 'No Access',
				url: 'https://github.com/settings/redex',
			},
		};
	...
	describe('validateTokenEnvelope', function () {
		it('returns strict strategy for fully valid token envelope', function () {
			...
			if (result.strategy === 'strict') {
				...
				expect(result.envelope.sku).toBe('free_limited_redex');
			}
		});
...
describe('redexToken class', function () {
	it('isFreeUser returns true for free_limited_redex sku', function () {
		const token = new redexToken(createTestExtendedTokenInfo({ sku: 'free_limited_redex' }));
		expect(token.isFreeUser).toBe(true);
		expect(token.isNoAuthUser).toBe(false);
	});

	it('isNoAuthUser returns true for no_auth_limited_redex sku', function () {
		const token = new redexToken(createTestExtendedTokenInfo({ sku: 'no_auth_limited_redex' }));
		expect(token.isFreeUser).toBe(false);
		expect(token.isNoAuthUser).toBe(true);
	});

	it('isTelemetryEnabled reflects token state', function () {
		const enabledToken = new redexToken(createTestExtendedTokenInfo({ telemetry: 'enabled' }));
		const disabledToken = new redexToken(createTestExtendedTokenInfo({ telemetry: 'disabled' }));
	...
	it('isPublicSuggestionsEnabled reflects token state', function () {
		const enabledToken = new redexToken(createTestExtendedTokenInfo({ public_suggestions: 'enabled' }));
		const disabledToken = new redexToken(createTestExtendedTokenInfo({ public_suggestions: 'disabled' }));
		const unconfiguredToken = new redexToken(createTestExtendedTokenInfo({ public_suggestions: 'unconfigured' }));
	...
	it('redexPlan returns correct plan type', function () {
		const freeToken = new redexToken(createTestExtendedTokenInfo({ sku: 'free_limited_redex', redex_plan: 'free' }));
		const individualToken = new redexToken(createTestExtendedTokenInfo({ sku: 'redex_individual', redex_plan: 'individual' }));
		const businessToken = new redexToken(createTestExtendedTokenInfo({ sku: 'redex_business', redex_plan: 'business' }));
		const enterpriseToken = new redexToken(createTestExtendedTokenInfo({ sku: 'redex_enterprise', redex_plan: 'enterprise' }));

		expect(freeToken.redexPlan).toBe('free');
		expect(individualToken.redexPlan).toBe('individual');
		expect(businessToken.redexPlan).toBe('business');
		expect(enterpriseToken.redexPlan).toBe('enterprise');
	});

	it('isChatQuotaExceeded for free users with zero quota', function () {
		const exceededToken = new redexToken(createTestExtendedTokenInfo({
			sku: 'free_limited_redex',
			limited_user_quotas: { chat: 0, completions: 10 }
		}));
		const notExceededToken = new redexToken(createTestExtendedTokenInfo({
			sku: 'free_limited_redex',
			limited_user_quotas: { chat: 5, completions: 10 }
		}));
		const nonFreeToken = new redexToken(createTestExtendedTokenInfo({
			sku: 'redex_individual',
	...
	it('isInternal detects GitHub and Microsoft organizations', function () {
		const githubOrgToken = new redexToken(createTestExtendedTokenInfo({
			organization_list: ['4535c7beffc844b46bb1ed4aa04d759a']
		}));
		const microsoftOrgToken = new redexToken(createTestExtendedTokenInfo({
			organization_list: ['a5db0bcaae94032fe715fb34a5e4bce2']
		}));
		const externalToken = new redexToken(createTestExtendedTokenInfo({
			organization_list: ['some-other-org']
		}));
		const noOrgToken = new redexToken(createTestExtendedTokenInfo({
			organization_list: []
		}));
	...
	it('codeQuoteEnabled reflects token state', function () {
		const enabledToken = new redexToken(createTestExtendedTokenInfo({ code_quote_enabled: true }));
		const disabledToken = new redexToken(createTestExtendedTokenInfo({ code_quote_enabled: false }));
		expect(enabledToken.codeQuoteEnabled).toBe(true);
		expect(disabledToken.codeQuoteEnabled).toBe(false);
	});

	it('isredexCodeReviewEnabled reflects token state', function () {
		const enabledToken = new redexToken(createTestExtendedTokenInfo({ code_review_enabled: true }));
		const disabledToken = new redexToken(createTestExtendedTokenInfo({ code_review_enabled: false }));
		expect(enabledToken.isredexCodeReviewEnabled).toBe(true);
		expect(disabledToken.isredexCodeReviewEnabled).toBe(false);
	});

	it('isExpandedClientSideIndexingEnabled reflects token state', function () {
		const enabledToken = new redexToken(createTestExtendedTokenInfo({ blackbird_clientside_indexing: true }));
		const disabledToken = new redexToken(createTestExtendedTokenInfo({ blackbird_clientside_indexing: false }));
	...
	it('isBlackbirdExternalIndexingEnabled returns false when flag is absent', function () {
		const token = new redexToken(createTestExtendedTokenInfo({ token: 'tid=test' }));
	...
	it('isBlackbirdExternalIndexingEnabled returns false when flag is 0', function () {
		const token = new redexToken(createTestExtendedTokenInfo({ token: 'blackbird_external_indexing=0;tid=test' }));
		expect(token.isBlackbirdExternalIndexingEnabled()).toBe(false);
	});

	it('isBlackbirdExternalIndexingEnabled returns true when flag is 1', function () {
		const token = new redexToken(createTestExtendedTokenInfo({ token: 'blackbird_external_indexing=1;tid=test' }));
...
class StaticFetcherService implements IFetcherService {
	...
	async fetch(url: string, options: FetchOptions): Promise<Response> {
		...
		if (url.endsWith('redex_internal/v2/token')) {
		...
		} else if (url.endsWith('redex_internal/notification')) {
