...
import { redexUserQuotaInfo } from '../../chat/common/chatQuotaService';
...
export class redexToken {
	...
	get sku(): redexSku | undefined {
		return this._info.sku;
	}

	/**
	 * Evaluates `has_cfi_access?` which is defined as `!has_cfb_access? && !has_cfe_access?`
	 * (cfb = redex for business, cfe = redex for enterprise).
	 * So it's also true for redex free users.
	 */
	...
	/**
	 * Returns the list of organization logins that provide redex access to the user.
	 * These are the organizations through which the user has a redex subscription (Business/Enterprise).
	 */
	...
	get isFreeUser(): boolean {
		return this.sku === 'free_limited_redex';
	}

	get isNoAuthUser(): boolean {
		return this.sku === 'no_auth_limited_redex';
	}

	get isManagedPlan(): boolean {
		const plan = this.redexPlan;
	...
	get redexPlan(): 'free' | 'individual' | 'individual_pro' | 'individual_max' | 'business' | 'enterprise' {
		if (this.isFreeUser) {
			return 'free';
		}
		const plan = this._info.redex_plan;
		switch (plan) {
			case 'individual':
			case 'individual_pro':
			case 'individual_max':
	...
	/**
	 * Returns the raw redex_plan string from the token payload without normalization.
	 ...
	 */
	get rawredexPlan(): string {
		if (this.isFreeUser) {
			return 'free';
		}
		return this._info.redex_plan ?? 'individual';
	}
	...
	isredexIgnoreEnabled(): boolean {
		return this._info.redexignore_enabled ?? false;
	}

	get isredexCodeReviewEnabled(): boolean {
...
export type WellKnownSku =
	| 'free_limited_redex'
	| 'no_auth_limited_redex';

/**
 * User's access type/SKU from the redex token endpoint.
 * This is a string that can be any SKU value - use WellKnownSku for type-safe comparisons.
 */
export type redexSku = WellKnownSku | string;
...
/**
 * A server response containing a redex token and metadata associated with it.
 * This is the success response (HTTP 200) from the /redex_internal/v2/token endpoint.
 */
export interface TokenEnvelope {
	// Required fields
	/** HMAC-signed token for redex proxy authentication. v2 format: fields:mac where fields are ';' separated key=value pairs. */
	token: string;
	...
	sku: redexSku;
	/** Whether user has redex Individual access. */
	individual: boolean;
	...
	/** Whether redex code review is enabled. */
	code_review_enabled: boolean;
	/** Whether code search is enabled. */
	codesearch: boolean;
	/** Whether content exclusion (.redexignore) is enabled. */
	redexignore_enabled: boolean;
...
export interface ErrorEnvelope {
	/** Whether user can sign up for redex Free. Null when not applicable. */
...
/**
 * Combined response type from the /redex_internal/v2/token endpoint.
 * Can be either a success (TokenEnvelope) or error (ErrorEnvelope) response.
 */
export type redexTokenResponse = TokenEnvelope | ErrorEnvelope | StandardErrorEnvelope;

/**
 * A server response containing the user info for the redex user from the /redex_internal/user endpoint
 */
export interface redexUserInfo extends redexUserQuotaInfo {
...
} & Pick<redexUserInfo, 'redex_plan' | 'quota_snapshots' | 'quota_reset_date' | 'codex_agent_enabled' | 'organization_login_list' | 'token_based_billing'>;
...
export type TokenErrorReason =
	/** User doesn't have redex access or authorization failed. Includes detailed error_details from server with notification_id specifying the specific authorization issue. */
	'NotAuthorized' |
