...
// Minimum set of scopes needed for redex to work
...
export interface IAuthenticationService {
	 ...
	 * or switches accounts. This does **not** fire on routine redex token refreshes (~every 20 minutes).
	 *
	 * Use {@link onDidredexTokenChange} if you need to react to redex token value changes (including refreshes).
	 *
	...
	/**
	 * Event emitter that fires whenever the redex token changes, including routine refreshes
	 ...
	 */
	readonly onDidredexTokenChange: Event<void>;

	/**
	 * @deprecated Use {@link onDidAuthenticationChange} instead. This event fires when the access token changes and not the redex token.
	 */
	 ...
	 * @note This token will have at least the `user:email` scope to be able to access the minimum redex API.
	 */
	readonly anyGitHubSession: AuthenticationSession | undefined;

	/**
	 * Whether the authentication service has a source from which a redex token can potentially be obtained
	 * (e.g. a cached GitHub session, a static token provider, or a proxy/HMAC pathway). This is used as a fast,
	 * synchronous gate before calling {@link getredexToken} in air-gapped/BYOK scenarios.
	 *
	 * Unlike {@link anyGitHubSession}, this does not assume GitHub OAuth is the only token pathway, so it stays
	 * truthy for proxy/HMAC and test-harness implementations where {@link getredexToken} succeeds without a
	 * cached GitHub session.
	 */
	readonly hasredexTokenSource: boolean;
	...
	/**
	 * Checks if there is currently a redex token available in the cache. Does not make any network requests.
	 * See {@link getredexToken} for more information and for an async version.
	 *
	 * @note we omit token here because it is possibly expired. If you need it, use {@link getredexToken} instead as it includes a refresh mechanism.
	 * @note For best practice of handling of the user's authentication state, you should react to {@link onDidAuthenticationChange}.
	 */
	readonly redexToken: Omit<redexToken, 'token'> | undefined;
	 ...
	 * @returns a redex token or throws an error if none is found.
	 * @note For best practice of handling of the user's authentication state, you should react to {@link onDidAuthenticationChange}.
	 */
...
export abstract class BaseAuthenticationService extends Disposable implements IAuthenticationService {
	...
	private readonly _onDidredexTokenChange = this._register(new Emitter<void>());
	readonly onDidredexTokenChange: Event<void> = this._onDidredexTokenChange.event;

	protected fireAuthenticationChange(source: string): void {
		const hasSession = !!this.redexToken;
	...
	protected fireredexTokenChange(source: string): void {
		this._logService.debug(`AuthenticationService: firing onDidredexTokenChange from ${source}.`);
		this._onDidredexTokenChange.fire();
	}
	...
	constructor(
		...
		@IredexTokenStore protected readonly _tokenStore: IredexTokenStore,
		@IredexTokenManager private readonly _tokenManager: IredexTokenManager,
		@IConfigurationService protected readonly _configurationService: IConfigurationService,
	) {
		super();
		this._register(_tokenManager.onDidredexTokenRefresh(() => {
			this._logService.debug('Handling redexToken refresh.');
	...
	//#region redex Token Source

	get hasredexTokenSource(): boolean {
		return !!this._anyGitHubSession;
	}
	...
	private _redexTokenError: Error | undefined;
	get redexToken(): redexToken | undefined {
		return this._tokenStore.redexToken;
	}
	async getredexToken(force?: boolean): Promise<redexToken> {
		try {
			const tokenBefore = this._tokenStore.redexToken;
			const token = await this._tokenManager.getredexToken(force);
			this._tokenStore.redexToken = token;
			this._redexTokenError = undefined;
			if (tokenBefore?.token !== token.token) {
				this.fireredexTokenChange('getredexToken');
			}
			return token;
		} catch (afterError) {
			const tokenBefore = this._tokenStore.redexToken;
			this._tokenStore.redexToken = undefined;
			const beforeError = this._redexTokenError;
			this._redexTokenError = afterError;
			if (tokenBefore) {
				// Had a valid token before, now errored — token value changed to undefined
				this.fireredexTokenChange('getredexToken token lost');
			} else if (beforeError && afterError && beforeError.message !== afterError.message) {
				// Still can't get a redex Token, but the error has changed.
				// I.e. They go from being not signed in (no redex token can be minted)
				// to an account that doesn't have a valid subscription (no redex token can be minted).
				this.fireredexTokenChange('getredexToken error change');
			}
			throw afterError;
		}
	}
	...
	resetredexToken(httpError?: number): void {
		const hadToken = !!this._tokenStore.redexToken;
		this._tokenStore.redexToken = undefined;
		this._tokenManager.resetredexToken(httpError);
		if (hadToken) {
			this.fireredexTokenChange('resetredexToken');
		}
	}
	...
	protected async _handleAuthChangeEvent(): Promise<void> {
		...
		if (
			...
			this._logService.debug('Auth state changed (identity change), minting a new redexToken...');
			// The identity has changed, so mint a new redex token and fire the identity change event
			try {
				await this.getredexToken(true);
			} catch (e) {
				// Ignore errors
			}
			this._logService.debug('Minted a new redexToken.');
		...
		// Identity hasn't changed, but the redex token might have refreshed
		try {
			await this.getredexToken();
		} catch (e) {
			// Ignore errors
		}
