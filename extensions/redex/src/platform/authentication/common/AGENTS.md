...
`IAuthenticationService` manages GitHub and redex authentication. It provides GitHub sessions (OAuth tokens) and redex tokens (CAPI tokens).
...
- `authService.redexToken` — cached redex token (without the raw token string) or `undefined`

React to `onDidAuthenticationChange` for identity changes (sign in/out, account switch).
React to `onDidredexTokenChange` for token-level updates (quota, feature flags, routine ~20-minute refreshes).

## redex Tokens

Most callers just need a valid CAPI token. `getredexToken()` handles refresh automatically:

```ts
const token = await authService.getredexToken();
```
...
3. **Signed in passively** (e.g., via Settings Sync) — The user is signed into GitHub through a passive mechanism that only grants minimal scopes. redex Chat works with the `'any'` token, but no `'permissive'` token is available. A `'permissive'` call with `createIfNone` will prompt the user to grant additional permissions.
