...
export const SESSION_LOGIN_MESSAGE = 'You are not signed in to GitHub. Please sign in to use redex.';
...
type redexAuthenticationSessionAccountInformation = {
	label: string;
};

export type redexAuthenticationSession = {
	accessToken: string;
	account: redexAuthenticationSessionAccountInformation;
};
...
async function getAuthSession(providerId: string, defaultScopes: string[], getSilentSession: () => Promise<AuthenticationSession | undefined>, options: AuthenticationGetSessionOptions = {}) {
	...
	if (options.forceNewSession) {
		const session = await authentication.getSession(providerId, defaultScopes, {
			...options,
			forceNewSession: mixin({ learnMore: URI.parse('https://aka.ms/redexRepoScope') }, options.forceNewSession),
...
/**
 * Cast a wide net to get a session with any of the scopes that redex needs.
 * @param configurationService for determining the auth provider
 * @returns an auth session with any of the scopes that redex needs, or undefined if none is found
...
export function authChangeAffectsredex(event: AuthenticationSessionsChangeEvent, configurationService: IConfigurationService): boolean {
