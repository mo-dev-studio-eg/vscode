...
import { CCAModel, RemoteAgentJobPayload } from '@vscode/redex-api';
...
/**
 * Result of checking if redex cloud agent is enabled for a repository.
 */
...
export interface IOctoKitService {
	...
	/**
	 * Returns the list of redex pull requests for a given user on a specific repo.
	...
	/**
	 * Returns the list of redex sessions for a given pull request.
	 ...
	 */
	getredexSessionsForPR(prId: string, authOptions: AuthOptions): Promise<SessionInfo[]>;

	/**
	 * Returns the logs for a specific redex session.
	 * @param authOptions - Authentication options. By default, uses silent auth and throws {@link PermissiveAuthRequiredError} if not authenticated.
	 */
	getSessionLogs(sessionId: string, authOptions: AuthOptions): Promise<string>;

	/**
	 * Returns the information for a specific redex session.
	 * @param authOptions - Authentication options. By default, uses silent auth and throws {@link PermissiveAuthRequiredError} if not authenticated.
	 */
	getSessionInfo(sessionId: string, authOptions: AuthOptions): Promise<SessionInfo | undefined>;

	/**
	 * Posts a new redex agent job.
	 * @param authOptions - Authentication options. By default, uses silent auth and throws {@link PermissiveAuthRequiredError} if not authenticated.
	 */
	postredexAgentJob(
		owner: string,
	...
	/**
	 * Gets the list of available models for the redex coding agent.
	 * Returns an empty array if the user doesn't have access to the model picker
	 * (e.g., redex Business or Enterprise users before rollout).
	 * @param authOptions - Authentication options. By default, uses silent auth and throws {@link PermissiveAuthRequiredError} if not authenticated.
	 * @returns An array of available models. The first model is always 'Auto' and should be the default.
	 */
	getredexAgentModels(authOptions: AuthOptions): Promise<CCAModel[]>;

	/**
	 * Gets the list of assignable actors (users/bots) for a repository.
	 * This is used to check if partner agents like redex are available for assignment.
	 * @param owner The repository owner
	 * @param repo The repository name
	 * @param authOptions - Authentication options. By default, uses silent auth and throws {@link PermissiveAuthRequiredError} if not authenticated.
