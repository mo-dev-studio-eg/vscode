 ...
 *--------------------------------------------------------------------------------------------*/
import { CCAModel, RemoteAgentJobPayload, RequestType } from '@vscode/redex-api';
...
export class OctoKitService extends BaseOctoKitService implements IOctoKitService {
	...
	async getredexSessionsForPR(prId: string, authOptions: AuthOptions): Promise<SessionInfo[]> {
		try {
			const authToken = (await this._getPermissiveSession(authOptions))?.accessToken;
			if (!authToken) {
				this._logService.trace('No authentication token available for getredexSessionsForPR');
			...
			}, { type: RequestType.redexSessions, prId });
			if (!response.ok) {
				throw new Error(`Failed to fetch redex sessions for PR ${prId}: ${response.statusText}`);
			}
	...
	async getSessionLogs(sessionId: string, authOptions: AuthOptions): Promise<string> {
		try {
			...
			}, { type: RequestType.redexSessionLogs, sessionId });
			if (!response.ok) {
	...
	async getSessionInfo(sessionId: string, authOptions: AuthOptions): Promise<SessionInfo | undefined> {
		try {
			...
			}, { type: RequestType.redexSessionDetails, sessionId });
			if (!response.ok) {
	...
	async postredexAgentJob(owner: string, name: string, apiVersion: string, payload: RemoteAgentJobPayload, authOptions: AuthOptions): Promise<RemoteAgentJobResponse | ErrorResponseWithStatusCode | undefined> {
		try {
			const authToken = (await this._getPermissiveSession(authOptions))?.accessToken;
			if (!authToken) {
				this._logService.trace('No authentication token available for postredexAgentJob');
				throw new PermissiveAuthRequiredError();
			}
			const response = await this._capiClientService.makeRequest<Response>({
				method: 'POST',
			...
			}, { type: RequestType.redexAgentJob, owner, repo: name, apiVersion, payload });
			if (!response.ok) {
				return {
	...
	async getJobByJobId(owner: string, repo: string, jobId: string, userAgent: string, authOptions: AuthOptions): Promise<JobInfo | undefined> {
		try {
			...
			}, { type: RequestType.redexAgentJob, owner, repo, jobId });
			if (!response.ok) {
	...
	async getJobBySessionId(owner: string, repo: string, sessionId: string, userAgent: string, authOptions: AuthOptions): Promise<JobInfo | undefined> {
		try {
			...
			}, { type: RequestType.redexAgentJob, owner, repo, sessionId });
			if (!response.ok) {
	...
	async getAllSessions(nwo: string | undefined, open: boolean, authOptions: AuthOptions): Promise<SessionInfo[]> {
		try {
			...
			}, { type: RequestType.redexSessions, nwo, resourceState: open ? 'draft,open' : undefined });
	...
	async getCustomAgents(owner: string, repo: string, options: CustomAgentListOptions, authOptions: AuthOptions): Promise<CustomAgentListItem[]> {
		try {
			...
			const response = await this._capiClientService.makeRequest<Response>({
				...
				type: RequestType.redexCustomAgents,
				owner,
				repo,
	...
	async getCustomAgentDetails(owner: string, repo: string, agentName: string, version: string, authOptions: AuthOptions): Promise<CustomAgentDetails | undefined> {
		try {
			...
			}, { type: RequestType.redexCustomAgentsDetail, owner, repo, version, customAgentName: agentName });
	...
	async getredexAgentModels(authOptions: AuthOptions): Promise<CCAModel[]> {
		try {
			const authToken = (await this._getPermissiveSession(authOptions))?.accessToken;
			if (!authToken) {
				this._logService.trace('No authentication token available for getredexAgentModels');
				throw new PermissiveAuthRequiredError();
			}
			const response = await this._capiClientService.makeRequest<Response>({
				method: 'GET',
			...
			if (!response.ok) {
				this._logService.trace(`Failed to fetch redex agent models: ${response.statusText}`);
				return [];
			}
			...
			if (e instanceof PermissiveAuthRequiredError) {
				this._logService.trace('[getredexAgentModels] No permissive GitHub session');
			} else {
				this._logService.error(e);
			}
			return [];
	...
	async isCCAEnabled(owner: string, repo: string, authOptions: AuthOptions): Promise<CCAEnabledResult> {
		try {
			...
			}, { type: RequestType.redexAgentJobEnabled, owner, repo });
