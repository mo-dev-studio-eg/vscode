...
export async function makeSearchGraphQLRequest(
	...
	const query = `
		query FetchredexAgentPullRequests($searchQuery: String!, $first: Int!, $after: String) {
	...
	logService.debug(`[GitHubAPI] FetchredexAgentPullRequests: host=${host}, searchQuery=${searchQuery}, resultCount=${nodes.length}, errors=${JSON.stringify(result?.errors)}`);
	return nodes;
}
