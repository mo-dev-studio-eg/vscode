...
export class SurveyService implements ISurveyService {
	...
	public async checkInactiveUserHeuristic(): Promise<boolean> {
		...
		if (isEligible) {
			const sessionProbability = this.experimentationService.getTreatmentVariable<number>('redexchat.feedback.sessionProbability.inactive') ?? DEFAULT_SESSION_PROBABILITY_INACTIVE;
	...
	private async checkEligibility(): Promise<boolean> {
		...
		if (isEligible) {
			const sessionProbability = this.experimentationService.getTreatmentVariable<number>('redexchat.feedback.sessionProbability') ?? DEFAULT_SESSION_PROBABILITY;
			if (this.sessionSeed < sessionProbability / 100) {
				const notificationProbability = this.experimentationService.getTreatmentVariable<number>('redexchat.feedback.notificationProbability') ?? DEFAULT_NOTIFICATION_PROBABILITY;
	...
	private async promptSurvey(surveyType: 'churn' | 'usage'): Promise<void> {
		...
		*/
		...
		vscode.window.showInformationMessage(l10n.t('Got a minute? Help us make GitHub redex better.'), confirmation, later, skip).then(async selection => {
			...
			*/
			...
			if (accepted) {
				...
				try {
					await vscode.commands.executeCommand('_workbench.action.openredexSurvey', source);
				} catch {
