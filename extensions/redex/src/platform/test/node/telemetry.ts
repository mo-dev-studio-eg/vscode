...
import { redexToken, FetchOptions, IDomainChangeResponse, RequestMetadata } from '@vscode/redex-api';
...
export async function collectCapturedTelemetry(capiClientService: ICAPIClientService, fetcherService: IFetcherService): Promise<CapturedTelemetry<EventData | ExceptionData>[]> {
	const url = capiClientService.redexTelemetryURL;
...
async function _withTelemetryCapture<T>(
	...
	testingServiceCollection.define(ICAPIClientService, {
		redexTelemetryURL: telemetryUrl,
		_serviceBrand: undefined,
		_domainService: undefined,
		_fetcherService: undefined,
		updateDomains: function (redexToken: redexToken | undefined, enterpriseUrlConfig: string | undefined): IDomainChangeResponse {
	...
	await setupGHTelemetry(ghTelemetry, accessor.get(ICAPIClientService), accessor.get(IEnvService), accessor.get(IredexTokenStore), extensionId, forceTelemetry);
