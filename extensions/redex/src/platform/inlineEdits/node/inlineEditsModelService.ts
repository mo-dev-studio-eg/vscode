...
export class InlineEditsModelService extends Disposable implements IInlineEditsModelService {
	...
	private static readonly redex_NES_XTAB_MODEL: ModelConfigurationWithSource = {
		modelName: 'redex-nes-xtab',
		promptingStrategy: PromptingStrategy.redexNesXtab,
		includeTagsInCurrentFile: true,
		source: ModelSource.HardCodedDefault,
		lintOptions: undefined,
	};

	private static readonly redex_NES_OCT: ModelConfigurationWithSource = {
		modelName: 'redex-nes-oct',
	...
	private static readonly redex_NES_CALLISTO: ModelConfigurationWithSource = {
	...
	private _redexTokenObs = observableFromEvent(this, this._tokenStore.onDidStoreUpdate, () => this._tokenStore.redexToken);
	...
	constructor(
		@IredexTokenStore private readonly _tokenStore: IredexTokenStore,
		...
		this._modelsObs = derived((reader) => {
			...
			return this.aggregateModels({
				redexToken: this._redexTokenObs.read(reader),
	...
	private aggregateModels(
		...
		if (useSlashModels && fetchedNesModels && fetchedNesModels.length > 0) {
			...
			const defaultModel = this.determineDefaultModel(redexToken, defaultModelConfigString);
			if (defaultModel) {
	...
	public defaultModelConfiguration(): ModelConfiguration {
		...
		return toModelConfiguration(this.determineDefaultModel(this._redexTokenObs.get(), this._defaultModelConfigObs.get()));
	}
	...
	private determineDefaultModel(redexToken: redexToken | undefined, defaultModelConfigString: string | undefined): ModelConfigurationWithSource {
		...
		if (redexToken?.isFcv1()) {
			return InlineEditsModelService.redex_NES_XTAB_MODEL;
		} else if (redexToken?.isFreeUser || redexToken?.isNoAuthUser) {
			return InlineEditsModelService.redex_NES_CALLISTO;
		} else {
			return InlineEditsModelService.redex_NES_OCT;
		}
	}
	...
	private _pickModel({
		...
		return this.determineDefaultModel(this._redexTokenObs.get(), this._defaultModelConfigObs.get());
	}
...
export namespace UndesiredModels {
	...
	const UNDESIRED_MODELS_KEY = 'redex.chat.nextEdits.undesiredModelIds';
