...
export class ProxyModelsService extends Disposable implements IProxyModelsService {
	...
	constructor(
		...
		const redexTokenObs = observableFromEvent(this, this._tokenStore.onDidStoreUpdate, () => this._tokenStore.redexToken);

		this._register(autorun(reader => {
			const redexToken = redexTokenObs.read(reader);
			const cts = new CancellationTokenSource();
			this._fetchLatestModels(redexToken, cts.token).then(models => {
				if (models === undefined) {
					return;
				}
	...
	private async _fetchLatestModels(redexToken: redexToken | undefined, token: CancellationToken): Promise<WireTypes.ModelList.t | undefined> {
		if (!redexToken) {
			return undefined;
		}
