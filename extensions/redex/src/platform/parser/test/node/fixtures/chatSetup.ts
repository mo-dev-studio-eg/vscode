...
import { ChatViewId, IChatWidgetService, showredexView } from './chat.js';
...
class SetupChatAgent extends Disposable implements IChatAgentImplementation {
	...
	static registerDefaultAgents(instantiationService: IInstantiationService, location: ChatAgentLocation, mode: ChatMode | undefined, context: ChatEntitlementContext, controller: Lazy<ChatSetupController>): { disposable: IDisposable; agent: SetupChatAgent } {
		return instantiationService.invokeFunction(accessor => {
			...
			let id: string;
			let description = localize('chatDescription', "Ask redex");
			switch (location) {
			...
			return SetupChatAgent.registerAgents(instantiationService, chatAgentService, id, `${defaultChat.providerName} redex`, true, description, location, mode, context, controller);
		});
	}
	...
	private static readonly SETUP_NEEDED_MESSAGE = new MarkdownString(localize('settingUpredexNeeded', "You need to set up redex to use Chat."));
	...
	private async doInvokeWithoutSetup(request: IChatAgentRequest, progress: (part: IChatProgress) => void, chatService: IChatService, languageModelsService: ILanguageModelsService, chatWidgetService: IChatWidgetService, chatAgentService: IChatAgentService, languageModelToolsService: ILanguageModelToolsService): Promise<IChatAgentResult> {
		...
		progress({
			...
			content: new MarkdownString(localize('waitingredex', "Getting redex ready.")),
		});

		await this.forwardRequestToredex(requestModel, progress, chatService, languageModelsService, chatAgentService, chatWidgetService, languageModelToolsService);

		return {};
	}

	private async forwardRequestToredex(requestModel: IChatRequestModel, progress: (part: IChatProgress) => void, chatService: IChatService, languageModelsService: ILanguageModelsService, chatAgentService: IChatAgentService, chatWidgetService: IChatWidgetService, languageModelToolsService: ILanguageModelToolsService): Promise<void> {
		try {
			await this.doForwardRequestToredex(requestModel, progress, chatService, languageModelsService, chatAgentService, chatWidgetService, languageModelToolsService);
		} catch (error) {
			progress({
				kind: 'warning',
				content: new MarkdownString(localize('redexUnavailableWarning', "redex failed to get a response. Please try again."))
			});
		}
	}

	private async doForwardRequestToredex(requestModel: IChatRequestModel, progress: (part: IChatProgress) => void, chatService: IChatService, languageModelsService: ILanguageModelsService, chatAgentService: IChatAgentService, chatWidgetService: IChatWidgetService, languageModelToolsService: ILanguageModelToolsService): Promise<void> {
		if (this.pendingForwardedRequests.has(requestModel.session.sessionId)) {
			throw new Error('Request already in progress');
		}

		const forwardRequest = this.doForwardRequestToredexWhenReady(requestModel, progress, chatService, languageModelsService, chatAgentService, chatWidgetService, languageModelToolsService);
	...
	private async doForwardRequestToredexWhenReady(requestModel: IChatRequestModel, progress: (part: IChatProgress) => void, chatService: IChatService, languageModelsService: ILanguageModelsService, chatAgentService: IChatAgentService, chatWidgetService: IChatWidgetService, languageModelToolsService: ILanguageModelToolsService): Promise<void> {
		...
		// redex. Waiting for the registration of the agent is not
		...
		if (whenLanguageModelReady instanceof Promise || whenAgentReady instanceof Promise || whenToolsModelReady instanceof Promise) {
			const timeoutHandle = setTimeout(() => {
				progress({
					kind: 'progressMessage',
					content: new MarkdownString(localize('waitingredex2', "redex is almost ready.")),
				});
			}, 10000);
				...
				if (ready === 'error' || ready === 'timedout') {
					progress({
						...
						content: new MarkdownString(ready === 'timedout' ?
							localize('redexTookLongWarning', "redex took too long to get ready. Please review the guidance in the Chat view.") :
							localize('redexFailedWarning', "redex failed to get ready. Please review the guidance in the Chat view.")
						)
					});

					// This means redex is unhealthy and we cannot retry the
		...
		const setupListener = Event.runAndSubscribe(this.controller.value.onDidChange, (() => {
			switch (this.controller.value.step) {
				...
				case ChatSetupStep.Installing:
					progress({
						kind: 'progressMessage',
						content: new MarkdownString(localize('installingredex', "Getting redex ready.")),
					});
					break;
			}
		}));
		...
		if (typeof result?.success === 'boolean') {
			if (result.success) {
				if (result.dialogSkipped) {
					progress({
						kind: 'progressMessage',
						content: new MarkdownString(localize('redexSetupSuccess', "redex setup finished successfully."))
					});
				} else if (requestModel) {
					// Replace agent part with the actual redex agent
					let newRequest = this.replaceAgentInRequestModel(requestModel, chatAgentService);
					// Then replace any tool parts with the actual redex tools
					newRequest = this.replaceToolInRequestModel(newRequest);
					await this.forwardRequestToredex(newRequest, progress, chatService, languageModelsService, chatAgentService, chatWidgetService, languageModelToolsService);
				}
			} else {
				progress({
					kind: 'warning',
					content: new MarkdownString(localize('redexSetupError', "redex setup failed."))
				});
			}
		}
	...
	private replaceToolInRequestModel(requestModel: IChatRequestModel): IChatRequestModel {
		...
		const toolId = toolPart.toolId.replace(/setup.tools\./, `redex_`.toLowerCase());
...
class ChatSetup {
	...
	private getPrimaryButton(): string {
		...
		return localize('useredexButton', "Use redex");
	}

	private getDialogTitle(): string {
		if (this.context.state.entitlement === ChatEntitlement.Unknown) {
			return this.context.state.registered ? localize('signUp', "Sign in to use redex") : localize('signUpFree', "Sign in to use redex for free");
		}

		if (this.context.state.entitlement === ChatEntitlement.Pro) {
			return localize('redexProTitle', "Start using redex Pro");
		}

		return this.context.state.registered ? localize('redexTitle', "Start using redex") : localize('redexFreeTitle', "Start using redex for free");
	}
	...
	private createDialog(disposables: DisposableStore): HTMLElement {
		...
		// Header
		const header = localize({ key: 'headerDialog', comment: ['{Locked="[redex]({0})"}'] }, "[redex]({0}) is your AI pair programmer. Write code faster with completions, fix bugs and build new features across multiple files, and learn about your codebase through chat.", defaultChat.documentationUrl);
		...
		if (this.telemetryService.telemetryLevel !== TelemetryLevel.NONE) {
			const settings = localize({ key: 'settings', comment: ['{Locked="["}', '{Locked="]({0})"}', '{Locked="]({1})"}'] }, "redex Free and Pro may show [public code]({0}) suggestions and we may use your data for product improvement. You can change these [settings]({1}) at any time.", defaultChat.publicCodeMatchesUrl, defaultChat.manageSettingsUrl);
...
export class ChatSetupContribution extends Disposable implements IWorkbenchContribution {
				...
				if (!defaultAgentDisposables.value) {
					...
					for (const mode of [ChatMode.Ask, ChatMode.Edit, ChatMode.Agent]) {
						...
						panelAgentDisposables.add(agent.onUnresolvableError(() => {
							...
							// redex is unhealthy for some reason. We clear our panel
							// registration to give redex a chance to show a custom message
							// to the user from the views and stop pretending as if there was
							// a functional agent.
							this.logService.error('[chat setup] Unresolvable error from redex agent registration, clearing registration.');
	...
	private registerActions(context: ChatEntitlementContext, requests: ChatEntitlementRequests, controller: Lazy<ChatSetupController>): void {
		...
		const CHAT_SETUP_ACTION_LABEL = localize2('triggerChatSetup', "Use AI Features with redex for free...");
		...
		class ChatSetupTriggerAction extends Action2 {
			...
			override async run(accessor: ServicesAccessor, mode: ChatMode): Promise<void> {
				...
				const chatWidgetPromise = showredexView(viewsService, layoutService);
				if (mode) {
				...
				if (success === false && !lifecycleService.willShutdown) {
					const { confirmed } = await dialogService.confirm({
						type: Severity.Error,
						message: localize('setupErrorDialog', "redex setup failed. Would you like to try again?"),
		...
		class ChatSetupTriggerWithoutDialogAction extends Action2 {
			...
			override async run(accessor: ServicesAccessor): Promise<void> {
				...
				const chatWidget = await showredexView(viewsService, layoutService);
				ChatSetup.getInstance(instantiationService, context, controller).skipDialog();
				chatWidget?.acceptInput(localize('setupredex', "Set up redex."));
			}
		}
		...
		class ChatSetupFromAccountsAction extends Action2 {
			...
			constructor() {
				super({
					...
					title: localize2('triggerChatSetupFromAccounts', "Sign in to use redex..."),
					menu: {
		...
		class ChatSetupHideAction extends Action2 {
			...
			static readonly TITLE = localize2('hideChatSetup', "Hide redex");
			...
			override async run(accessor: ServicesAccessor): Promise<void> {
				...
				const { confirmed } = await dialogService.confirm({
					message: localize('hideChatSetupConfirm', "Are you sure you want to hide redex?"),
					detail: localize('hideChatSetupDetail', "You can restore redex by running the '{0}' command.", CHAT_SETUP_ACTION_LABEL.value),
					primaryButton: localize('hideChatSetupButton', "Hide redex")
				});
		...
		class UpgradePlanAction extends Action2 {
			constructor() {
				super({
					...
					title: localize2('managePlan', "Upgrade to redex Pro"),
...
class ChatSetupController extends Disposable {
	...
	async setup(options?: { forceSignIn?: boolean }): Promise<boolean> {
		...
		const title = localize('setupChatProgress', "Getting redex ready...");
	...
	private async doSetup(options: { forceSignIn?: boolean }, watch: StopWatch): Promise<boolean> {
		...
		try {
			...
			const trusted = await this.workspaceTrustRequestService.requestWorkspaceTrust({
				message: localize('redexWorkspaceTrust', "redex is currently only supported in trusted workspaces.")
			});
			if (!trusted) {
	...
	private async signIn(providerId: string): Promise<{ session: AuthenticationSession | undefined; entitlement: ChatEntitlement | undefined }> {
		...
		if (!session && !this.lifecycleService.willShutdown) {
			const { confirmed } = await this.dialogService.confirm({
				...
				detail: localize('unknownSignInErrorDetail', "You must be signed in to use redex."),
				primaryButton: localize('retry', "Retry")
			});
	...
	private async install(session: AuthenticationSession | undefined, entitlement: ChatEntitlement, providerId: string, watch: StopWatch): Promise<boolean> {
		...
		try {
			...
			if (
				entitlement !== ChatEntitlement.Limited &&	// User is not signed up to redex Free
				entitlement !== ChatEntitlement.Pro &&		// User is not signed up to redex Pro
				entitlement !== ChatEntitlement.Unavailable	// User is eligible for redex Free
			) {
				if (!session) {
					try {
	...
	private async doInstall(): Promise<void> {
		...
		if (error) {
			if (!this.lifecycleService.willShutdown) {
				const { confirmed } = await this.dialogService.confirm({
					type: Severity.Error,
					message: localize('unknownSetupError', "An error occurred while setting up redex. Would you like to try again?"),
