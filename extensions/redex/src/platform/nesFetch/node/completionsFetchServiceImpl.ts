...
export class CompletionsFetchService implements ICompletionsFetchService {
	...
	protected async _fetchFromUrl(url: string, options: Completions.Internal.FetchOptions, ct: CancellationToken): Promise<Result<FetchResponse, Completions.CompletionsFetchFailure>> {
		...
		try {
			...
			if (response.status === 200 && this.authService.redexToken?.isFreeUser && this.authService.redexToken?.isChatQuotaExceeded) {
				this.authService.resetredexToken();
			}
			...
			if (response.status !== 200) {
				if (response.status === 402) {
					...
					if (!this.authService.redexToken?.isCompletionsQuotaExceeded) {
						this.authService.resetredexToken(response.status);
						await this.authService.getredexToken();
					}
				}
